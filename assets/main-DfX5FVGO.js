...
`),i=new Blob([s],{type:"text/csv;charset=utf-8"});return console.log("✅ CSV generated successfully"),i}catch(t){throw console.error("❌ CSV generation failed:",t),t}}async generateBothFiles(e){try{const[t,o]=await Promise.all([this.generatePDF(e),this.generateCSV(e)]);return{pdfBlob:t,csvBlob:o}}catch(t){throw console.error("❌ File generation failed:",t),t}}getSelectedProducts(){const e=JSON.parse(localStorage.getItem("selection")||"[]"),t=JSON.parse(localStorage.getItem(M.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");return t.length>0?t:e.map(o=>({product:o,room:o.Room||"",notes:o.Notes||"",quantity:o.Quantity||1}))}calculateTotalPrice(e,t){const s=(parseFloat(e.toString().replace(/[^0-9.]/g,""))||0)*(t||1);return s>0?s.toFixed(2):""}cleanForCSV(e){return e?e.toString().replace(/"/g,'""').replace(/[\r\n]/g," "):""}async generateQuotePDF(e){return await this.generatePDF(e)}async generateReportPDF(e){return await this.generatePDF(e)}generateFileName(e,t){const o=new Date,s=String(o.getDate()).padStart(2,"0"),i=String(o.getMonth()+1).padStart(2,"0"),n=String(o.getFullYear()).slice(-2),r=String(o.getHours()).padStart(2,"0"),a=String(o.getMinutes()).padStart(2,"0");return`${(e.project||"seima-selection").replace(/[^a-zA-Z0-9\s]/g,"")}-${s}${i}${n}.${r}${a}.${t}`}downloadFile(e,t){const o=URL.createObjectURL(e),s=document.createElement("a");s.href=o,s.download=t,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(o)}getSelectionSummary(){const e=this.getSelectedProducts(),t=e.length,s=new Set(e.map(n=>n.room).filter(Boolean)).size||1;let i=0;return e.forEach(n=>{var c,d;const r=parseFloat((((c=n.product)==null?void 0:c.UserEditedPrice)||((d=n.product)==null?void 0:d.RRP_EX)||"0").toString().replace(/[^0-9.]/g,""))||0,a=n.quantity||1;i+=r*a}),{totalProducts:t,totalRooms:s,totalValue:i,hasProducts:t>0}}}const Wt=new qt,Vt={Utils:x},Xt=N;class Yt{constructor(){this.modules={dataLayer:R,pdfGenerator:Wt,pdfCore:it,pdfLayouts:wt,StorageManager:E,utils:Vt},this.isInitialized=!1,this.initStatus={}}async init(){try{console.log("🚀 Initializing modular components...");const e=await this.initModule("dataLayer",this.modules.dataLayer);return this.initStatus.dataLayer=e,this.initStatus.pdfGenerator="deferred",this.isInitialized=!0,console.log("✅ Module initialization complete:",this.initStatus),this.initStatus}catch(e){return console.error("❌ Module coordinator initialization failed:",e),!1}}async initModule(e,t){try{if(t&&typeof t.init=="function"){const o=await t.init();return console.log(`✅ ${e} initialized:`,o),o}else return console.log(`ℹ️ ${e} does not require initialization`),!0}catch(o){throw console.error(`❌ Failed to initialize ${e}:`,o),o}}async searchProducts(e,t=10){return this.modules.dataLayer.isLoaded||await this.modules.dataLayer.init(),this.modules.dataLayer.searchProducts(e,t)}async findProductByCode(e){return this.modules.dataLayer.isLoaded||await this.modules.dataLayer.init(),this.modules.dataLayer.findProductByCode(e)}async addProductToSelection(e,{room:t="",notes:o="",quantity:s=1}={}){return this.modules.dataLayer.addProductToSelection(e,{room:t,notes:o,quantity:s})}getSelectedProducts(){return this.modules.dataLayer.getSelectedProducts()}getSelectionSummary(){return this.modules.dataLayer.getSelectionSummary()}async generatePDF(e){return await this.modules.pdfGenerator.generatePDF(e)}async generateCSV(e){return await this.modules.pdfGenerator.generateCSV(e)}async generateBothFiles(e){return await this.modules.pdfGenerator.generateBothFiles(e)}async sendEmail(e,t){return await this.modules.emailService.sendEmailWithPDF(e,t)}clearSelection(){return this.modules.dataLayer.clearSelection()}getModuleStatus(){return{initialized:this.isInitialized,moduleStatus:this.initStatus,dataLayer:{loaded:this.modules.dataLayer.isLoaded,productCount:this.modules.dataLayer.products.length},selection:{count:this.getSelectedProducts().length,summary:this.getSelectionSummary()}}}async reinitializeModule(e){if(this.modules[e])try{return this.initStatus[e]=await this.initModule(e,this.modules[e]),this.initStatus[e]}catch(t){return console.error(`❌ Failed to reinitialize ${e}:`,t),!1}return!1}async batchAddProducts(e){const t=[];for(const{product:o,room:s,notes:i,quantity:n}of e)try{const r=await this.addProductToSelection(o,{room:s,notes:i,quantity:n});t.push({success:!0,result:r})}catch(r){t.push({success:!1,error:r.message,product:o})}return t}exportState(){return{moduleStatus:this.getModuleStatus(),config:Xt,timestamp:new Date().toISOString()}}}const Jt=new Yt;Jt.init().catch(g=>{console.error("❌ Auto-initialization failed:",g)});class Qt{constructor(){this.currentScreen="welcome",this.currentSearchResults=[]}async init(){if(!R.isLoaded)try{await R.init()}catch(e){console.error("Failed to load product catalog:",e)}this.updateSelectionCount(),this.setVersion()}setVersion(){const e=document.getElementById("version-number");e&&(e.innerText=M.VERSION)}async showProductLookupScreen(){try{const t=await(await fetch("./screens/product-grid.html")).text();document.body.innerHTML=t,this.currentScreen="product-grid";const o=document.createElement("script");o.type="module",o.src="js/app.js",document.body.appendChild(o),setTimeout(()=>{document.querySelectorAll(".back-btn").forEach(s=>s.remove())},100),window.productGridManager&&window.productGridManager.init(),await this.loadVersion(),setTimeout(()=>this.loadVersion(),1e3)}catch(e){console.error("Failed to load product grid screen:",e)}}setupSplitInterface(){const e=document.getElementById("back-to-home");e&&(e.onclick=()=>location.reload());const t=document.getElementById("download-btn"),o=document.getElementById("clear-all-btn");t&&(t.onclick=()=>this.showDownloadFormModal()),o&&(o.onclick=()=>this.showClearConfirmModal()),this.setupSplitProductSearch(),this.setupReviewTable(),this.renderReviewTable(),this.loadInitialSearchResults()}setupSplitProductSearch(){const e=document.getElementById("product-search-input"),t=document.getElementById("search-results-list"),o=document.getElementById("search-loading"),s=document.getElementById("search-no-results");if(!e||!t)return;const i=[],n=x.debounce(r=>{this.performSplitProductSearch(r,t,i,o,s)},200);e.addEventListener("input",()=>{const r=e.value.trim();r?n(r):this.loadInitialSearchResults()}),t.addEventListener("click",r=>{const a=r.target.closest(".result-item");if(!a)return;const c=parseInt(a.getAttribute("data-idx"),10),d=i.length>0?i:this.currentSearchResults||[];!isNaN(c)&&d[c]&&this.showSplitProductDetails(d[c])})}performSplitProductSearch(e,t,o,s,i){if(!R.isLoaded){s.style.display="flex",i.style.display="none",t.innerHTML="";return}o.length=0,o.push(...R.searchProducts(e)),s.style.display="none",o.length===0?(i.style.display="flex",t.innerHTML=""):(i.style.display="none",t.innerHTML=o.map((n,r)=>`
        ...
        `).join(""))}async loadInitialSearchResults(){const e=document.getElementById("search-results-list"),t=document.getElementById("search-loading"),o=document.getElementById("search-no-results");if(!e)return;if(!R.isLoaded){t.style.display="flex",o.style.display="none",e.innerHTML="",R.init().then(()=>this.loadInitialSearchResults()).catch(()=>{});return}const s=R.getAllProducts().slice(0,50);t.style.display="none",o.style.display="none",e.innerHTML=s.map((i,n)=>`
      ...
      <div class="review-room-group">
        ...
        ${n.map((r,a)=>{const c=r.product,d=c.Description||c.description||c.productName||c["Product Name"]||"Product",l=c.OrderCode||c.orderCode||"",u=c.Image_URL||c.imageUrl||"assets/no-image.png",m=c.RRP_EX||c["RRP EX GST"]||c.RRP_EX||c.rrpExGst||c.RRP_EXGST||c["PL1 - RRP EX GST"]||c.RRP_INCGST||c["RRP INC GST"]||"0";return`
    ...
    `,document.body.appendChild(o);const i=Rt();Xe("./assets/seima-logo.png",async(n,r,a)=>{const c=JSON.parse(localStorage.getItem("selection")||"[]"),d=JSON.parse(localStorage.getItem(M.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let l=[];d.length>0?l=d.map(v=>({...v.product,Room:v.room,Notes:v.notes,Quantity:v.quantity,Timestamp:new Date(v.timestamp).toISOString()})):l=c;const u=document.getElementById("sort-by"),m=u?u.value:"room";switch(m){case"code":l.sort((v,S)=>{const P=v.OrderCode||v.Code||"",C=S.OrderCode||S.Code||"";return P.localeCompare(C)});break;case"product":l.sort((v,S)=>{const P=v.Description||v.ProductName||"",C=S.Description||S.ProductName||"";return P.localeCompare(C)});break;case"room":default:{let v=[];try{const S=localStorage.getItem("customRoomOrder");v=S?JSON.parse(S):[]}catch{v=[]}l.sort((S,P)=>{const C=S.Room||"Blank",_=P.Room||"Blank";if(C===_)return 0;if(C==="Blank")return 1;if(_==="Blank")return-1;const I=v.indexOf(C),$=v.indexOf(_);return I!==-1&&$!==-1?I-$:I!==-1?-1:$!==-1?1:C.localeCompare(_)});break}}if(!l.length){alert("No products selected."),t&&(t.style.display="none");return}if(ct()>0)console.log(`📷 Using ${ct()} pre-cached images (skipping duplicate preload)`);else{const v=document.getElementById("pdf-processing-notification");if(v){const S=document.createElement("span");S.id="preload-progress",S.style.cssText="display: block; font-size: 12px; margin-top: 8px; color: #1e40af;",S.textContent="Loading images: 0%",v.appendChild(S)}console.log("📷 Starting image preload for",l.length,"products"),vt(l).then(S=>{const P=document.getElementById("preload-progress");P&&(P.textContent=`✓ ${S} images ready`,P.style.color="#059669")}).catch(S=>{console.warn("Image preloading error:",S)})}const p={};m==="room"?l.forEach(v=>{const S=v.Room||"Blank";p[S]||(p[S]=[]),p[S].push(v)}):p.__all__=l,await i;const{jsPDF:f}=window.jspdf,y=new f({orientation:"landscape",unit:"pt",format:"a4",compress:!0,putOnlyUsedFonts:!0,precision:16,userUnit:1,floatPrecision:16}),b=y.internal.pageSize.getWidth(),w=y.internal.pageSize.getHeight();Xe("./assets/seima-logo.png",async(v,S,P)=>{const C=T.getCurrentUser(),_=E.getUserSettings(),I=C?{name:C.name,email:C.email,phone:C.phone,position:C.position}:_;let $=null;try{const{get:U}=await O(async()=>{const{get:H}=await import("./vendor-idb-DwnyWBFG.js");return{get:H}},[]);$=await U("customerLogo")}catch{}It(y,{pageWidth:b,pageHeight:w,seimaLogoDataUrl:v,seimaLogoNaturalW:S,seimaLogoNaturalH:P,customerLogoDataUrl:$,userDetails:g,staffContact:I,footerHeight:dt.footerHeight}),y.addPage(),Xe("./assets/seima-logo-white.png",(U,H,q)=>{const ye=g.showRrp&&!g.excludePrice,$e=!g.excludePrice,j=!g.excludeQty,He=Lt(b,{leftMargin:32,rightMargin:32,showRrp:ye,showPrice:$e,showQty:j,showTotal:$e&&j}),{colX:le,colW:Q,headers:ke}=He,De=dt.footerHeight;mt();const rt=(D,L,J,ie,G,W,F)=>{if(!L||typeof L!="string"||L.length<10||!L.startsWith("http://")&&!L.startsWith("https://")&&!L.startsWith("data:")){F&&F();return}if((B=>{if(!B||B.length<25||/\/images\/\d+$/.test(B)||B.endsWith("/0"))return!0;const A=/\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(B),z=B.startsWith("data:");return!A&&!z})(L)){console.warn("Skipping malformed image URL:",(L==null?void 0:L.substring(0,50))+"..."),F&&F();return}if(oe.totalImages++,g.emailCompatible){oe.failedImages++,F&&F();return}const ne=At(L);if(ne&&ne.dataUrl)try{const B=ne.width/ne.height;let A=G,z=G/B;z>W&&(z=W,A=W*B),D.addImage(ne.dataUrl,ne.format,J,ie,A,z,void 0,"FAST"),oe.optimizedImages++,F&&F();return}catch{console.warn("Failed to use cached image, falling back to direct load")}let ue=!1;const Se=["https://wsrv.nl/?url=","https://images.weserv.nl/?url=","https://api.codetabs.com/v1/proxy?quest="];let re=0;function ae(){if(ue)return;const B=new Image;B.crossOrigin="Anonymous";let A=null;B.onload=function(){if(!ue){ue=!0,A&&clearTimeout(A);try{const me=St(0),Ne=me.imageMaxWidth,Ee=G,ce=W;try{const V=document.createElement("canvas"),de=V.getContext("2d"),{width:pe,height:xe}=no(B.width,B.height,Ne);V.width=pe,V.height=xe,de.imageSmoothingEnabled=!0,de.imageSmoothingQuality="high",de.drawImage(B,0,0,pe,xe);let We,Ve="JPEG";const Pt=io(V,de),_t=so(B);Pt||_t?(We=V.toDataURL("image/png",me.imageQuality),Ve="PNG"):(We=V.toDataURL("image/jpeg",me.imageQuality),Ve="JPEG");const kt=`img_${ho(L)}`;D.addImage(We,Ve,J,ie,Ee,ce,kt,"FAST"),oe.optimizedImages++,F&&F()}catch(V){console.warn(`Failed to optimize image: ${L}`,V),console.warn("Error details:",V.message,V.stack);try{D.addImage(B,"JPEG",J,ie,Ee,ce),oe.optimizedImages++,F&&F()}catch(de){console.error(`Fallback also failed for: ${L}`,de),oe.failedImages++,F&&F()}}}catch(me){console.warn("Failed to add image to PDF:",me),oe.failedImages++,F&&F()}}},B.onerror=function(){ue||(A&&clearTimeout(A),console.warn(`Failed to load image with proxy ${re}: ${L}`),console.warn(`Error details for: ${L} - Proxy: ${Se[re]}`),re++,re<Se.length?setTimeout(()=>{ae()},200):(ue=!0,console.warn("All proxies failed, skipping image"),oe.failedImages++,F&&F()))},A=setTimeout(()=>{ue||(console.warn(`⏰ Timeout with proxy ${re}: ${L}`),B.src="",B.onload=null,B.onerror=null,re++,re<Se.length?setTimeout(()=>{ae()},200):(ue=!0,console.warn("All proxies timed out, skipping image"),oe.failedImages++,F&&F()))},3e3);let z=L;re<Se.length&&(z=Se[re]+encodeURIComponent(L)),B.src=z}ae()},te=[];Object.keys(p).forEach((D,L)=>{const J=p[D];if(!J||!Array.isArray(J)){console.warn("⚠️ Skipping invalid room items:",D,J);return}J.forEach((ie,G)=>{if(!ie){console.warn("⚠️ Skipping null item in room:",D,"at index:",G);return}te.push({item:ie,room:D,rIdx:L,iIdx:G,isFirstInRoom:G===0,roomCount:J.length})})}),te.reduce((D,L)=>{if(!L||!L.item)return console.warn("⚠️ Skipping null row in data analysis:",L),D;const J=String(L.item.Description||""),ie=String(L.item.LongDescription||""),G=String(L.item.Notes||""),W=String(L.item.OrderCode||"");return D+J.length+ie.length+G.length+W.length},0);let Re=0,Be=0;const Ae=4,Fe=8,je=Math.floor((w-80)/Ae);let at=De+8;function qe(){if(!te||!Array.isArray(te)){console.error("❌ Critical error: rowsToDraw is not a valid array:",te),ut(new Error("Invalid product data structure"),"generating PDF","unknown.pdf");return}if(Re>=te.length){const G=y.internal.getNumberOfPages()-1;for(let A=2;A<=G+1;A++){y.setPage(A);const me=(A-2)*Ae,Ne=Math.min(me+Ae,te.length);let Ee=!1;for(let ce=me;ce<Ne;ce++)if(te[ce]&&te[ce].item&&Tt(te[ce].item)){Ee=!0;break}$t(y,{pageWidth:b,colX:le,colW:Q,leftMargin:32,footerHeight:De,logoDataUrl:U,logoNaturalW:H,logoNaturalH:q,headers:ke,userDetails:g,skipWelsHeader:!Ee}),Dt(y,{pageWidth:b,pageHeight:w,leftMargin:32,footerHeight:De,pageNumber:A-1,totalPages:G})}const W=new Date,F=String(W.getDate()).padStart(2,"0"),be=String(W.getMonth()+1).padStart(2,"0"),ne=String(W.getFullYear()).slice(-2),ue=String(W.getHours()).padStart(2,"0"),Se=String(W.getMinutes()).padStart(2,"0"),ae=`${g.project.replace(/[^a-zA-Z0-9\s]/g,"")}-${F}${be}${ne}.${ue}${Se}.pdf`,B=document.getElementById("pdf-processing-notification");B&&B.remove(),go(g.emailCompatible);try{const A=y.output("blob"),z=y.output("string"),me=z?z.match(/\/Type\s*\/XObject/g):null,Ne=z?z.match(/Tj\s/g):null,Ee=z?z.match(/\/A\s*<</g):null;g.pdfSize=A.size;const ce=ao(A,ae);if(g.sendEmail&&A.size>15*1024*1024){console.warn(`❌ PDF too large for email (${(A.size/1024/1024).toFixed(1)}MB), offering email-compatible version`),fo(g,ae);return}const V=ro(A,ce.settings);if(g.sendEmail&&g.email)if(g.exportCsv){const de=ae.replace(/\.pdf$/,".csv");lt(g,de).then(pe=>{window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:g,pdfBlob:V,csvBlob:pe}}))}).catch(pe=>{console.error("Async CSV generation for email failed:",pe),window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:g,pdfBlob:V,csvBlob:null}}))})}else window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:g,pdfBlob:V,csvBlob:null}}));else(async()=>{const de=await vo(V);if(await tt(de,ae,"PDF"),g.exportCsv){const pe=ae.replace(/\.pdf$/,".csv");try{const xe=await lt(g,pe);xe&&await tt(xe,pe,"CSV")}catch(xe){console.error("CSV generation failed:",xe)}}})()}catch(A){console.error("PDF generation failed:",A),ut(A,"generating PDF",ae);const z=document.getElementById("pdf-processing-notification");z&&z.remove()}t&&(t.style.display="none");return}Be>=Ae&&(y.addPage(),at=De+8,Be=0);const D=te[Re];if(!D||!D.item){console.warn(`⚠️  Skipping invalid row at index ${Re}:`,D),Re++,qe();return}const L=at+je*Be;D.isFirstInRoom&&m==="room"&&D.room!=="__all__"&&Bt(y,D.room,D.roomCount,32,L);const J=le[0],ie=J+90+12;rt(y,D.item.Image_URL||"",J,L+Fe+16,90,je-Fe*2,()=>{rt(y,D.item.Diagram_URL||"",ie,L+Fe+16,90,je-Fe*2,()=>{const G=L+28,W=le[1]+Q[1]/2;y.setFontSize(10),y.setTextColor("#222"),y.text(String(D.item.OrderCode||""),W,G+10,{align:"center"}),Ft(y,D.item,W,G+35);const F=Q[2]-10;Nt(y,D.item,le[2],G+10,F,g.excludeLongDescription);const be=ke.indexOf("WELS")+1;if(be>0&&le[be]){const ne=le[be]+Q[be]/2;Mt(y,D.item,ne,G+10)}Ut(y,D.item,le,Q,ke,G+10,{excludePrice:g.excludePrice,includeGst:g.includeGst}),Re++,Be++,qe()})})}qe()})})})}function Xe(g,e){const t=new window.Image;t.crossOrigin="Anonymous",t.onload=function(){const o=document.createElement("canvas"),s=o.getContext("2d"),i=400,n=150;let r=t.width,a=t.height;if(r>i||a>n){const d=i/r,l=n/a,u=Math.min(d,l);r=Math.round(r*u),a=Math.round(a*u)}o.width=r,o.height=a,s.imageSmoothingEnabled=!0,s.imageSmoothingQuality="high",s.drawImage(t,0,0,r,a);const c=o.toDataURL("image/png",.9);e(c,r,a)},t.src=g}function Kt(){if(!document.getElementById("pdf-spinner")){const g=document.createElement("div");if(g.id="pdf-spinner",g.style.display="none",g.style.position="fixed",g.style.top="0",g.style.left="0",g.style.width="100vw",g.style.height="100vh",g.style.zIndex="9999",g.style.background="rgba(255,255,255,0.7)",g.style.alignItems="center",g.style.justifyContent="center",g.innerHTML='<div style="border:6px solid #e0e0e0;border-top:6px solid #2563eb;border-radius:50%;width:54px;height:54px;animation:spin 1s linear infinite;"></div>',document.body.appendChild(g),!document.getElementById("pdf-spinner-style")){const e=document.createElement("style");e.id="pdf-spinner-style",e.innerHTML="@keyframes spin { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);} }",document.head.appendChild(e)}}}async function lt(g,e){return new Promise(async t=>{if(!window.Papa)try{await x.loadScript("https://cdn.jsdelivr.net/npm/papaparse@5.4.1/papaparse.min.js")}catch(n){console.error("Failed to load PapaParse:",n),t(null);return}const o=JSON.parse(localStorage.getItem("selection")||"[]"),s=JSON.parse(localStorage.getItem(M.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let i=[];if(s.length>0?i=s.map(n=>({...n.product,Room:n.room,Notes:n.notes,Quantity:n.quantity,Timestamp:new Date(n.timestamp).toISOString()})):i=o,!i.length){t(null);return}setTimeout(()=>{const n=i.map(r=>{let a,c,d,l;const u=g.excludePrice;let m=0;if(r.UserEditedPrice!==void 0&&r.UserEditedPrice!==null&&r.UserEditedPrice!=="")m=parseFloat(r.UserEditedPrice.toString().replace(/,/g,""));else{const y=r.RRP_EX||r["RRP EX GST"]||r.RRP_EX||r.RRP_EXGST||"";m=parseFloat((y||"0").toString().replace(/,/g,""))}a=m,d="Price ea ex GST",l="Price Total ex GST",c=!isNaN(a)&&a>=0?(a*(r.Quantity||1)).toFixed(2):"";const h=r["WELS STAR"]||r.WELS_STAR||r.WELS_STAR||r.WelsStar||"",p=h&&h.toString().trim()?h.toString().replace(/[^\d.]/g,"").trim():"",f={Code:he(r.OrderCode||""),Description:he(r.Description||""),"WELS Star":he(p),Quantity:r.Quantity||1,Notes:he(r.Notes||""),Room:he(r.Room||""),"Image URL":he(r.Image_URL||""),"Diagram URL":he(r.Diagram_URL||""),"Datasheet URL":he(r.Datasheet_URL||""),"Website URL":he(r.Website_URL||"")};return f[d]=u?"0.00":a>=0?a.toFixed(2):"",f[l]=u?"0.00":c,f});setTimeout(()=>{const r=window.Papa.unparse(n,{quotes:!0,quoteChar:'"',delimiter:",",header:!0,newline:`\r
`,skipEmptyLines:!1,escapeChar:'"',transform:{value(a,c){return typeof a=="string"?a.replace(/\0/g,"").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g,""):a}}});g.sendEmail?setTimeout(()=>{try{const a=btoa(unescape(encodeURIComponent(r)));t({name:e,data:a,contentType:"text/csv",originalSize:r.length,base64Size:a.length})}catch(a){console.error("CSV base64 encoding failed:",a),t(new Blob([r],{type:"text/csv"}))}},0):t(new Blob([r],{type:"text/csv"}))},0)},0)})}function he(g){return typeof g!="string"&&(g=String(g)),g=g.replace(/\0/g,"").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g,"").replace(/\r?\n|\r/g," ").trim(),g}async function Zt(g,e,t="file"){try{if("showSaveFilePicker"in window){const s=await(await window.showSaveFilePicker({suggestedName:e,types:[{description:`${t} files`,accept:{[g.type]:[`.${e.split(".").pop()}`]}}]})).createWritable();return await s.write(g),await s.close(),!0}}catch(o){console.warn("File System Access API failed:",o)}return!1}function eo(g,e,t="file"){try{if(g.size>2*1024*1024)return console.warn("File too large for data URI method"),!1;const o=new FileReader;return o.onload=function(s){try{const i=document.createElement("a");i.href=s.target.result,i.download=e,i.style.display="none",document.body.appendChild(i),i.click(),document.body.removeChild(i)}catch(i){console.error("Data URI download failed:",i)}},o.readAsDataURL(g),!0}catch(o){return console.warn("Data URI method failed:",o),!1}}function to(g,e,t="file"){const o=URL.createObjectURL(g),s=document.createElement("div");s.style.cssText=`
  ...
  `,s.appendChild(i),document.body.appendChild(s),document.getElementById("manual-download-close").onclick=()=>{URL.revokeObjectURL(o),document.body.removeChild(s)},document.getElementById("manual-download-retry").onclick=()=>{URL.revokeObjectURL(o),document.body.removeChild(s),setTimeout(()=>{bt(g,e,t)},1e3)},document.getElementById("copy-url-btn").onclick=()=>{const n=document.getElementById("manual-download-url");n.select(),n.setSelectionRange(0,99999);try{navigator.clipboard.writeText(o).then(()=>{const r=document.getElementById("copy-url-btn");r.textContent="Copied!",r.style.background="#059669",setTimeout(()=>{r.textContent="Copy",r.style.background="#059669"},2e3)}).catch(()=>{document.execCommand("copy");const r=document.getElementById("copy-url-btn");r.textContent="Copied!",setTimeout(()=>r.textContent="Copy",2e3)})}catch{alert("Copy failed. Please select the URL manually and copy it.")}},s.onclick=n=>{n.target===s&&(URL.revokeObjectURL(o),document.body.removeChild(s))},setTimeout(()=>{s.parentElement&&(URL.revokeObjectURL(o),document.body.removeChild(s))},5*60*1e3)}async function bt(g,e,t="file"){try{if(await oo(g,e))return}catch(o){console.warn("Standard download failed:",o)}await Zt(g,e,t)||eo(g,e,t)||to(g,e,t)}function oo(g,e){return new Promise(t=>{try{const o=URL.createObjectURL(g),s=document.createElement("a");s.href=o,s.download=e,s.style.display="none",document.body.appendChild(s);const i=setTimeout(()=>{n(),t(!1)},3e3),n=()=>{clearTimeout(i),s.parentElement&&document.body.removeChild(s),setTimeout(()=>URL.revokeObjectURL(o),1e3)};s.onclick=()=>{n(),t(!0)},s.click(),setTimeout(()=>{n(),t(!0)},500)}catch(o){console.error("Standard download error:",o),t(!1)}})}function so(g){const e=document.createElement("canvas"),t=e.getContext("2d");e.width=Math.min(100,g.width),e.height=Math.min(100,g.height),t.drawImage(g,0,0,e.width,e.height);const s=t.getImageData(0,0,e.width,e.height).data,i=new Set;for(let n=0;n<s.length;n+=4){const r=`${s[n]},${s[n+1]},${s[n+2]}`;i.add(r)}return i.size<1e3}function io(g,e){const o=e.getImageData(0,0,g.width,g.height).data;for(let s=3;s<o.length;s+=4)if(o[s]<255)return!0;return!1}function no(g,e,t){if(g<=t)return{width:g,height:e};const o=e/g;return{width:t,height:Math.round(t*o)}}function St(g){return g>25*1024*1024?{compressionLevel:"aggressive",imageQuality:.6,imageMaxWidth:300,removeImages:!1,usePNG:!0,message:"Aggressive compression - maintaining technical diagram clarity"}:g>20*1024*1024?{compressionLevel:"high",imageQuality:.65,imageMaxWidth:350,removeImages:!1,usePNG:!0,message:"High compression - preserving technical diagram details"}:g>15*1024*1024?{compressionLevel:"medium",imageQuality:.7,imageMaxWidth:400,removeImages:!1,usePNG:!0,message:"Medium compression - optimal for technical documentation"}:g>10*1024*1024?{compressionLevel:"light",imageQuality:.75,imageMaxWidth:450,removeImages:!1,usePNG:!0,message:"Light compression - excellent technical diagram quality"}:{compressionLevel:"minimal",imageQuality:.8,imageMaxWidth:500,removeImages:!1,usePNG:!0,message:"Minimal compression - maximum technical diagram quality"}}function ro(g,e){return g}function ao(g,e){const t=(g.size/1048576).toFixed(2),o=St(g.size);if(g.size>15*1024*1024){console.warn(`Large file detected (${t} MB) - exceeds typical email limit, may need email-compatible version`);const s=document.createElement("div");s.style.cssText=`
    ...
    `,s.innerHTML=`
      ...
      ">OK</button>
    `,document.body.appendChild(s),setTimeout(()=>{s.parentElement&&s.remove()},8e3)}else g.size>3*1024*1024;return{size:g.size,sizeInMB:parseFloat(t),settings:o}}function ut(g,e="",t=""){console.error("Detailed error:",g);const o={type:co(g),message:g.message||"Unknown error",context:e,filename:t,timestamp:new Date().toISOString(),userAgent:navigator.userAgent},s=document.createElement("div");s.style.cssText=`
    position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
  ...
  `,s.appendChild(i),document.body.appendChild(s),document.getElementById("error-close").onclick=()=>{document.body.removeChild(s)},document.getElementById("error-retry").onclick=()=>{document.body.removeChild(s),console.log("Retry requested for:",e)},document.getElementById("error-report").onclick=()=>{po(o),alert("Error details copied to clipboard. Please send this to support.")},s.onclick=n=>{n.target===s&&document.body.removeChild(s)},o}function co(g){var o,s;const e=((o=g.message)==null?void 0:o.toLowerCase())||"",t=((s=g.stack)==null?void 0:s.toLowerCase())||"";return e.includes("network")||e.includes("fetch")?"network":e.includes("permission")||e.includes("denied")?"permission":e.includes("memory")||e.includes("quota")?"memory":e.includes("blob")||e.includes("url")?"download":e.includes("canvas")||e.includes("image")?"rendering":t.includes("jspdf")||e.includes("pdf")?"pdf":"unknown"}function lo(g){return{network:"Network Connection Error",permission:"Permission Required",memory:"Insufficient Memory",download:"Download Failed",rendering:"Display Error",pdf:"PDF Generation Error",unknown:"Unexpected Error"}[g]||"Error Occurred"}function uo(g,e,t){return{network:"Unable to load required resources. Please check your internet connection and try again.",permission:`Browser permission required to save ${t}. Please allow downloads and try again.`,memory:"Not enough memory to process this large file. Try closing other browser tabs or use fewer products.",download:`Failed to download ${t}. This may be due to browser security settings or storage limitations.`,rendering:"Unable to display product images properly. Some images may be missing from the final output.",pdf:`PDF generation failed while ${e}. The file may be too large or contain problematic data.`,unknown:`An unexpected error occurred while ${e}. Please try again or contact support.`}[g]||"An unknown error has occurred."}function mo(g){const e={network:`
      ...
      </div>`};return e[g]||e.unknown}function xt(){const g=navigator.userAgent;return g.includes("Chrome")?"Chrome":g.includes("Firefox")?"Firefox":g.includes("Safari")?"Safari":g.includes("Edge")?"Edge":g.includes("SamsungBrowser")?"Samsung Internet":"Unknown"}function po(g){const e=`
...
========================
  `.trim();try{navigator.clipboard.writeText(e)}catch(t){console.error("Failed to copy error report:",t)}}let oe={totalImages:0,optimizedImages:0,failedImages:0,totalSavings:0};function ho(g){let e=0;if(g.length===0)return e.toString();for(let t=0;t<g.length;t++){const o=g.charCodeAt(t);e=(e<<5)-e+o,e=e&e}return Math.abs(e).toString(36)}function mt(){oe={totalImages:0,optimizedImages:0,failedImages:0,totalSavings:0}}function go(g=!1){oe.totalImages>0}function fo(g,e){const t=document.createElement("div");t.style.cssText=`
    position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
    background: rgba(0,0,0,0.8); z-index: 10001; display: flex; 
    align-items: center; justify-content: center; padding: 20px;
  ...
  `,t.appendChild(o),document.body.appendChild(t),document.getElementById("email-regular-version").onclick=()=>{t.remove();const s=new CustomEvent("sendEmailRegular",{detail:{userDetails:g,originalFilename:e}});window.dispatchEvent(s)},document.getElementById("email-optimized-version").onclick=()=>{t.remove(),g.emailCompatible=!0,ot(g)}}const yo="tipTailSettings";async function vo(g){let e={};if(window._currentTipTailSettings)e=window._currentTipTailSettings,window._currentTipTailSettings=null;else try{e=JSON.parse(localStorage.getItem(yo)||"{}")}catch(a){console.warn("Could not read tipTailSettings from localStorage:",a)}const{tipAsset:t,tipUpload:o,tailAsset:s,tailUpload:i}=e;if(!t&&!o&&!s&&!i)return g;async function n(a,c,d="file"){if(c&&a)try{const l=atob(a),u=new Uint8Array(l.length);for(let m=0;m<l.length;m++)u[m]=l.charCodeAt(m);return u.buffer}catch(l){return console.warn(`⚠️ Error converting base64 to ArrayBuffer for ${d}:`,l),null}if(a)try{const l=await fetch(a);return l.ok?await l.arrayBuffer():(console.warn(`⚠️ Failed to fetch ${d} file: ${a} (${l.status} ${l.statusText})`),null)}catch(l){return console.warn(`⚠️ Error fetching ${d} file: ${a}`,l),null}return null}async function r(a,c="file",d="unknown"){if(!a)return null;try{return await PDFLib.PDFDocument.load(a)}catch(l){return console.warn(`⚠️ Failed to parse ${c} PDF: ${d}`,l),null}}try{const a=await g.arrayBuffer(),c=await PDFLib.PDFDocument.load(a),d=await PDFLib.PDFDocument.create(),[l]=await d.copyPages(c,[0]);d.addPage(l);let u=null,m=null;if(o){const y=await n(o,!0,"tip");y?(u=await r(y,"tip","uploaded file"),u||(m="The uploaded tip file is not a valid PDF or could not be loaded.")):m="Failed to process the uploaded tip file."}else if(t){const y=await n(t,!1,"tip");y?(u=await r(y,"tip",t),u||(m=`The tip file "${t.split("/").pop()}" is not a valid PDF or could not be loaded.`)):m=`The tip file "${t.split("/").pop()}" could not be found or accessed.`}if(u){const y=Array.from({length:u.getPageCount()},(w,v)=>v);(await d.copyPages(u,y)).forEach(w=>d.addPage(w))}else m&&(console.warn(`⚠️ Tip file error: ${m}`),Ye("Tip File Issue",m));if(c.getPageCount()>1){const y=Array.from({length:c.getPageCount()-1},(w,v)=>v+1);(await d.copyPages(c,y)).forEach(w=>d.addPage(w))}let h=null,p=null;if(i){const y=await n(i,!0,"tail");y?(h=await r(y,"tail","uploaded file"),h||(p="The uploaded tail file is not a valid PDF or could not be loaded.")):p="Failed to process the uploaded tail file."}else if(s){const y=await n(s,!1,"tail");y?(h=await r(y,"tail",s),h||(p=`The tail file "${s.split("/").pop()}" is not a valid PDF or could not be loaded.`)):p=`The tail file "${s.split("/").pop()}" could not be found or accessed.`}if(h){const y=Array.from({length:h.getPageCount()},(w,v)=>v);(await d.copyPages(h,y)).forEach(w=>d.addPage(w))}else p&&(console.warn(`⚠️ Tail file error: ${p}`),Ye("Tail File Issue",p));const f=await d.save({useObjectStreams:!0,addDefaultPage:!1,objectsPerTick:20});return new Blob([f],{type:"application/pdf"})}catch(a){return console.error("❌ Error during PDF merging:",a),Ye("PDF Merging Error","An error occurred while merging the PDF files. The main PDF will be generated without tip/tail content."),g}}function Ye(g,e){const t=document.createElement("div");t.style.cssText=`
  ...
  `,document.body.appendChild(t),setTimeout(()=>{t.parentElement&&t.remove()},8e3)}const Je="fredChatHistory",ze="fredChatMessages",Ie="fredFeedback",Pe="fredQuestionLog",Qe=16,X={PROXY_URL:"https://seima-ai-proxy.seima.workers.dev",MODEL:"gpt-4o",MAX_TOKENS:2e3,TEMPERATURE:.2,MAX_CATALOG_RESULTS:60};function Ke(){return{"Content-Type":"application/json",...T.getAuthHeaders()}}const wo=`You are a product specification parser for building/construction projects. Extract ALL product references from the provided text.
...
Return format: [{"code":"...","name":"...","brand":"...","category":"...","quantity":1,"finish":"...","section":"...","specCode":"...","style":"..."}]`;class bo{constructor(){this.conversationHistory=[],this._historyReady=this._loadHistory(),this.isProcessing=!1,this._catalogChecked=!1,this._syncing=!1,this.lastCompetitorProducts=new Map}async chat(e,t,o=null,s=null,i=null){var n,r;if(this.isProcessing)throw new Error("A request is already in progress");this.isProcessing=!0,await this._historyReady,this._catalogChecked||(this._catalogChecked=!0,this._checkCatalogFreshness());try{const a=typeof e=="string"?e:((r=(n=e.find)==null?void 0:n.call(e,d=>d.type==="text"))==null?void 0:r.text)||"";let c;try{c=await this._agentChatStream(a,o,i,s)}catch(d){console.warn("Agent chat failed, falling back to legacy flow:",d.message),c=null}if(c||(c=await this._legacyChat(e,a,t,o,s)),!c)throw new Error("No response from AI");return this.conversationHistory.push({role:"user",content:a||e},{role:"assistant",content:c}),this.conversationHistory.length>Qe&&(this.conversationHistory=this.conversationHistory.slice(-Qe)),await this._saveHistory(),{content:c,usage:null}}finally{this.isProcessing=!1}}async _agentChatStream(e,t,o,s){var r;const i=new AbortController,n=setTimeout(()=>i.abort(),6e4);try{const a=this._deriveIntentProfile(e),c=await fetch(`${X.PROXY_URL}/v1/agent-chat`,{method:"POST",headers:Ke(),body:JSON.stringify({message:e,history:this.conversationHistory.slice(-12),model:X.MODEL,staffMode:T.isStaffMode(),powerUser:T.isPowerUser(),userEmail:((r=T.getCurrentUser())==null?void 0:r.email)||"",intentProfile:a,selectedProducts:(t==null?void 0:t.length)>0?t.map(f=>({OrderCode:f.OrderCode,ProductName:f.ProductName||f.Description,room:f.room})):void 0}),signal:i.signal});if(!c.ok){const f=await c.json().catch(()=>({}));throw new Error(f.details||f.error||`Request failed (${c.status})`)}if((c.headers.get("content-type")||"").includes("application/json")){const f=await c.json();return f.needsSync?(this._ensureCatalogSync(),null):f.content||null}const l=c.body.getReader(),u=new TextDecoder;let m="",h="",p="";for(this.lastCompetitorProducts.clear();;){const{done:f,value:y}=await l.read();if(f)break;h+=u.decode(y,{stream:!0});const b=h.split(`
`);h=b.pop();for(const w of b)if(w.startsWith("event: "))p=w.slice(7).trim();else if(w.startsWith("data: ")){try{const v=JSON.parse(w.slice(6));if(p==="status"&&o)o(v.text);else if(p==="token")m+=v.text,s&&s(v.text);else if(p==="comp_products")for(const S of v.products||[])S.code&&this.lastCompetitorProducts.set(S.code,S);else if(p==="error")throw new Error(v.message||"Agent chat failed")}catch(v){if(v instanceof SyntaxError)continue;throw v}p=""}}return m||null}catch(a){throw a.name==="AbortError"?new Error("Request timed out."):a}finally{clearTimeout(n)}}async _legacyChat(e,t,o,s,i){var l,u,m;const n=[],r=await this._findRelevantProductsAI(t,o),a=this._deriveIntentProfile(t);if(r?n.push({role:"system",content:`SEIMA PRODUCT DATA (${r.count} products):
${r.text}
...
`);return console.log(`PDF text extraction: ${o.numPages} pages, ${s.length} with text, ${i.length} chars total`),i}catch(t){return console.warn("PDF text extraction failed, will use vision API:",t),null}}_fileToBase64(e){return new Promise((t,o)=>{const s=new FileReader;s.onload=()=>t(s.result.split(",")[1]),s.onerror=()=>o(new Error("Failed to read file")),s.readAsDataURL(e)})}async _compressPdf(e,t){if(!window.pdfjsLib)throw new Error("PDF.js must be loaded before compression");if(!window.jspdf){const u=document.createElement("script");u.src="https://cdn.jsdelivr.net/npm/jspdf@2.5.2/dist/jspdf.umd.min.js",document.head.appendChild(u),await new Promise((m,h)=>{u.onload=m,u.onerror=()=>h(new Error("Failed to load jsPDF")),setTimeout(()=>h(new Error("jsPDF load timeout")),1e4)})}const o=await e.arrayBuffer(),s=await window.pdfjsLib.getDocument({data:o}).promise,{jsPDF:i}=window.jspdf,n=1.5,r=.65;let a=null;for(let u=1;u<=s.numPages;u++){t&&t(u,s.numPages);const m=await s.getPage(u),h=m.getViewport({scale:n}),p=document.createElement("canvas");p.width=h.width,p.height=h.height;const f=p.getContext("2d");await m.render({canvasContext:f,viewport:h}).promise;const y=p.toDataURL("image/jpeg",r),b=h.width>h.height?"landscape":"portrait";u===1?a=new i({orientation:b,unit:"px",format:[h.width,h.height]}):a.addPage([h.width,h.height],b),a.addImage(y,"JPEG",0,0,h.width,h.height),p.width=0,p.height=0}const c=a.output("blob"),d=new File([c],e.name,{type:"application/pdf"}),l=Math.round((1-d.size/e.size)*100);return console.log(`PDF compressed: ${(e.size/(1024*1024)).toFixed(1)}MB → ${(d.size/(1024*1024)).toFixed(1)}MB (${l}% reduction)`),d.size<e.size*.9?d:(console.log("Compression did not significantly reduce size, using original"),e)}_showSpecResults(e){const t=e.filter(u=>["verified","seima-own","alternative","catalog-suggestion"].includes(u.status)),o=e.filter(u=>u.status==="unmatched"||u.status==="ai-suggested"),s=[...new Set(e.map(u=>u.section).filter(Boolean))],i=document.getElementById("spec-results-content");if(!i)return;let n="";if(this._projectMeta){const u=this._projectMeta;[u.name,u.architect,u.builder,u.location].filter(Boolean).length>0&&(n=`<div class="spec-project-meta" style="margin-bottom:12px;padding:8px 12px;background:#f0f9ff;border-radius:6px;border:1px solid #bae6fd;">
    ...
    `}_updateImportCount(){const e=document.querySelectorAll(".spec-row-check:checked").length,t=document.getElementById("spec-add-all-btn");t&&(t.textContent=`Import Selected to Grid (${e})`,t.disabled=e===0)}_importSelectedToGrid(){var c,d;if(!this._specResults)return;const e=document.querySelectorAll(".spec-row-check:checked");this.importMode==="override"&&(E.clearAllSelections(),console.log("Cleared all existing data for override mode (spec import)"));const t=new Set;e.forEach(l=>{const u=parseInt(l.dataset.index),m=this._specResults[u];m!=null&&m.section&&t.add(m.section)}),t.forEach(l=>E.addCustomRoom(l));const o=[];e.forEach(l=>{var y,b;const u=parseInt(l.dataset.index),m=this._specResults[u];if(!m)return;const h=this._buildSpecNote(m),p=m.section||"Imported";if((m.status==="seima-own"||m.status==="verified")&&m.seimaProduct){const w=m.seimaProduct.OrderCode||m.seimaProduct["Order Code"]||((b=(y=m.seimaMatches)==null?void 0:y[0])==null?void 0:b.SeimaSKU),v=R.findProductByCode(w);if(v){o.push({product:v,notes:h,room:p,quantity:m.quantity||1});return}}let f=h;if(m.seimaProduct&&m.status!=="seima-own"&&m.status!=="verified"){const w=m.seimaProduct.OrderCode||"",v=m.seimaProduct.ProductName||m.seimaProduct.Description||"";f+=` | SEIMA SUGGESTION: ${w} ${v}`}o.push({product:{OrderCode:"",Description:h,_placeholder:!0},notes:f,room:p,quantity:m.quantity||1})});const s=E.addProductsBatch(o),i=s?o.length:0;s||alert(`Could not save all products — browser storage is full.
...
This can be restored later.`))try{const t=await ve.deleteSelections([e.id]);t.success?(this.showToast("Selection deleted"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to delete")}catch(t){this.showError(t.message)}}async restoreSelection(e){try{const t=await ve.restoreSelections([e.id]);t.success?(this.showToast("Selection restored"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to restore")}catch(t){this.showError(t.message)}}showToast(e){const t=document.createElement("div");t.style.cssText=`
    ...
    `;const a=n.querySelector(".toast-close");if(a.onclick=()=>this.dismiss(i),s&&s.callback){const c=n.querySelector(".toast-action");c.onclick=()=>{s.callback(),this.dismiss(i)}}return this.container.appendChild(n),this.toasts.set(i,n),requestAnimationFrame(()=>{n.classList.add("toast-enter")}),o>0&&setTimeout(()=>this.dismiss(i),o),i}dismiss(e){const t=this.toasts.get(e);t&&(t.classList.add("toast-exit"),t.addEventListener("animationend",()=>{t.remove(),this.toasts.delete(e)}))}dismissAll(){this.toasts.forEach((e,t)=>this.dismiss(t))}success(e,t={}){return this.show({message:e,type:"success",...t})}error(e,t={}){return this.show({message:e,type:"error",duration:6e3,...t})}warning(e,t={}){return this.show({message:e,type:"warning",...t})}info(e,t={}){return this.show({message:e,type:"info",...t})}withUndo(e,t,o=5e3){return this.show({message:e,type:"info",duration:o,action:{label:"Undo",callback:t}})}}const ge=new ko;window.toast=ge;const gt=20;let Ue=0;function ft(g){return{orderCode:String(g["Order Code"]||g.OrderCode||g.Code||"").trim(),name:g.Description||g["Product Name"]||g.ProductName||"",longDescription:g.LongDescription||g["Long Description"]||"",range:g.Range||"",group:g.Group||"",subGroup:g.SubGroup||g["Sub Group"]||"",rrpExGst:g.RRP_EX||g["RRP EX GST"]||"",rrpIncGst:g.RRP_INCGST||g["RRP INC GST"]||"",imageUrl:g.Image_URL||g["Image URL"]||g.imageUrl||"",dimX:g.DimX||g["X Dimension (mm)"]||"",dimY:g.DimY||g["Y Dimension (mm)"]||"",dimZ:g.DimZ||g["Z Dimension (mm)"]||"",welsStar:g.WELS_STAR||g["WELS Star"]||g["WELS STAR"]||"",colour:g.Colour||g.Color||"",_raw:g}}class Ro{constructor(){this._overlay=null,this._dataLayer=null,this._seimaProducts=null,this._seimaByCode={},this._seimaFormatted=[],this._currentCompetitor=null,this._currentCompetitorProduct=null,this._onProductSelected=null,this._expandedCard=null}async open(e,t){if(!this._overlay&&(this._onProductSelected=t||null,await this._ensureSeimaData(),this._render(),e)){const o=this._overlay.querySelector("#uv-comp-search");o&&(o.value=e,this._onCompetitorSearch(e))}}close(){this._overlay&&(this._overlay.remove(),this._overlay=null),this._onProductSelected=null,this._expandedCard=null}async _ensureSeimaData(){if(!this._seimaProducts)try{const{dataLayer:e}=await O(async()=>{const{dataLayer:o}=await import("./data-layer-C5VcJsaC.js").then(s=>s.a);return{dataLayer:o}},__vite__mapDeps([0,1,2,3,4,5,6]));e.isLoaded||await e.init(),this._dataLayer=e;const t=e.products||[];this._seimaProducts=t,this._seimaByCode={},this._seimaFormatted=[];for(const o of t){const s=ft(o),i=s.orderCode;i&&(this._seimaByCode[i]=o),i&&/^\d+/.test(i)&&this._seimaFormatted.push(s)}}catch(e){console.error("UserVerify: failed to load Seima catalog",e),this._seimaProducts=[]}}_render(){const e=T.getCurrentUser(),t=document.createElement("div");t.className="uv-overlay",t.innerHTML=`
      <div class="uv-modal">
        <div class="uv-header">
    ...
    `}_bindCompetitorSearch(){const e=this._overlay.querySelector("#uv-comp-search");if(!e)return;let t;e.addEventListener("input",()=>{clearTimeout(t),t=setTimeout(()=>this._onCompetitorSearch(e.value),250)}),e.addEventListener("focus",()=>{e.value.length>=2&&this._onCompetitorSearch(e.value)})}async _onCompetitorSearch(e){const t=this._overlay.querySelector("#uv-comp-results");if(!t)return;if(!e||e.length<2){t.style.display="none";return}if(await k._ensureIndex(),!k._index){t.style.display="none";return}const o=e.toLowerCase().replace(/[:\-]/g," ").split(/\s+/).filter(Boolean),s=[];for(const i of k._index.searchable)if(o.every(n=>i.searchText.includes(n))&&(s.push(i),s.length>=15))break;if(s.length===0){t.innerHTML='<div class="uv-empty">No competitor products found</div>',t.style.display="block";return}t.innerHTML=s.map((i,n)=>{const r=i.product,a=r.image_url||"",c=r.product_code||"",d=r.product_name||r.collection||"",l=r.product_type||"";return`
        <div class="uv-search-item" data-comp-idx="${n}">
          ...
          <div class="uv-search-item-info">
            ...
            ${l?`<span class="uv-search-item-type">${this._esc(l)}</span>`:""}
          </div>
        </div>
    ...
    `}_renderCompetitorPanel(e,t){const o=e.image_url||"",s=this._extractCompetitorDimensions(e),i=e.features||"",r=[["Name",e.product_name||e.collection||""],["Code",e.product_code],["Brand",t||e.brand],["Category",[e.product_type,e.subcategory].filter(Boolean).join(" / ")],["Collection",e.collection],["Finish",e.finish||e.colour],["Material",e.material],["Dimensions",s],["Style",e.style],["WELS",e.wels_rating]].filter(([,c])=>c&&String(c).trim()).map(([c,d])=>`<tr><td>${this._esc(c)}</td><td>${c==="Name"?`<strong>${this._esc(d)}</strong>`:this._esc(d)}</td></tr>`).join(""),a=i?`<tr><td>Features</td><td class="uv-features-cell">${this._esc(i).replace(/;\s*/g,"<br>")}</td></tr>`:"";return`
    ...
    `}_buildReasons(e){const t=[];return e.finishScore>=60&&t.push("finish match"),e.materialScore>=80?t.push("material match"):e.materialScore===0&&t.push("material mismatch"),t.push(`text: ${e.textScore}%`),t.join(", ")}_renderSeimaCard(e,t={}){const o=e.orderCode,s=e.name,i=e.range||"",n=e.group||"",r=e.subGroup||"",a=e.imageUrl||"assets/no-image.png",c=this._seimaDimensions(e),d=e.rrpIncGst?`$${Number(e.rrpIncGst).toLocaleString("en-AU",{minimumFractionDigits:2})}`:"",l=e.longDescription||"",u=e.welsStar||"",m=this._extractFinishFromSeima(e),h=this._extractMaterialFromSeima(e),p=t.score!=null?`<span class="uv-badge uv-badge-${t.score>=50?"high":t.score>=30?"med":"low"}">${t.score}%</span>`:"",f=[o,i,r||n].filter(Boolean).join(" · "),b=[m,h,c].filter(Boolean).join(" · "),w=l?l.length>120?l.slice(0,120)+"…":l:"";return`
      <div class="uv-match-card" data-seima-code="${this._esc(o)}">
        ...
        <div class="uv-match-body">
          ...
          <div class="uv-match-meta">${this._esc(f)} ${p}</div>
          ${b?`<div class="uv-match-spec">${this._esc(b)}</div>`:""}
        ...
        <div class="uv-match-right">
          <div class="uv-match-price">${d}</div>
    ...
    `,n.querySelector(".uv-confirm-cancel").addEventListener("click",()=>n.remove()),n.querySelector(".uv-confirm-done").addEventListener("click",()=>{this._submitAndClose(e)}),n.addEventListener("click",a=>{a.target===n&&n.remove()}),t.appendChild(n)}async _submitAndClose(e){if(!this._currentCompetitor||!this._currentCompetitorProduct)return;const t=T.getCurrentUser();if(!(t!=null&&t.email)){this._showToast("Please sign in to submit.","error");return}if(Ue>=gt){this._showToast("Session limit reached. Thank you for your contributions!","error");return}const o=this._currentCompetitor,s=String(this._currentCompetitorProduct.product_code||""),i=this._seimaByCode[e],n=this._onProductSelected;i&&n&&n(i),this.close(),this._showToast("Thanks! Your suggestion has been submitted for review.");try{await k.submitUserVerification(o,s,e,t.email,""),Ue++}catch(r){console.error("UserVerify: submit failed",r),this._showToast("Submission failed — please try again later.","error")}}async _submitNoEquivalent(){if(!this._currentCompetitor||!this._currentCompetitorProduct)return;const e=T.getCurrentUser();if(!(e!=null&&e.email)){this._showToast("Please sign in to submit.","error");return}if(Ue>=gt){this._showToast("Session limit reached. Thank you for your contributions!","error");return}const t=String(this._currentCompetitorProduct.product_code||"");try{await k.submitUserVerification(this._currentCompetitor,t,"NO_EQUIVALENT",e.email,"User indicated no Seima equivalent exists"),Ue++,this.close(),this._showToast("Thanks! We've noted that this product has no Seima equivalent.")}catch(o){console.error("UserVerify: no-equivalent submit failed",o),this._showToast("Failed to submit. Please try again.","error")}}_quickMatch(e,t=6){if(!this._seimaFormatted||this._seimaFormatted.length===0)return[];const o=(e.product_type||e.subcategory||"").toLowerCase(),s=this._normalizeFinish(e.finish||e.colour||""),i=parseFloat(e.rrp_ex_gst||e.rrp||e.price)||0,n=(e.material||"").toLowerCase(),r=this._tokenize(`${e.product_name||""} ${e.product_type||""} ${e.collection||""} ${e.finish||""} ${e.material||""}`),a=[Ot(e.product_name),o].filter(Boolean);let c;for(const m of a)if(c=zt[m.toLowerCase()],c)break;const d=[];for(const m of this._seimaFormatted){if(c){const _=`${m.subGroup} ${m.group} ${m.range} ${m.name}`.toUpperCase();if(!c.some(I=>_.includes(I)))continue}const h=[m.subGroup,m.group,m.range,m.name,m.longDescription].filter(Boolean).join(" ").toLowerCase(),p=this._tokenize(h);let f=0;for(const _ of r)p.includes(_)&&f++;const y=r.length>0?f/r.length*100:0,b=this._normalizeFinish(`${m.group||""} ${m.name||""}`),w=s&&b&&s===b?100:s&&b&&(s.includes(b)||b.includes(s))?60:0;let v=50;if(n){const _=(m.longDescription||"").toLowerCase();_.includes(n)?v=100:n.split(/\s+/).some(I=>I.length>=4&&_.includes(I))?v=60:v=0}const S=parseFloat(m.rrpExGst)||0;let P=50;if(i>0&&S>0){const _=Math.min(i,S)/Math.max(i,S);P=_>=.8?100:_>=.6?70:_>=.4?40:10}const C=Math.round(y*.35+w*.25+v*.2+P*.2);C<15||d.push({seima:m,score:C,textScore:Math.round(y),finishScore:w,materialScore:v,priceScore:P})}d.sort((m,h)=>h.score-m.score);const l=new Set,u=[];for(const m of d)if(!l.has(m.seima.orderCode)&&(l.add(m.seima.orderCode),u.push(m),u.length>=t))break;return u}_normalizeFinish(e){const t=(e||"").toLowerCase(),o={chrome:["chrome"," cr",",cr","cp","polished chrome"],"matte black":["matte black","matt black"," mb",",mb","mblk","black"],"brushed nickel":["brushed nickel"," bn",",bn","satin nickel","nickel"],"brushed brass":["brushed brass"," bb",",bb","brass","warm brass"],"brushed gold":["brushed gold"," bg",",bg","gold","light gold"],"gun metal":["gun metal","gunmetal"," gm",",gm"],white:["white"," wh",",wh","gloss white"],"stainless steel":["stainless steel"," ss",",ss","stainless"]};for(const[s,i]of Object.entries(o))if(i.some(n=>t.includes(n)))return s;return t.trim()}_tokenize(e){return(e||"").toLowerCase().replace(/[^a-z0-9\s]/g," ").split(/\s+/).filter(t=>t.length>=2)}_showToast(e,t="success"){const o=document.querySelector(".uv-toast");o&&o.remove();const s=document.createElement("div");s.className=`uv-toast uv-toast-${t}`,s.textContent=e,document.body.appendChild(s),setTimeout(()=>s.classList.add("uv-toast-visible"),10),setTimeout(()=>{s.classList.remove("uv-toast-visible"),setTimeout(()=>s.remove(),300)},3500)}_extractCompetitorDimensions(e){if(!e)return"";if(e.dimensions_mm)return e.dimensions_mm;let t=0,o=0,s=0;for(const i of Object.keys(e)){const n=i.toLowerCase();n.includes("width")&&!t&&(t=parseFloat(e[i])||0),n.includes("depth")&&!o&&(o=parseFloat(e[i])||0),n.includes("height")&&!s&&(s=parseFloat(e[i])||0)}if(t>0||o>0||s>0){const i=[];return t&&i.push(t),o&&i.push(o),s&&i.push(s),i.join(" × ")+"mm"}return""}_esc(e){const t=document.createElement("div");return t.textContent=e||"",t.innerHTML}}const Te=new Ro;class Io{constructor(){this.activeDropdown=null,this.updatePositionHandler=null,document.addEventListener("click",e=>{e.target.closest(".global-search-dropdown")||this.hideDropdown()})}showDropdown(e,t,o,s=!1){this.hideDropdown();const i=document.createElement("ul");i.className="global-search-dropdown";const n=300,r=e.getBoundingClientRect(),a=window.innerWidth,c=r.width;let d=r.left;d+c>a-8&&(d=a-c-8),d<8&&(d=8);const l={position:"fixed",top:"0px",left:`${d}px`,width:`${c}px`,minWidth:`${c}px`,maxWidth:`${c}px`,background:"#fff",border:"1px solid #d1d5db",borderRadius:"8px",boxShadow:"0 10px 25px -5px rgba(0, 0, 0, 0.15), 0 8px 10px -6px rgba(0, 0, 0, 0.10)",maxHeight:`${n}px`,overflowY:"auto",overflowX:"hidden",zIndex:"10010",listStyle:"none",margin:"0",padding:"4px 0",whiteSpace:"normal",wordWrap:"break-word",display:"block",pointerEvents:"auto",transform:"none",contain:"none",isolation:"isolate",visibility:"hidden"};if(Object.keys(l).forEach(u=>{i.style.setProperty(u,l[u],"important")}),i.maxDropdownHeight=n,t.length===0?i.innerHTML='<li style="padding: 12px 16px; color: #6b7280; font-style: italic; background: #fff;">No products found</li>':i.innerHTML=t.map(u=>{if(u._crosshairValidate){const v=u._crosshairValidate,S=v.imageUrl?`<img src="${x.sanitizeInput(v.imageUrl)}" alt="" style="width:32px; height:32px; object-fit:contain; border-radius:4px; margin-right:10px; vertical-align:middle; flex-shrink:0;">`:"";return`<li data-validate-competitor="${x.sanitizeInput(v.competitor)}" data-validate-code="${x.sanitizeInput(v.productCode)}"
            ...
            <div style="flex:1; min-width:0;">
              <div style="font-weight: 600; color: #92400e;">${x.sanitizeInput(v.productName)}</div>
              <div style="color: #78716c; font-size: 11px;">${x.sanitizeInput(v.competitor)} · #${x.sanitizeInput(v.productCode)}${v.finish?" · "+x.sanitizeInput(v.finish):""} · ${v.matchCount>0?v.matchCount+" suggested match"+(v.matchCount!==1?"es":""):"no Seima match yet"}</div>
            </div>
            <span style="color: #d97706; font-size: 11px; font-weight: 600; white-space: nowrap; margin-left: 8px;">Review →</span>
          </li>`}const m=u.OrderCode||u.Code||"",h=u.Description||u.ProductName||u["Product Name"]||"",p=u.Image_URL||u.image_url||"",f=u._crosshairMatch,y=f?`<span style="display: inline-block; background: ${f.status==="Verified"?"#e8f5e9":"#e3f2fd"}; color: ${f.status==="Verified"?"#2e7d32":"#1565c0"}; font-size: 10px; font-weight: 600; padding: 1px 6px; border-radius: 8px; margin-left: 8px;">↔ ${x.sanitizeInput(f.competitorLabel||f.competitor)}</span>`:"",b=p?`<img src="${x.sanitizeInput(p)}" alt="" style="width:36px; height:36px; object-fit:contain; border-radius:4px; margin-right:10px; flex-shrink:0;" onerror="this.style.display='none'">`:"",w=f?Object.fromEntries(Object.entries(u).filter(([v])=>v!=="_crosshairMatch")):u;return`<li data-product='${JSON.stringify(w).replace(/'/g,"&apos;")}'
      ...
      `,s.style.display="flex"),!k.isEnabled())try{await k.preload()}catch{}if(e===this._matchSuggestionsRunId){if(!k.isEnabled()){s&&(s.innerHTML=`
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="flex-shrink:0">
        ...
        `,setTimeout(()=>{s&&(s.style.display="none")},8e3));return}for(const d of t){if(e!==this._matchSuggestionsRunId)return;const l=d.dataset.rowId,u=this.gridRows.find(C=>C.id===l);if(!((n=u==null?void 0:u.product)!=null&&n._placeholder))continue;const m=u.product.Description||"",h=this.extractCandidateCodes(m);if(h.length===0)continue;const p=d.closest(".grid-product-cell"),f=p==null?void 0:p.querySelector(".grid-placeholder-hint"),y=[];for(const C of h)try{const _=await k.findCompetitorEntryByCode(C);if(e!==this._matchSuggestionsRunId)return;if(_){const I=_.matchedCode??((r=_.competitorProduct)==null?void 0:r.product_code)??C;y.push({code:C,competitor:_.competitor,searchCode:I})}else console.log(`Crosshair: no competitor match for code "${C}" (row ${l})`)}catch{}if(f&&y.length>0&&(f.innerHTML=this.buildPlaceholderHintHtml(m,y)),!R.isLoaded)continue;let b=null,w="";for(const C of h)try{const _=await k.findSeimaMatches(C);if(e!==this._matchSuggestionsRunId)return;if(((a=_==null?void 0:_.matches)==null?void 0:a.length)>0){const I=String(_.matches[0].SeimaSKU).trim(),$=R.findProductByCode(I);if($){b=$;const H=(_.competitorProduct||{}).product_name||$.Description||"";w=`${_.competitor}: ${C} → ${I}${H?" – "+H:""}`;break}}}catch{}if((c=u==null?void 0:u.product)!=null&&c._placeholder&&s){i++;const C=Math.round(i/o.length*100),_=s.querySelector("span");_&&(_.textContent=`Cross-referencing ${i}/${o.length} products (${C}%)...`)}if(!b||d.querySelector(".grid-seima-match-option"))continue;const v=document.createElement("button");v.type="button",v.className="grid-seima-match-btn grid-seima-match-option";const S=(b.Description||"").trim(),P=S.length>45?S.slice(0,45)+"…":S;v.textContent=`Matched Seima: ${b.OrderCode||""} – ${P}`,v.title=w,v.addEventListener("click",()=>{this._applyVerifiedSeimaToRow(l,b.OrderCode||b.Code||"")}),d.appendChild(v)}if(e===this._matchSuggestionsRunId&&s){const d=document.querySelectorAll(".grid-seima-match-option").length;s.innerHTML=`
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="flex-shrink:0">
    ...
    `,document.body.appendChild(s),document.body.appendChild(i);const n=()=>this.closeValidatorModal(o);i.querySelector(".gvm-close").addEventListener("click",n),s.addEventListener("click",n);const r=a=>{a.key==="Escape"&&n()};document.addEventListener("keydown",r),this._validatorModal={overlay:s,modal:i,escHandler:r,competitor:e,productCode:t,rowId:o};try{await k._ensureIndex();const a=await k.getMatches(e,!0),d=(await k.getProducts(e)).find(p=>String(p.product_code)===String(t));if(!d){i.querySelector(".gvm-body").innerHTML=`<div class="gvm-no-matches">Product ${x.sanitizeInput(t)} not found in ${x.sanitizeInput(e)} data.</div>`;return}const l=a.filter(p=>String(p.CompetitorSKU)===String(t)).sort((p,f)=>(p.Rank||99)-(f.Rank||99)),u={Verified:0,Manual:1,Rejected:2,"AI-Suggested":3},m=new Map;for(const p of l){const f=String(p.SeimaSKU),y=m.get(f);(!y||(u[p.Status]??9)<(u[y.Status]??9))&&m.set(f,p)}const h=[...m.values()].sort((p,f)=>(p.Rank||99)-(f.Rank||99));this._renderValidatorModal(d,h,e)}catch(a){console.error("Validator modal load failed:",a),i.querySelector(".gvm-body").innerHTML='<div class="gvm-no-matches">Failed to load data. Check console.</div>'}}_renderValidatorModal(e,t,o){var u;const s=(u=this._validatorModal)==null?void 0:u.modal;if(!s)return;const i=e.product_name||e.product_code,n=e.image_url||e.Image||"",r=m=>x.escapeHtml(m||"");s.querySelector(".gvm-title").textContent=`${i} — ${o}`;let a;t.length===0?a='<div class="gvm-no-matches">No Seima matches found. Use the search below to add one.</div>':a=t.map(m=>{const h=String(m.SeimaSKU||"").trim(),p=R.findProductByCode(h),f=(p==null?void 0:p.Description)||(p==null?void 0:p.ProductName)||h,y=(p==null?void 0:p.Image_URL)||(p==null?void 0:p.imageUrl)||"",b=(p==null?void 0:p.RRP_INCGST)||(p==null?void 0:p["RRP INC GST"])||"",w=b?`$${Number(b).toLocaleString("en-AU",{minimumFractionDigits:2})}`:"",v=Number(m.Confidence)||0,S=v>=70?"high":v>=40?"med":"low",P=(m.Status||"").toLowerCase().replace(/[\s-]+/g,"-");let C;return m.Status==="Verified"||m.Status==="Manual"?C=`<span class="gvm-status-label gvm-status-verified">✓ Verified</span>
            ...
            <button class="gvm-btn gvm-btn-reject" data-action="reject" data-sku="${r(h)}">✗ Reject</button>`,`<div class="gvm-match-card status-${P}" data-seima-sku="${r(h)}" style="cursor:pointer;">
          ...
          <div class="gvm-match-info">
            ...
            <div class="gvm-match-sku">${r(h)} <span class="gvm-badge gvm-badge-${S}">${v}%</span></div>
        ...
        </div>`}).join("");const c=e.rrp_ex_gst?`$${Number(e.rrp_ex_gst).toLocaleString("en-AU",{minimumFractionDigits:2})}`:"",l=[["Code",e.product_code],["Brand",e.brand||o],["Category",[e.product_type,e.subcategory].filter(Boolean).join(" / ")],["Collection",e.collection],["Finish",e.finish||e.colour],["RRP",c]].filter(([,m])=>m&&String(m).trim()).map(([m,h])=>`<tr><td>${r(m)}</td><td>${r(h)}</td></tr>`).join("");s.querySelector(".gvm-body").innerHTML=`
    ...
    `,this._wireValidatorModalActions(e,t,o)}_wireValidatorModalActions(e,t,o){var a;const s=(a=this._validatorModal)==null?void 0:a.modal;if(!s)return;s.querySelectorAll(".gvm-match-card").forEach(c=>{c.addEventListener("click",d=>{if(d.target.closest(".gvm-btn"))return;const l=c.dataset.seimaSku;l&&this._showSeimaDetailInModal(l)})}),s.querySelectorAll(".gvm-btn-verify, .gvm-btn-reject, .gvm-btn-undo").forEach(c=>{c.addEventListener("click",async d=>{var h;d.stopPropagation();const l=c.dataset.sku,u=c.dataset.action,m=u==="verify"?"Verified":"Rejected";c.disabled=!0,c.textContent="…";try{await k.updateMatch(o,String(e.product_code),l,m);const p=t.find(f=>String(f.SeimaSKU)===l);if(p&&(p.Status=m),m==="Verified"&&((h=this._validatorModal)!=null&&h.rowId)){this._applyVerifiedSeimaToRow(this._validatorModal.rowId,l),this.closeValidatorModal(this._validatorModal.rowId);return}this._renderValidatorModal(e,t,o)}catch(p){console.error("Match update failed:",p),c.disabled=!1,c.textContent=u==="verify"?"✓ Verify":"✗ Reject"}})});const i=s.querySelector(".gvm-search-add-input"),n=s.querySelector(".gvm-search-results");if(!i||!n)return;let r;i.addEventListener("input",()=>{clearTimeout(r),r=setTimeout(()=>{this._renderValidatorSeimaSearch(i.value,n,e,t,o)},250)})}_renderValidatorSeimaSearch(e,t,o,s,i){if(!e||e.length<2){t.innerHTML="";return}const n=R.searchProducts(e,20),r=new Set(s.map(d=>String(d.SeimaSKU))),a=n.filter(d=>{const l=String(d.OrderCode||d.Code||"");return l&&!r.has(l)});if(a.length===0){t.innerHTML='<div class="gvm-no-matches">No Seima products found.</div>';return}const c=d=>x.sanitizeInput(d||"");t.innerHTML=a.slice(0,15).map(d=>{const l=d.OrderCode||d.Code||"",u=d.Description||d.ProductName||"",m=d.Image_URL||d.imageUrl||"",h=d.RRP_INCGST||d["RRP INC GST"]||"",p=h?`$${Number(h).toLocaleString("en-AU",{minimumFractionDigits:2})}`:"";return`<div class="gvm-search-result" data-seima-code="${c(l)}">
      ...
      </div>`}).join(""),t.querySelectorAll(".gvm-search-result").forEach(d=>{const l=d.querySelector(".gvm-btn-verify");l==null||l.addEventListener("click",async u=>{var h;u.stopPropagation();const m=l.dataset.seimaCode;l.disabled=!0,l.textContent="…";try{const p=s.length+1;if(await k.addMatch(i,String(o.product_code),m,p,"Manual verification"),await k.updateMatch(i,String(o.product_code),m,"Verified"),s.push({CompetitorSKU:String(o.product_code),SeimaSKU:m,Rank:p,Confidence:"",MatchReason:"Manual verification",Status:"Verified"}),(h=this._validatorModal)!=null&&h.rowId){this._applyVerifiedSeimaToRow(this._validatorModal.rowId,m),this.closeValidatorModal(this._validatorModal.rowId);return}this._renderValidatorModal(o,s,i)}catch(p){console.error("Add match failed:",p),l.disabled=!1,l.textContent="✓ Verify"}})})}closeValidatorModal(e){this._validatorModal&&(this._closeSeimaDetailPanel(),this._validatorModal.overlay.remove(),this._validatorModal.modal.remove(),this._validatorModal.escHandler&&document.removeEventListener("keydown",this._validatorModal.escHandler),this._validatorModal=null,k._index=null,this.attachSeimaMatchSuggestions())}async openPlaceholderSearch(e){var n,r,a;const t=this.gridRows.find(c=>c.id===e);if(!((n=t==null?void 0:t.product)!=null&&n._placeholder))return;const o=String(t.product.Description||""),s=this.extractCandidateCodes(o);if(k.isEnabled()&&s.length>0)try{await k.preload()}catch{}for(const c of s)try{const d=await k.findCompetitorEntryByCode(c);if(!d)continue;const l=d.matchedCode??((r=d.competitorProduct)==null?void 0:r.product_code)??c;k.isWriteEnabled()?await this.openValidatorModal(d.competitor,String(l),e):Te.open(`${d.competitor}: ${l}`,u=>{const m=(u==null?void 0:u.OrderCode)||(u==null?void 0:u.Code)||"";m&&this._applyVerifiedSeimaToRow(e,m)});return}catch{}const i=[];for(const c of s.slice(0,3))try{const d=await k.findSeimaMatches(c);if((a=d==null?void 0:d.matches)!=null&&a.length)for(const l of d.matches){const u=String(l.SeimaSKU||"").trim();u&&!i.includes(u)&&i.push(u)}}catch{}this.openSeimaSearchModal(e,o,i,s)}openSeimaSearchModal(e,t,o=[],s=[]){var b,w;this.closeSeimaSearchModal();const i=v=>x.sanitizeInput(v||""),n=this._derivePlaceholderSearchQuery(t),r=document.createElement("div");r.className="grid-validator-overlay";const a=document.createElement("div");a.className="grid-validator-modal";const c=o.map(v=>{const S=R.findProductByCode(v);if(!S)return"";const P=S.Description||S.ProductName||v;return`
          <button type="button" class="gvm-search-result gvm-search-result-pick" data-seima-code="${i(v)}">
            <div class="gvm-match-info">
              <div class="gvm-match-name">${i(P)}</div>
    ...
    `,document.body.appendChild(r),document.body.appendChild(a);const d=()=>this.closeSeimaSearchModal();(b=a.querySelector(".gvm-close"))==null||b.addEventListener("click",d),r.addEventListener("click",d);const l=v=>{v.key==="Escape"&&d()};document.addEventListener("keydown",l),this._seimaSearchModal={overlay:r,modal:a,escHandler:l,rowId:e},a.querySelectorAll(".gvm-search-result-pick").forEach(v=>{v.addEventListener("click",async()=>{const S=v.dataset.seimaCode||"";S&&this._applyVerifiedSeimaToRow(e,S)&&(this.closeSeimaSearchModal(),await this._openVerificationFlowFromPlaceholder(e,t,s,S))})});const u=a.querySelector(".gvm-search-add-input"),m=a.querySelector(".gvm-search-add .gvm-search-results");if(!u||!m)return;(w=a.querySelector('[data-action="web-image-search"]'))==null||w.addEventListener("click",()=>{this._openExternalImageSearch(n||t||"")});let h;const p=a.querySelector(".gvm-explorer-category"),f=a.querySelector(".gvm-explorer-group"),y={query:n||"",category:"",group:""};this._populateModalCategoryOptions(p),this._populateModalGroupOptions(f,""),p==null||p.addEventListener("change",()=>{y.category=p.value,y.group="",f&&(f.value=""),this._populateModalGroupOptions(f,y.category),this._renderStandaloneSeimaSearch(y,m,e,t,s)}),f==null||f.addEventListener("change",()=>{y.group=f.value,this._renderStandaloneSeimaSearch(y,m,e,t,s)}),u.addEventListener("input",()=>{clearTimeout(h),h=setTimeout(()=>{y.query=u.value,this._renderStandaloneSeimaSearch(y,m,e,t,s)},200)}),this._renderStandaloneSeimaSearch(y,m,e,t,s),u.focus()}_renderStandaloneSeimaSearch(e,t,o,s="",i=[]){const n=String((e==null?void 0:e.query)||"").trim(),r=String((e==null?void 0:e.category)||"").trim(),a=String((e==null?void 0:e.group)||"").trim(),d=(n.length>=2?R.searchProducts(n,300):R.getAllProducts()).filter(u=>{const m=String(u.SubGroup||u.Subgroup||u["Sub Group"]||"").trim(),h=String(u.Group||"").trim();return!(r&&m!==r||a&&h!==a)});if(d.length===0){t.innerHTML=`
    ...
    `,document.body.appendChild(c),document.body.appendChild(d);const l=()=>this._closeSeimaDetailPanel();d.querySelector(".gvm-close").addEventListener("click",l),c.addEventListener("click",l),this._seimaDetailPanel={overlay:c,panel:d}}_closeSeimaDetailPanel(){this._seimaDetailPanel&&(this._seimaDetailPanel.overlay.remove(),this._seimaDetailPanel.panel.remove(),this._seimaDetailPanel=null)}_showProductDetailModal(e){const t=document.querySelector(".grid-product-modal-overlay");t&&t.remove();const o=q=>x.sanitizeInput(q||""),s=e.OrderCode||e.Code||"",i=s?String(parseInt(s,10)):"",n=e.Description||e.ProductName||e["Product Name"]||"",r=e.Image_URL||e.imageUrl||e.Image||"assets/no-image.png",a=e["Long Description"]||e.LongDescription||"",c=e.Range||"",d=e.SubGroup||e.Subgroup||e["Sub Group"]||"",l=e.Finish||"",u=e.Colour||"",m=e.DimX||e["X Dimension (mm)"]||"",h=e.DimY||e["Y Dimension (mm)"]||"",p=e.DimZ||e["Z Dimension (mm)"]||"",f=m&&m!=="0"?`${m} × ${h||0} × ${p||0}mm`:"",y=e.WELS_STAR||e["WELS Star"]||e["WELS STAR"]||"",b=e.RRP_EX||e["RRP EX GST"]||e.RRP_EXGST||"",w=e.RRP_INCGST||e["RRP INC GST"]||"",v=[];i&&v.push(["Order code",i]),c&&v.push(["Range",c]),d&&v.push(["Type",d]),f&&v.push(["Dimensions",f]),l&&v.push(["Finish",l]),u&&u!==l&&v.push(["Colour",u]),y&&v.push(["WELS",`${y} star`]);const S=b?`$${parseFloat(b).toLocaleString("en-AU",{minimumFractionDigits:2})} ex GST`:w?`$${parseFloat(w).toLocaleString("en-AU",{minimumFractionDigits:2})} inc GST`:"",P=e.Diagram_URL||e["Diagram URL"]||"",C=e.Datasheet_URL||e["Datasheet URL"]||"",_=e.Website_URL||e["Website URL"]||"",I=[];P&&I.push(`<a href="${o(P)}" target="_blank" rel="noopener" class="grid-pdm-link">Diagram</a>`),C&&I.push(`<a href="${o(C)}" target="_blank" rel="noopener" class="grid-pdm-link">Datasheet</a>`),_&&I.push(`<a href="${o(_)}" target="_blank" rel="noopener" class="grid-pdm-link">View on Website</a>`);const $=document.createElement("div");$.className="grid-product-modal-overlay",$.innerHTML=`
    ...
    `;this.injectSaveDialogStyles(),document.body.insertAdjacentHTML("beforeend",i);const n=document.getElementById("save-dialog"),r=document.getElementById("save-doc-name"),a=document.getElementById("save-notes");r==null||r.focus(),r==null||r.select(),n.querySelectorAll("button[data-action]").forEach(d=>{d.addEventListener("click",async()=>{const l=d.dataset.action;if(l==="cancel"){n.remove();return}const u=(r==null?void 0:r.value.trim())||"Untitled Selection",m=(a==null?void 0:a.value.trim())||"";this.currentSelectionName=u;const h={...this.prepareSelectionData(),documentName:u,notes:m};n.querySelectorAll("button").forEach(p=>p.disabled=!0),d.textContent="Saving...";try{let p;l==="save-update"?p=await we.updateSelection(this.currentSelectionId,h):(p=await we.saveSelection(h),p.success&&p.id&&(this.currentSelectionId=p.id)),n.remove(),p.success?(this.hasUnsavedChanges=!1,this.lastSaveTime=new Date,this.updateContextHeader(),ge.success(l==="save-update"?"Selection updated!":"Selection saved!")):ge.error("Failed to save: "+(p.error||"Unknown error"))}catch(p){n.remove(),ge.error("Failed to save: "+p.message)}})}),n.addEventListener("click",d=>{d.target===n&&n.remove()});const c=d=>{d.key==="Escape"&&(n.remove(),document.removeEventListener("keydown",c))};document.addEventListener("keydown",c)}injectSaveDialogStyles(){if(document.getElementById("save-dialog-styles"))return;document.head.insertAdjacentHTML("beforeend",`
      <style id="save-dialog-styles">
        .save-dialog-overlay {
          position: fixed;
          top: 0;
    ...
    `,document.head.appendChild(e)}getSteps(){return[{icon:"logo",title:"Welcome to Product Presenter",text:"Create beautiful PDF presentations of Seima products for your clients in minutes.",features:[]},{icon:"📦",title:"Add Your Products",text:"There are three ways to get started:",features:[{icon:"📁",title:"Import a File",desc:"Upload CSV or Excel files with product codes"},{icon:"📂",title:"Load a Selection",desc:"Continue from a previous saved selection"},{icon:"🔍",title:"Search Products",desc:"Search and add products one by one"}]},{icon:"🏠",title:"Organise by Room",text:"Group products by room or area. Drag to reorder, and easily manage your selection.",features:[{icon:"🎨",title:"Colour-coded",desc:"Rooms are visually distinct for quick reference"},{icon:"📊",title:"Sort Options",desc:"Sort by Room/Group, Product Code, or Product Name"},{icon:"💾",title:"Auto-saves",desc:"Your work is automatically preserved"}]},{icon:"📄",title:"Create Your PDF",text:'Click "Create PDF" to customise and generate a professional presentation with your branding.',features:[{icon:"💰",title:"Pricing Options",desc:"Show RRP, add GST, or hide pricing entirely"},{icon:"📝",title:"Content Control",desc:"Include descriptions and custom notes"},{icon:"📑",title:"Cover Pages",desc:"Add branded cover and appendix pages"}]}]}renderStep(){const e=this.getSteps(),t=e[this.currentStep],o=document.getElementById("onboarding-content"),s=document.getElementById("onboarding-dots"),i=document.getElementById("onboarding-next");if(!o||!s)return;let n="";t.features.length>0&&(n=t.features.map(a=>`
      ...
      ${n}
    `,s.innerHTML=e.map((a,c)=>`<div class="onboarding-dot ${c===this.currentStep?"active":""}"></div>`).join(""),i&&(i.textContent=this.currentStep===e.length-1?"Get Started":"Next")}nextStep(){const e=this.getSteps();this.currentStep<e.length-1?(this.currentStep++,this.renderStep()):this.complete()}complete(){localStorage.setItem(et,JSON.stringify({version:yt,completedAt:new Date().toISOString()})),this.overlay&&(this.overlay.style.animation="fadeIn 0.2s ease reverse",setTimeout(()=>{this.overlay.remove(),this.overlay=null},200))}reset(){localStorage.removeItem(et)}}const Ct=new To;class $o{constructor(){this.panel=null,this.messagesContainer=null,this.input=null,this.sendBtn=null,this.isOpen=!1,this._lastUserMessage=null,this._conversationId=null,this._userEmail=null,this._historyOpen=!1}init(){this._createPanel(),this._bindEvents(),this._restoreMessages(),this._initUserEmail()}_createPanel(){const e=document.createElement("div");e.id="ai-chat-panel",e.className="ai-chat-panel",e.innerHTML=`
...
`).filter(l=>l.trim());if(s.length<2)return o;const i=l=>l.split("|").slice(1,-1).map(u=>u.trim()),n=l=>/^\|[\s:|-]+\|$/.test(l.trim());let r=i(s[0]),a=1;s.length>1&&n(s[1])&&(a=2);let c="<thead><tr>"+r.map(l=>`<th>${l}</th>`).join("")+"</tr></thead>",d="<tbody>"+s.slice(a).filter(l=>!n(l)).map(l=>"<tr>"+i(l).map(u=>`<td>${u}</td>`).join("")+"</tr>").join("")+"</tbody>";return`<table class="ai-comparison-table">${c}${d}</table>`}),t=t.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),t=t.replace(/`([^`]+)`/g,"<code>$1</code>"),t=t.replace(/^### (.+)$/gm,"<h4>$1</h4>"),t=t.replace(/^## (.+)$/gm,"<h3>$1</h3>"),t=t.replace(/^- (.+)$/gm,"<li>$1</li>"),t=t.replace(/(<li>.*<\/li>\n?)+/g,o=>`<ul>${o}</ul>`),t=t.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),t=t.replace(/\n\n/g,"</p><p>"),t=`<p>${t}</p>`,t=t.replace(/<p><\/p>/g,""),t=t.replace(/<p>(<h[34]>)/g,"$1"),t=t.replace(/(<\/h[34]>)<\/p>/g,"$1"),t=t.replace(/<p>(<ul>)/g,"$1"),t=t.replace(/(<\/ul>)<\/p>/g,"$1"),t=t.replace(/<p>(<table)/g,"$1"),t=t.replace(/(<\/table>)<\/p>/g,"$1"),t=t.replace(/\b(\d{6})\b/g,'<button class="ai-add-product-btn" data-code="$1" title="Add $1 to selection">$1 <span class="ai-add-icon">+</span></button>'),t}_getBasePath(){var e;return(((e=document.querySelector("base"))==null?void 0:e.href)||window.location.origin+"/").replace(/\/$/,"")}_getProductImage(e){return e.Image_URL||e.imageUrl||e.Image||`${this._getBasePath()}/assets/no-image.png`}_getProductName(e){return e.Description||e.ProductName||e["Product Name"]||""}_getProductMeta(e){const t=e.Finish||e.Colour||"",o=e.DimX||e["X Dimension (mm)"]||"",s=e.DimY||e["Y Dimension (mm)"]||"",i=e.DimZ||e["Z Dimension (mm)"]||"",n=o&&o!=="0"?`${o} × ${s||0} × ${i||0}mm`:"";return[t,n].filter(Boolean).join("  ·  ")}_getProductPrice(e){const t=e["RRP INC GST"]||e.RRP_INCGST||e["RRP EX GST"]||e.RRP_EXGST||"";return t?`$${parseFloat(t).toLocaleString("en-AU",{minimumFractionDigits:2})}`:""}_enrichTableProducts(e){const t=this._getBasePath(),o=se.lastCompetitorProducts;e.querySelectorAll(".ai-comparison-table").forEach(s=>{const i=s.querySelector("thead"),n=s.querySelector("tbody");if(!i||!n)return;const a=i.querySelectorAll("th").length;if(a<2)return;const c=new Array(a).fill(null);if(n.querySelectorAll("tr").forEach(l=>{l.querySelectorAll("td").forEach((m,h)=>{if(c[h])return;const p=m.querySelector("strong"),f=(p?p.textContent:m.textContent).trim();if(f){if(/^\d{6}$/.test(f)){const y=R.findProductByCode(f);y&&(c[h]={src:this._getProductImage(y),click:()=>this._showProductModal(f)})}else if(o)for(const[y,b]of o){const w=String(y);if(w===f||f.includes(w)||w.includes(f)){c[h]={src:b.imageUrl||"",click:()=>this._showCompetitorModal(b.brand,y)};break}}}})}),!c.some(Boolean))return;const d=document.createElement("tr");d.className="ai-table-image-row";for(let l=0;l<a;l++){const u=document.createElement("td"),m=c[l];if(m&&m.src){const h=document.createElement("img");h.className="ai-table-product-img",h.src=m.src,h.alt="",h.onerror=()=>{h.src=`${t}/assets/no-image.png`},u.appendChild(h),m.click&&(u.style.cursor="pointer",u.addEventListener("click",m.click))}d.appendChild(u)}n.insertBefore(d,n.firstChild)})}_injectProductCards(e){e.querySelectorAll("table .ai-add-product-btn").forEach(s=>{const i=document.createElement("strong");i.textContent=s.dataset.code,s.replaceWith(i)});const t=e.querySelectorAll(".ai-add-product-btn");if(!t.length)return;const o=this._getBasePath();t.forEach(s=>{const i=s.dataset.code,n=R.findProductByCode(i);if(!n)return;const r=s.closest("li");if(!(r||s.parentElement))return;const c=this._getProductImage(n),d=this._getProductName(n),l=this._getProductMeta(n),u=this._getProductPrice(n),m=document.createElement("div");if(m.className="ai-product-card",m.dataset.code=i,m.innerHTML=`
      ...
      `,h.addEventListener("click",()=>{this._showCompetitorModal(m,a.code)});const p=n.parentElement;n.replaceWith(h),this._removeTrailingProductDetail(p||h)}}_showCompetitorModal(e,t){var l,u,m,h;if(!T.isStaffMode())return;const o=document.querySelector(".ai-product-modal-overlay");o&&o.remove();const s=this._getBasePath(),i=this.messagesContainer.querySelector(`.ai-comp-card[data-brand="${e}"][data-code="${t}"]`),n=((l=i==null?void 0:i.querySelector(".ai-product-card-name"))==null?void 0:l.textContent)||t,r=((u=i==null?void 0:i.querySelector(".ai-product-card-img"))==null?void 0:u.src)||"",a=((m=i==null?void 0:i.querySelector(".ai-product-card-meta"))==null?void 0:m.textContent)||"",c=((h=i==null?void 0:i.querySelector(".ai-product-card-price"))==null?void 0:h.textContent)||"",d=document.createElement("div");d.className="ai-product-modal-overlay",d.innerHTML=`
    ...
    `,d.addEventListener("click",p=>{p.target===d&&d.remove()}),d.querySelector(".ai-product-modal-close").addEventListener("click",()=>d.remove()),document.addEventListener("keydown",function p(f){f.key==="Escape"&&(d.remove(),document.removeEventListener("keydown",p))}),document.body.appendChild(d),this._enrichCompetitorModal(d,e,t,s)}async _enrichCompetitorModal(e,t,o,s){const i=e.querySelector(".ai-product-modal-body"),n=i==null?void 0:i.querySelector(".ai-comp-modal-loading");try{const r=await k.findCompetitorEntryByCode(o);if(!(r!=null&&r.competitorProduct)){n&&n.remove();return}const a=r.competitorProduct,c=a.image_url||a.Image_URL||`${s}/assets/no-image.png`,d=a.product_name||a.ProductName||o,l=a.description||a.product_description||a.features||"",u=a.rrp_ex_gst||a.rrp||a.price||a.rrp_inc_gst||"",m=u?`$${parseFloat(u).toLocaleString("en-AU",{minimumFractionDigits:2})}`:"",h=a.product_url||"",p=[];a.product_code&&p.push(["Product Code",a.product_code]),a.collection&&p.push(["Collection",a.collection]),a.product_type&&p.push(["Category",a.product_type]),a.subcategory&&p.push(["Subcategory",a.subcategory]),(a.finish||a.colour)&&p.push(["Finish",a.finish||a.colour]),a.material&&p.push(["Material",a.material]),a.dimensions_mm&&p.push(["Dimensions",a.dimensions_mm]),a.wels_rating&&p.push(["WELS",a.wels_rating]),i.innerHTML=`
        ...
        ${p.length?`<dl class="ai-product-modal-specs">${p.map(([f,y])=>`<dt>${this._escapeHtml(f)}</dt><dd>${this._escapeHtml(y)}</dd>`).join("")}</dl>`:""}
      ...
      <div class="ai-product-modal">
        ...
        <div class="ai-product-modal-body">
          ...
          <h3 class="ai-product-modal-title">${this._escapeHtml(n)}</h3>
          ${w.length?`<dl class="ai-product-modal-specs">${w.map(([S,P])=>`<dt>${this._escapeHtml(S)}</dt><dd>${this._escapeHtml(P)}</dd>`).join("")}</dl>`:""}
    ...
    `,v.addEventListener("click",S=>{S.target===v&&v.remove()}),v.querySelector(".ai-product-modal-close").addEventListener("click",()=>v.remove()),v.querySelector(".ai-product-modal-add").addEventListener("click",S=>{this._addProductToGrid(e,null);const P=S.currentTarget;P.classList.add("added"),P.disabled=!0,P.textContent="Added to Selection ✓"}),document.addEventListener("keydown",function S(P){P.key==="Escape"&&(v.remove(),document.removeEventListener("keydown",S))}),document.body.appendChild(v)}_appendFeedbackRow(e,t,o){const s=document.createElement("div");s.className="ai-feedback-row";const i=this._getFeedbackEntry(t),n=(i==null?void 0:i.rating)||null;s.innerHTML=`
    ...
    `,s.querySelectorAll(".ai-feedback-btn").forEach(a=>{a.addEventListener("click",()=>{const c=a.dataset.rating;s.querySelectorAll(".ai-feedback-btn").forEach(d=>d.classList.remove("active")),a.classList.add("active"),this._saveFeedback(t,o,c)})});const r=e.querySelector(".ai-msg-bubble");r?e.insertBefore(s,r.nextSibling):e.appendChild(s)}async _saveFeedback(e,t,o){try{const s=await this._loadFeedback(),i=s.findIndex(a=>a.id===e),n={id:e,question:this._lastUserMessage||"",answer:(t||"").slice(0,500),rating:o,timestamp:Date.now(),synced:!1};i>=0?s[i]={...s[i],...n}:s.push(n);const{set:r}=await O(async()=>{const{set:a}=await import("./vendor-idb-DwnyWBFG.js");return{set:a}},[]);await r(Ie,s)}catch{}}_getFeedbackEntry(e){return this._loadFeedback().then(t=>t.find(o=>o.id===e)||null)}async _loadFeedback(){try{const{get:e}=await O(async()=>{const{get:o}=await import("./vendor-idb-DwnyWBFG.js");return{get:o}},[]),t=await e(Ie);return Array.isArray(t)?t:[]}catch{return[]}}_escapeHtml(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}_scrollToBottom(){requestAnimationFrame(()=>{this.messagesContainer.scrollTop=this.messagesContainer.scrollHeight})}_enrichResponseBubble(e){this._injectProductCards(e),T.isPowerUser()&&(this._enrichTableProducts(e),this._injectCompetitorCards(e)),this._cleanupCardOrphans(e),this._collapseProductCards(e),this._attachProductButtons(e)}_persistMessages(){clearTimeout(this._persistTimer),this._persistTimer=setTimeout(()=>this._doPersistMessages(),500)}async _doPersistMessages(){try{const e=[];this.messagesContainer.querySelectorAll(".ai-msg").forEach(o=>{var s,i;o.classList.contains("ai-msg-user")?e.push({role:"user",content:((s=o.querySelector(".ai-msg-bubble"))==null?void 0:s.textContent)||""}):o.classList.contains("ai-msg-assistant")&&!o.querySelector(".ai-msg-thinking")&&e.push({role:"assistant",content:((i=o.querySelector(".ai-msg-bubble"))==null?void 0:i.dataset.rawContent)||"",feedbackId:o.dataset.feedbackId||null})});const{set:t}=await O(async()=>{const{set:o}=await import("./vendor-idb-DwnyWBFG.js");return{set:o}},[]);await t(ze,e)}catch{}}async _restoreMessages(){try{localStorage.removeItem(ze);const{get:e}=await O(async()=>{const{get:i}=await import("./vendor-idb-DwnyWBFG.js");return{get:i}},[]),t=await e(ze);if(!Array.isArray(t)||t.length===0)return;const o=this.messagesContainer.querySelector(".ai-chat-welcome");o&&o.remove();for(const i of t)i.role==="user"&&(this._lastUserMessage=i.content),this._addMessage(i.role,i.content,{skipPersist:!0,feedbackId:i.feedbackId||null});const s=document.createElement("div");s.className="ai-chat-restored",s.textContent="Previous conversation restored",this.messagesContainer.prepend(s)}catch{}}_generateId(){return Date.now().toString(36)+Math.random().toString(36).substr(2)}async _trackQuestion(e){try{const{get:t,set:o}=await O(async()=>{const{get:n,set:r}=await import("./vendor-idb-DwnyWBFG.js");return{get:n,set:r}},[]),s=await t(Pe)||{},i=e.trim();if(i.length<5)return;s[i]=(s[i]||0)+1,await o(Pe,s)}catch{}}async _syncFeedback(){try{const{get:e,set:t,del:o}=await O(async()=>{const{get:a,set:c,del:d}=await import("./vendor-idb-DwnyWBFG.js");return{get:a,set:c,del:d}},[]),s=await this._loadFeedback(),i=s.filter(a=>!a.synced),n=await e(Pe)||null;if(i.length===0&&!n)return;if((await fetch(`${X.PROXY_URL}/v1/feedback`,{method:"POST",headers:{"Content-Type":"application/json",...T.getAuthHeaders()},body:JSON.stringify({entries:i,questionLog:n})})).ok){for(const a of i)a.synced=!0;this._pruneFeedback(s),await t(Ie,s),await o(Pe)}}catch{}}_pruneFeedback(e){const t=Date.now()-6048e5;for(let o=e.length-1;o>=0;o--)e[o].synced&&e[o].timestamp<t&&e.splice(o,1)}async _clearChat(){await se.clearHistory();try{const{del:e}=await O(async()=>{const{del:t}=await import("./vendor-idb-DwnyWBFG.js");return{del:t}},[]);await e(Ie),await e(Pe)}catch{}localStorage.removeItem(Ie),localStorage.removeItem(Pe),this._showWelcome()}async _initUserEmail(){try{const e=T.getCurrentUser();this._userEmail=(e==null?void 0:e.email)||null}catch{}}async _saveConversation(){var s;if(!this._userEmail)return;const e=[];if(this.messagesContainer.querySelectorAll(".ai-msg").forEach(i=>{var n,r;i.classList.contains("ai-msg-user")?e.push({role:"user",content:((n=i.querySelector(".ai-msg-bubble"))==null?void 0:n.textContent)||""}):i.classList.contains("ai-msg-assistant")&&!i.querySelector(".ai-msg-thinking")&&e.push({role:"assistant",content:((r=i.querySelector(".ai-msg-bubble"))==null?void 0:r.dataset.rawContent)||""})}),e.length<2)return;this._conversationId||(this._conversationId=Date.now().toString(36)+Math.random().toString(36).slice(2,8));const t=((s=e.find(i=>i.role==="user"))==null?void 0:s.content)||"Untitled",o=t.length>50?t.slice(0,47)+"...":t;try{await fetch(`${X.PROXY_URL}/v1/conversations/${this._conversationId}`,{method:"PUT",headers:{"Content-Type":"application/json",...T.getAuthHeaders()},body:JSON.stringify({email:this._userEmail,title:o,messages:e})})}catch{}}_startNewConversation(){this._saveConversation(),this._conversationId=null,this._clearChat()}_toggleHistory(e){const t=this.panel.querySelector("#ai-chat-history-panel");this._historyOpen=e!==void 0?e:!this._historyOpen,t.classList.toggle("open",this._historyOpen),this._historyOpen&&this._loadConversationList()}async _loadConversationList(){if(!this._userEmail){this.panel.querySelector("#ai-chat-history-list").innerHTML='<div class="ai-chat-history-empty">Sign in to see conversation history.</div>';return}const e=this.panel.querySelector("#ai-chat-history-list");e.innerHTML='<div class="ai-chat-history-empty">Loading...</div>';try{const t=`${X.PROXY_URL}/v1/conversations?email=${encodeURIComponent(this._userEmail)}`,o=await fetch(t,{headers:T.getAuthHeaders()});if(!o.ok)throw new Error("Failed");const i=(await o.json()).conversations||[];if(i.length===0){e.innerHTML='<div class="ai-chat-history-empty">No previous conversations.</div>';return}e.innerHTML=i.map(n=>`
      ...
      `).join(""),e.querySelectorAll(".ai-chat-history-item").forEach(n=>{n.addEventListener("click",()=>this._loadConversation(n.dataset.id))})}catch{e.innerHTML='<div class="ai-chat-history-empty">Failed to load conversations.</div>'}}async _loadConversation(e){try{const t=`${X.PROXY_URL}/v1/conversations/${e}`,o=await fetch(t,{headers:T.getAuthHeaders()});if(!o.ok)throw new Error("Failed");const s=await o.json();this._conversationId=e,se.clearHistory(),this.messagesContainer.innerHTML="";const i=this.messagesContainer.querySelector(".ai-chat-welcome");i&&i.remove();for(const r of s.messages||[])r.role==="user"&&(this._lastUserMessage=r.content),this._addMessage(r.role,r.content,{skipPersist:!0}),(r.role==="user"||r.role==="assistant")&&se.conversationHistory.push(r);this._persistMessages(),this._toggleHistory(!1);const n=document.createElement("div");n.className="ai-chat-restored",n.textContent="Conversation restored",this.messagesContainer.prepend(n)}catch{console.error("Failed to load conversation")}}_formatHistoryTime(e){if(!e)return"";const t=new Date(e),s=new Date-t,i=Math.floor(s/864e5);return i===0?"Today":i===1?"Yesterday":i<7?`${i}d ago`:t.toLocaleDateString("en-AU",{day:"numeric",month:"short"})}open(){this.panel.style.right="",this.panel.classList.add("open"),this.isOpen=!0,setTimeout(()=>this.input.focus(),300),this._syncFeedback()}close(){const e=this.panel.offsetWidth||620;this.panel.style.right=-(e+20)+"px",this.panel.classList.remove("open"),this.isOpen=!1}toggle(){this.isOpen?this.close():this.open()}}const st=new $o;class Do{constructor(){this.navigationManager=null,this.fileImportManager=new So,this.productGridManager=new Lo,this.isInitialized=!1,Z.log("SeimaScanner application starting",ee.INFO)}async init(){var e,t,o;try{Z.log("Initializing application modules",ee.INFO),T.configure({googleSheetsUrl:(e=M.PRESENTATION_RECORDING)==null?void 0:e.GOOGLE_SHEETS_URL,email:M.EMAIL}),K.configure({logoSrc:"assets/seima-logo.png",brandName:"Seima",appName:"Product Presenter"});const s=(t=M.PRESENTATION_RECORDING)==null?void 0:t.GOOGLE_SHEETS_URL;if(s&&Gt(s).catch(r=>{console.warn("Synonyms preload failed:",(r==null?void 0:r.message)||r)}),(o=M.CROSSHAIR)!=null&&o.ENABLED){const r=T.getCurrentUser();k.configure(M.CROSSHAIR,(r==null?void 0:r.email)||""),k.preload().catch(a=>{console.warn("Crosshair preload failed (user may not be logged in):",a.message)})}this.navigationManager=new Qt;const i=Ce.getCompatibilityReport();Z.log(`Browser compatibility: ${i.score}% (${i.browserName})`,ee.INFO),Ce.shouldShowCompatibilityWarning()&&this.showCompatibilityWarning(),await this.navigationManager.init(),this.fileImportManager.init(),this.setupGlobalEventListeners(),this.productGridManager.init(),st.init();const n=document.getElementById("ai-chat-trigger");return n&&n.addEventListener("click",()=>st.toggle()),window.navigationManager=this.navigationManager,window.productGridManager=this.productGridManager,window.browserCompatibility=Ce,window.downloadWithFallback=tt,window.showPdfFormScreen=ot,this.isInitialized=!0,Z.log("Seima Scanner initialized successfully",ee.INFO),this._dismissLoadingOverlay(),setTimeout(()=>{Ct.show()},500),!0}catch(s){return Z.handleError({message:"Failed to initialize application",error:s,category:Le.UI,level:ee.CRITICAL,context:"app-init"}),this._dismissLoadingOverlay(),!1}}_dismissLoadingOverlay(){const e=document.getElementById("app-loading-overlay");e&&(e.classList.add("fade-out"),e.addEventListener("transitionend",()=>e.remove(),{once:!0}),setTimeout(()=>e.remove(),400))}showCompatibilityWarning(){const e=Ce.getCompatibilityReport(),t=e.recommendations;if(t.length===0)return;const o=t.filter(n=>n.type==="critical"),s=e.score<N.get("compatibility.minCompatibilityScore",70);if(o.length===0&&!s)return;const i=document.createElement("div");i.style.cssText=`
    ...
    `,document.body.insertBefore(i,document.body.firstChild)}setupGlobalEventListeners(){window.addEventListener("generatePdf",e=>{const{tipTailSettings:t,...o}=e.detail;Kt(),ot(o,t||null)}),window.addEventListener("beforeunload",()=>{}),Ce.features.memoryAPI&&setInterval(()=>{const e=Ce.memoryInfo;e.memoryPressure==="high"&&console.warn("High memory usage detected:",e)},6e4)}getSelectedProducts(){return E.getSelectedProducts()}clearSelection(){return E.clearAllSelections()}addProduct(e,{notes:t="",room:o="",quantity:s=1}={}){return E.addProductToSelection(e,{notes:t,room:o,quantity:s})}updateSelectionCount(){this.navigationManager&&this.navigationManager.updateSelectionCount()}showError(e){alert(e)}}document.addEventListener("DOMContentLoaded",()=>{window.seimaScanner=new Do,window.seimaScanner.init(),window._appVersion=M.VERSION;const g=document.getElementById("menu-version-footer");g&&(g.textContent=`Ver: ${M.VERSION}`),Bo(),Ao();const e=document.getElementById("changelog-close");e&&e.addEventListener("click",()=>{document.getElementById("changelog-modal").style.display="none"})});function Bo(){const g=document.getElementById("help-btn");g&&g.addEventListener("click",()=>{Fo()});const e=document.getElementById("crosshair-btn");e&&e.addEventListener("click",()=>{window.location.href="screens/validator.html"});const t=document.getElementById("pending-btn");t&&t.addEventListener("click",()=>{window.location.href="screens/admin-pending.html"});const o=document.getElementById("landscape-btn");o&&o.addEventListener("click",()=>{window.location.href="screens/landscape.html"});const s=document.getElementById("admin-btn");s&&s.addEventListener("click",()=>{window.location.href="screens/admin.html"});const i=document.getElementById("quick-start-btn");i&&i.addEventListener("click",()=>{Ct.showForced()})}function Ao(){const g=document.getElementById("user-menu-container"),e=document.getElementById("user-menu-trigger"),t=document.getElementById("user-menu-dropdown"),o=document.getElementById("sign-in-btn"),s=document.getElementById("user-avatar"),i=document.getElementById("user-name-display");function n(c){var d,l,u,m,h,p,f,y,b,w;if(c){(d=M.CROSSHAIR)!=null&&d.ENABLED&&(k.configure(M.CROSSHAIR,c.email||""),k.preload().catch(ye=>{console.warn("Crosshair preload after login failed:",ye)})),g&&(g.style.display="block"),o&&(o.style.display="none");const v=document.getElementById("crosshair-btn"),S=document.getElementById("pending-btn"),P=T.isStaffMode(),C=T.isPowerUser(),_=T.isAdmin();v&&(v.style.display=(l=M.CROSSHAIR)!=null&&l.ENABLED&&P?"":"none"),S&&(S.style.display=P?"":"none");const I=document.getElementById("landscape-btn");I&&(I.style.display=(u=M.CROSSHAIR)!=null&&u.ENABLED&&(_||C)?"":"none");const $=document.getElementById("admin-btn");$&&($.style.display=_?"":"none");const U=document.getElementById("staff-divider"),H=(v==null?void 0:v.style.display)!=="none"||(S==null?void 0:S.style.display)!=="none"||(I==null?void 0:I.style.display)!=="none"||($==null?void 0:$.style.display)!=="none";U&&(U.style.display=H?"":"none"),document.querySelectorAll("#import-file-btn, #entry-import").forEach(ye=>{ye&&(ye.style.display=C?"":"none")});const q=document.getElementById("ai-chat-trigger");q&&(q.style.display="");const fe=document.getElementById("guided-ask-fred");fe&&(fe.style.display="");const _e=r(c.name);s&&(s.textContent=_e),i&&(i.textContent=((m=c.name)==null?void 0:m.split(" ")[0])||"User");const Ge=(c.email||"").toLowerCase().endsWith("@seima.com.au")&&c.emailVerified===!1;let Y=document.getElementById("verify-email-banner");if(Ge?(Y||(Y=document.createElement("div"),Y.id="verify-email-banner",Y.className="verify-email-banner",Y.innerHTML=`
            <span>Verify your email to access staff features.</span>
            <button id="verify-email-btn" class="verify-email-btn">Verify now</button>
          `,document.body.insertBefore(Y,document.body.firstChild),Y.querySelector("#verify-email-btn").addEventListener("click",()=>{K.showVerifyEmail(c.email,()=>{n(T.getCurrentUser()),Y==null||Y.remove()})})),Y.style.display=""):Y&&(Y.style.display="none"),t){const ye=Ge?`
            ...
            </button>`:"",$e=window._appVersion?`Ver: ${window._appVersion}`:"Loading...";t.innerHTML=`
          ...
          <div class="user-menu-footer" id="menu-version-footer" title="Click to view what's new">${$e}</div>
        `,(h=document.getElementById("menu-verify-email"))==null||h.addEventListener("click",()=>{t.style.display="none",K.showVerifyEmail(c.email,()=>{var j;n(T.getCurrentUser()),(j=document.getElementById("verify-email-banner"))==null||j.remove()})}),(p=document.getElementById("menu-refresh"))==null||p.addEventListener("click",async()=>{t.style.display="none";try{const{del:j,keys:nt}=await O(async()=>{const{del:Q,keys:ke}=await import("./vendor-idb-DwnyWBFG.js");return{del:Q,keys:ke}},[]),He=await nt(),le=["productCatalogCsv","customerLogo","fredChatHistory","fredChatMessages","fredFeedback","fredQuestionLog"];await Promise.all(He.filter(Q=>typeof Q=="string"&&(le.includes(Q)||Q.startsWith("crosshair_"))).map(Q=>j(Q)))}catch{}localStorage.removeItem("configPreferences"),localStorage.removeItem("pdfWizardSettings"),Object.keys(localStorage).filter(j=>j.startsWith("crosshair_")||j.startsWith("fred")).forEach(j=>localStorage.removeItem(j)),window.location.reload()}),(f=document.getElementById("menu-version-footer"))==null||f.addEventListener("click",()=>{t.style.display="none",Mo()}),(y=document.getElementById("menu-profile"))==null||y.addEventListener("click",()=>{t.style.display="none",K.showEditProfile(j=>{E.clearUserSettings(),n(j)})}),(b=document.getElementById("menu-password"))==null||b.addEventListener("click",()=>{t.style.display="none",K.showChangePassword()}),(w=document.getElementById("menu-logout"))==null||w.addEventListener("click",()=>{var j;t.style.display="none",E.clearUserSettings(),k.clearAllCaches(),(j=window.productGridManager)!=null&&j.searchCache&&window.productGridManager.searchCache.clear(),T.logout(),n(null)})}}else{g&&(g.style.display="none"),o&&(o.style.display="block"),document.querySelectorAll("#import-file-btn, #entry-import").forEach(U=>{U&&(U.style.display="none")});const v=document.getElementById("ai-chat-trigger");v&&(v.style.display="none");const S=document.getElementById("guided-ask-fred");S&&(S.style.display="none"),st.close();const P=document.getElementById("crosshair-btn");P&&(P.style.display="none");const C=document.getElementById("pending-btn");C&&(C.style.display="none");const _=document.getElementById("landscape-btn");_&&(_.style.display="none");const I=document.getElementById("admin-btn");I&&(I.style.display="none");const $=document.getElementById("staff-divider");$&&($.style.display="none")}}function r(c){if(!c)return"?";const d=c.trim().split(" ");return d.length>=2?(d[0][0]+d[d.length-1][0]).toUpperCase():c.substring(0,2).toUpperCase()}function a(c){const d=document.createElement("div");return d.textContent=c||"",d.innerHTML}e&&t&&(e.addEventListener("click",c=>{c.stopPropagation();const d=t.style.display!=="none";t.style.display=d?"none":"block"}),document.addEventListener("click",c=>{g!=null&&g.contains(c.target)||(t.style.display="none")})),o&&o.addEventListener("click",()=>{K.showLogin(c=>{n(c)})}),T.onAuthChange=n,n(T.getCurrentUser())}function Fo(){const g=document.getElementById("user-guide-modal"),e=document.getElementById("user-guide-content");if(!g||!e)return;e.innerHTML=No(),g.style.display="flex";const t=document.getElementById("user-guide-close");t&&(t.onclick=()=>{g.style.display="none"}),g.onclick=s=>{s.target===g&&(g.style.display="none")};const o=s=>{s.key==="Escape"&&(g.style.display="none",document.removeEventListener("keydown",o))};document.addEventListener("keydown",o)}function No(){return`
  ...
  `}async function Mo(){try{const g=document.getElementById("changelog-modal"),e=document.getElementById("changelog-content"),s=(await(await fetch("./version.txt")).text()).trim().split(`
`);if(s.length===0){e.innerHTML="<p>No changelog available.</p>",g.style.display="flex";return}const i=r=>String(r).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");let n="";s.forEach(r=>{if(r.trim()){const a=r.indexOf(" - ");if(a>0){const c=r.substring(0,a).trim(),d=r.substring(a+3).trim();n+=`
            <div style="margin-bottom: 30px; border-bottom: 1px solid #eee; padding-bottom: 20px;">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                <h4 style="margin: 0; color: #a09484;">v${i(c)}</h4>
              </div>
              <p style="margin: 10px 0; color: #555; line-height: 1.5;">${i(d)}</p>
            </div>
