...
`)}isAvailable(){return this.emailJsLoaded&&!!this.config.SERVICE_ID&&!!this.config.TEMPLATE_ID}static validateEmail(e){return/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)}}new EmailService;function isTechnicalDiagram$1(c,e){try{const t=Math.min(100,c.width),o=Math.min(100,c.height),i=e.getImageData(0,0,t,o).data,n=new Set;for(let s=0;s<i.length;s+=4){const a=`${i[s]},${i[s+1]},${i[s+2]}`;if(n.add(a),n.size>1e3)return!1}return n.size<1e3}catch(t){return console.warn("Could not analyze image for diagram detection:",t),!1}}function detectTransparency$1(c,e){try{const o=e.getImageData(0,0,c.width,c.height).data;for(let r=3;r<o.length;r+=4)if(o[r]<255)return!0;return!1}catch(t){return console.warn("Could not detect transparency:",t),!1}}const preloadedImageCache=new Map;async function preloadAllProductImages(c,e={}){const{batchSize:t=10,maxWidth:o=400,quality:r=.8}=e;preloadedImageCache.clear();const i=new Set;c.forEach(u=>{u.Image_URL&&u.Image_URL.length>10&&i.add(u.Image_URL),u.Diagram_URL&&u.Diagram_URL.length>10&&i.add(u.Diagram_URL)});const n=Array.from(i);if(n.length===0)return console.log("📷 No images to preload"),0;console.log(`📷 Preloading ${n.length} images in parallel...`);const s=Date.now();let a=0,l=0;for(let u=0;u<n.length;u+=t){const p=n.slice(u,u+t);(await Promise.allSettled(p.map(h=>preloadSingleImage(h,o,r)))).forEach((h,v)=>{const y=p[v];h.status==="fulfilled"&&h.value?(preloadedImageCache.set(y,h.value),a++):l++});const m=document.getElementById("preload-progress");if(m){const h=Math.min(100,Math.round((u+p.length)/n.length*100));m.textContent=`Loading images: ${h}%`}}const d=((Date.now()-s)/1e3).toFixed(1);return console.log(`✅ Preloaded ${a}/${n.length} images in ${d}s (${l} failed)`),a}async function preloadSingleImage(c,e=400,t=.8){return preloadedImageCache.has(c)?preloadedImageCache.get(c):new Promise(o=>{const r=new Image;r.crossOrigin="anonymous";const i=setTimeout(()=>{o(null)},5e3);r.onload=function(){clearTimeout(i);try{const n=document.createElement("canvas"),s=n.getContext("2d");let a=r.width,l=r.height;a>e&&(l=l*e/a,a=e),n.width=a,n.height=l,s.imageSmoothingEnabled=!0,s.imageSmoothingQuality="high",s.drawImage(r,0,0,a,l);const d=detectTransparency$1(n,s),u=isTechnicalDiagram$1(n,s),p=d||u?"PNG":"JPEG",g=u?.9:t,m=n.toDataURL(`image/${p.toLowerCase()}`,g);o({dataUrl:m,width:a,height:l,format:p})}catch{console.warn("Failed to optimize image:",c.substring(0,50)),o(null)}},r.onerror=()=>{if(clearTimeout(i),c.includes("corsproxy"))o(null);else{const n="https://corsproxy.io/?"+encodeURIComponent(c),s=new Image;s.crossOrigin="anonymous",s.onload=function(){try{const a=document.createElement("canvas"),l=a.getContext("2d");let d=s.width,u=s.height;d>e&&(u=u*e/d,d=e),a.width=d,a.height=u,l.drawImage(s,0,0,d,u);const p="JPEG",g=a.toDataURL("image/jpeg",t);o({dataUrl:g,width:d,height:u,format:p})}catch{o(null)}},s.onerror=()=>o(null),s.src=n}},r.src=c})}function getCachedImage(c){return preloadedImageCache.get(c)}function getImageCacheSize(){return preloadedImageCache.size}function isSamsungDevice$1(){const c=navigator.userAgent;return/SM-|SCH-|SPH-|SGH-|GT-|Galaxy/i.test(c)||/SamsungBrowser/i.test(c)}function showSamsungDownloadHelp(c,e,t="PDF"){const o=document.createElement("div");o.style.cssText=`
  ...
  `,o.appendChild(r),document.body.appendChild(o),document.getElementById("samsung-help-close").onclick=()=>{document.body.removeChild(o)},document.getElementById("samsung-help-retry").onclick=()=>{document.body.removeChild(o),setTimeout(()=>{downloadWithFallback(c,e,t)},1e3)},o.onclick=i=>{i.target===o&&document.body.removeChild(o)}}async function downloadWithFallback(c,e,t="file"){if(isSamsungDevice$1())try{if(await attemptStandardDownload(c,e))return;console.warn("Download may have failed on Samsung device"),showSamsungDownloadHelp(c,e,t)}catch(o){console.error("Samsung download failed:",o),showSamsungDownloadHelp(c,e,t)}else await downloadWithEnhancedFallbacks(c,e,t)}function showBrowserCompatibilityWarning(){if(isSamsungDevice$1()){if(document.getElementById("samsung-browser-warning"))return;const e=document.createElement("div");e.id="samsung-browser-warning",e.style.cssText=`
    ...
    `,document.body.appendChild(o),loadImageAsDataURL("./assets/seima-logo.png",(i,n,s)=>{const a=JSON.parse(localStorage.getItem("selection")||"[]"),l=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let d=[];if(l.length>0?d=l.map(E=>({...E.product,Room:E.room,Notes:E.notes,Quantity:E.quantity,Timestamp:new Date(E.timestamp).toISOString()})):d=a,!d.length){alert("No products selected."),t&&(t.style.display="none");return}if(getImageCacheSize()>0)console.log(`📷 Using ${getImageCacheSize()} pre-cached images (skipping duplicate preload)`);else{const E=document.getElementById("pdf-processing-notification");if(E){const P=document.createElement("span");P.id="preload-progress",P.style.cssText="display: block; font-size: 12px; margin-top: 8px; color: #1e40af;",P.textContent="Loading images: 0%",E.appendChild(P)}console.log("📷 Starting image preload for",d.length,"products"),preloadAllProductImages(d).then(P=>{const O=document.getElementById("preload-progress");O&&(O.textContent=`✓ ${P} images ready`,O.style.color="#059669")}).catch(P=>{console.warn("Image preloading error:",P)})}const p={};d.forEach(E=>{p[E.Room]||(p[E.Room]=[]),p[E.Room].push(E)});const{jsPDF:g}=window.jspdf,m=new g({orientation:"landscape",unit:"pt",format:"a4",compress:!0,putOnlyUsedFonts:!0,precision:16,userUnit:1,floatPrecision:16}),h=m.internal.pageSize.getWidth(),v=m.internal.pageSize.getHeight(),y=h/2,b=320,f=90,S=(h-b)/2,k=70;m.setFillColor(255,255,255),m.rect(S,k,b,f,"F");const U=localStorage.getItem("customerLogo");if(U)try{const E=new Image;E.onload=function(){const P=b-20,O=f-20,ce=E.width/E.height;let Y=P,J=P/ce;J>O&&(J=O,Y=O*ce);const re=S+(b-Y)/2,ie=k+(f-J)/2;m.addImage(U,"PNG",re,ie,Y,J,void 0,"FAST")},E.src=U}catch(E){console.warn("Failed to draw customer logo:",E)}loadImageAsDataURL("./assets/seima-logo.png",(E,P,O)=>{const Y=O&&P?250*O/P:65,J=(h-250)/2,re=k+f+80;m.addImage(E,"PNG",J,re,250,Y,void 0,"FAST"),m.setFontSize(15),m.setTextColor("#444");const ie=y;let le=re+Y+50;const q=[];c!=null&&c.name&&c.name.trim()&&q.push({label:"Name:",value:c.name.trim(),bold:!0}),c!=null&&c.project&&c.project.trim()&&q.push({label:"Project:",value:c.project.trim(),bold:!0}),c!=null&&c.address&&c.address.trim()&&q.push({label:"Address:",value:c.address.trim(),bold:!0}),c!=null&&c.email&&c.email.trim()&&q.push({label:"Email:",value:c.email.trim(),bold:!0}),c!=null&&c.telephone&&c.telephone.trim()&&q.push({label:"Telephone:",value:c.telephone.trim(),bold:!0});const W=32;q.length>0&&q.forEach(X=>{m.setFont("helvetica","normal"),m.text(X.label,ie-80,le,{align:"right"}),m.setFont("helvetica","bold"),m.text(X.value,ie-35,le,{align:"left"}),le+=26}),m.setFillColor("#9B9184"),m.rect(0,v-W,h,W,"F"),m.setTextColor("#fff"),m.setFontSize(13);const ue=new Date,ke=["January","February","March","April","May","June","July","August","September","October","November","December"],Te=ue.getDate(),De=ke[ue.getMonth()],Ae=ue.getFullYear(),Be=String(ue.getHours()).padStart(2,"0"),Fe=String(ue.getMinutes()).padStart(2,"0"),Ne=`Printed ${Te} ${De} ${Ae}, ${Be}:${Fe}`;m.text(Ne,16,v-10),m.text("www.seima.com.au",h-140,v-10);const pe=StorageManager.getUserSettings(),_=pe.staffName||"",Q=pe.staffPosition||"",Z=pe.staffEmail||"",ee=pe.staffPhone||"";let te="For more information";_&&ee&&Z?te=`For more information, please contact ${Q?`${_}, ${Q}`:_} on ${ee} or email ${Z}`:_&&ee?te=`For more information, please contact ${Q?`${_}, ${Q}`:_} on ${ee}`:_&&Z?te=`For more information, please contact ${Q?`${_}, ${Q}`:_} or email ${Z}`:ee&&Z?te=`For more information, please call ${ee} or email ${Z}`:_?te=`For more information, please contact ${Q?`${_}, ${Q}`:_}`:ee?te=`For more information, please call ${ee}`:Z&&(te=`For more information, please email ${Z}`),m.setFont("helvetica","normal"),m.setFontSize(14),m.setTextColor("#111"),m.text(te,y,v-W-18,{align:"center"}),m.addPage(),loadImageAsDataURL("./assets/seima-logo-white.png",(X,Ie,Ce)=>{const fe=h-200,ye=h-120,Pe=h-80,oe=[32,236,321,fe,ye,Pe],ne=[90,90,fe-321,ye-fe,Pe-ye,80];resetImageOptimizationStats();const Re=(w,I,N,V,T,M,C)=>{if(!I||typeof I!="string"||I.length<10||!I.startsWith("http://")&&!I.startsWith("https://")&&!I.startsWith("data:")){C&&C();return}if(imageOptimizationStats.totalImages++,c.emailCompatible){imageOptimizationStats.failedImages++,C&&C();return}const D=getCachedImage(I);if(D&&D.dataUrl)try{const x=D.width/D.height;let $=T,R=T/x;R>M&&(R=M,$=M*x),w.addImage(D.dataUrl,D.format,N,V,$,R,void 0,"FAST"),imageOptimizationStats.optimizedImages++,C&&C();return}catch{console.warn("Failed to use cached image, falling back to direct load")}let B=!1;const G=["https://wsrv.nl/?url=","https://images.weserv.nl/?url=","https://api.codetabs.com/v1/proxy?quest="];let F=0;function ae(){if(B)return;const x=new Image;x.crossOrigin="Anonymous";let $=null;x.onload=function(){if(!B){B=!0,$&&clearTimeout($);try{const A=getOptimizedFileSettings(0),L=A.imageMaxWidth,de=T,Se=M;try{const z=document.createElement("canvas"),j=z.getContext("2d"),{width:K,height:H}=calculateOptimizedDimensions(x.width,x.height,L);z.width=K,z.height=H,j.imageSmoothingEnabled=!0,j.imageSmoothingQuality="high",j.drawImage(x,0,0,K,H);let xe,Ee="JPEG";const Oe=detectTransparency(z,j),Me=isTechnicalDiagram(x);Oe||Me?(xe=z.toDataURL("image/png",A.imageQuality),Ee="PNG"):(xe=z.toDataURL("image/jpeg",A.imageQuality),Ee="JPEG");const $e=`img_${generateImageHash(I)}`;w.addImage(xe,Ee,N,V,de,Se,$e,"FAST"),imageOptimizationStats.optimizedImages++,C&&C()}catch(z){console.warn(`Failed to optimize image: ${I}`,z),console.warn("Error details:",z.message,z.stack);try{w.addImage(x,"JPEG",N,V,de,Se),imageOptimizationStats.optimizedImages++,C&&C()}catch(j){console.error(`Fallback also failed for: ${I}`,j),imageOptimizationStats.failedImages++,C&&C()}}}catch(A){console.warn("Failed to add image to PDF:",A),imageOptimizationStats.failedImages++,C&&C()}}},x.onerror=function(){B||($&&clearTimeout($),console.warn(`Failed to load image with proxy ${F}: ${I}`),console.warn(`Error details for: ${I} - Proxy: ${G[F]}`),F++,F<G.length?setTimeout(()=>{ae()},200):(B=!0,console.warn("All proxies failed, skipping image"),imageOptimizationStats.failedImages++,C&&C()))},$=setTimeout(()=>{B||(console.warn(`⏰ Timeout with proxy ${F}: ${I}`),x.src="",x.onload=null,x.onerror=null,F++,F<G.length?setTimeout(()=>{ae()},200):(B=!0,console.warn("All proxies timed out, skipping image"),imageOptimizationStats.failedImages++,C&&C()))},3e3);let R=I;F<G.length&&(R=G[F]+encodeURIComponent(I)),x.src=R}ae()},se=[];Object.keys(p).forEach((w,I)=>{const N=p[w];if(!N||!Array.isArray(N)){console.warn("⚠️ Skipping invalid room items:",w,N);return}N.forEach((V,T)=>{if(!V){console.warn("⚠️ Skipping null item in room:",w,"at index:",T);return}se.push({item:V,room:w,rIdx:I,iIdx:T,isFirstInRoom:T===0,roomCount:N.length})})}),se.reduce((w,I)=>{if(!I||!I.item)return console.warn("⚠️ Skipping null row in data analysis:",I),w;const N=String(I.item.Description||""),V=String(I.item.LongDescription||""),T=String(I.item.Notes||""),M=String(I.item.OrderCode||"");return w+N.length+V.length+T.length+M.length},0);let me=0,ge=0;const Le=4,he=8,we=Math.floor((v-80)/Le);let ve=W+8;function be(){if(!se||!Array.isArray(se)){console.error("❌ Critical error: rowsToDraw is not a valid array:",se),showDetailedErrorMessage(new Error("Invalid product data structure"),"generating PDF","unknown.pdf");return}if(me>=se.length){const T=m.internal.getNumberOfPages()-1;for(let R=2;R<=T+1;R++)m.setPage(R),drawPDFHeader(m,h,oe,ne,32,W,X,Ie,Ce,c),ve=W+8,m.setFillColor("#9B9184"),m.rect(0,v-W,h,W,"F"),m.setTextColor("#fff"),m.setFontSize(11),m.text("www.seima.com.au",h-140,v-10),m.text(`Page ${R-1} of ${T}`,32,v-10);const M=new Date,C=String(M.getDate()).padStart(2,"0"),D=String(M.getMonth()+1).padStart(2,"0"),B=String(M.getFullYear()).slice(-2),G=String(M.getHours()).padStart(2,"0"),F=String(M.getMinutes()).padStart(2,"0"),x=`${c.project.replace(/[^a-zA-Z0-9\s]/g,"")}-${C}${D}${B}.${G}${F}.pdf`,$=document.getElementById("pdf-processing-notification");$&&$.remove(),showImageOptimizationSummary(c.emailCompatible);try{const R=m.output("blob"),A=m.output("string"),L=A?A.match(/\/Type\s*\/XObject/g):null,de=A?A.match(/Tj\s/g):null,Se=A?A.match(/\/A\s*<</g):null;c.pdfSize=R.size;const z=showFileSizeInfo(R,x);if(c.sendEmail&&R.size>15*1024*1024){console.warn(`❌ PDF too large for email (${(R.size/1024/1024).toFixed(1)}MB), offering email-compatible version`),showEmailCompatibleOption(c,x);return}const j=createOptimizedBlob(R,z.settings);if(c.sendEmail&&c.email)if(c.exportCsv){const K=x.replace(/\.pdf$/,".csv");generateCsvBlobAsync(c,K).then(H=>{window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:c,pdfBlob:j,csvBlob:H}}))}).catch(H=>{console.error("Async CSV generation for email failed:",H),window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:c,pdfBlob:j,csvBlob:null}}))})}else window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:c,pdfBlob:j,csvBlob:null}}));else if((async()=>{const K=await mergeWithTipTail(j);downloadWithFallback(K,x,"PDF")})(),c.exportCsv){const K=x.replace(/\.pdf$/,".csv");setTimeout(()=>{generateCsvBlobAsync(c,K).then(H=>{H&&downloadWithFallback(H,K,"CSV")}).catch(H=>{console.error("CSV generation failed:",H)})},1e3)}}catch(R){console.error("PDF generation failed:",R),showDetailedErrorMessage(R,"generating PDF",x);const A=document.getElementById("pdf-processing-notification");A&&A.remove()}t&&(t.style.display="none");return}ge>=Le&&(m.addPage(),drawPDFHeader(m,h,oe,ne,32,W,X,Ie,Ce,c),ve=W+8,ge=0);const w=se[me];if(!w||!w.item){console.warn(`⚠️  Skipping invalid row at index ${me}:`,w),me++,be();return}const I=ve+we*ge;w.isFirstInRoom&&(m.setFontSize(12),m.setFont("helvetica","bold"),m.setTextColor("#333"),m.text(`${w.room} (${w.roomCount})`,32,I+10),m.setFont("helvetica","normal"));const N=oe[0],V=N+90+12;Re(m,w.item.Image_URL||"",N,I+he+16,90,we-he*2,()=>{Re(m,w.item.Diagram_URL||"",V,I+he+16,90,we-he*2,()=>{m.setFontSize(10),m.setTextColor("#222");const T=I+28,C=oe[1]+ne[1]/2;m.text(String(w.item.OrderCode||""),C,T+10,{align:"center"});let D=T+35;if(w.item.Datasheet_URL&&w.item.Datasheet_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Datasheet",C,D,{url:w.item.Datasheet_URL,align:"center"});const L=m.getTextWidth("Datasheet");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(C-L/2,D+1.5,C+L/2,D+1.5),D+=16}if(w.item.Website_URL&&w.item.Website_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Website",C,D,{url:w.item.Website_URL,align:"center"});const L=m.getTextWidth("Website");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(C-L/2,D+1.5,C+L/2,D+1.5),D+=16}let B=T+10;m.setFontSize(10),m.setTextColor("#222");const G=ne[2]-10,F=oe[2],ae=m.splitTextToSize(String(w.item.Description||""),G);if(m.text(ae,F+5,B),B+=ae.length*12,!c.excludeLongDescription&&(w.item.LongDescription||w.item["Long Description"]||w.item.longDescription)){const L=w.item.LongDescription||w.item["Long Description"]||w.item.longDescription;m.setFontSize(9),m.setTextColor("#444");const de=m.splitTextToSize(String(L),G);m.text(de,F+5,B),B+=de.length*11}if(w.item.Notes){m.setFont("helvetica","italic"),m.setFontSize(9),m.setTextColor("#444");const L=m.splitTextToSize(`Notes: ${String(w.item.Notes).replace(/\r?\n|\r/g," ")}`,G);m.text(L,F+5,B),B+=L.length*11,m.setFont("helvetica","normal")}m.setFontSize(10),m.setTextColor("#222");let x=NaN;if(w.item.UserEditedPrice!==void 0&&w.item.UserEditedPrice!==null&&w.item.UserEditedPrice!=="")x=parseFloat(w.item.UserEditedPrice.toString().replace(/,/g,""));else{const L=w.item.RRP_EX||w.item["RRP EX GST"]||w.item.RRP_EX||w.item.RRP_EXGST;L&&(x=parseFloat(L.toString().replace(/,/g,"")))}!isNaN(x)&&x>0&&c.includeGst&&(x=x*1.1);const $=!isNaN(x)&&x>=0?`$${x.toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2})}`:"";if(!c.excludePrice&&!c.excludeQty){const L=oe[3]+ne[3]/2;m.text($,L,T+10,{align:"center"})}if(m.setFontSize(10),m.setTextColor("#222"),!c.excludeQty){const L=oe[4]+ne[4]/2;m.text(String(w.item.Quantity||1),L,T+10,{align:"center"})}m.setFontSize(10),m.setTextColor("#222");const R=x*(w.item.Quantity||1),A=!isNaN(x)&&x>=0?`$${R.toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2})}`:"";if(!c.excludePrice&&!c.excludeQty){const L=oe[5]+ne[5]/2;m.text(A,L,T+10,{align:"center"})}me++,ge++,be()})})}be()})})})}function drawPDFHeader(c,e,t,o,r,i,n,s,a,l){const d=i+5.7;if(c.setFillColor("#8B6C2B"),c.rect(0,0,e,d,"F"),n&&s&&a){const y=d*.55,b=s/a;let f=y*b;f>80&&(f=80);const S=(d-y)/2;c.addImage(n,"PNG",r,S,f,y)}c.setFontSize(10),c.setTextColor("#f4f4f4"),c.setFont("helvetica","normal");const u=d-8,p=t[1]+o[1]/2,g=t[2]+o[2]/2,m=t[3]+o[3]/2,h=t[4]+o[4]/2,v=t[5]+o[5]/2;if(c.text("Code",p,u,{align:"center"}),c.text("Description",g,u,{align:"center"}),!l.excludePrice&&!l.excludeQty){if(l.includeGst){c.setFont("helvetica","normal");const y=c.getTextWidth("Price ea "),b=m-c.getTextWidth("Price ea INC GST")/2;c.text("Price ea ",b,u),c.setFont("helvetica","bold"),c.text("INC GST",b+y,u),c.setFont("helvetica","normal")}else{c.setFont("helvetica","normal");const y=c.getTextWidth("Price ea "),b=m-c.getTextWidth("Price ea EX GST")/2;c.text("Price ea ",b,u),c.setFont("helvetica","bold"),c.text("EX GST",b+y,u),c.setFont("helvetica","normal")}c.text("Qty",h,u,{align:"center"}),c.text("Total",v,u,{align:"center"})}else l.excludePrice&&!l.excludeQty&&c.text("Qty",h,u,{align:"center"})}function loadImageAsDataURL(c,e){const t=new window.Image;t.crossOrigin="Anonymous",t.onload=function(){const o=document.createElement("canvas"),r=o.getContext("2d"),i=400,n=150;let s=t.width,a=t.height;if(s>i||a>n){const d=i/s,u=n/a,p=Math.min(d,u);s=Math.round(s*p),a=Math.round(a*p)}o.width=s,o.height=a,r.imageSmoothingEnabled=!0,r.imageSmoothingQuality="high",r.drawImage(t,0,0,s,a);const l=o.toDataURL("image/png",.9);e(l,s,a)},t.src=c}function ensurePdfSpinner(){if(!document.getElementById("pdf-spinner")){const c=document.createElement("div");if(c.id="pdf-spinner",c.style.display="none",c.style.position="fixed",c.style.top="0",c.style.left="0",c.style.width="100vw",c.style.height="100vh",c.style.zIndex="9999",c.style.background="rgba(255,255,255,0.7)",c.style.alignItems="center",c.style.justifyContent="center",c.innerHTML='<div style="border:6px solid #e0e0e0;border-top:6px solid #2563eb;border-radius:50%;width:54px;height:54px;animation:spin 1s linear infinite;"></div>',document.body.appendChild(c),!document.getElementById("pdf-spinner-style")){const e=document.createElement("style");e.id="pdf-spinner-style",e.innerHTML="@keyframes spin { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);} }",document.head.appendChild(e)}}}async function generateCsvBlobAsync(c,e){return new Promise(async t=>{if(!window.Papa)try{await Utils$1.loadScript("https://cdn.jsdelivr.net/npm/papaparse@5.4.1/papaparse.min.js")}catch(n){console.error("Failed to load PapaParse:",n),t(null);return}const o=JSON.parse(localStorage.getItem("selection")||"[]"),r=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let i=[];if(r.length>0?i=r.map(n=>({...n.product,Room:n.room,Notes:n.notes,Quantity:n.quantity,Timestamp:new Date(n.timestamp).toISOString()})):i=o,!i.length){t(null);return}setTimeout(()=>{const n=i.map(s=>{let a,l,d,u;const p=c.excludePrice;let g=0;if(s.UserEditedPrice!==void 0&&s.UserEditedPrice!==null&&s.UserEditedPrice!=="")g=parseFloat(s.UserEditedPrice.toString().replace(/,/g,""));else{const h=s.RRP_EX||s["RRP EX GST"]||s.RRP_EX||s.RRP_EXGST||"";g=parseFloat((h||"0").toString().replace(/,/g,""))}a=g,d="Price ea ex GST",u="Price Total ex GST",l=!isNaN(a)&&a>=0?(a*(s.Quantity||1)).toFixed(2):"";const m={Code:sanitizeCSVField(s.OrderCode||""),Description:sanitizeCSVField(s.Description||""),Quantity:s.Quantity||1,Notes:sanitizeCSVField(s.Notes||""),Room:sanitizeCSVField(s.Room||""),"Image URL":sanitizeCSVField(s.Image_URL||""),"Diagram URL":sanitizeCSVField(s.Diagram_URL||""),"Datasheet URL":sanitizeCSVField(s.Datasheet_URL||""),"Website URL":sanitizeCSVField(s.Website_URL||"")};return m[d]=p?"0.00":a>=0?a.toFixed(2):"",m[u]=p?"0.00":l,m});setTimeout(()=>{const s=window.Papa.unparse(n,{quotes:!0,quoteChar:'"',delimiter:",",header:!0,newline:`\r
