...
`).filter(s=>s.trim()!==""),i=r.length>0?r[r.length-1]:"Unknown",n=i.split(" - ")[0]||i;o.innerText=n,o.innerText.trim()||(o.innerText="v2.1.0")}}else throw new Error("Version file not found")}catch{const t=document.getElementById("version-number");if(t){const o=config.get("app.version")||"v2.1.0";t.innerText=o}else setTimeout(()=>{const o=document.getElementById("version-number");if(o&&!o.innerText.trim()){const r=config.get("app.version")||"v2.1.0";o.innerText=r}},1e3);console.info("Version loaded from config (GitHub Pages mode)")}}async showProductLookupScreen(){try{const t=await(await fetch("./screens/product-grid.html")).text();document.body.innerHTML=t,this.currentScreen="product-grid";const o=document.createElement("script");o.type="module",o.src="js/app.js",document.body.appendChild(o),setTimeout(()=>{document.querySelectorAll(".back-btn").forEach(r=>r.remove())},100),window.productGridManager&&window.productGridManager.init(),await this.loadVersion(),setTimeout(()=>this.loadVersion(),1e3)}catch(e){console.error("Failed to load product grid screen:",e)}}setupSplitInterface(){const e=document.getElementById("back-to-home");e&&(e.onclick=()=>location.reload());const t=document.getElementById("download-btn"),o=document.getElementById("clear-all-btn");t&&(t.onclick=()=>this.showDownloadFormModal()),o&&(o.onclick=()=>this.showClearConfirmModal()),this.setupSplitProductSearch(),this.setupReviewTable(),this.renderReviewTable(),this.loadInitialSearchResults()}setupSplitProductSearch(){const e=document.getElementById("product-search-input"),t=document.getElementById("search-results-list"),o=document.getElementById("search-loading"),r=document.getElementById("search-no-results");if(!e||!t)return;const i=[],n=Utils$1.debounce(s=>{this.performSplitProductSearch(s,t,i,o,r)},200);e.addEventListener("input",()=>{const s=e.value.trim();s?n(s):this.loadInitialSearchResults()}),t.addEventListener("click",s=>{const c=s.target.closest(".result-item");if(!c)return;const l=parseInt(c.getAttribute("data-idx"),10),d=i.length>0?i:this.currentSearchResults||[];!isNaN(l)&&d[l]&&this.showSplitProductDetails(d[l])})}performSplitProductSearch(e,t,o,r,i){if(!dataLayer.isLoaded){r.style.display="flex",i.style.display="none",t.innerHTML="";return}o.length=0,o.push(...dataLayer.searchProducts(e)),r.style.display="none",o.length===0?(i.style.display="flex",t.innerHTML=""):(i.style.display="none",t.innerHTML=o.map((n,s)=>`
      ...
      `}).join("")}getRoomOptions(e){let t=`<option value="Blank"${e==="Blank"?" selected":""}>Blank</option>`;return config.get("rooms.predefined",[]).forEach(i=>{t+=`<option value="${i.name}"${e===i.name?" selected":""}>${i.name}</option>`}),StorageManager.getCustomRooms().forEach(i=>{t+=`<option value="${i.name}"${e===i.name?" selected":""}>${i.name}</option>`}),t+='<option value="__ADD_NEW_ROOM__" style="font-weight: bold; color: #2563eb;">➕ Add new room...</option>',t}handleQuantityChange(e){const t=parseInt(e.getAttribute("data-index")),o=Math.max(1,parseInt(e.value)||1),r=StorageManager.getSelectedProducts();r[t]&&(r[t].quantity=o,StorageManager.setSelectedProducts(r),this.renderReviewTable(),this.updateSelectionCount())}handleRoomChange(e){const t=parseInt(e.getAttribute("data-index"));let o=e.value;if(o==="__ADD_NEW_ROOM__"){const i=prompt("Enter new room name:");if(i&&i.trim()){const n=i.trim();if(StorageManager.addCustomRoom(n)){o=n,console.log("✅ Added new room:",n),this.renderSelectionTable();return}else{alert("Room name already exists or is invalid");const s=StorageManager.getSelectedProducts();s[t]&&(e.value=s[t].room||"Blank");return}}else{const n=StorageManager.getSelectedProducts();n[t]&&(e.value=n[t].room||"Blank");return}}const r=StorageManager.getSelectedProducts();r[t]&&(r[t].room=o,StorageManager.setSelectedProducts(r),this.updateSelectionCount())}handleRemoveProduct(e){const t=parseInt(e.getAttribute("data-index")),o=StorageManager.getSelectedProducts();o[t]&&(o.splice(t,1),StorageManager.setSelectedProducts(o),this.renderReviewTable(),this.updateSelectionCount())}async showReviewScreen(){try{const t=await(await fetch("./screens/review.html")).text();document.body.innerHTML=t,this.currentScreen="review",this.setupReviewScreenHandlers(),this.renderReviewList();const o=StorageManager.getSelectedProducts();o.length>0&&(console.log(`🖼️ Preloading ${o.length} product images in background...`),preloadAllProductImages(o).then(r=>{console.log(`✅ ${r} images preloaded and ready for PDF`)}).catch(r=>{console.warn("Background preloading failed:",r)}))}catch(e){console.error("Failed to load review screen:",e)}}setupReviewScreenHandlers(){const e=document.getElementById("back-to-grid"),t=document.getElementById("add-more-btn"),o=document.getElementById("quick-pdf-btn");e&&(e.onclick=()=>this.showProductLookupScreen()),t&&(t.onclick=()=>this.showProductLookupScreen()),o&&(o.onclick=()=>this.showDownloadFormModal())}renderReviewList(){const e=document.getElementById("review-list"),t=document.getElementById("review-empty");if(!e)return;const o=StorageManager.getSelectedProducts();if(o.length===0){e.innerHTML="",t&&(t.style.display="block");return}t&&(t.style.display="none");const r={};o.forEach(i=>{const n=i.room||"Unassigned";r[n]||(r[n]=[]),r[n].push(i)}),e.innerHTML=Object.entries(r).map(([i,n])=>`
      <div class="review-room-group">
        <div class="review-room-header">${i} <span class="room-count">(${n.length})</span></div>
        ${n.map((s,c)=>{const l=s.product,d=l.Description||l.description||l.productName||l["Product Name"]||"Product",u=l.OrderCode||l.orderCode||"",p=l.Image_URL||l.imageUrl||"assets/no-image.png",m=l.RRP_EX||l["RRP EX GST"]||l.RRP_EX||l.rrpExGst||l.RRP_EXGST||l.RRP_INCGST||l["RRP INC GST"]||"0";return`
  ...
  `,o.appendChild(r),document.body.appendChild(o),document.getElementById("samsung-help-close").onclick=()=>{document.body.removeChild(o)},document.getElementById("samsung-help-retry").onclick=()=>{document.body.removeChild(o),setTimeout(()=>{downloadWithFallback(a,e,t)},1e3)},o.onclick=i=>{i.target===o&&document.body.removeChild(o)}}async function downloadWithFallback(a,e,t="file"){if(isSamsungDevice$1())try{if(await attemptStandardDownload(a,e))return;console.warn("Download may have failed on Samsung device"),showSamsungDownloadHelp(a,e,t)}catch(o){console.error("Samsung download failed:",o),showSamsungDownloadHelp(a,e,t)}else await downloadWithEnhancedFallbacks(a,e,t)}function showBrowserCompatibilityWarning(){if(isSamsungDevice$1()){if(document.getElementById("samsung-browser-warning"))return;const e=document.createElement("div");e.id="samsung-browser-warning",e.style.cssText=`
    ...
    `,document.body.appendChild(o),loadImageAsDataURL("./assets/seima-logo.png",(i,n,s)=>{const c=JSON.parse(localStorage.getItem("selection")||"[]"),l=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let d=[];if(l.length>0?d=l.map(E=>({...E.product,Room:E.room,Notes:E.notes,Quantity:E.quantity,Timestamp:new Date(E.timestamp).toISOString()})):d=c,!d.length){alert("No products selected."),t&&(t.style.display="none");return}if(!a.emailCompatible){const E=document.getElementById("pdf-processing-notification");if(E){const P=document.createElement("span");P.id="preload-progress",P.style.cssText="display: block; font-size: 12px; margin-top: 8px; color: #1e40af;",P.textContent="Loading images: 0%",E.appendChild(P)}preloadAllProductImages(d).then(P=>{const R=document.getElementById("preload-progress");R&&(R.textContent=`✓ ${P} images ready`,R.style.color="#059669")}).catch(P=>{console.warn("Image preloading error:",P)})}const u={};d.forEach(E=>{u[E.Room]||(u[E.Room]=[]),u[E.Room].push(E)});const{jsPDF:p}=window.jspdf,m=new p({orientation:"landscape",unit:"pt",format:"a4",compress:!0,putOnlyUsedFonts:!0,precision:16,userUnit:1,floatPrecision:16}),h=m.internal.pageSize.getWidth(),g=m.internal.pageSize.getHeight(),S=h/2,y=320,v=90,f=(h-y)/2,b=70;m.setFillColor(255,255,255),m.rect(f,b,y,v,"F");const T=localStorage.getItem("customerLogo");if(T)try{const E=new Image;E.onload=function(){const P=y-20,R=v-20,ae=E.width/E.height;let V=P,Y=P/ae;Y>R&&(Y=R,V=R*ae);const oe=f+(y-V)/2,re=b+(v-Y)/2;m.addImage(T,"PNG",oe,re,V,Y,void 0,"FAST")},E.src=T}catch(E){console.warn("Failed to draw customer logo:",E)}loadImageAsDataURL("./assets/seima-logo.png",(E,P,R)=>{const V=R&&P?250*R/P:65,Y=(h-250)/2,oe=b+v+80;m.addImage(E,"PNG",Y,oe,250,V,void 0,"FAST"),m.setFontSize(15),m.setTextColor("#444");const re=S;let ce=oe+V+50;const q=[];a!=null&&a.name&&a.name.trim()&&q.push({label:"Name:",value:a.name.trim(),bold:!0}),a!=null&&a.project&&a.project.trim()&&q.push({label:"Project:",value:a.project.trim(),bold:!0}),a!=null&&a.address&&a.address.trim()&&q.push({label:"Address:",value:a.address.trim(),bold:!0}),a!=null&&a.email&&a.email.trim()&&q.push({label:"Email:",value:a.email.trim(),bold:!0}),a!=null&&a.telephone&&a.telephone.trim()&&q.push({label:"Telephone:",value:a.telephone.trim(),bold:!0});const M=32;q.length>0&&q.forEach(J=>{m.setFont("helvetica","normal"),m.text(J.label,re-80,ce,{align:"right"}),m.setFont("helvetica","bold"),m.text(J.value,re-35,ce,{align:"left"}),ce+=26}),m.setFillColor("#9B9184"),m.rect(0,g-M,h,M,"F"),m.setTextColor("#fff"),m.setFontSize(13);const de=new Date,Le=["January","February","March","April","May","June","July","August","September","October","November","December"],ke=de.getDate(),Te=Le[de.getMonth()],De=de.getFullYear(),Ae=String(de.getHours()).padStart(2,"0"),Be=String(de.getMinutes()).padStart(2,"0"),Oe=`Printed ${ke} ${Te} ${De}, ${Ae}:${Be}`;m.text(Oe,16,g-10),m.text("www.seima.com.au",h-140,g-10);const me=StorageManager.getUserSettings(),_=me.staffName||"",K=me.staffPosition||"",Q=me.staffEmail||"",Z=me.staffPhone||"";let ee="For more information";_&&Z&&Q?ee=`For more information, please contact ${K?`${_}, ${K}`:_} on ${Z} or email ${Q}`:_&&Z?ee=`For more information, please contact ${K?`${_}, ${K}`:_} on ${Z}`:_&&Q?ee=`For more information, please contact ${K?`${_}, ${K}`:_} or email ${Q}`:Z&&Q?ee=`For more information, please call ${Z} or email ${Q}`:_?ee=`For more information, please contact ${K?`${_}, ${K}`:_}`:Z?ee=`For more information, please call ${Z}`:Q&&(ee=`For more information, please email ${Q}`),m.setFont("helvetica","normal"),m.setFontSize(14),m.setTextColor("#111"),m.text(ee,S,g-M-18,{align:"center"}),m.addPage(),loadImageAsDataURL("./assets/seima-logo-white.png",(J,Ee,Ie)=>{const he=h-200,fe=h-120,Ce=h-80,te=[32,236,321,he,fe,Ce],ie=[90,90,he-321,fe-he,Ce-fe,80];resetImageOptimizationStats();const Pe=(w,I,N,W,D,$,C)=>{if(!I||typeof I!="string"||I.length<10||!I.startsWith("http://")&&!I.startsWith("https://")&&!I.startsWith("data:")){C&&C();return}if(imageOptimizationStats.totalImages++,a.emailCompatible){imageOptimizationStats.failedImages++,C&&C();return}const A=getCachedImage(I);if(A&&A.dataUrl)try{const x=A.width/A.height;let U=D,L=D/x;L>$&&(L=$,U=$*x),w.addImage(A.dataUrl,A.format,N,W,U,L,void 0,"FAST"),imageOptimizationStats.optimizedImages++,C&&C();return}catch{console.warn("Failed to use cached image, falling back to direct load")}let O=!1;const G=["https://api.codetabs.com/v1/proxy?quest=","https://corsproxy.io/?"];let F=0;function se(){if(O)return;const x=new Image;x.crossOrigin="Anonymous";let U=null;x.onload=function(){if(!O){O=!0,U&&clearTimeout(U);try{const B=getOptimizedFileSettings(0),k=B.imageMaxWidth,le=D,be=$;try{const z=document.createElement("canvas"),j=z.getContext("2d"),{width:X,height:H}=calculateOptimizedDimensions(x.width,x.height,k);z.width=X,z.height=H,j.imageSmoothingEnabled=!0,j.imageSmoothingQuality="high",j.drawImage(x,0,0,X,H);let Se,xe="JPEG";const Fe=detectTransparency(z,j),Ne=isTechnicalDiagram(x);Fe||Ne?(Se=z.toDataURL("image/png",B.imageQuality),xe="PNG"):(Se=z.toDataURL("image/jpeg",B.imageQuality),xe="JPEG");const Me=`img_${generateImageHash(I)}`;w.addImage(Se,xe,N,W,le,be,Me,"FAST"),imageOptimizationStats.optimizedImages++,C&&C()}catch(z){console.warn(`Failed to optimize image: ${I}`,z),console.warn("Error details:",z.message,z.stack);try{w.addImage(x,"JPEG",N,W,le,be),imageOptimizationStats.optimizedImages++,C&&C()}catch(j){console.error(`Fallback also failed for: ${I}`,j),imageOptimizationStats.failedImages++,C&&C()}}}catch(B){console.warn("Failed to add image to PDF:",B),imageOptimizationStats.failedImages++,C&&C()}}},x.onerror=function(){O||(U&&clearTimeout(U),console.warn(`Failed to load image with proxy ${F}: ${I}`),console.warn(`Error details for: ${I} - Proxy: ${G[F]}`),F++,F<G.length?setTimeout(()=>{se()},200):(O=!0,console.warn("All proxies failed, skipping image"),imageOptimizationStats.failedImages++,C&&C()))},U=setTimeout(()=>{O||(console.warn(`⏰ Timeout with proxy ${F}: ${I}`),x.src="",x.onload=null,x.onerror=null,F++,F<G.length?setTimeout(()=>{se()},200):(O=!0,console.warn("All proxies timed out, skipping image"),imageOptimizationStats.failedImages++,C&&C()))},3e3);let L=I;F<G.length&&(L=G[F]+encodeURIComponent(I)),x.src=L}se()},ne=[];Object.keys(u).forEach((w,I)=>{const N=u[w];if(!N||!Array.isArray(N)){console.warn("⚠️ Skipping invalid room items:",w,N);return}N.forEach((W,D)=>{if(!W){console.warn("⚠️ Skipping null item in room:",w,"at index:",D);return}ne.push({item:W,room:w,rIdx:I,iIdx:D,isFirstInRoom:D===0,roomCount:N.length})})}),ne.reduce((w,I)=>{if(!I||!I.item)return console.warn("⚠️ Skipping null row in data analysis:",I),w;const N=String(I.item.Description||""),W=String(I.item.LongDescription||""),D=String(I.item.Notes||""),$=String(I.item.OrderCode||"");return w+N.length+W.length+D.length+$.length},0);let ue=0,pe=0;const Re=4,ge=8,ye=Math.floor((g-80)/Re);let we=M+8;function ve(){if(!ne||!Array.isArray(ne)){console.error("❌ Critical error: rowsToDraw is not a valid array:",ne),showDetailedErrorMessage(new Error("Invalid product data structure"),"generating PDF","unknown.pdf");return}if(ue>=ne.length){const D=m.internal.getNumberOfPages()-1;for(let L=2;L<=D+1;L++)m.setPage(L),drawPDFHeader(m,h,te,ie,32,M,J,Ee,Ie,a),we=M+8,m.setFillColor("#9B9184"),m.rect(0,g-M,h,M,"F"),m.setTextColor("#fff"),m.setFontSize(11),m.text("www.seima.com.au",h-140,g-10),m.text(`Page ${L-1} of ${D}`,32,g-10);const $=new Date,C=String($.getDate()).padStart(2,"0"),A=String($.getMonth()+1).padStart(2,"0"),O=String($.getFullYear()).slice(-2),G=String($.getHours()).padStart(2,"0"),F=String($.getMinutes()).padStart(2,"0"),x=`${a.project.replace(/[^a-zA-Z0-9\s]/g,"")}-${C}${A}${O}.${G}${F}.pdf`,U=document.getElementById("pdf-processing-notification");U&&U.remove(),showImageOptimizationSummary(a.emailCompatible);try{const L=m.output("blob"),B=m.output("string"),k=B?B.match(/\/Type\s*\/XObject/g):null,le=B?B.match(/Tj\s/g):null,be=B?B.match(/\/A\s*<</g):null;a.pdfSize=L.size;const z=showFileSizeInfo(L,x);if(a.sendEmail&&L.size>15*1024*1024){console.warn(`❌ PDF too large for email (${(L.size/1024/1024).toFixed(1)}MB), offering email-compatible version`),showEmailCompatibleOption(a,x);return}const j=createOptimizedBlob(L,z.settings);if(a.sendEmail&&a.email)if(a.exportCsv){const X=x.replace(/\.pdf$/,".csv");generateCsvBlobAsync(a,X).then(H=>{window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:j,csvBlob:H}}))}).catch(H=>{console.error("Async CSV generation for email failed:",H),window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:j,csvBlob:null}}))})}else window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:j,csvBlob:null}}));else if((async()=>{const X=await mergeWithTipTail(j);downloadWithFallback(X,x,"PDF")})(),a.exportCsv){const X=x.replace(/\.pdf$/,".csv");setTimeout(()=>{generateCsvBlobAsync(a,X).then(H=>{H&&downloadWithFallback(H,X,"CSV")}).catch(H=>{console.error("CSV generation failed:",H)})},1e3)}}catch(L){console.error("PDF generation failed:",L),showDetailedErrorMessage(L,"generating PDF",x);const B=document.getElementById("pdf-processing-notification");B&&B.remove()}t&&(t.style.display="none");return}pe>=Re&&(m.addPage(),drawPDFHeader(m,h,te,ie,32,M,J,Ee,Ie,a),we=M+8,pe=0);const w=ne[ue];if(!w||!w.item){console.warn(`⚠️  Skipping invalid row at index ${ue}:`,w),ue++,ve();return}const I=we+ye*pe;w.isFirstInRoom&&(m.setFontSize(12),m.setFont("helvetica","bold"),m.setTextColor("#333"),m.text(`${w.room} (${w.roomCount})`,32,I+10),m.setFont("helvetica","normal"));const N=te[0],W=N+90+12;Pe(m,w.item.Image_URL||"",N,I+ge+16,90,ye-ge*2,()=>{Pe(m,w.item.Diagram_URL||"",W,I+ge+16,90,ye-ge*2,()=>{m.setFontSize(10),m.setTextColor("#222");const D=I+28,C=te[1]+ie[1]/2;m.text(String(w.item.OrderCode||""),C,D+10,{align:"center"});let A=D+35;if(w.item.Datasheet_URL&&w.item.Datasheet_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Datasheet",C,A,{url:w.item.Datasheet_URL,align:"center"});const k=m.getTextWidth("Datasheet");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(C-k/2,A+1.5,C+k/2,A+1.5),A+=16}if(w.item.Website_URL&&w.item.Website_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Website",C,A,{url:w.item.Website_URL,align:"center"});const k=m.getTextWidth("Website");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(C-k/2,A+1.5,C+k/2,A+1.5),A+=16}let O=D+10;m.setFontSize(10),m.setTextColor("#222");const G=ie[2]-10,F=te[2],se=m.splitTextToSize(String(w.item.Description||""),G);if(m.text(se,F+5,O),O+=se.length*12,!a.excludeLongDescription&&(w.item.LongDescription||w.item["Long Description"]||w.item.longDescription)){const k=w.item.LongDescription||w.item["Long Description"]||w.item.longDescription;m.setFontSize(9),m.setTextColor("#444");const le=m.splitTextToSize(String(k),G);m.text(le,F+5,O),O+=le.length*11}if(w.item.Notes){m.setFont("helvetica","italic"),m.setFontSize(9),m.setTextColor("#444");const k=m.splitTextToSize(`Notes: ${String(w.item.Notes).replace(/\r?\n|\r/g," ")}`,G);m.text(k,F+5,O),O+=k.length*11,m.setFont("helvetica","normal")}m.setFontSize(10),m.setTextColor("#222");let x=NaN;if(w.item.UserEditedPrice!==void 0&&w.item.UserEditedPrice!==null&&w.item.UserEditedPrice!=="")x=parseFloat(w.item.UserEditedPrice.toString().replace(/,/g,""));else{const k=w.item.RRP_EX||w.item["RRP EX GST"]||w.item.RRP_EX||w.item.RRP_EXGST;k&&(x=parseFloat(k.toString().replace(/,/g,"")))}!isNaN(x)&&x>0&&a.includeGst&&(x=x*1.1);const U=!isNaN(x)&&x>=0?`$${x.toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2})}`:"";if(!a.excludePrice&&!a.excludeQty){const k=te[3]+ie[3]/2;m.text(U,k,D+10,{align:"center"})}if(m.setFontSize(10),m.setTextColor("#222"),!a.excludeQty){const k=te[4]+ie[4]/2;m.text(String(w.item.Quantity||1),k,D+10,{align:"center"})}m.setFontSize(10),m.setTextColor("#222");const L=x*(w.item.Quantity||1),B=!isNaN(x)&&x>=0?`$${L.toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2})}`:"";if(!a.excludePrice&&!a.excludeQty){const k=te[5]+ie[5]/2;m.text(B,k,D+10,{align:"center"})}ue++,pe++,ve()})})}ve()})})})}function drawPDFHeader(a,e,t,o,r,i,n,s,c,l){const d=i+5.7;if(a.setFillColor("#8B6C2B"),a.rect(0,0,e,d,"F"),n&&s&&c){const y=d*.55,v=s/c;let f=y*v;f>80&&(f=80);const b=(d-y)/2;a.addImage(n,"PNG",r,b,f,y)}a.setFontSize(10),a.setTextColor("#f4f4f4"),a.setFont("helvetica","normal");const u=d-8,p=t[1]+o[1]/2,m=t[2]+o[2]/2,h=t[3]+o[3]/2,g=t[4]+o[4]/2,S=t[5]+o[5]/2;if(a.text("Code",p,u,{align:"center"}),a.text("Description",m,u,{align:"center"}),!l.excludePrice&&!l.excludeQty){if(l.includeGst){a.setFont("helvetica","normal");const y=a.getTextWidth("Price ea "),v=h-a.getTextWidth("Price ea INC GST")/2;a.text("Price ea ",v,u),a.setFont("helvetica","bold"),a.text("INC GST",v+y,u),a.setFont("helvetica","normal")}else{a.setFont("helvetica","normal");const y=a.getTextWidth("Price ea "),v=h-a.getTextWidth("Price ea EX GST")/2;a.text("Price ea ",v,u),a.setFont("helvetica","bold"),a.text("EX GST",v+y,u),a.setFont("helvetica","normal")}a.text("Qty",g,u,{align:"center"}),a.text("Total",S,u,{align:"center"})}else l.excludePrice&&!l.excludeQty&&a.text("Qty",g,u,{align:"center"})}function loadImageAsDataURL(a,e){const t=new window.Image;t.crossOrigin="Anonymous",t.onload=function(){const o=document.createElement("canvas"),r=o.getContext("2d"),i=400,n=150;let s=t.width,c=t.height;if(s>i||c>n){const d=i/s,u=n/c,p=Math.min(d,u);s=Math.round(s*p),c=Math.round(c*p)}o.width=s,o.height=c,r.imageSmoothingEnabled=!0,r.imageSmoothingQuality="high",r.drawImage(t,0,0,s,c);const l=o.toDataURL("image/png",.9);e(l,s,c)},t.src=a}function ensurePdfSpinner(){if(!document.getElementById("pdf-spinner")){const a=document.createElement("div");if(a.id="pdf-spinner",a.style.display="none",a.style.position="fixed",a.style.top="0",a.style.left="0",a.style.width="100vw",a.style.height="100vh",a.style.zIndex="9999",a.style.background="rgba(255,255,255,0.7)",a.style.alignItems="center",a.style.justifyContent="center",a.innerHTML='<div style="border:6px solid #e0e0e0;border-top:6px solid #2563eb;border-radius:50%;width:54px;height:54px;animation:spin 1s linear infinite;"></div>',document.body.appendChild(a),!document.getElementById("pdf-spinner-style")){const e=document.createElement("style");e.id="pdf-spinner-style",e.innerHTML="@keyframes spin { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);} }",document.head.appendChild(e)}}}async function generateCsvBlobAsync(a,e){return new Promise(async t=>{if(!window.Papa)try{await Utils$1.loadScript("https://cdn.jsdelivr.net/npm/papaparse@5.4.1/papaparse.min.js")}catch(n){console.error("Failed to load PapaParse:",n),t(null);return}const o=JSON.parse(localStorage.getItem("selection")||"[]"),r=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let i=[];if(r.length>0?i=r.map(n=>({...n.product,Room:n.room,Notes:n.notes,Quantity:n.quantity,Timestamp:new Date(n.timestamp).toISOString()})):i=o,!i.length){t(null);return}setTimeout(()=>{const n=i.map(s=>{let c,l,d,u;const p=a.excludePrice;let m=0;if(s.UserEditedPrice!==void 0&&s.UserEditedPrice!==null&&s.UserEditedPrice!=="")m=parseFloat(s.UserEditedPrice.toString().replace(/,/g,""));else{const g=s.RRP_EX||s["RRP EX GST"]||s.RRP_EX||s.RRP_EXGST||"";m=parseFloat((g||"0").toString().replace(/,/g,""))}c=m,d="Price ea ex GST",u="Price Total ex GST",l=!isNaN(c)&&c>=0?(c*(s.Quantity||1)).toFixed(2):"";const h={Code:sanitizeCSVField(s.OrderCode||""),Description:sanitizeCSVField(s.Description||""),Quantity:s.Quantity||1,Notes:sanitizeCSVField(s.Notes||""),Room:sanitizeCSVField(s.Room||""),"Image URL":sanitizeCSVField(s.Image_URL||""),"Diagram URL":sanitizeCSVField(s.Diagram_URL||""),"Datasheet URL":sanitizeCSVField(s.Datasheet_URL||""),"Website URL":sanitizeCSVField(s.Website_URL||"")};return h[d]=p?"0.00":c>=0?c.toFixed(2):"",h[u]=p?"0.00":l,h});setTimeout(()=>{const s=window.Papa.unparse(n,{quotes:!0,quoteChar:'"',delimiter:",",header:!0,newline:`\r
...
This can be restored later.`))try{const t=await presentationLoader.deleteSelections([e.id]);t.success?(this.showToast("Selection deleted"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to delete")}catch(t){this.showError(t.message)}}async restoreSelection(e){try{const t=await presentationLoader.restoreSelections([e.id]);t.success?(this.showToast("Selection restored"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to restore")}catch(t){this.showError(t.message)}}showToast(e){const t=document.createElement("div");t.style.cssText=`
    ...
    `;this.injectSaveDialogStyles(),document.body.insertAdjacentHTML("beforeend",i);const n=document.getElementById("save-dialog"),s=document.getElementById("save-doc-name"),c=document.getElementById("save-notes");s==null||s.focus(),s==null||s.select(),n.querySelectorAll("button[data-action]").forEach(d=>{d.addEventListener("click",async()=>{const u=d.dataset.action;if(u==="cancel"){n.remove();return}const p=(s==null?void 0:s.value.trim())||"Untitled Selection",m=(c==null?void 0:c.value.trim())||"";this.currentSelectionName=p;const h={...this.prepareSelectionData(),documentName:p,notes:m};n.querySelectorAll("button").forEach(g=>g.disabled=!0),d.textContent="Saving...";try{let g;u==="save-update"?g=await presentationRecorder.updateSelection(this.currentSelectionId,h):(g=await presentationRecorder.saveSelection(h),g.success&&g.id&&(this.currentSelectionId=g.id)),n.remove(),g.success?(this.hasUnsavedChanges=!1,this.lastSaveTime=new Date,this.updateContextHeader(),toast.success(u==="save-update"?"Selection updated!":"Selection saved!")):toast.error("Failed to save: "+(g.error||"Unknown error"))}catch(g){n.remove(),toast.error("Failed to save: "+g.message)}})}),n.addEventListener("click",d=>{d.target===n&&n.remove()});const l=d=>{d.key==="Escape"&&(n.remove(),document.removeEventListener("keydown",l))};document.addEventListener("keydown",l)}injectSaveDialogStyles(){if(document.getElementById("save-dialog-styles"))return;document.head.insertAdjacentHTML("beforeend",`
      <style id="save-dialog-styles">
        .save-dialog-overlay {
          position: fixed;
          top: 0;
    ...
    `;const r=t.filter(n=>n.type==="critical"),i=e.score<config.get("compatibility.minCompatibilityScore",70);r.length>0||i?o.innerHTML=`
      ...
      `,document.body.insertBefore(o,document.body.firstChild),!i&&r.length===0&&setTimeout(()=>{o.parentElement&&o.remove()},15e3)}setupGlobalEventListeners(){window.addEventListener("generatePdf",e=>{const{tipTailSettings:t,...o}=e.detail;ensurePdfSpinner(),showPdfFormScreen$1(o,t||null)}),window.addEventListener("beforeunload",()=>{}),browserCompatibility.features.memoryAPI&&setInterval(()=>{const e=browserCompatibility.memoryInfo;e.memoryPressure==="high"&&console.warn("High memory usage detected:",e)},6e4)}getSelectedProducts(){return StorageManager.getSelectedProducts()}clearSelection(){return StorageManager.clearAllSelections()}addProduct(e,t,o,r){return StorageManager.addProductToSelection(e,t,o,r)}updateSelectionCount(){this.navigationManager&&this.navigationManager.updateSelectionCount()}showError(e){alert(e)}}document.addEventListener("DOMContentLoaded",()=>{window.seimaScanner=new SeimaScanner,window.seimaScanner.init()});window.addEventListener("DOMContentLoaded",()=>{fetch("./version.txt").then(a=>a.text()).then(a=>{const e=document.getElementById("app-version");if(e){const t=a.trim().split(/\s+|-/)[0];e.textContent=`Ver: ${t}`,e.addEventListener("click",()=>{showChangelog(a.trim(),t)})}}),setupHelpButton(),setupUserMenu()});function setupHelpButton(){const a=document.getElementById("help-btn");a&&a.addEventListener("click",()=>{showUserGuide()});const e=document.getElementById("quick-start-btn");e&&e.addEventListener("click",()=>{onboarding.showForced()})}function setupUserMenu(){const a=document.getElementById("user-menu-container"),e=document.getElementById("user-menu-trigger"),t=document.getElementById("user-menu-dropdown"),o=document.getElementById("sign-in-btn"),r=document.getElementById("user-avatar"),i=document.getElementById("user-name-display");function n(l){var d,u,p,m;if(l){a&&(a.style.display="block"),o&&(o.style.display="none");const h=s(l.name);r&&(r.textContent=h),i&&(i.textContent=((d=l.name)==null?void 0:d.split(" ")[0])||"User"),t&&(t.innerHTML=`
        ...
        `,(u=document.getElementById("menu-profile"))==null||u.addEventListener("click",()=>{t.style.display="none",showProfileModal(l)}),(p=document.getElementById("menu-password"))==null||p.addEventListener("click",()=>{t.style.display="none",showChangePasswordModal()}),(m=document.getElementById("menu-logout"))==null||m.addEventListener("click",()=>{t.style.display="none",authService.logout(),n(null)}))}else a&&(a.style.display="none"),o&&(o.style.display="block")}function s(l){if(!l)return"?";const d=l.trim().split(" ");return d.length>=2?(d[0][0]+d[d.length-1][0]).toUpperCase():l.substring(0,2).toUpperCase()}function c(l){const d=document.createElement("div");return d.textContent=l||"",d.innerHTML}e&&t&&(e.addEventListener("click",l=>{l.stopPropagation();const d=t.style.display!=="none";t.style.display=d?"none":"block"}),document.addEventListener("click",l=>{a!=null&&a.contains(l.target)||(t.style.display="none")})),o&&o.addEventListener("click",()=>{authUI.showLogin(l=>{n(l)})}),authService.onAuthChange=n,n(authService.getCurrentUser())}function showProfileModal(a){var t;const e=`
