...
`).filter(s=>s.trim()!==""),i=r.length>0?r[r.length-1]:"Unknown",n=i.split(" - ")[0]||i;o.innerText=n,o.innerText.trim()||(o.innerText="v2.1.0")}}else throw new Error("Version file not found")}catch{const t=document.getElementById("version-number");if(t){const o=config.get("app.version")||"v2.1.0";t.innerText=o}else setTimeout(()=>{const o=document.getElementById("version-number");if(o&&!o.innerText.trim()){const r=config.get("app.version")||"v2.1.0";o.innerText=r}},1e3);console.info("Version loaded from config (GitHub Pages mode)")}}async showProductLookupScreen(){try{const t=await(await fetch("./screens/product-grid.html")).text();document.body.innerHTML=t,this.currentScreen="product-grid";const o=document.createElement("script");o.type="module",o.src="js/app.js",document.body.appendChild(o),setTimeout(()=>{document.querySelectorAll(".back-btn").forEach(r=>r.remove())},100),window.productGridManager&&window.productGridManager.init(),await this.loadVersion(),setTimeout(()=>this.loadVersion(),1e3)}catch(e){console.error("Failed to load product grid screen:",e)}}setupSplitInterface(){const e=document.getElementById("back-to-home");e&&(e.onclick=()=>location.reload());const t=document.getElementById("download-btn"),o=document.getElementById("clear-all-btn");t&&(t.onclick=()=>this.showDownloadFormModal()),o&&(o.onclick=()=>this.showClearConfirmModal()),this.setupSplitProductSearch(),this.setupReviewTable(),this.renderReviewTable(),this.loadInitialSearchResults()}setupSplitProductSearch(){const e=document.getElementById("product-search-input"),t=document.getElementById("search-results-list"),o=document.getElementById("search-loading"),r=document.getElementById("search-no-results");if(!e||!t)return;const i=[],n=Utils$1.debounce(s=>{this.performSplitProductSearch(s,t,i,o,r)},200);e.addEventListener("input",()=>{const s=e.value.trim();s?n(s):this.loadInitialSearchResults()}),t.addEventListener("click",s=>{const c=s.target.closest(".result-item");if(!c)return;const l=parseInt(c.getAttribute("data-idx"),10),d=i.length>0?i:this.currentSearchResults||[];!isNaN(l)&&d[l]&&this.showSplitProductDetails(d[l])})}performSplitProductSearch(e,t,o,r,i){if(!dataLayer.isLoaded){r.style.display="flex",i.style.display="none",t.innerHTML="";return}o.length=0,o.push(...dataLayer.searchProducts(e)),r.style.display="none",o.length===0?(i.style.display="flex",t.innerHTML=""):(i.style.display="none",t.innerHTML=o.map((n,s)=>`
  ...
  `,o.appendChild(r),document.body.appendChild(o),document.getElementById("samsung-help-close").onclick=()=>{document.body.removeChild(o)},document.getElementById("samsung-help-retry").onclick=()=>{document.body.removeChild(o),setTimeout(()=>{downloadWithFallback(a,e,t)},1e3)},o.onclick=i=>{i.target===o&&document.body.removeChild(o)}}async function downloadWithFallback(a,e,t="file"){if(isSamsungDevice$1())try{if(await attemptStandardDownload(a,e))return;console.warn("Download may have failed on Samsung device"),showSamsungDownloadHelp(a,e,t)}catch(o){console.error("Samsung download failed:",o),showSamsungDownloadHelp(a,e,t)}else await downloadWithEnhancedFallbacks(a,e,t)}function showBrowserCompatibilityWarning(){if(isSamsungDevice$1()){if(document.getElementById("samsung-browser-warning"))return;const e=document.createElement("div");e.id="samsung-browser-warning",e.style.cssText=`
    ...
    `,document.body.appendChild(o),loadImageAsDataURL("./assets/seima-logo.png",(i,n,s)=>{const c=JSON.parse(localStorage.getItem("selection")||"[]"),l=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let d=[];if(l.length>0?d=l.map(E=>({...E.product,Room:E.room,Notes:E.notes,Quantity:E.quantity,Timestamp:new Date(E.timestamp).toISOString()})):d=c,!d.length){alert("No products selected."),t&&(t.style.display="none");return}if(!a.emailCompatible){const E=document.getElementById("pdf-processing-notification");if(E){const R=document.createElement("span");R.id="preload-progress",R.style.cssText="display: block; font-size: 12px; margin-top: 8px; color: #1e40af;",R.textContent="Loading images: 0%",E.appendChild(R)}preloadAllProductImages(d).then(R=>{const L=document.getElementById("preload-progress");L&&(L.textContent=`✓ ${R} images ready`,L.style.color="#059669")}).catch(R=>{console.warn("Image preloading error:",R)})}const u={};d.forEach(E=>{u[E.Room]||(u[E.Room]=[]),u[E.Room].push(E)});const{jsPDF:p}=window.jspdf,m=new p({orientation:"landscape",unit:"pt",format:"a4",compress:!0,putOnlyUsedFonts:!0,precision:16,userUnit:1,floatPrecision:16}),h=m.internal.pageSize.getWidth(),g=m.internal.pageSize.getHeight(),S=h/2,y=320,v=90,f=(h-y)/2,b=70;m.setFillColor(255,255,255),m.rect(f,b,y,v,"F");const T=localStorage.getItem("customerLogo");if(T)try{const E=new Image;E.onload=function(){const R=y-20,L=v-20,se=E.width/E.height;let V=R,Y=R/se;Y>L&&(Y=L,V=L*se);const te=f+(y-V)/2,oe=b+(v-Y)/2;m.addImage(T,"PNG",te,oe,V,Y,void 0,"FAST")},E.src=T}catch(E){console.warn("Failed to draw customer logo:",E)}loadImageAsDataURL("./assets/seima-logo.png",(E,R,L)=>{const V=L&&R?250*L/R:65,Y=(h-250)/2,te=b+v+80;m.addImage(E,"PNG",Y,te,250,V,void 0,"FAST"),m.setFontSize(15),m.setTextColor("#444");const oe=S;let ae=te+V+50;const H=[];a!=null&&a.name&&a.name.trim()&&H.push({label:"Name:",value:a.name.trim(),bold:!0}),a!=null&&a.project&&a.project.trim()&&H.push({label:"Project:",value:a.project.trim(),bold:!0}),a!=null&&a.address&&a.address.trim()&&H.push({label:"Address:",value:a.address.trim(),bold:!0}),a!=null&&a.email&&a.email.trim()&&H.push({label:"Email:",value:a.email.trim(),bold:!0}),a!=null&&a.telephone&&a.telephone.trim()&&H.push({label:"Telephone:",value:a.telephone.trim(),bold:!0});const $=32;H.length>0&&H.forEach(J=>{m.setFont("helvetica","normal"),m.text(J.label,oe-80,ae,{align:"right"}),m.setFont("helvetica","bold"),m.text(J.value,oe-35,ae,{align:"left"}),ae+=26}),m.setFillColor("#9B9184"),m.rect(0,g-$,h,$,"F"),m.setTextColor("#fff"),m.setFontSize(13);const ce=new Date,ke=["January","February","March","April","May","June","July","August","September","October","November","December"],Te=ce.getDate(),De=ke[ce.getMonth()],Ae=ce.getFullYear(),Be=String(ce.getHours()).padStart(2,"0"),Oe=String(ce.getMinutes()).padStart(2,"0"),Fe=`Printed ${Te} ${De} ${Ae}, ${Be}:${Oe}`;m.text(Fe,16,g-10),m.text("www.seima.com.au",h-140,g-10);const ue=StorageManager.getUserSettings(),G=ue.staffName||"",X=ue.staffPosition||"",K=ue.staffEmail||"",Q=ue.staffPhone||"";let Z="For more information";G&&Q&&K?Z=`For more information, please contact ${X?`${G}, ${X}`:G} on ${Q} or email ${K}`:G&&Q?Z=`For more information, please contact ${X?`${G}, ${X}`:G} on ${Q}`:G&&K?Z=`For more information, please contact ${X?`${G}, ${X}`:G} or email ${K}`:Q&&K?Z=`For more information, please call ${Q} or email ${K}`:G?Z=`For more information, please contact ${X?`${G}, ${X}`:G}`:Q?Z=`For more information, please call ${Q}`:K&&(Z=`For more information, please email ${K}`),m.setFont("helvetica","normal"),m.setFontSize(14),m.setTextColor("#111"),m.text(Z,S,g-$-18,{align:"center"}),m.addPage(),loadImageAsDataURL("./assets/seima-logo-white.png",(J,Ee,Ie)=>{const he=h-200,fe=h-120,Ce=h-80,ee=[32,236,321,he,fe,Ce],re=[90,90,he-321,fe-he,Ce-fe,80];resetImageOptimizationStats();const Pe=(w,I,F,q,D,U,C)=>{if(!I||typeof I!="string"||I.length<10||!I.startsWith("http://")&&!I.startsWith("https://")&&!I.startsWith("data:")){C&&C();return}if(imageOptimizationStats.totalImages++,a.emailCompatible){imageOptimizationStats.failedImages++,C&&C();return}const A=getCachedImage(I);if(A&&A.dataUrl)try{const x=A.width/A.height;let _=D,k=D/x;k>U&&(k=U,_=U*x),w.addImage(A.dataUrl,A.format,F,q,_,k,void 0,"FAST"),imageOptimizationStats.optimizedImages++,C&&C();return}catch{console.warn("Failed to use cached image, falling back to direct load")}let B=!1;const W=["","https://images.weserv.nl/?url=","https://api.allorigins.win/raw?url="];let N=0;function ne(){if(B)return;const x=new Image;x.crossOrigin="Anonymous";let _=null;x.onload=function(){if(!B){B=!0,_&&clearTimeout(_);try{const P=getOptimizedFileSettings(0),de=P.imageMaxWidth,be=D,ge=U;try{const O=document.createElement("canvas"),z=O.getContext("2d"),{width:j,height:Le}=calculateOptimizedDimensions(x.width,x.height,de);O.width=j,O.height=Le,z.imageSmoothingEnabled=!0,z.imageSmoothingQuality="high",z.drawImage(x,0,0,j,Le);let Se,xe="JPEG";const Ne=detectTransparency(O,z),Me=isTechnicalDiagram(x);Ne||Me?(Se=O.toDataURL("image/png",P.imageQuality),xe="PNG"):(Se=O.toDataURL("image/jpeg",P.imageQuality),xe="JPEG");const $e=`img_${generateImageHash(I)}`;w.addImage(Se,xe,F,q,be,ge,$e,"FAST"),imageOptimizationStats.optimizedImages++,C&&C()}catch(O){console.warn(`Failed to optimize image: ${I}`,O),console.warn("Error details:",O.message,O.stack);try{w.addImage(x,"JPEG",F,q,be,ge),imageOptimizationStats.optimizedImages++,C&&C()}catch(z){console.error(`Fallback also failed for: ${I}`,z),imageOptimizationStats.failedImages++,C&&C()}}}catch(P){console.warn("Failed to add image to PDF:",P),imageOptimizationStats.failedImages++,C&&C()}}},x.onerror=function(){B||(_&&clearTimeout(_),console.warn(`Failed to load image with proxy ${N}: ${I}`),console.warn(`Error details for: ${I} - Proxy: ${W[N]||"direct"}`),N++,N<W.length?setTimeout(()=>{ne()},100):(B=!0,console.warn("All proxies failed, skipping image"),imageOptimizationStats.failedImages++,C&&C()))},_=setTimeout(()=>{B||(console.warn(`⏰ Timeout with proxy ${N}: ${I}`),x.src="",x.onload=null,x.onerror=null,N++,N<W.length?setTimeout(()=>{ne()},50):(B=!0,console.warn("All proxies timed out, skipping image"),imageOptimizationStats.failedImages++,C&&C()))},1500);let k=I;const M=W[N];M&&(k=M+encodeURIComponent(I)),x.src=k}ne()},ie=[];Object.keys(u).forEach((w,I)=>{const F=u[w];if(!F||!Array.isArray(F)){console.warn("⚠️ Skipping invalid room items:",w,F);return}F.forEach((q,D)=>{if(!q){console.warn("⚠️ Skipping null item in room:",w,"at index:",D);return}ie.push({item:q,room:w,rIdx:I,iIdx:D,isFirstInRoom:D===0,roomCount:F.length})})}),ie.reduce((w,I)=>{if(!I||!I.item)return console.warn("⚠️ Skipping null row in data analysis:",I),w;const F=String(I.item.Description||""),q=String(I.item.LongDescription||""),D=String(I.item.Notes||""),U=String(I.item.OrderCode||"");return w+F.length+q.length+D.length+U.length},0);let le=0,me=0;const Re=4,pe=8,ye=Math.floor((g-80)/Re);let we=$+8;function ve(){if(!ie||!Array.isArray(ie)){console.error("❌ Critical error: rowsToDraw is not a valid array:",ie),showDetailedErrorMessage(new Error("Invalid product data structure"),"generating PDF","unknown.pdf");return}if(le>=ie.length){const D=m.internal.getNumberOfPages()-1;for(let k=2;k<=D+1;k++)m.setPage(k),drawPDFHeader(m,h,ee,re,32,$,J,Ee,Ie,a),we=$+8,m.setFillColor("#9B9184"),m.rect(0,g-$,h,$,"F"),m.setTextColor("#fff"),m.setFontSize(11),m.text("www.seima.com.au",h-140,g-10),m.text(`Page ${k-1} of ${D}`,32,g-10);const U=new Date,C=String(U.getDate()).padStart(2,"0"),A=String(U.getMonth()+1).padStart(2,"0"),B=String(U.getFullYear()).slice(-2),W=String(U.getHours()).padStart(2,"0"),N=String(U.getMinutes()).padStart(2,"0"),x=`${a.project.replace(/[^a-zA-Z0-9\s]/g,"")}-${C}${A}${B}.${W}${N}.pdf`,_=document.getElementById("pdf-processing-notification");_&&_.remove(),showImageOptimizationSummary(a.emailCompatible);try{const k=m.output("blob"),M=m.output("string"),P=M?M.match(/\/Type\s*\/XObject/g):null,de=M?M.match(/Tj\s/g):null,be=M?M.match(/\/A\s*<</g):null;a.pdfSize=k.size;const ge=showFileSizeInfo(k,x);if(a.sendEmail&&k.size>15*1024*1024){console.warn(`❌ PDF too large for email (${(k.size/1024/1024).toFixed(1)}MB), offering email-compatible version`),showEmailCompatibleOption(a,x);return}const O=createOptimizedBlob(k,ge.settings);if(a.sendEmail&&a.email)if(a.exportCsv){const z=x.replace(/\.pdf$/,".csv");generateCsvBlobAsync(a,z).then(j=>{window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:O,csvBlob:j}}))}).catch(j=>{console.error("Async CSV generation for email failed:",j),window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:O,csvBlob:null}}))})}else window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:O,csvBlob:null}}));else if((async()=>{const z=await mergeWithTipTail(O);downloadWithFallback(z,x,"PDF")})(),a.exportCsv){const z=x.replace(/\.pdf$/,".csv");setTimeout(()=>{generateCsvBlobAsync(a,z).then(j=>{j&&downloadWithFallback(j,z,"CSV")}).catch(j=>{console.error("CSV generation failed:",j)})},1e3)}}catch(k){console.error("PDF generation failed:",k),showDetailedErrorMessage(k,"generating PDF",x);const M=document.getElementById("pdf-processing-notification");M&&M.remove()}t&&(t.style.display="none");return}me>=Re&&(m.addPage(),drawPDFHeader(m,h,ee,re,32,$,J,Ee,Ie,a),we=$+8,me=0);const w=ie[le];if(!w||!w.item){console.warn(`⚠️  Skipping invalid row at index ${le}:`,w),le++,ve();return}const I=we+ye*me;w.isFirstInRoom&&(m.setFontSize(12),m.setFont("helvetica","bold"),m.setTextColor("#333"),m.text(`${w.room} (${w.roomCount})`,32,I+10),m.setFont("helvetica","normal"));const F=ee[0],q=F+90+12;Pe(m,w.item.Image_URL||"",F,I+pe+16,90,ye-pe*2,()=>{Pe(m,w.item.Diagram_URL||"",q,I+pe+16,90,ye-pe*2,()=>{m.setFontSize(10),m.setTextColor("#222");const D=I+28,C=ee[1]+re[1]/2;m.text(String(w.item.OrderCode||""),C,D+10,{align:"center"});let A=D+35;if(w.item.Datasheet_URL&&w.item.Datasheet_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Datasheet",C,A,{url:w.item.Datasheet_URL,align:"center"});const P=m.getTextWidth("Datasheet");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(C-P/2,A+1.5,C+P/2,A+1.5),A+=16}if(w.item.Website_URL&&w.item.Website_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Website",C,A,{url:w.item.Website_URL,align:"center"});const P=m.getTextWidth("Website");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(C-P/2,A+1.5,C+P/2,A+1.5),A+=16}let B=D+10;m.setFontSize(10),m.setTextColor("#222");const W=re[2]-10,N=ee[2],ne=m.splitTextToSize(String(w.item.Description||""),W);if(m.text(ne,N+5,B),B+=ne.length*12,!a.excludeLongDescription&&(w.item.LongDescription||w.item["Long Description"]||w.item.longDescription)){const P=w.item.LongDescription||w.item["Long Description"]||w.item.longDescription;m.setFontSize(9),m.setTextColor("#444");const de=m.splitTextToSize(String(P),W);m.text(de,N+5,B),B+=de.length*11}if(w.item.Notes){m.setFont("helvetica","italic"),m.setFontSize(9),m.setTextColor("#444");const P=m.splitTextToSize(`Notes: ${String(w.item.Notes).replace(/\r?\n|\r/g," ")}`,W);m.text(P,N+5,B),B+=P.length*11,m.setFont("helvetica","normal")}m.setFontSize(10),m.setTextColor("#222");let x=NaN;if(w.item.UserEditedPrice!==void 0&&w.item.UserEditedPrice!==null&&w.item.UserEditedPrice!=="")x=parseFloat(w.item.UserEditedPrice.toString().replace(/,/g,""));else{const P=w.item.RRP_EX||w.item["RRP EX GST"]||w.item.RRP_EX||w.item.RRP_EXGST;P&&(x=parseFloat(P.toString().replace(/,/g,"")))}!isNaN(x)&&x>0&&a.includeGst&&(x=x*1.1);const _=!isNaN(x)&&x>=0?`$${x.toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2})}`:"";if(!a.excludePrice&&!a.excludeQty){const P=ee[3]+re[3]/2;m.text(_,P,D+10,{align:"center"})}if(m.setFontSize(10),m.setTextColor("#222"),!a.excludeQty){const P=ee[4]+re[4]/2;m.text(String(w.item.Quantity||1),P,D+10,{align:"center"})}m.setFontSize(10),m.setTextColor("#222");const k=x*(w.item.Quantity||1),M=!isNaN(x)&&x>=0?`$${k.toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2})}`:"";if(!a.excludePrice&&!a.excludeQty){const P=ee[5]+re[5]/2;m.text(M,P,D+10,{align:"center"})}le++,me++,ve()})})}ve()})})})}function drawPDFHeader(a,e,t,o,r,i,n,s,c,l){const d=i+5.7;if(a.setFillColor("#8B6C2B"),a.rect(0,0,e,d,"F"),n&&s&&c){const y=d*.55,v=s/c;let f=y*v;f>80&&(f=80);const b=(d-y)/2;a.addImage(n,"PNG",r,b,f,y)}a.setFontSize(10),a.setTextColor("#f4f4f4"),a.setFont("helvetica","normal");const u=d-8,p=t[1]+o[1]/2,m=t[2]+o[2]/2,h=t[3]+o[3]/2,g=t[4]+o[4]/2,S=t[5]+o[5]/2;if(a.text("Code",p,u,{align:"center"}),a.text("Description",m,u,{align:"center"}),!l.excludePrice&&!l.excludeQty){if(l.includeGst){a.setFont("helvetica","normal");const y=a.getTextWidth("Price ea "),v=h-a.getTextWidth("Price ea INC GST")/2;a.text("Price ea ",v,u),a.setFont("helvetica","bold"),a.text("INC GST",v+y,u),a.setFont("helvetica","normal")}else{a.setFont("helvetica","normal");const y=a.getTextWidth("Price ea "),v=h-a.getTextWidth("Price ea EX GST")/2;a.text("Price ea ",v,u),a.setFont("helvetica","bold"),a.text("EX GST",v+y,u),a.setFont("helvetica","normal")}a.text("Qty",g,u,{align:"center"}),a.text("Total",S,u,{align:"center"})}else l.excludePrice&&!l.excludeQty&&a.text("Qty",g,u,{align:"center"})}function loadImageAsDataURL(a,e){const t=new window.Image;t.crossOrigin="Anonymous",t.onload=function(){const o=document.createElement("canvas"),r=o.getContext("2d"),i=400,n=150;let s=t.width,c=t.height;if(s>i||c>n){const d=i/s,u=n/c,p=Math.min(d,u);s=Math.round(s*p),c=Math.round(c*p)}o.width=s,o.height=c,r.imageSmoothingEnabled=!0,r.imageSmoothingQuality="high",r.drawImage(t,0,0,s,c);const l=o.toDataURL("image/png",.9);e(l,s,c)},t.src=a}function ensurePdfSpinner(){if(!document.getElementById("pdf-spinner")){const a=document.createElement("div");if(a.id="pdf-spinner",a.style.display="none",a.style.position="fixed",a.style.top="0",a.style.left="0",a.style.width="100vw",a.style.height="100vh",a.style.zIndex="9999",a.style.background="rgba(255,255,255,0.7)",a.style.alignItems="center",a.style.justifyContent="center",a.innerHTML='<div style="border:6px solid #e0e0e0;border-top:6px solid #2563eb;border-radius:50%;width:54px;height:54px;animation:spin 1s linear infinite;"></div>',document.body.appendChild(a),!document.getElementById("pdf-spinner-style")){const e=document.createElement("style");e.id="pdf-spinner-style",e.innerHTML="@keyframes spin { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);} }",document.head.appendChild(e)}}}async function generateCsvBlobAsync(a,e){return new Promise(async t=>{if(!window.Papa)try{await Utils$1.loadScript("https://cdn.jsdelivr.net/npm/papaparse@5.4.1/papaparse.min.js")}catch(n){console.error("Failed to load PapaParse:",n),t(null);return}const o=JSON.parse(localStorage.getItem("selection")||"[]"),r=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let i=[];if(r.length>0?i=r.map(n=>({...n.product,Room:n.room,Notes:n.notes,Quantity:n.quantity,Timestamp:new Date(n.timestamp).toISOString()})):i=o,!i.length){t(null);return}setTimeout(()=>{const n=i.map(s=>{let c,l,d,u;const p=a.excludePrice;let m=0;if(s.UserEditedPrice!==void 0&&s.UserEditedPrice!==null&&s.UserEditedPrice!=="")m=parseFloat(s.UserEditedPrice.toString().replace(/,/g,""));else{const g=s.RRP_EX||s["RRP EX GST"]||s.RRP_EX||s.RRP_EXGST||"";m=parseFloat((g||"0").toString().replace(/,/g,""))}c=m,d="Price ea ex GST",u="Price Total ex GST",l=!isNaN(c)&&c>=0?(c*(s.Quantity||1)).toFixed(2):"";const h={Code:sanitizeCSVField(s.OrderCode||""),Description:sanitizeCSVField(s.Description||""),Quantity:s.Quantity||1,Notes:sanitizeCSVField(s.Notes||""),Room:sanitizeCSVField(s.Room||""),"Image URL":sanitizeCSVField(s.Image_URL||""),"Diagram URL":sanitizeCSVField(s.Diagram_URL||""),"Datasheet URL":sanitizeCSVField(s.Datasheet_URL||""),"Website URL":sanitizeCSVField(s.Website_URL||"")};return h[d]=p?"0.00":c>=0?c.toFixed(2):"",h[u]=p?"0.00":l,h});setTimeout(()=>{const s=window.Papa.unparse(n,{quotes:!0,quoteChar:'"',delimiter:",",header:!0,newline:`\r
