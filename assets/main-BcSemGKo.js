...
`).filter(s=>s.trim()!==""),i=r.length>0?r[r.length-1]:"Unknown",n=i.split(" - ")[0]||i;o.innerText=n,o.innerText.trim()||(o.innerText="v2.1.0")}}else throw new Error("Version file not found")}catch{const t=document.getElementById("version-number");if(t){const o=config.get("app.version")||"v2.1.0";t.innerText=o}else setTimeout(()=>{const o=document.getElementById("version-number");if(o&&!o.innerText.trim()){const r=config.get("app.version")||"v2.1.0";o.innerText=r}},1e3);console.info("Version loaded from config (GitHub Pages mode)")}}async showProductLookupScreen(){try{const t=await(await fetch("./screens/product-grid.html")).text();document.body.innerHTML=t,this.currentScreen="product-grid";const o=document.createElement("script");o.type="module",o.src="js/app.js",document.body.appendChild(o),setTimeout(()=>{document.querySelectorAll(".back-btn").forEach(r=>r.remove())},100),window.productGridManager&&window.productGridManager.init(),await this.loadVersion(),setTimeout(()=>this.loadVersion(),1e3)}catch(e){console.error("Failed to load product grid screen:",e)}}setupSplitInterface(){const e=document.getElementById("back-to-home");e&&(e.onclick=()=>location.reload());const t=document.getElementById("download-btn"),o=document.getElementById("clear-all-btn");t&&(t.onclick=()=>this.showDownloadFormModal()),o&&(o.onclick=()=>this.showClearConfirmModal()),this.setupSplitProductSearch(),this.setupReviewTable(),this.renderReviewTable(),this.loadInitialSearchResults()}setupSplitProductSearch(){const e=document.getElementById("product-search-input"),t=document.getElementById("search-results-list"),o=document.getElementById("search-loading"),r=document.getElementById("search-no-results");if(!e||!t)return;const i=[],n=Utils.debounce(s=>{this.performSplitProductSearch(s,t,i,o,r)},200);e.addEventListener("input",()=>{const s=e.value.trim();s?n(s):this.loadInitialSearchResults()}),t.addEventListener("click",s=>{const a=s.target.closest(".result-item");if(!a)return;const l=parseInt(a.getAttribute("data-idx"),10),d=i.length>0?i:this.currentSearchResults||[];!isNaN(l)&&d[l]&&this.showSplitProductDetails(d[l])})}performSplitProductSearch(e,t,o,r,i){if(!dataLayer.isLoaded){r.style.display="flex",i.style.display="none",t.innerHTML="";return}o.length=0,o.push(...dataLayer.searchProducts(e)),r.style.display="none",o.length===0?(i.style.display="flex",t.innerHTML=""):(i.style.display="none",t.innerHTML=o.map((n,s)=>`
      ...
      <div class="review-room-group">
        ...
        ${n.map((s,a)=>{const l=s.product,d=l.Description||l.description||l.productName||l["Product Name"]||"Product",u=l.OrderCode||l.orderCode||"",p=l.Image_URL||l.imageUrl||"assets/no-image.png",h=l.RRP_EX||l["RRP EX GST"]||l.RRP_EX||l.rrpExGst||l.RRP_EXGST||l.RRP_INCGST||l["RRP INC GST"]||"0";return`
    ...
    `,document.body.appendChild(o),loadImageAsDataURL("./assets/seima-logo.png",(i,n,s)=>{const a=JSON.parse(localStorage.getItem("selection")||"[]"),l=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let d=[];if(l.length>0?d=l.map(E=>({...E.product,Room:E.room,Notes:E.notes,Quantity:E.quantity,Timestamp:new Date(E.timestamp).toISOString()})):d=a,!d.length){alert("No products selected."),t&&(t.style.display="none");return}if(getImageCacheSize()>0)console.log(`📷 Using ${getImageCacheSize()} pre-cached images (skipping duplicate preload)`);else{const E=document.getElementById("pdf-processing-notification");if(E){const P=document.createElement("span");P.id="preload-progress",P.style.cssText="display: block; font-size: 12px; margin-top: 8px; color: #1e40af;",P.textContent="Loading images: 0%",E.appendChild(P)}console.log("📷 Starting image preload for",d.length,"products"),preloadAllProductImages(d).then(P=>{const O=document.getElementById("preload-progress");O&&(O.textContent=`✓ ${P} images ready`,O.style.color="#059669")}).catch(P=>{console.warn("Image preloading error:",P)})}const p={};d.forEach(E=>{p[E.Room]||(p[E.Room]=[]),p[E.Room].push(E)});const{jsPDF:h}=window.jspdf,m=new h({orientation:"landscape",unit:"pt",format:"a4",compress:!0,putOnlyUsedFonts:!0,precision:16,userUnit:1,floatPrecision:16}),g=m.internal.pageSize.getWidth(),w=m.internal.pageSize.getHeight(),y=g/2,v=320,f=90,S=(g-v)/2,D=70;m.setFillColor(255,255,255),m.rect(S,D,v,f,"F");const z=localStorage.getItem("customerLogo");if(z)try{const E=new Image;E.onload=function(){const P=v-20,O=f-20,ae=E.width/E.height;let J=P,X=P/ae;X>O&&(X=O,J=O*ae);const re=S+(v-J)/2,ie=D+(f-X)/2;m.addImage(z,"PNG",re,ie,J,X,void 0,"FAST")},E.src=z}catch(E){console.warn("Failed to draw customer logo:",E)}loadImageAsDataURL("./assets/seima-logo.png",(E,P,O)=>{const J=O&&P?250*O/P:65,X=(g-250)/2,re=D+f+80;m.addImage(E,"PNG",X,re,250,J,void 0,"FAST"),m.setFontSize(15),m.setTextColor("#444");const ie=y;let ce=re+J+50;const W=[];c!=null&&c.name&&c.name.trim()&&W.push({label:"Name:",value:c.name.trim(),bold:!0}),c!=null&&c.project&&c.project.trim()&&W.push({label:"Project:",value:c.project.trim(),bold:!0}),c!=null&&c.address&&c.address.trim()&&W.push({label:"Address:",value:c.address.trim(),bold:!0}),c!=null&&c.email&&c.email.trim()&&W.push({label:"Email:",value:c.email.trim(),bold:!0}),c!=null&&c.telephone&&c.telephone.trim()&&W.push({label:"Telephone:",value:c.telephone.trim(),bold:!0});const V=32;W.length>0&&W.forEach(K=>{m.setFont("helvetica","normal"),m.text(K.label,ie-80,ce,{align:"right"}),m.setFont("helvetica","bold"),m.text(K.value,ie-35,ce,{align:"left"}),ce+=26}),m.setFillColor("#9B9184"),m.rect(0,w-V,g,V,"F"),m.setTextColor("#fff"),m.setFontSize(13);const le=new Date,Te=["January","February","March","April","May","June","July","August","September","October","November","December"],De=le.getDate(),Ae=Te[le.getMonth()],Be=le.getFullYear(),Ne=String(le.getHours()).padStart(2,"0"),Fe=String(le.getMinutes()).padStart(2,"0"),Oe=`Printed ${De} ${Ae} ${Be}, ${Ne}:${Fe}`;m.text(Oe,16,w-10),m.text("www.seima.com.au",g-140,w-10);const me=StorageManager.getUserSettings(),G=me.staffName||"",Q=me.staffPosition||"",Z=me.staffEmail||"",ee=me.staffPhone||"";let te="For more information";G&&ee&&Z?te=`For more information, please contact ${Q?`${G}, ${Q}`:G} on ${ee} or email ${Z}`:G&&ee?te=`For more information, please contact ${Q?`${G}, ${Q}`:G} on ${ee}`:G&&Z?te=`For more information, please contact ${Q?`${G}, ${Q}`:G} or email ${Z}`:ee&&Z?te=`For more information, please call ${ee} or email ${Z}`:G?te=`For more information, please contact ${Q?`${G}, ${Q}`:G}`:ee?te=`For more information, please call ${ee}`:Z&&(te=`For more information, please email ${Z}`),m.setFont("helvetica","normal"),m.setFontSize(14),m.setTextColor("#111"),m.text(te,y,w-V-18,{align:"center"}),m.addPage(),loadImageAsDataURL("./assets/seima-logo-white.png",(K,Ie,Ce)=>{const fe=g-200,ye=g-120,Pe=g-80,oe=[32,236,321,fe,ye,Pe],ne=[90,90,fe-321,ye-fe,Pe-ye,80];resetImageOptimizationStats();const Re=(b,x,F,Y,A,M,I)=>{if(!x||typeof x!="string"||x.length<10||!x.startsWith("http://")&&!x.startsWith("https://")&&!x.startsWith("data:")){I&&I();return}if((C=>{if(!C||C.length<25||/\/images\/\d+$/.test(C)||C.endsWith("/0"))return!0;const R=/\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(C),T=C.startsWith("data:");return!R&&!T})(x)){console.warn("Skipping malformed image URL:",(x==null?void 0:x.substring(0,50))+"..."),I&&I();return}if(imageOptimizationStats.totalImages++,c.emailCompatible){imageOptimizationStats.failedImages++,I&&I();return}const B=getCachedImage(x);if(B&&B.dataUrl)try{const C=B.width/B.height;let R=A,T=A/C;T>M&&(T=M,R=M*C),b.addImage(B.dataUrl,B.format,F,Y,R,T,void 0,"FAST"),imageOptimizationStats.optimizedImages++,I&&I();return}catch{console.warn("Failed to use cached image, falling back to direct load")}let U=!1;const H=["https://wsrv.nl/?url=","https://images.weserv.nl/?url=","https://api.codetabs.com/v1/proxy?quest="];let $=0;function k(){if(U)return;const C=new Image;C.crossOrigin="Anonymous";let R=null;C.onload=function(){if(!U){U=!0,R&&clearTimeout(R);try{const L=getOptimizedFileSettings(0),ue=L.imageMaxWidth,Se=A,he=M;try{const N=document.createElement("canvas"),_=N.getContext("2d"),{width:q,height:ke}=calculateOptimizedDimensions(C.width,C.height,ue);N.width=q,N.height=ke,_.imageSmoothingEnabled=!0,_.imageSmoothingQuality="high",_.drawImage(C,0,0,q,ke);let xe,Ee="JPEG";const Me=detectTransparency(N,_),Ue=isTechnicalDiagram(C);Me||Ue?(xe=N.toDataURL("image/png",L.imageQuality),Ee="PNG"):(xe=N.toDataURL("image/jpeg",L.imageQuality),Ee="JPEG");const $e=`img_${generateImageHash(x)}`;b.addImage(xe,Ee,F,Y,Se,he,$e,"FAST"),imageOptimizationStats.optimizedImages++,I&&I()}catch(N){console.warn(`Failed to optimize image: ${x}`,N),console.warn("Error details:",N.message,N.stack);try{b.addImage(C,"JPEG",F,Y,Se,he),imageOptimizationStats.optimizedImages++,I&&I()}catch(_){console.error(`Fallback also failed for: ${x}`,_),imageOptimizationStats.failedImages++,I&&I()}}}catch(L){console.warn("Failed to add image to PDF:",L),imageOptimizationStats.failedImages++,I&&I()}}},C.onerror=function(){U||(R&&clearTimeout(R),console.warn(`Failed to load image with proxy ${$}: ${x}`),console.warn(`Error details for: ${x} - Proxy: ${H[$]}`),$++,$<H.length?setTimeout(()=>{k()},200):(U=!0,console.warn("All proxies failed, skipping image"),imageOptimizationStats.failedImages++,I&&I()))},R=setTimeout(()=>{U||(console.warn(`⏰ Timeout with proxy ${$}: ${x}`),C.src="",C.onload=null,C.onerror=null,$++,$<H.length?setTimeout(()=>{k()},200):(U=!0,console.warn("All proxies timed out, skipping image"),imageOptimizationStats.failedImages++,I&&I()))},3e3);let T=x;$<H.length&&(T=H[$]+encodeURIComponent(x)),C.src=T}k()},se=[];Object.keys(p).forEach((b,x)=>{const F=p[b];if(!F||!Array.isArray(F)){console.warn("⚠️ Skipping invalid room items:",b,F);return}F.forEach((Y,A)=>{if(!Y){console.warn("⚠️ Skipping null item in room:",b,"at index:",A);return}se.push({item:Y,room:b,rIdx:x,iIdx:A,isFirstInRoom:A===0,roomCount:F.length})})}),se.reduce((b,x)=>{if(!x||!x.item)return console.warn("⚠️ Skipping null row in data analysis:",x),b;const F=String(x.item.Description||""),Y=String(x.item.LongDescription||""),A=String(x.item.Notes||""),M=String(x.item.OrderCode||"");return b+F.length+Y.length+A.length+M.length},0);let de=0,pe=0;const Le=4,ge=8,we=Math.floor((w-80)/Le);let ve=V+8;function be(){if(!se||!Array.isArray(se)){console.error("❌ Critical error: rowsToDraw is not a valid array:",se),showDetailedErrorMessage(new Error("Invalid product data structure"),"generating PDF","unknown.pdf");return}if(de>=se.length){const A=m.internal.getNumberOfPages()-1;for(let R=2;R<=A+1;R++)m.setPage(R),drawPDFHeader(m,g,oe,ne,32,V,K,Ie,Ce,c),ve=V+8,m.setFillColor("#9B9184"),m.rect(0,w-V,g,V,"F"),m.setTextColor("#fff"),m.setFontSize(11),m.text("www.seima.com.au",g-140,w-10),m.text(`Page ${R-1} of ${A}`,32,w-10);const M=new Date,I=String(M.getDate()).padStart(2,"0"),j=String(M.getMonth()+1).padStart(2,"0"),B=String(M.getFullYear()).slice(-2),U=String(M.getHours()).padStart(2,"0"),H=String(M.getMinutes()).padStart(2,"0"),k=`${c.project.replace(/[^a-zA-Z0-9\s]/g,"")}-${I}${j}${B}.${U}${H}.pdf`,C=document.getElementById("pdf-processing-notification");C&&C.remove(),showImageOptimizationSummary(c.emailCompatible);try{const R=m.output("blob"),T=m.output("string"),L=T?T.match(/\/Type\s*\/XObject/g):null,ue=T?T.match(/Tj\s/g):null,Se=T?T.match(/\/A\s*<</g):null;c.pdfSize=R.size;const he=showFileSizeInfo(R,k);if(c.sendEmail&&R.size>15*1024*1024){console.warn(`❌ PDF too large for email (${(R.size/1024/1024).toFixed(1)}MB), offering email-compatible version`),showEmailCompatibleOption(c,k);return}const N=createOptimizedBlob(R,he.settings);if(c.sendEmail&&c.email)if(c.exportCsv){const _=k.replace(/\.pdf$/,".csv");generateCsvBlobAsync(c,_).then(q=>{window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:c,pdfBlob:N,csvBlob:q}}))}).catch(q=>{console.error("Async CSV generation for email failed:",q),window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:c,pdfBlob:N,csvBlob:null}}))})}else window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:c,pdfBlob:N,csvBlob:null}}));else if((async()=>{const _=await mergeWithTipTail(N);downloadWithFallback(_,k,"PDF")})(),c.exportCsv){const _=k.replace(/\.pdf$/,".csv");setTimeout(()=>{generateCsvBlobAsync(c,_).then(q=>{q&&downloadWithFallback(q,_,"CSV")}).catch(q=>{console.error("CSV generation failed:",q)})},1e3)}}catch(R){console.error("PDF generation failed:",R),showDetailedErrorMessage(R,"generating PDF",k);const T=document.getElementById("pdf-processing-notification");T&&T.remove()}t&&(t.style.display="none");return}pe>=Le&&(m.addPage(),drawPDFHeader(m,g,oe,ne,32,V,K,Ie,Ce,c),ve=V+8,pe=0);const b=se[de];if(!b||!b.item){console.warn(`⚠️  Skipping invalid row at index ${de}:`,b),de++,be();return}const x=ve+we*pe;b.isFirstInRoom&&(m.setFontSize(12),m.setFont("helvetica","bold"),m.setTextColor("#333"),m.text(`${b.room} (${b.roomCount})`,32,x+10),m.setFont("helvetica","normal"));const F=oe[0],Y=F+90+12;Re(m,b.item.Image_URL||"",F,x+ge+16,90,we-ge*2,()=>{Re(m,b.item.Diagram_URL||"",Y,x+ge+16,90,we-ge*2,()=>{m.setFontSize(10),m.setTextColor("#222");const A=x+28,I=oe[1]+ne[1]/2;m.text(String(b.item.OrderCode||""),I,A+10,{align:"center"});let j=A+35;if(b.item.Datasheet_URL&&b.item.Datasheet_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Datasheet",I,j,{url:b.item.Datasheet_URL,align:"center"});const L=m.getTextWidth("Datasheet");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(I-L/2,j+1.5,I+L/2,j+1.5),j+=16}if(b.item.Website_URL&&b.item.Website_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Website",I,j,{url:b.item.Website_URL,align:"center"});const L=m.getTextWidth("Website");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(I-L/2,j+1.5,I+L/2,j+1.5),j+=16}let B=A+10;m.setFontSize(10),m.setTextColor("#222");const U=ne[2]-10,H=oe[2],$=m.splitTextToSize(String(b.item.Description||""),U);if(m.text($,H+5,B),B+=$.length*12,!c.excludeLongDescription&&(b.item.LongDescription||b.item["Long Description"]||b.item.longDescription)){const L=b.item.LongDescription||b.item["Long Description"]||b.item.longDescription;m.setFontSize(9),m.setTextColor("#444");const ue=m.splitTextToSize(String(L),U);m.text(ue,H+5,B),B+=ue.length*11}if(b.item.Notes){m.setFont("helvetica","italic"),m.setFontSize(9),m.setTextColor("#444");const L=m.splitTextToSize(`Notes: ${String(b.item.Notes).replace(/\r?\n|\r/g," ")}`,U);m.text(L,H+5,B),B+=L.length*11,m.setFont("helvetica","normal")}m.setFontSize(10),m.setTextColor("#222");let k=NaN;if(b.item.UserEditedPrice!==void 0&&b.item.UserEditedPrice!==null&&b.item.UserEditedPrice!=="")k=parseFloat(b.item.UserEditedPrice.toString().replace(/,/g,""));else{const L=b.item.RRP_EX||b.item["RRP EX GST"]||b.item.RRP_EX||b.item.RRP_EXGST;L&&(k=parseFloat(L.toString().replace(/,/g,"")))}!isNaN(k)&&k>0&&c.includeGst&&(k=k*1.1);const C=!isNaN(k)&&k>=0?`$${k.toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2})}`:"";if(!c.excludePrice&&!c.excludeQty){const L=oe[3]+ne[3]/2;m.text(C,L,A+10,{align:"center"})}if(m.setFontSize(10),m.setTextColor("#222"),!c.excludeQty){const L=oe[4]+ne[4]/2;m.text(String(b.item.Quantity||1),L,A+10,{align:"center"})}m.setFontSize(10),m.setTextColor("#222");const R=k*(b.item.Quantity||1),T=!isNaN(k)&&k>=0?`$${R.toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2})}`:"";if(!c.excludePrice&&!c.excludeQty){const L=oe[5]+ne[5]/2;m.text(T,L,A+10,{align:"center"})}de++,pe++,be()})})}be()})})})}function drawPDFHeader(c,e,t,o,r,i,n,s,a,l){const d=i+5.7;if(c.setFillColor("#8B6C2B"),c.rect(0,0,e,d,"F"),n&&s&&a){const y=d*.55,v=s/a;let f=y*v;f>80&&(f=80);const S=(d-y)/2;c.addImage(n,"PNG",r,S,f,y)}c.setFontSize(10),c.setTextColor("#f4f4f4"),c.setFont("helvetica","normal");const u=d-8,p=t[1]+o[1]/2,h=t[2]+o[2]/2,m=t[3]+o[3]/2,g=t[4]+o[4]/2,w=t[5]+o[5]/2;if(c.text("Code",p,u,{align:"center"}),c.text("Description",h,u,{align:"center"}),!l.excludePrice&&!l.excludeQty){if(l.includeGst){c.setFont("helvetica","normal");const y=c.getTextWidth("Price ea "),v=m-c.getTextWidth("Price ea INC GST")/2;c.text("Price ea ",v,u),c.setFont("helvetica","bold"),c.text("INC GST",v+y,u),c.setFont("helvetica","normal")}else{c.setFont("helvetica","normal");const y=c.getTextWidth("Price ea "),v=m-c.getTextWidth("Price ea EX GST")/2;c.text("Price ea ",v,u),c.setFont("helvetica","bold"),c.text("EX GST",v+y,u),c.setFont("helvetica","normal")}c.text("Qty",g,u,{align:"center"}),c.text("Total",w,u,{align:"center"})}else l.excludePrice&&!l.excludeQty&&c.text("Qty",g,u,{align:"center"})}function loadImageAsDataURL(c,e){const t=new window.Image;t.crossOrigin="Anonymous",t.onload=function(){const o=document.createElement("canvas"),r=o.getContext("2d"),i=400,n=150;let s=t.width,a=t.height;if(s>i||a>n){const d=i/s,u=n/a,p=Math.min(d,u);s=Math.round(s*p),a=Math.round(a*p)}o.width=s,o.height=a,r.imageSmoothingEnabled=!0,r.imageSmoothingQuality="high",r.drawImage(t,0,0,s,a);const l=o.toDataURL("image/png",.9);e(l,s,a)},t.src=c}function ensurePdfSpinner(){if(!document.getElementById("pdf-spinner")){const c=document.createElement("div");if(c.id="pdf-spinner",c.style.display="none",c.style.position="fixed",c.style.top="0",c.style.left="0",c.style.width="100vw",c.style.height="100vh",c.style.zIndex="9999",c.style.background="rgba(255,255,255,0.7)",c.style.alignItems="center",c.style.justifyContent="center",c.innerHTML='<div style="border:6px solid #e0e0e0;border-top:6px solid #2563eb;border-radius:50%;width:54px;height:54px;animation:spin 1s linear infinite;"></div>',document.body.appendChild(c),!document.getElementById("pdf-spinner-style")){const e=document.createElement("style");e.id="pdf-spinner-style",e.innerHTML="@keyframes spin { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);} }",document.head.appendChild(e)}}}async function generateCsvBlobAsync(c,e){return new Promise(async t=>{if(!window.Papa)try{await Utils.loadScript("https://cdn.jsdelivr.net/npm/papaparse@5.4.1/papaparse.min.js")}catch(n){console.error("Failed to load PapaParse:",n),t(null);return}const o=JSON.parse(localStorage.getItem("selection")||"[]"),r=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let i=[];if(r.length>0?i=r.map(n=>({...n.product,Room:n.room,Notes:n.notes,Quantity:n.quantity,Timestamp:new Date(n.timestamp).toISOString()})):i=o,!i.length){t(null);return}setTimeout(()=>{const n=i.map(s=>{let a,l,d,u;const p=c.excludePrice;let h=0;if(s.UserEditedPrice!==void 0&&s.UserEditedPrice!==null&&s.UserEditedPrice!=="")h=parseFloat(s.UserEditedPrice.toString().replace(/,/g,""));else{const g=s.RRP_EX||s["RRP EX GST"]||s.RRP_EX||s.RRP_EXGST||"";h=parseFloat((g||"0").toString().replace(/,/g,""))}a=h,d="Price ea ex GST",u="Price Total ex GST",l=!isNaN(a)&&a>=0?(a*(s.Quantity||1)).toFixed(2):"";const m={Code:sanitizeCSVField(s.OrderCode||""),Description:sanitizeCSVField(s.Description||""),Quantity:s.Quantity||1,Notes:sanitizeCSVField(s.Notes||""),Room:sanitizeCSVField(s.Room||""),"Image URL":sanitizeCSVField(s.Image_URL||""),"Diagram URL":sanitizeCSVField(s.Diagram_URL||""),"Datasheet URL":sanitizeCSVField(s.Datasheet_URL||""),"Website URL":sanitizeCSVField(s.Website_URL||"")};return m[d]=p?"0.00":a>=0?a.toFixed(2):"",m[u]=p?"0.00":l,m});setTimeout(()=>{const s=window.Papa.unparse(n,{quotes:!0,quoteChar:'"',delimiter:",",header:!0,newline:`\r
...
This can be restored later.`))try{const t=await presentationLoader.deleteSelections([e.id]);t.success?(this.showToast("Selection deleted"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to delete")}catch(t){this.showError(t.message)}}async restoreSelection(e){try{const t=await presentationLoader.restoreSelections([e.id]);t.success?(this.showToast("Selection restored"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to restore")}catch(t){this.showError(t.message)}}showToast(e){const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.insertBefore(i,document.body.firstChild)}setupGlobalEventListeners(){window.addEventListener("generatePdf",e=>{const{tipTailSettings:t,...o}=e.detail;ensurePdfSpinner(),showPdfFormScreen$1(o,t||null)}),window.addEventListener("beforeunload",()=>{}),browserCompatibility.features.memoryAPI&&setInterval(()=>{const e=browserCompatibility.memoryInfo;e.memoryPressure==="high"&&console.warn("High memory usage detected:",e)},6e4)}getSelectedProducts(){return StorageManager.getSelectedProducts()}clearSelection(){return StorageManager.clearAllSelections()}addProduct(e,t,o,r){return StorageManager.addProductToSelection(e,t,o,r)}updateSelectionCount(){this.navigationManager&&this.navigationManager.updateSelectionCount()}showError(e){alert(e)}}document.addEventListener("DOMContentLoaded",()=>{window.seimaScanner=new SeimaScanner,window.seimaScanner.init()});window.addEventListener("DOMContentLoaded",()=>{fetch("./version.txt").then(c=>c.text()).then(c=>{const e=document.getElementById("app-version");if(e){const t=c.trim().split(/\s+|-/)[0];e.textContent=`Ver: ${t}`,e.addEventListener("click",()=>{showChangelog(c.trim(),t)})}}),setupHelpButton(),setupUserMenu()});function setupHelpButton(){const c=document.getElementById("help-btn");c&&c.addEventListener("click",()=>{showUserGuide()});const e=document.getElementById("quick-start-btn");e&&e.addEventListener("click",()=>{onboarding.showForced()})}function setupUserMenu(){const c=document.getElementById("user-menu-container"),e=document.getElementById("user-menu-trigger"),t=document.getElementById("user-menu-dropdown"),o=document.getElementById("sign-in-btn"),r=document.getElementById("user-avatar"),i=document.getElementById("user-name-display");function n(l){var d,u,p,h;if(l){c&&(c.style.display="block"),o&&(o.style.display="none");const m=s(l.name);r&&(r.textContent=m),i&&(i.textContent=((d=l.name)==null?void 0:d.split(" ")[0])||"User"),t&&(t.innerHTML=`
        ...
        `,(u=document.getElementById("menu-profile"))==null||u.addEventListener("click",()=>{t.style.display="none",authUI.showEditProfile(g=>{StorageManager.clearUserSettings(),n(g)})}),(p=document.getElementById("menu-password"))==null||p.addEventListener("click",()=>{t.style.display="none",authUI.showChangePassword()}),(h=document.getElementById("menu-logout"))==null||h.addEventListener("click",()=>{t.style.display="none",StorageManager.clearUserSettings(),authService.logout(),n(null)}))}else c&&(c.style.display="none"),o&&(o.style.display="block")}function s(l){if(!l)return"?";const d=l.trim().split(" ");return d.length>=2?(d[0][0]+d[d.length-1][0]).toUpperCase():l.substring(0,2).toUpperCase()}function a(l){const d=document.createElement("div");return d.textContent=l||"",d.innerHTML}e&&t&&(e.addEventListener("click",l=>{l.stopPropagation();const d=t.style.display!=="none";t.style.display=d?"none":"block"}),document.addEventListener("click",l=>{c!=null&&c.contains(l.target)||(t.style.display="none")})),o&&o.addEventListener("click",()=>{authUI.showLogin(l=>{n(l)})}),authService.onAuthChange=n,n(authService.getCurrentUser())}function showUserGuide(){const c=document.getElementById("user-guide-modal"),e=document.getElementById("user-guide-content");if(!c||!e)return;e.innerHTML=getUserGuideContent(),c.style.display="flex";const t=document.getElementById("user-guide-close");t&&(t.onclick=()=>{c.style.display="none"}),c.onclick=r=>{r.target===c&&(c.style.display="none")};const o=r=>{r.key==="Escape"&&(c.style.display="none",document.removeEventListener("keydown",o))};document.addEventListener("keydown",o)}function getUserGuideContent(){return`
