...
`).filter(r=>r.trim()!==""),n=s.length>0?s[s.length-1]:"Unknown",i=n.split(" - ")[0]||n;o.innerText=i,o.innerText.trim()||(o.innerText="v2.1.0")}}else throw new Error("Version file not found")}catch{const t=document.getElementById("version-number");if(t){const o=$.get("app.version")||"v2.1.0";t.innerText=o}else setTimeout(()=>{const o=document.getElementById("version-number");if(o&&!o.innerText.trim()){const s=$.get("app.version")||"v2.1.0";o.innerText=s}},1e3);console.info("Version loaded from config (GitHub Pages mode)")}}async showProductLookupScreen(){try{const t=await(await fetch("./screens/product-grid.html")).text();document.body.innerHTML=t,this.currentScreen="product-grid";const o=document.createElement("script");o.type="module",o.src="js/app.js",document.body.appendChild(o),setTimeout(()=>{document.querySelectorAll(".back-btn").forEach(s=>s.remove())},100),window.productGridManager&&window.productGridManager.init(),await this.loadVersion(),setTimeout(()=>this.loadVersion(),1e3)}catch(e){console.error("Failed to load product grid screen:",e)}}setupSplitInterface(){const e=document.getElementById("back-to-home");e&&(e.onclick=()=>location.reload());const t=document.getElementById("download-btn"),o=document.getElementById("clear-all-btn");t&&(t.onclick=()=>this.showDownloadFormModal()),o&&(o.onclick=()=>this.showClearConfirmModal()),this.setupSplitProductSearch(),this.setupReviewTable(),this.renderReviewTable(),this.loadInitialSearchResults()}setupSplitProductSearch(){const e=document.getElementById("product-search-input"),t=document.getElementById("search-results-list"),o=document.getElementById("search-loading"),s=document.getElementById("search-no-results");if(!e||!t)return;const n=[],i=S.debounce(r=>{this.performSplitProductSearch(r,t,n,o,s)},200);e.addEventListener("input",()=>{const r=e.value.trim();r?i(r):this.loadInitialSearchResults()}),t.addEventListener("click",r=>{const a=r.target.closest(".result-item");if(!a)return;const c=parseInt(a.getAttribute("data-idx"),10),d=n.length>0?n:this.currentSearchResults||[];!isNaN(c)&&d[c]&&this.showSplitProductDetails(d[c])})}performSplitProductSearch(e,t,o,s,n){if(!T.isLoaded){s.style.display="flex",n.style.display="none",t.innerHTML="";return}o.length=0,o.push(...T.searchProducts(e)),s.style.display="none",o.length===0?(n.style.display="flex",t.innerHTML=""):(n.style.display="none",t.innerHTML=o.map((i,r)=>`
    ...
    `,document.body.appendChild(o);const n=bt();Ne("./assets/seima-logo.png",async(i,r,a)=>{const c=JSON.parse(localStorage.getItem("selection")||"[]"),d=JSON.parse(localStorage.getItem(F.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let l=[];d.length>0?l=d.map(w=>({...w.product,Room:w.room,Notes:w.notes,Quantity:w.quantity,Timestamp:new Date(w.timestamp).toISOString()})):l=c;const u=document.getElementById("sort-by"),m=u?u.value:"room";switch(m){case"code":l.sort((w,E)=>{const P=w.OrderCode||w.Code||"",I=E.OrderCode||E.Code||"";return P.localeCompare(I)});break;case"product":l.sort((w,E)=>{const P=w.Description||w.ProductName||"",I=E.Description||E.ProductName||"";return P.localeCompare(I)});break;case"room":default:l.sort((w,E)=>{const P=w.Room||"Blank",I=E.Room||"Blank";return P.localeCompare(I)});break}if(!l.length){alert("No products selected."),t&&(t.style.display="none");return}if(Ke()>0)console.log(`📷 Using ${Ke()} pre-cached images (skipping duplicate preload)`);else{const w=document.getElementById("pdf-processing-notification");if(w){const E=document.createElement("span");E.id="preload-progress",E.style.cssText="display: block; font-size: 12px; margin-top: 8px; color: #1e40af;",E.textContent="Loading images: 0%",w.appendChild(E)}console.log("📷 Starting image preload for",l.length,"products"),ct(l).then(E=>{const P=document.getElementById("preload-progress");P&&(P.textContent=`✓ ${E} images ready`,P.style.color="#059669")}).catch(E=>{console.warn("Image preloading error:",E)})}const f={};m==="room"?l.forEach(w=>{const E=w.Room||"Blank";f[E]||(f[E]=[]),f[E].push(w)}):f.__all__=l,await n;const{jsPDF:v}=window.jspdf,h=new v({orientation:"landscape",unit:"pt",format:"a4",compress:!0,putOnlyUsedFonts:!0,precision:16,userUnit:1,floatPrecision:16}),x=h.internal.pageSize.getWidth(),y=h.internal.pageSize.getHeight();Ne("./assets/seima-logo.png",(w,E,P)=>{const I=j.getCurrentUser(),M=b.getUserSettings(),_=I?{name:I.name,email:I.email,phone:I.phone,position:I.position}:M,K=localStorage.getItem("customerLogo");St(h,{pageWidth:x,pageHeight:y,seimaLogoDataUrl:w,seimaLogoNaturalW:E,seimaLogoNaturalH:P,customerLogoDataUrl:K,userDetails:p,staffContact:_,footerHeight:Ze.footerHeight}),h.addPage(),Ne("./assets/seima-logo-white.png",(de,le,ue)=>{const gt=p.showRrp&&!p.excludePrice,Ye=!p.excludePrice,Xe=!p.excludeQty,ft=xt(x,{leftMargin:32,rightMargin:32,showRrp:gt,showPrice:Ye,showQty:Xe,showTotal:Ye&&Xe}),{colX:pe,colW:be,headers:De}=ft,Ee=Ze.footerHeight;ot();const Je=(R,C,z,W,N,O,B)=>{if(!C||typeof C!="string"||C.length<10||!C.startsWith("http://")&&!C.startsWith("https://")&&!C.startsWith("data:")){B&&B();return}if((L=>{if(!L||L.length<25||/\/images\/\d+$/.test(L)||L.endsWith("/0"))return!0;const D=/\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(L),A=L.startsWith("data:");return!D&&!A})(C)){console.warn("Skipping malformed image URL:",(C==null?void 0:C.substring(0,50))+"..."),B&&B();return}if(H.totalImages++,p.emailCompatible){H.failedImages++,B&&B();return}const V=It(C);if(V&&V.dataUrl)try{const L=V.width/V.height;let D=N,A=N/L;A>O&&(A=O,D=O*L),R.addImage(V.dataUrl,V.format,z,W,D,A,void 0,"FAST"),H.optimizedImages++,B&&B();return}catch{console.warn("Failed to use cached image, falling back to direct load")}let Z=!1;const he=["https://wsrv.nl/?url=","https://images.weserv.nl/?url=","https://api.codetabs.com/v1/proxy?quest="];let Y=0;function X(){if(Z)return;const L=new Image;L.crossOrigin="Anonymous";let D=null;L.onload=function(){if(!Z){Z=!0,D&&clearTimeout(D);try{const ee=ut(0),Ie=ee.imageMaxWidth,ve=N,J=O;try{const U=document.createElement("canvas"),Q=U.getContext("2d"),{width:te,height:ge}=Jt(L.width,L.height,Ie);U.width=te,U.height=ge,Q.imageSmoothingEnabled=!0,Q.imageSmoothingQuality="high",Q.drawImage(L,0,0,te,ge);let _e,Ae="JPEG";const yt=Xt(U,Q),vt=Yt(L);yt||vt?(_e=U.toDataURL("image/png",ee.imageQuality),Ae="PNG"):(_e=U.toDataURL("image/jpeg",ee.imageQuality),Ae="JPEG");const wt=`img_${no(C)}`;R.addImage(_e,Ae,z,W,ve,J,wt,"FAST"),H.optimizedImages++,B&&B()}catch(U){console.warn(`Failed to optimize image: ${C}`,U),console.warn("Error details:",U.message,U.stack);try{R.addImage(L,"JPEG",z,W,ve,J),H.optimizedImages++,B&&B()}catch(Q){console.error(`Fallback also failed for: ${C}`,Q),H.failedImages++,B&&B()}}}catch(ee){console.warn("Failed to add image to PDF:",ee),H.failedImages++,B&&B()}}},L.onerror=function(){Z||(D&&clearTimeout(D),console.warn(`Failed to load image with proxy ${Y}: ${C}`),console.warn(`Error details for: ${C} - Proxy: ${he[Y]}`),Y++,Y<he.length?setTimeout(()=>{X()},200):(Z=!0,console.warn("All proxies failed, skipping image"),H.failedImages++,B&&B()))},D=setTimeout(()=>{Z||(console.warn(`⏰ Timeout with proxy ${Y}: ${C}`),L.src="",L.onload=null,L.onerror=null,Y++,Y<he.length?setTimeout(()=>{X()},200):(Z=!0,console.warn("All proxies timed out, skipping image"),H.failedImages++,B&&B()))},3e3);let A=C;Y<he.length&&(A=he[Y]+encodeURIComponent(C)),L.src=A}X()},G=[];Object.keys(f).forEach((R,C)=>{const z=f[R];if(!z||!Array.isArray(z)){console.warn("⚠️ Skipping invalid room items:",R,z);return}z.forEach((W,N)=>{if(!W){console.warn("⚠️ Skipping null item in room:",R,"at index:",N);return}G.push({item:W,room:R,rIdx:C,iIdx:N,isFirstInRoom:N===0,roomCount:z.length})})}),G.reduce((R,C)=>{if(!C||!C.item)return console.warn("⚠️ Skipping null row in data analysis:",C),R;const z=String(C.item.Description||""),W=String(C.item.LongDescription||""),N=String(C.item.Notes||""),O=String(C.item.OrderCode||"");return R+z.length+W.length+N.length+O.length},0);let Se=0,Pe=0;const Ce=4,ke=8,Be=Math.floor((y-80)/Ce);let Qe=Ee+8;function $e(){if(!G||!Array.isArray(G)){console.error("❌ Critical error: rowsToDraw is not a valid array:",G),tt(new Error("Invalid product data structure"),"generating PDF","unknown.pdf");return}if(Se>=G.length){const N=h.internal.getNumberOfPages()-1;for(let D=2;D<=N+1;D++){h.setPage(D);const ee=(D-2)*Ce,Ie=Math.min(ee+Ce,G.length);let ve=!1;for(let J=ee;J<Ie;J++)if(G[J]&&G[J].item&&Et(G[J].item)){ve=!0;break}Pt(h,{pageWidth:x,colX:pe,colW:be,leftMargin:32,footerHeight:Ee,logoDataUrl:de,logoNaturalW:le,logoNaturalH:ue,headers:De,userDetails:p,skipWelsHeader:!ve}),Ct(h,{pageWidth:x,pageHeight:y,leftMargin:32,footerHeight:Ee,pageNumber:D-1,totalPages:N})}const O=new Date,B=String(O.getDate()).padStart(2,"0"),me=String(O.getMonth()+1).padStart(2,"0"),V=String(O.getFullYear()).slice(-2),Z=String(O.getHours()).padStart(2,"0"),he=String(O.getMinutes()).padStart(2,"0"),X=`${p.project.replace(/[^a-zA-Z0-9\s]/g,"")}-${B}${me}${V}.${Z}${he}.pdf`,L=document.getElementById("pdf-processing-notification");L&&L.remove(),io(p.emailCompatible);try{const D=h.output("blob"),A=h.output("string"),ee=A?A.match(/\/Type\s*\/XObject/g):null,Ie=A?A.match(/Tj\s/g):null,ve=A?A.match(/\/A\s*<</g):null;p.pdfSize=D.size;const J=Kt(D,X);if(p.sendEmail&&D.size>15*1024*1024){console.warn(`❌ PDF too large for email (${(D.size/1024/1024).toFixed(1)}MB), offering email-compatible version`),ro(p,X);return}const U=Qt(D,J.settings);if(p.sendEmail&&p.email)if(p.exportCsv){const Q=X.replace(/\.pdf$/,".csv");et(p,Q).then(te=>{window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:p,pdfBlob:U,csvBlob:te}}))}).catch(te=>{console.error("Async CSV generation for email failed:",te),window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:p,pdfBlob:U,csvBlob:null}}))})}else window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:p,pdfBlob:U,csvBlob:null}}));else(async()=>{const Q=await co(U);if(await He(Q,X,"PDF"),p.exportCsv){const te=X.replace(/\.pdf$/,".csv");try{const ge=await et(p,te);ge&&await He(ge,te,"CSV")}catch(ge){console.error("CSV generation failed:",ge)}}})()}catch(D){console.error("PDF generation failed:",D),tt(D,"generating PDF",X);const A=document.getElementById("pdf-processing-notification");A&&A.remove()}t&&(t.style.display="none");return}Pe>=Ce&&(h.addPage(),Qe=Ee+8,Pe=0);const R=G[Se];if(!R||!R.item){console.warn(`⚠️  Skipping invalid row at index ${Se}:`,R),Se++,$e();return}const C=Qe+Be*Pe;R.isFirstInRoom&&m==="room"&&R.room!=="__all__"&&kt(h,R.room,R.roomCount,32,C);const z=pe[0],W=z+90+12;Je(h,R.item.Image_URL||"",z,C+ke+16,90,Be-ke*2,()=>{Je(h,R.item.Diagram_URL||"",W,C+ke+16,90,Be-ke*2,()=>{const N=C+28,O=pe[1]+be[1]/2;h.setFontSize(10),h.setTextColor("#222"),h.text(String(R.item.OrderCode||""),O,N+10,{align:"center"}),Rt(h,R.item,O,N+35);const B=be[2]-10;Lt(h,R.item,pe[2],N+10,B,p.excludeLongDescription);const me=De.indexOf("WELS")+1;if(me>0&&pe[me]){const V=pe[me]+be[me]/2;Tt(h,R.item,V,N+10)}Dt(h,R.item,pe,be,De,N+10,{excludePrice:p.excludePrice,includeGst:p.includeGst}),Se++,Pe++,$e()})})}$e()})})})}function Ne(p,e){const t=new window.Image;t.crossOrigin="Anonymous",t.onload=function(){const o=document.createElement("canvas"),s=o.getContext("2d"),n=400,i=150;let r=t.width,a=t.height;if(r>n||a>i){const d=n/r,l=i/a,u=Math.min(d,l);r=Math.round(r*u),a=Math.round(a*u)}o.width=r,o.height=a,s.imageSmoothingEnabled=!0,s.imageSmoothingQuality="high",s.drawImage(t,0,0,r,a);const c=o.toDataURL("image/png",.9);e(c,r,a)},t.src=p}function Gt(){if(!document.getElementById("pdf-spinner")){const p=document.createElement("div");if(p.id="pdf-spinner",p.style.display="none",p.style.position="fixed",p.style.top="0",p.style.left="0",p.style.width="100vw",p.style.height="100vh",p.style.zIndex="9999",p.style.background="rgba(255,255,255,0.7)",p.style.alignItems="center",p.style.justifyContent="center",p.innerHTML='<div style="border:6px solid #e0e0e0;border-top:6px solid #2563eb;border-radius:50%;width:54px;height:54px;animation:spin 1s linear infinite;"></div>',document.body.appendChild(p),!document.getElementById("pdf-spinner-style")){const e=document.createElement("style");e.id="pdf-spinner-style",e.innerHTML="@keyframes spin { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);} }",document.head.appendChild(e)}}}async function et(p,e){return new Promise(async t=>{if(!window.Papa)try{await S.loadScript("https://cdn.jsdelivr.net/npm/papaparse@5.4.1/papaparse.min.js")}catch(i){console.error("Failed to load PapaParse:",i),t(null);return}const o=JSON.parse(localStorage.getItem("selection")||"[]"),s=JSON.parse(localStorage.getItem(F.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let n=[];if(s.length>0?n=s.map(i=>({...i.product,Room:i.room,Notes:i.notes,Quantity:i.quantity,Timestamp:new Date(i.timestamp).toISOString()})):n=o,!n.length){t(null);return}setTimeout(()=>{const i=n.map(r=>{let a,c,d,l;const u=p.excludePrice;let m=0;if(r.UserEditedPrice!==void 0&&r.UserEditedPrice!==null&&r.UserEditedPrice!=="")m=parseFloat(r.UserEditedPrice.toString().replace(/,/g,""));else{const h=r.RRP_EX||r["RRP EX GST"]||r.RRP_EX||r.RRP_EXGST||"";m=parseFloat((h||"0").toString().replace(/,/g,""))}a=m,d="Price ea ex GST",l="Price Total ex GST",c=!isNaN(a)&&a>=0?(a*(r.Quantity||1)).toFixed(2):"";const g=r["WELS STAR"]||r.WELS_STAR||r.WELS_STAR||r.WelsStar||"",f=g&&g.toString().trim()?g.toString().replace(/[^\d.]/g,"").trim():"",v={Code:oe(r.OrderCode||""),Description:oe(r.Description||""),"WELS Star":oe(f),Quantity:r.Quantity||1,Notes:oe(r.Notes||""),Room:oe(r.Room||""),"Image URL":oe(r.Image_URL||""),"Diagram URL":oe(r.Diagram_URL||""),"Datasheet URL":oe(r.Datasheet_URL||""),"Website URL":oe(r.Website_URL||"")};return v[d]=u?"0.00":a>=0?a.toFixed(2):"",v[l]=u?"0.00":c,v});setTimeout(()=>{const r=window.Papa.unparse(i,{quotes:!0,quoteChar:'"',delimiter:",",header:!0,newline:`\r
`,skipEmptyLines:!1,escapeChar:'"',transform:{value(a,c){return typeof a=="string"?a.replace(/\0/g,"").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g,""):a}}});p.sendEmail?setTimeout(()=>{try{const a=btoa(unescape(encodeURIComponent(r)));t({name:e,data:a,contentType:"text/csv",originalSize:r.length,base64Size:a.length})}catch(a){console.error("CSV base64 encoding failed:",a),t(new Blob([r],{type:"text/csv"}))}},0):t(new Blob([r],{type:"text/csv"}))},0)},0)})}function oe(p){return typeof p!="string"&&(p=String(p)),p=p.replace(/\0/g,"").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g,"").replace(/\r?\n|\r/g," ").trim(),p}async function Ht(p,e,t="file"){try{if("showSaveFilePicker"in window){const s=await(await window.showSaveFilePicker({suggestedName:e,types:[{description:`${t} files`,accept:{[p.type]:[`.${e.split(".").pop()}`]}}]})).createWritable();return await s.write(p),await s.close(),!0}}catch(o){console.warn("File System Access API failed:",o)}return!1}function qt(p,e,t="file"){try{if(p.size>2*1024*1024)return console.warn("File too large for data URI method"),!1;const o=new FileReader;return o.onload=function(s){try{const n=document.createElement("a");n.href=s.target.result,n.download=e,n.style.display="none",document.body.appendChild(n),n.click(),document.body.removeChild(n)}catch(n){console.error("Data URI download failed:",n)}},o.readAsDataURL(p),!0}catch(o){return console.warn("Data URI method failed:",o),!1}}function Wt(p,e,t="file"){const o=URL.createObjectURL(p),s=document.createElement("div");s.style.cssText=`
...
========================
  `.trim();try{navigator.clipboard.writeText(e)}catch(t){console.error("Failed to copy error report:",t)}}let H={totalImages:0,optimizedImages:0,failedImages:0,totalSavings:0};function no(p){let e=0;if(p.length===0)return e.toString();for(let t=0;t<p.length;t++){const o=p.charCodeAt(t);e=(e<<5)-e+o,e=e&e}return Math.abs(e).toString(36)}function ot(){H={totalImages:0,optimizedImages:0,failedImages:0,totalSavings:0}}function io(p=!1){H.totalImages>0}function ro(p,e){const t=document.createElement("div");t.style.cssText=`
    position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
    background: rgba(0,0,0,0.8); z-index: 10001; display: flex; 
    align-items: center; justify-content: center; padding: 20px;
...
`);if(e.length>0){const o=e[0].split(" - ")[0].trim(),s=document.getElementById("app-version");s&&(s.textContent=`Ver: ${o}`,s.style.cursor="pointer",s.addEventListener("click",Lo))}}),Co(),ko()});function Co(){const p=document.getElementById("help-btn");p&&p.addEventListener("click",()=>{Io()});const e=document.getElementById("crosshair-btn");e&&e.addEventListener("click",()=>{window.location.href="screens/validator.html"});const t=document.getElementById("landscape-btn");t&&t.addEventListener("click",()=>{window.location.href="screens/landscape.html"});const o=document.getElementById("admin-btn");o&&o.addEventListener("click",()=>{window.location.href="screens/admin.html"});const s=document.getElementById("quick-start-btn");s&&s.addEventListener("click",()=>{ht.showForced()})}function ko(){const p=document.getElementById("user-menu-container"),e=document.getElementById("user-menu-trigger"),t=document.getElementById("user-menu-dropdown"),o=document.getElementById("sign-in-btn"),s=document.getElementById("user-avatar"),n=document.getElementById("user-name-display");function i(c){var d,l,u,m,g,f,v;if(c){(d=F.CROSSHAIR)!=null&&d.ENABLED&&(k.configure(F.CROSSHAIR,c.email||""),k.preload().catch(E=>{console.warn("Crosshair preload after login failed:",E)})),p&&(p.style.display="block"),o&&(o.style.display="none");const h=document.getElementById("crosshair-btn");h&&(h.style.display=(l=F.CROSSHAIR)!=null&&l.ENABLED&&j.isStaffMode()?"":"none");const x=document.getElementById("landscape-btn");x&&(x.style.display=(u=F.CROSSHAIR)!=null&&u.ENABLED&&j.isAdmin()?"":"none");const y=document.getElementById("admin-btn");y&&(y.style.display=j.isAdmin()?"":"none");const w=r(c.name);s&&(s.textContent=w),n&&(n.textContent=((m=c.name)==null?void 0:m.split(" ")[0])||"User"),t&&(t.innerHTML=`
        ...
        `,(g=document.getElementById("menu-profile"))==null||g.addEventListener("click",()=>{t.style.display="none",q.showEditProfile(E=>{b.clearUserSettings(),i(E)})}),(f=document.getElementById("menu-password"))==null||f.addEventListener("click",()=>{t.style.display="none",q.showChangePassword()}),(v=document.getElementById("menu-logout"))==null||v.addEventListener("click",()=>{t.style.display="none",b.clearUserSettings(),j.logout(),i(null)}))}else p&&(p.style.display="none"),o&&(o.style.display="block")}function r(c){if(!c)return"?";const d=c.trim().split(" ");return d.length>=2?(d[0][0]+d[d.length-1][0]).toUpperCase():c.substring(0,2).toUpperCase()}function a(c){const d=document.createElement("div");return d.textContent=c||"",d.innerHTML}e&&t&&(e.addEventListener("click",c=>{c.stopPropagation();const d=t.style.display!=="none";t.style.display=d?"none":"block"}),document.addEventListener("click",c=>{p!=null&&p.contains(c.target)||(t.style.display="none")})),o&&o.addEventListener("click",()=>{q.showLogin(c=>{i(c)})}),j.onAuthChange=i,i(j.getCurrentUser())}function Io(){const p=document.getElementById("user-guide-modal"),e=document.getElementById("user-guide-content");if(!p||!e)return;e.innerHTML=Ro(),p.style.display="flex";const t=document.getElementById("user-guide-close");t&&(t.onclick=()=>{p.style.display="none"}),p.onclick=s=>{s.target===p&&(p.style.display="none")};const o=s=>{s.key==="Escape"&&(p.style.display="none",document.removeEventListener("keydown",o))};document.addEventListener("keydown",o)}function Ro(){return`
