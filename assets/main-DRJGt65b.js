...
`).filter(r=>r.trim()!==""),n=s.length>0?s[s.length-1]:"Unknown",i=n.split(" - ")[0]||n;o.innerText=i,o.innerText.trim()||(o.innerText="v2.1.0")}}else throw new Error("Version file not found")}catch{const t=document.getElementById("version-number");if(t){const o=F.get("app.version")||"v2.1.0";t.innerText=o}else setTimeout(()=>{const o=document.getElementById("version-number");if(o&&!o.innerText.trim()){const s=F.get("app.version")||"v2.1.0";o.innerText=s}},1e3);console.info("Version loaded from config (GitHub Pages mode)")}}async showProductLookupScreen(){try{const t=await(await fetch("./screens/product-grid.html")).text();document.body.innerHTML=t,this.currentScreen="product-grid";const o=document.createElement("script");o.type="module",o.src="js/app.js",document.body.appendChild(o),setTimeout(()=>{document.querySelectorAll(".back-btn").forEach(s=>s.remove())},100),window.productGridManager&&window.productGridManager.init(),await this.loadVersion(),setTimeout(()=>this.loadVersion(),1e3)}catch(e){console.error("Failed to load product grid screen:",e)}}setupSplitInterface(){const e=document.getElementById("back-to-home");e&&(e.onclick=()=>location.reload());const t=document.getElementById("download-btn"),o=document.getElementById("clear-all-btn");t&&(t.onclick=()=>this.showDownloadFormModal()),o&&(o.onclick=()=>this.showClearConfirmModal()),this.setupSplitProductSearch(),this.setupReviewTable(),this.renderReviewTable(),this.loadInitialSearchResults()}setupSplitProductSearch(){const e=document.getElementById("product-search-input"),t=document.getElementById("search-results-list"),o=document.getElementById("search-loading"),s=document.getElementById("search-no-results");if(!e||!t)return;const n=[],i=E.debounce(r=>{this.performSplitProductSearch(r,t,n,o,s)},200);e.addEventListener("input",()=>{const r=e.value.trim();r?i(r):this.loadInitialSearchResults()}),t.addEventListener("click",r=>{const c=r.target.closest(".result-item");if(!c)return;const a=parseInt(c.getAttribute("data-idx"),10),l=n.length>0?n:this.currentSearchResults||[];!isNaN(a)&&l[a]&&this.showSplitProductDetails(l[a])})}performSplitProductSearch(e,t,o,s,n){if(!L.isLoaded){s.style.display="flex",n.style.display="none",t.innerHTML="";return}o.length=0,o.push(...L.searchProducts(e)),s.style.display="none",o.length===0?(n.style.display="flex",t.innerHTML=""):(n.style.display="none",t.innerHTML=o.map((i,r)=>`
      ...
      <div class="review-room-group">
        ...
        ${i.map((r,c)=>{const a=r.product,l=a.Description||a.description||a.productName||a["Product Name"]||"Product",d=a.OrderCode||a.orderCode||"",u=a.Image_URL||a.imageUrl||"assets/no-image.png",g=a.RRP_EX||a["RRP EX GST"]||a.RRP_EX||a.rrpExGst||a.RRP_EXGST||a["PL1 - RRP EX GST"]||a.RRP_INCGST||a["RRP INC GST"]||"0";return`
...
This can be restored later.`))try{const t=await le.deleteSelections([e.id]);t.success?(this.showToast("Selection deleted"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to delete")}catch(t){this.showError(t.message)}}async restoreSelection(e){try{const t=await le.restoreSelections([e.id]);t.success?(this.showToast("Selection restored"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to restore")}catch(t){this.showError(t.message)}}showToast(e){const t=document.createElement("div");t.style.cssText=`
    ...
    `,document.body.appendChild(s),document.body.appendChild(n);const i=()=>this.closeValidatorModal(o);n.querySelector(".gvm-close").addEventListener("click",i),s.addEventListener("click",i);const r=c=>{c.key==="Escape"&&i()};document.addEventListener("keydown",r),this._validatorModal={overlay:s,modal:n,escHandler:r,competitor:e,productCode:t};try{await R._ensureIndex();const c=await R.getMatches(e),l=(await R.getProducts(e)).find(p=>String(p.product_code)===String(t));if(!l){n.querySelector(".gvm-body").innerHTML=`<div class="gvm-no-matches">Product ${E.sanitizeInput(t)} not found in ${E.sanitizeInput(e)} data.</div>`;return}const d=c.filter(p=>String(p.CompetitorSKU)===String(t)).sort((p,y)=>(p.Rank||99)-(y.Rank||99)),u={Verified:0,Manual:1,Rejected:2,"AI-Suggested":3},g=new Map;for(const p of d){const y=String(p.SeimaSKU),f=g.get(y);(!f||(u[p.Status]??9)<(u[f.Status]??9))&&g.set(y,p)}const h=[...g.values()].sort((p,y)=>(p.Rank||99)-(y.Rank||99));this._renderValidatorModal(l,h,e)}catch(c){console.error("Validator modal load failed:",c),n.querySelector(".gvm-body").innerHTML='<div class="gvm-no-matches">Failed to load data. Check console.</div>'}}_renderValidatorModal(e,t,o){var d;const s=(d=this._validatorModal)==null?void 0:d.modal;if(!s)return;const n=e.product_name||e.product_code,i=e.image_url||e.Image||"",r=u=>E.sanitizeInput(u||"");s.querySelector(".gvm-title").textContent=`${n} — ${o}`;let c;t.length===0?c='<div class="gvm-no-matches">No Seima matches found. Use the search below to add one.</div>':c=t.map(u=>{const g=String(u.SeimaSKU||"").trim(),h=L.findProductByCode(g),p=(h==null?void 0:h.Description)||(h==null?void 0:h.ProductName)||g,y=(h==null?void 0:h.Image_URL)||(h==null?void 0:h.imageUrl)||"",f=(h==null?void 0:h.RRP_INCGST)||(h==null?void 0:h["RRP INC GST"])||"",w=f?`$${Number(f).toLocaleString("en-AU",{minimumFractionDigits:2})}`:"",v=Number(u.Confidence)||0,b=v>=70?"high":v>=40?"med":"low",S=(u.Status||"").toLowerCase().replace(/[\s-]+/g,"-");let P;return u.Status==="Verified"||u.Status==="Manual"?P=`<span class="gvm-status-label gvm-status-verified">✓ Verified</span>
