...
Return format: [{"code":"...","name":"...","brand":"...","category":"...","quantity":1,"finish":"...","section":"...","specCode":"...","style":"..."}]`;class bo{constructor(){this.conversationHistory=[],this._historyReady=this._loadHistory(),this.isProcessing=!1,this._catalogChecked=!1,this._syncing=!1,this.lastCompetitorProducts=new Map}async chat(e,t,o=null,s=null,i=null){var n,r;if(this.isProcessing)throw new Error("A request is already in progress");this.isProcessing=!0,await this._historyReady,this._catalogChecked||(this._catalogChecked=!0,this._checkCatalogFreshness());try{const a=typeof e=="string"?e:((r=(n=e.find)==null?void 0:n.call(e,d=>d.type==="text"))==null?void 0:r.text)||"";let c;try{c=await this._agentChatStream(a,o,i,s)}catch(d){console.warn("Agent chat failed, falling back to legacy flow:",d.message),c=null}if(c||(c=await this._legacyChat(e,a,t,o,s)),!c)throw new Error("No response from AI");return this.conversationHistory.push({role:"user",content:a||e},{role:"assistant",content:c}),this.conversationHistory.length>Ge&&(this.conversationHistory=this.conversationHistory.slice(-Ge)),await this._saveHistory(),{content:c,usage:null}}finally{this.isProcessing=!1}}async _agentChatStream(e,t,o,s){const i=new AbortController,n=setTimeout(()=>i.abort(),6e4);try{const r=await fetch(`${U.PROXY_URL}/v1/agent-chat`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${U.APP_SECRET}`},body:JSON.stringify({message:e,history:this.conversationHistory.slice(-12),model:U.MODEL,staffMode:O.isStaffMode(),selectedProducts:(t==null?void 0:t.length)>0?t.map(h=>({OrderCode:h.OrderCode,ProductName:h.ProductName||h.Description,room:h.room})):void 0}),signal:i.signal});if(!r.ok){const h=await r.json().catch(()=>({}));throw new Error(h.details||h.error||`Request failed (${r.status})`)}if((r.headers.get("content-type")||"").includes("application/json")){const h=await r.json();return h.needsSync?(this._ensureCatalogSync(),null):h.content||null}const c=r.body.getReader(),d=new TextDecoder;let l="",u="",p="";for(this.lastCompetitorProducts.clear();;){const{done:h,value:m}=await c.read();if(h)break;u+=d.decode(m,{stream:!0});const y=u.split(`
`);u=y.pop();for(const g of y)if(g.startsWith("event: "))p=g.slice(7).trim();else if(g.startsWith("data: ")){try{const w=JSON.parse(g.slice(6));if(p==="status"&&o)o(w.text);else if(p==="token")l+=w.text,s&&s(w.text);else if(p==="comp_products")for(const v of w.products||[])v.code&&this.lastCompetitorProducts.set(v.code,v);else if(p==="error")throw new Error(w.message||"Agent chat failed")}catch(w){if(w instanceof SyntaxError)continue;throw w}p=""}}return l||null}catch(r){throw r.name==="AbortError"?new Error("Request timed out."):r}finally{clearTimeout(n)}}async _legacyChat(e,t,o,s,i){var d,l,u;const n=[],r=await this._findRelevantProductsAI(t,o);if(r?n.push({role:"system",content:`SEIMA PRODUCT DATA (${r.count} products):
${r.text}
...
`).filter(l=>l.trim());if(s.length<2)return o;const i=l=>l.split("|").slice(1,-1).map(u=>u.trim()),n=l=>/^\|[\s:|-]+\|$/.test(l.trim());let r=i(s[0]),a=1;s.length>1&&n(s[1])&&(a=2);let c="<thead><tr>"+r.map(l=>`<th>${l}</th>`).join("")+"</tr></thead>",d="<tbody>"+s.slice(a).filter(l=>!n(l)).map(l=>"<tr>"+i(l).map(u=>`<td>${u}</td>`).join("")+"</tr>").join("")+"</tbody>";return`<table class="ai-comparison-table">${c}${d}</table>`}),t=t.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),t=t.replace(/`([^`]+)`/g,"<code>$1</code>"),t=t.replace(/^### (.+)$/gm,"<h4>$1</h4>"),t=t.replace(/^## (.+)$/gm,"<h3>$1</h3>"),t=t.replace(/^- (.+)$/gm,"<li>$1</li>"),t=t.replace(/(<li>.*<\/li>\n?)+/g,o=>`<ul>${o}</ul>`),t=t.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),t=t.replace(/\n\n/g,"</p><p>"),t=`<p>${t}</p>`,t=t.replace(/<p><\/p>/g,""),t=t.replace(/<p>(<h[34]>)/g,"$1"),t=t.replace(/(<\/h[34]>)<\/p>/g,"$1"),t=t.replace(/<p>(<ul>)/g,"$1"),t=t.replace(/(<\/ul>)<\/p>/g,"$1"),t=t.replace(/<p>(<table)/g,"$1"),t=t.replace(/(<\/table>)<\/p>/g,"$1"),t=t.replace(/\b(\d{6})\b/g,'<button class="ai-add-product-btn" data-code="$1" title="Add $1 to selection">$1 <span class="ai-add-icon">+</span></button>'),t}_getBasePath(){var e;return(((e=document.querySelector("base"))==null?void 0:e.href)||window.location.origin+"/").replace(/\/$/,"")}_getProductImage(e){return e.Image_URL||e.imageUrl||e.Image||`${this._getBasePath()}/assets/no-image.png`}_getProductName(e){return e.Description||e.ProductName||e["Product Name"]||""}_getProductMeta(e){const t=e.Finish||e.Colour||"",o=e.DimX||e["X Dimension (mm)"]||"",s=e.DimY||e["Y Dimension (mm)"]||"",i=e.DimZ||e["Z Dimension (mm)"]||"",n=o&&o!=="0"?`${o} × ${s||0} × ${i||0}mm`:"";return[t,n].filter(Boolean).join("  ·  ")}_getProductPrice(e){const t=e["RRP INC GST"]||e.RRP_INCGST||e["RRP EX GST"]||e.RRP_EXGST||"";return t?`$${parseFloat(t).toLocaleString("en-AU",{minimumFractionDigits:2})}`:""}_enrichTableProducts(e){const t=this._getBasePath(),o=ee.lastCompetitorProducts;e.querySelectorAll(".ai-comparison-table").forEach(s=>{const i=s.querySelector("thead"),n=s.querySelector("tbody");if(!i||!n)return;const a=i.querySelectorAll("th").length;if(a<2)return;const c=new Array(a).fill(null);if(n.querySelectorAll("tr").forEach(l=>{l.querySelectorAll("td").forEach((p,h)=>{if(c[h])return;const m=p.querySelector("strong"),y=(m?m.textContent:p.textContent).trim();if(y){if(/^\d{6}$/.test(y)){const g=D.findProductByCode(y);g&&(c[h]={src:this._getProductImage(g),click:()=>this._showProductModal(y)})}else if(o)for(const[g,w]of o){const v=String(g);if(v===y||y.includes(v)||v.includes(y)){c[h]={src:w.imageUrl||"",click:()=>this._showCompetitorModal(w.brand,g)};break}}}})}),!c.some(Boolean))return;const d=document.createElement("tr");d.className="ai-table-image-row";for(let l=0;l<a;l++){const u=document.createElement("td"),p=c[l];if(p&&p.src){const h=document.createElement("img");h.className="ai-table-product-img",h.src=p.src,h.alt="",h.onerror=()=>{h.src=`${t}/assets/no-image.png`},u.appendChild(h),p.click&&(u.style.cursor="pointer",u.addEventListener("click",p.click))}d.appendChild(u)}n.insertBefore(d,n.firstChild)})}_injectProductCards(e){e.querySelectorAll("table .ai-add-product-btn").forEach(s=>{const i=document.createElement("strong");i.textContent=s.dataset.code,s.replaceWith(i)});const t=e.querySelectorAll(".ai-add-product-btn");if(!t.length)return;const o=this._getBasePath();t.forEach(s=>{const i=s.dataset.code,n=D.findProductByCode(i);if(!n)return;const r=s.closest("li");if(!(r||s.parentElement))return;const c=this._getProductImage(n),d=this._getProductName(n),l=this._getProductMeta(n),u=this._getProductPrice(n),p=document.createElement("div");if(p.className="ai-product-card",p.dataset.code=i,p.innerHTML=`
    ...
    `,s.querySelectorAll(".ai-feedback-btn").forEach(a=>{a.addEventListener("click",()=>{const c=a.dataset.rating;s.querySelectorAll(".ai-feedback-btn").forEach(d=>d.classList.remove("active")),a.classList.add("active"),this._saveFeedback(t,o,c)})});const r=e.querySelector(".ai-msg-bubble");r?e.insertBefore(s,r.nextSibling):e.appendChild(s)}async _saveFeedback(e,t,o){try{const s=await this._loadFeedback(),i=s.findIndex(a=>a.id===e),n={id:e,question:this._lastUserMessage||"",answer:(t||"").slice(0,500),rating:o,timestamp:Date.now(),synced:!1};i>=0?s[i]={...s[i],...n}:s.push(n);const{set:r}=await z(async()=>{const{set:a}=await import("./index-DwnyWBFG.js");return{set:a}},[]);await r(Ce,s)}catch{}}_getFeedbackEntry(e){return this._loadFeedback().then(t=>t.find(o=>o.id===e)||null)}async _loadFeedback(){try{const{get:e}=await z(async()=>{const{get:o}=await import("./index-DwnyWBFG.js");return{get:o}},[]),t=await e(Ce);return Array.isArray(t)?t:[]}catch{return[]}}_escapeHtml(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}_scrollToBottom(){requestAnimationFrame(()=>{this.messagesContainer.scrollTop=this.messagesContainer.scrollHeight})}async _persistMessages(){try{const e=[];this.messagesContainer.querySelectorAll(".ai-msg").forEach(o=>{var s,i;o.classList.contains("ai-msg-user")?e.push({role:"user",content:((s=o.querySelector(".ai-msg-bubble"))==null?void 0:s.textContent)||""}):o.classList.contains("ai-msg-assistant")&&!o.querySelector(".ai-msg-thinking")&&e.push({role:"assistant",content:((i=o.querySelector(".ai-msg-bubble"))==null?void 0:i.dataset.rawContent)||"",feedbackId:o.dataset.feedbackId||null})});const{set:t}=await z(async()=>{const{set:o}=await import("./index-DwnyWBFG.js");return{set:o}},[]);await t(Fe,e)}catch{}}async _restoreMessages(){try{localStorage.removeItem(Fe);const{get:e}=await z(async()=>{const{get:i}=await import("./index-DwnyWBFG.js");return{get:i}},[]),t=await e(Fe);if(!Array.isArray(t)||t.length===0)return;const o=this.messagesContainer.querySelector(".ai-chat-welcome");o&&o.remove();for(const i of t)i.role==="user"&&(this._lastUserMessage=i.content),this._addMessage(i.role,i.content,{skipPersist:!0,feedbackId:i.feedbackId||null});const s=document.createElement("div");s.className="ai-chat-restored",s.textContent="Previous conversation restored",this.messagesContainer.prepend(s)}catch{}}_generateId(){return Date.now().toString(36)+Math.random().toString(36).substr(2)}async _trackQuestion(e){try{const{get:t,set:o}=await z(async()=>{const{get:n,set:r}=await import("./index-DwnyWBFG.js");return{get:n,set:r}},[]),s=await t(xe)||{},i=e.trim();if(i.length<5)return;s[i]=(s[i]||0)+1,await o(xe,s)}catch{}}async _syncFeedback(){try{const{get:e,set:t,del:o}=await z(async()=>{const{get:a,set:c,del:d}=await import("./index-DwnyWBFG.js");return{get:a,set:c,del:d}},[]),s=await this._loadFeedback(),i=s.filter(a=>!a.synced),n=await e(xe)||null;if(i.length===0&&!n)return;if((await fetch(`${U.PROXY_URL}/v1/feedback`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${U.APP_SECRET}`},body:JSON.stringify({entries:i,questionLog:n})})).ok){for(const a of i)a.synced=!0;this._pruneFeedback(s),await t(Ce,s),await o(xe)}}catch{}}_pruneFeedback(e){const t=Date.now()-6048e5;for(let o=e.length-1;o>=0;o--)e[o].synced&&e[o].timestamp<t&&e.splice(o,1)}async _clearChat(){await ee.clearHistory();try{const{del:e}=await z(async()=>{const{del:t}=await import("./index-DwnyWBFG.js");return{del:t}},[]);await e(Ce),await e(xe)}catch{}localStorage.removeItem(Ce),localStorage.removeItem(xe),this._showWelcome()}async _initUserEmail(){var e;try{const{authService:t}=await z(async()=>{const{authService:s}=await import("./index-BnLAUsuI.js");return{authService:s}},__vite__mapDeps([5,1,2,6])),o=((e=t.getUser)==null?void 0:e.call(t))||{};this._userEmail=o.email||null}catch{}}async _saveConversation(){var s;if(!this._userEmail)return;const e=[];if(this.messagesContainer.querySelectorAll(".ai-msg").forEach(i=>{var n,r;i.classList.contains("ai-msg-user")?e.push({role:"user",content:((n=i.querySelector(".ai-msg-bubble"))==null?void 0:n.textContent)||""}):i.classList.contains("ai-msg-assistant")&&!i.querySelector(".ai-msg-thinking")&&e.push({role:"assistant",content:((r=i.querySelector(".ai-msg-bubble"))==null?void 0:r.dataset.rawContent)||""})}),e.length<2)return;this._conversationId||(this._conversationId=Date.now().toString(36)+Math.random().toString(36).slice(2,8));const t=((s=e.find(i=>i.role==="user"))==null?void 0:s.content)||"Untitled",o=t.length>50?t.slice(0,47)+"...":t;try{await fetch(`${U.PROXY_URL}/v1/conversations/${this._conversationId}`,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${U.APP_SECRET}`},body:JSON.stringify({email:this._userEmail,title:o,messages:e})})}catch{}}_startNewConversation(){this._saveConversation(),this._conversationId=null,this._clearChat()}_toggleHistory(e){const t=this.panel.querySelector("#ai-chat-history-panel");this._historyOpen=e!==void 0?e:!this._historyOpen,t.classList.toggle("open",this._historyOpen),this._historyOpen&&this._loadConversationList()}async _loadConversationList(){if(!this._userEmail){this.panel.querySelector("#ai-chat-history-list").innerHTML='<div class="ai-chat-history-empty">Sign in to see conversation history.</div>';return}const e=this.panel.querySelector("#ai-chat-history-list");e.innerHTML='<div class="ai-chat-history-empty">Loading...</div>';try{const t=`${U.PROXY_URL}/v1/conversations?token=${encodeURIComponent(U.APP_SECRET)}&email=${encodeURIComponent(this._userEmail)}`,o=await fetch(t);if(!o.ok)throw new Error("Failed");const i=(await o.json()).conversations||[];if(i.length===0){e.innerHTML='<div class="ai-chat-history-empty">No previous conversations.</div>';return}e.innerHTML=i.map(n=>`
...
`);if(e.length>0){const o=e[0].split(" - ")[0].trim();window._appVersion=o;const s=document.getElementById("menu-version-footer");s&&(s.textContent=`Ver: ${o}`)}}),Ao(),Fo()});function Ao(){const f=document.getElementById("help-btn");f&&f.addEventListener("click",()=>{No()});const e=document.getElementById("crosshair-btn");e&&e.addEventListener("click",()=>{window.location.href="screens/validator.html"});const t=document.getElementById("landscape-btn");t&&t.addEventListener("click",()=>{window.location.href="screens/landscape.html"});const o=document.getElementById("admin-btn");o&&o.addEventListener("click",()=>{window.location.href="screens/admin.html"});const s=document.getElementById("quick-start-btn");s&&s.addEventListener("click",()=>{bt.showForced()})}function Fo(){const f=document.getElementById("user-menu-container"),e=document.getElementById("user-menu-trigger"),t=document.getElementById("user-menu-dropdown"),o=document.getElementById("sign-in-btn"),s=document.getElementById("user-avatar"),i=document.getElementById("user-name-display");function n(c){var d,l,u,p,h,m,y,g,w,v;if(c){(d=G.CROSSHAIR)!=null&&d.ENABLED&&(R.configure(G.CROSSHAIR,c.email||""),R.preload().catch(H=>{console.warn("Crosshair preload after login failed:",H)})),f&&(f.style.display="block"),o&&(o.style.display="none");const b=document.getElementById("crosshair-btn");b&&(b.style.display=(l=G.CROSSHAIR)!=null&&l.ENABLED&&O.isStaffMode()?"":"none");const S=document.getElementById("landscape-btn");S&&(S.style.display=(u=G.CROSSHAIR)!=null&&u.ENABLED&&O.isAdmin()?"":"none");const C=document.getElementById("admin-btn");C&&(C.style.display=O.isAdmin()?"":"none");const P=document.getElementById("staff-divider"),k=(b==null?void 0:b.style.display)!=="none"||(S==null?void 0:S.style.display)!=="none"||(C==null?void 0:C.style.display)!=="none";P&&(P.style.display=k?"":"none");const _=r(c.name);s&&(s.textContent=_),i&&(i.textContent=((p=c.name)==null?void 0:p.split(" ")[0])||"User");const I=(c.email||"").toLowerCase().endsWith("@seima.com.au")&&c.emailVerified===!1;let L=document.getElementById("verify-email-banner");if(I?(L||(L=document.createElement("div"),L.id="verify-email-banner",L.className="verify-email-banner",L.innerHTML=`
        ...
        `,(h=document.getElementById("menu-verify-email"))==null||h.addEventListener("click",()=>{t.style.display="none",Y.showVerifyEmail(c.email,()=>{var M;n(O.getCurrentUser()),(M=document.getElementById("verify-email-banner"))==null||M.remove()})}),(m=document.getElementById("menu-refresh"))==null||m.addEventListener("click",async()=>{t.style.display="none";try{const{del:M,keys:_e}=await z(async()=>{const{del:ce,keys:Re}=await import("./index-DwnyWBFG.js");return{del:ce,keys:Re}},[]),Ke=await _e(),Ze=["productCatalogCsv","customerLogo","fredChatHistory","fredChatMessages","fredFeedback","fredQuestionLog"];await Promise.all(Ke.filter(ce=>typeof ce=="string"&&(Ze.includes(ce)||ce.startsWith("crosshair_"))).map(ce=>M(ce)))}catch{}localStorage.removeItem("configPreferences"),localStorage.removeItem("pdfWizardSettings"),Object.keys(localStorage).filter(M=>M.startsWith("crosshair_")||M.startsWith("fred")).forEach(M=>localStorage.removeItem(M)),window.location.reload()}),(y=document.getElementById("menu-version-footer"))==null||y.addEventListener("click",()=>{t.style.display="none",Uo()}),(g=document.getElementById("menu-profile"))==null||g.addEventListener("click",()=>{t.style.display="none",Y.showEditProfile(M=>{E.clearUserSettings(),n(M)})}),(w=document.getElementById("menu-password"))==null||w.addEventListener("click",()=>{t.style.display="none",Y.showChangePassword()}),(v=document.getElementById("menu-logout"))==null||v.addEventListener("click",()=>{t.style.display="none",E.clearUserSettings(),O.logout(),n(null)})}}else f&&(f.style.display="none"),o&&(o.style.display="block")}function r(c){if(!c)return"?";const d=c.trim().split(" ");return d.length>=2?(d[0][0]+d[d.length-1][0]).toUpperCase():c.substring(0,2).toUpperCase()}function a(c){const d=document.createElement("div");return d.textContent=c||"",d.innerHTML}e&&t&&(e.addEventListener("click",c=>{c.stopPropagation();const d=t.style.display!=="none";t.style.display=d?"none":"block"}),document.addEventListener("click",c=>{f!=null&&f.contains(c.target)||(t.style.display="none")})),o&&o.addEventListener("click",()=>{Y.showLogin(c=>{n(c)})}),O.onAuthChange=n,n(O.getCurrentUser())}function No(){const f=document.getElementById("user-guide-modal"),e=document.getElementById("user-guide-content");if(!f||!e)return;e.innerHTML=Mo(),f.style.display="flex";const t=document.getElementById("user-guide-close");t&&(t.onclick=()=>{f.style.display="none"}),f.onclick=s=>{s.target===f&&(f.style.display="none")};const o=s=>{s.key==="Escape"&&(f.style.display="none",document.removeEventListener("keydown",o))};document.addEventListener("keydown",o)}function Mo(){return`
...
`);if(s.length===0){e.innerHTML="<p>No changelog available.</p>",f.style.display="flex";return}let i="";s.forEach(n=>{if(n.trim()){const r=n.indexOf(" - ");if(r>0){const a=n.substring(0,r).trim(),c=n.substring(r+3).trim();i+=`
          ...
          `}}}),e.innerHTML=i||"<p>No changelog available.</p>",f.style.display="flex"}catch(f){console.error("Error loading changelog:",f),document.getElementById("changelog-content").innerHTML='<p style="color: #999;">Error loading changelog.</p>',document.getElementById("changelog-modal").style.display="flex"}}document.addEventListener("DOMContentLoaded",()=>{const f=document.getElementById("changelog-close");f&&f.addEventListener("click",()=>{document.getElementById("changelog-modal").style.display="none"})});document.addEventListener("click",f=>{const e=document.getElementById("changelog-modal");f.target===e&&(e.style.display="none")});
