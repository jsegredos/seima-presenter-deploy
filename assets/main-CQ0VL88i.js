...
`).filter(r=>r.trim()!==""),i=s.length>0?s[s.length-1]:"Unknown",n=i.split(" - ")[0]||i;o.innerText=n,o.innerText.trim()||(o.innerText="v2.1.0")}}else throw new Error("Version file not found")}catch{const t=document.getElementById("version-number");if(t){const o=M.get("app.version")||"v2.1.0";t.innerText=o}else setTimeout(()=>{const o=document.getElementById("version-number");if(o&&!o.innerText.trim()){const s=M.get("app.version")||"v2.1.0";o.innerText=s}},1e3);console.info("Version loaded from config (GitHub Pages mode)")}}async showProductLookupScreen(){try{const t=await(await fetch("./screens/product-grid.html")).text();document.body.innerHTML=t,this.currentScreen="product-grid";const o=document.createElement("script");o.type="module",o.src="js/app.js",document.body.appendChild(o),setTimeout(()=>{document.querySelectorAll(".back-btn").forEach(s=>s.remove())},100),window.productGridManager&&window.productGridManager.init(),await this.loadVersion(),setTimeout(()=>this.loadVersion(),1e3)}catch(e){console.error("Failed to load product grid screen:",e)}}setupSplitInterface(){const e=document.getElementById("back-to-home");e&&(e.onclick=()=>location.reload());const t=document.getElementById("download-btn"),o=document.getElementById("clear-all-btn");t&&(t.onclick=()=>this.showDownloadFormModal()),o&&(o.onclick=()=>this.showClearConfirmModal()),this.setupSplitProductSearch(),this.setupReviewTable(),this.renderReviewTable(),this.loadInitialSearchResults()}setupSplitProductSearch(){const e=document.getElementById("product-search-input"),t=document.getElementById("search-results-list"),o=document.getElementById("search-loading"),s=document.getElementById("search-no-results");if(!e||!t)return;const i=[],n=x.debounce(r=>{this.performSplitProductSearch(r,t,i,o,s)},200);e.addEventListener("input",()=>{const r=e.value.trim();r?n(r):this.loadInitialSearchResults()}),t.addEventListener("click",r=>{const a=r.target.closest(".result-item");if(!a)return;const c=parseInt(a.getAttribute("data-idx"),10),d=i.length>0?i:this.currentSearchResults||[];!isNaN(c)&&d[c]&&this.showSplitProductDetails(d[c])})}performSplitProductSearch(e,t,o,s,i){if(!D.isLoaded){s.style.display="flex",i.style.display="none",t.innerHTML="";return}o.length=0,o.push(...D.searchProducts(e)),s.style.display="none",o.length===0?(i.style.display="flex",t.innerHTML=""):(i.style.display="none",t.innerHTML=o.map((n,r)=>`
    ...
    `,document.body.appendChild(o);const i=kt();He("./assets/seima-logo.png",async(n,r,a)=>{const c=JSON.parse(localStorage.getItem("selection")||"[]"),d=JSON.parse(localStorage.getItem(q.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let l=[];d.length>0?l=d.map(b=>({...b.product,Room:b.room,Notes:b.notes,Quantity:b.quantity,Timestamp:new Date(b.timestamp).toISOString()})):l=c;const u=document.getElementById("sort-by"),p=u?u.value:"room";switch(p){case"code":l.sort((b,S)=>{const C=b.OrderCode||b.Code||"",P=S.OrderCode||S.Code||"";return C.localeCompare(P)});break;case"product":l.sort((b,S)=>{const C=b.Description||b.ProductName||"",P=S.Description||S.ProductName||"";return C.localeCompare(P)});break;case"room":default:{let b=[];try{const S=localStorage.getItem("customRoomOrder");b=S?JSON.parse(S):[]}catch{b=[]}l.sort((S,C)=>{const P=S.Room||"Blank",k=C.Room||"Blank";if(P===k)return 0;if(P==="Blank")return 1;if(k==="Blank")return-1;const _=b.indexOf(P),I=b.indexOf(k);return _!==-1&&I!==-1?_-I:_!==-1?-1:I!==-1?1:P.localeCompare(k)});break}}if(!l.length){alert("No products selected."),t&&(t.style.display="none");return}if(it()>0)console.log(`📷 Using ${it()} pre-cached images (skipping duplicate preload)`);else{const b=document.getElementById("pdf-processing-notification");if(b){const S=document.createElement("span");S.id="preload-progress",S.style.cssText="display: block; font-size: 12px; margin-top: 8px; color: #1e40af;",S.textContent="Loading images: 0%",b.appendChild(S)}console.log("📷 Starting image preload for",l.length,"products"),ht(l).then(S=>{const C=document.getElementById("preload-progress");C&&(C.textContent=`✓ ${S} images ready`,C.style.color="#059669")}).catch(S=>{console.warn("Image preloading error:",S)})}const m={};p==="room"?l.forEach(b=>{const S=b.Room||"Blank";m[S]||(m[S]=[]),m[S].push(b)}):m.__all__=l,await i;const{jsPDF:y}=window.jspdf,g=new y({orientation:"landscape",unit:"pt",format:"a4",compress:!0,putOnlyUsedFonts:!0,precision:16,userUnit:1,floatPrecision:16}),v=g.internal.pageSize.getWidth(),w=g.internal.pageSize.getHeight();He("./assets/seima-logo.png",async(b,S,C)=>{const P=$.getCurrentUser(),k=E.getUserSettings(),_=P?{name:P.name,email:P.email,phone:P.phone,position:P.position}:k;let I=null;try{const{get:L}=await z(async()=>{const{get:O}=await import("./vendor-idb-DwnyWBFG.js");return{get:O}},[]);I=await L("customerLogo")}catch{}_t(g,{pageWidth:v,pageHeight:w,seimaLogoDataUrl:b,seimaLogoNaturalW:S,seimaLogoNaturalH:C,customerLogoDataUrl:I,userDetails:f,staffContact:_,footerHeight:nt.footerHeight}),g.addPage(),He("./assets/seima-logo-white.png",(L,O,ae)=>{const ce=f.showRrp&&!f.excludePrice,Re=!f.excludePrice,tt=!f.excludeQty,xt=Rt(v,{leftMargin:32,rightMargin:32,showRrp:ce,showPrice:Re,showQty:tt,showTotal:Re&&tt}),{colX:ye,colW:Pe,headers:Ne}=xt,Ie=nt.footerHeight;dt();const ot=(B,T,X,te,j,G,N)=>{if(!T||typeof T!="string"||T.length<10||!T.startsWith("http://")&&!T.startsWith("https://")&&!T.startsWith("data:")){N&&N();return}if((A=>{if(!A||A.length<25||/\/images\/\d+$/.test(A)||A.endsWith("/0"))return!0;const F=/\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(A),H=A.startsWith("data:");return!F&&!H})(T)){console.warn("Skipping malformed image URL:",(T==null?void 0:T.substring(0,50))+"..."),N&&N();return}if(Z.totalImages++,f.emailCompatible){Z.failedImages++,N&&N();return}const oe=$t(T);if(oe&&oe.dataUrl)try{const A=oe.width/oe.height;let F=j,H=j/A;H>G&&(H=G,F=G*A),B.addImage(oe.dataUrl,oe.format,X,te,F,H,void 0,"FAST"),Z.optimizedImages++,N&&N();return}catch{console.warn("Failed to use cached image, falling back to direct load")}let de=!1;const we=["https://wsrv.nl/?url=","https://images.weserv.nl/?url=","https://api.codetabs.com/v1/proxy?quest="];let se=0;function ie(){if(de)return;const A=new Image;A.crossOrigin="Anonymous";let F=null;A.onload=function(){if(!de){de=!0,F&&clearTimeout(F);try{const le=yt(0),$e=le.imageMaxWidth,Se=j,ne=G;try{const W=document.createElement("canvas"),re=W.getContext("2d"),{width:ue,height:be}=ro(A.width,A.height,$e);W.width=ue,W.height=be,re.imageSmoothingEnabled=!0,re.imageSmoothingQuality="high",re.drawImage(A,0,0,ue,be);let Oe,ze="JPEG";const Et=no(W,re),Pt=io(A);Et||Pt?(Oe=W.toDataURL("image/png",le.imageQuality),ze="PNG"):(Oe=W.toDataURL("image/jpeg",le.imageQuality),ze="JPEG");const Ct=`img_${go(T)}`;B.addImage(Oe,ze,X,te,Se,ne,Ct,"FAST"),Z.optimizedImages++,N&&N()}catch(W){console.warn(`Failed to optimize image: ${T}`,W),console.warn("Error details:",W.message,W.stack);try{B.addImage(A,"JPEG",X,te,Se,ne),Z.optimizedImages++,N&&N()}catch(re){console.error(`Fallback also failed for: ${T}`,re),Z.failedImages++,N&&N()}}}catch(le){console.warn("Failed to add image to PDF:",le),Z.failedImages++,N&&N()}}},A.onerror=function(){de||(F&&clearTimeout(F),console.warn(`Failed to load image with proxy ${se}: ${T}`),console.warn(`Error details for: ${T} - Proxy: ${we[se]}`),se++,se<we.length?setTimeout(()=>{ie()},200):(de=!0,console.warn("All proxies failed, skipping image"),Z.failedImages++,N&&N()))},F=setTimeout(()=>{de||(console.warn(`⏰ Timeout with proxy ${se}: ${T}`),A.src="",A.onload=null,A.onerror=null,se++,se<we.length?setTimeout(()=>{ie()},200):(de=!0,console.warn("All proxies timed out, skipping image"),Z.failedImages++,N&&N()))},3e3);let H=T;se<we.length&&(H=we[se]+encodeURIComponent(T)),A.src=H}ie()},K=[];Object.keys(m).forEach((B,T)=>{const X=m[B];if(!X||!Array.isArray(X)){console.warn("⚠️ Skipping invalid room items:",B,X);return}X.forEach((te,j)=>{if(!te){console.warn("⚠️ Skipping null item in room:",B,"at index:",j);return}K.push({item:te,room:B,rIdx:T,iIdx:j,isFirstInRoom:j===0,roomCount:X.length})})}),K.reduce((B,T)=>{if(!T||!T.item)return console.warn("⚠️ Skipping null row in data analysis:",T),B;const X=String(T.item.Description||""),te=String(T.item.LongDescription||""),j=String(T.item.Notes||""),G=String(T.item.OrderCode||"");return B+X.length+te.length+j.length+G.length},0);let Ce=0,Le=0;const Te=4,De=8,Me=Math.floor((w-80)/Te);let st=Ie+8;function Ue(){if(!K||!Array.isArray(K)){console.error("❌ Critical error: rowsToDraw is not a valid array:",K),ct(new Error("Invalid product data structure"),"generating PDF","unknown.pdf");return}if(Ce>=K.length){const j=g.internal.getNumberOfPages()-1;for(let F=2;F<=j+1;F++){g.setPage(F);const le=(F-2)*Te,$e=Math.min(le+Te,K.length);let Se=!1;for(let ne=le;ne<$e;ne++)if(K[ne]&&K[ne].item&&It(K[ne].item)){Se=!0;break}Lt(g,{pageWidth:v,colX:ye,colW:Pe,leftMargin:32,footerHeight:Ie,logoDataUrl:L,logoNaturalW:O,logoNaturalH:ae,headers:Ne,userDetails:f,skipWelsHeader:!Se}),Tt(g,{pageWidth:v,pageHeight:w,leftMargin:32,footerHeight:Ie,pageNumber:F-1,totalPages:j})}const G=new Date,N=String(G.getDate()).padStart(2,"0"),ve=String(G.getMonth()+1).padStart(2,"0"),oe=String(G.getFullYear()).slice(-2),de=String(G.getHours()).padStart(2,"0"),we=String(G.getMinutes()).padStart(2,"0"),ie=`${f.project.replace(/[^a-zA-Z0-9\s]/g,"")}-${N}${ve}${oe}.${de}${we}.pdf`,A=document.getElementById("pdf-processing-notification");A&&A.remove(),fo(f.emailCompatible);try{const F=g.output("blob"),H=g.output("string"),le=H?H.match(/\/Type\s*\/XObject/g):null,$e=H?H.match(/Tj\s/g):null,Se=H?H.match(/\/A\s*<</g):null;f.pdfSize=F.size;const ne=co(F,ie);if(f.sendEmail&&F.size>15*1024*1024){console.warn(`❌ PDF too large for email (${(F.size/1024/1024).toFixed(1)}MB), offering email-compatible version`),yo(f,ie);return}const W=ao(F,ne.settings);if(f.sendEmail&&f.email)if(f.exportCsv){const re=ie.replace(/\.pdf$/,".csv");at(f,re).then(ue=>{window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:f,pdfBlob:W,csvBlob:ue}}))}).catch(ue=>{console.error("Async CSV generation for email failed:",ue),window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:f,pdfBlob:W,csvBlob:null}}))})}else window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:f,pdfBlob:W,csvBlob:null}}));else(async()=>{const re=await wo(W);if(await Je(re,ie,"PDF"),f.exportCsv){const ue=ie.replace(/\.pdf$/,".csv");try{const be=await at(f,ue);be&&await Je(be,ue,"CSV")}catch(be){console.error("CSV generation failed:",be)}}})()}catch(F){console.error("PDF generation failed:",F),ct(F,"generating PDF",ie);const H=document.getElementById("pdf-processing-notification");H&&H.remove()}t&&(t.style.display="none");return}Le>=Te&&(g.addPage(),st=Ie+8,Le=0);const B=K[Ce];if(!B||!B.item){console.warn(`⚠️  Skipping invalid row at index ${Ce}:`,B),Ce++,Ue();return}const T=st+Me*Le;B.isFirstInRoom&&p==="room"&&B.room!=="__all__"&&Dt(g,B.room,B.roomCount,32,T);const X=ye[0],te=X+90+12;ot(g,B.item.Image_URL||"",X,T+De+16,90,Me-De*2,()=>{ot(g,B.item.Diagram_URL||"",te,T+De+16,90,Me-De*2,()=>{const j=T+28,G=ye[1]+Pe[1]/2;g.setFontSize(10),g.setTextColor("#222"),g.text(String(B.item.OrderCode||""),G,j+10,{align:"center"}),Bt(g,B.item,G,j+35);const N=Pe[2]-10;At(g,B.item,ye[2],j+10,N,f.excludeLongDescription);const ve=Ne.indexOf("WELS")+1;if(ve>0&&ye[ve]){const oe=ye[ve]+Pe[ve]/2;Ft(g,B.item,oe,j+10)}Nt(g,B.item,ye,Pe,Ne,j+10,{excludePrice:f.excludePrice,includeGst:f.includeGst}),Ce++,Le++,Ue()})})}Ue()})})})}function He(f,e){const t=new window.Image;t.crossOrigin="Anonymous",t.onload=function(){const o=document.createElement("canvas"),s=o.getContext("2d"),i=400,n=150;let r=t.width,a=t.height;if(r>i||a>n){const d=i/r,l=n/a,u=Math.min(d,l);r=Math.round(r*u),a=Math.round(a*u)}o.width=r,o.height=a,s.imageSmoothingEnabled=!0,s.imageSmoothingQuality="high",s.drawImage(t,0,0,r,a);const c=o.toDataURL("image/png",.9);e(c,r,a)},t.src=f}function Zt(){if(!document.getElementById("pdf-spinner")){const f=document.createElement("div");if(f.id="pdf-spinner",f.style.display="none",f.style.position="fixed",f.style.top="0",f.style.left="0",f.style.width="100vw",f.style.height="100vh",f.style.zIndex="9999",f.style.background="rgba(255,255,255,0.7)",f.style.alignItems="center",f.style.justifyContent="center",f.innerHTML='<div style="border:6px solid #e0e0e0;border-top:6px solid #2563eb;border-radius:50%;width:54px;height:54px;animation:spin 1s linear infinite;"></div>',document.body.appendChild(f),!document.getElementById("pdf-spinner-style")){const e=document.createElement("style");e.id="pdf-spinner-style",e.innerHTML="@keyframes spin { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);} }",document.head.appendChild(e)}}}async function at(f,e){return new Promise(async t=>{if(!window.Papa)try{await x.loadScript("https://cdn.jsdelivr.net/npm/papaparse@5.4.1/papaparse.min.js")}catch(n){console.error("Failed to load PapaParse:",n),t(null);return}const o=JSON.parse(localStorage.getItem("selection")||"[]"),s=JSON.parse(localStorage.getItem(q.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let i=[];if(s.length>0?i=s.map(n=>({...n.product,Room:n.room,Notes:n.notes,Quantity:n.quantity,Timestamp:new Date(n.timestamp).toISOString()})):i=o,!i.length){t(null);return}setTimeout(()=>{const n=i.map(r=>{let a,c,d,l;const u=f.excludePrice;let p=0;if(r.UserEditedPrice!==void 0&&r.UserEditedPrice!==null&&r.UserEditedPrice!=="")p=parseFloat(r.UserEditedPrice.toString().replace(/,/g,""));else{const g=r.RRP_EX||r["RRP EX GST"]||r.RRP_EX||r.RRP_EXGST||"";p=parseFloat((g||"0").toString().replace(/,/g,""))}a=p,d="Price ea ex GST",l="Price Total ex GST",c=!isNaN(a)&&a>=0?(a*(r.Quantity||1)).toFixed(2):"";const h=r["WELS STAR"]||r.WELS_STAR||r.WELS_STAR||r.WelsStar||"",m=h&&h.toString().trim()?h.toString().replace(/[^\d.]/g,"").trim():"",y={Code:me(r.OrderCode||""),Description:me(r.Description||""),"WELS Star":me(m),Quantity:r.Quantity||1,Notes:me(r.Notes||""),Room:me(r.Room||""),"Image URL":me(r.Image_URL||""),"Diagram URL":me(r.Diagram_URL||""),"Datasheet URL":me(r.Datasheet_URL||""),"Website URL":me(r.Website_URL||"")};return y[d]=u?"0.00":a>=0?a.toFixed(2):"",y[l]=u?"0.00":c,y});setTimeout(()=>{const r=window.Papa.unparse(n,{quotes:!0,quoteChar:'"',delimiter:",",header:!0,newline:`\r
...
`);if(t.length>0){const s=t[0].split(" - ")[0].trim();window._appVersion=s;const i=document.getElementById("menu-version-footer");i&&(i.textContent=`Ver: ${s}`)}}),Fo(),No();const f=document.getElementById("changelog-close");f&&f.addEventListener("click",()=>{document.getElementById("changelog-modal").style.display="none"})});function Fo(){const f=document.getElementById("help-btn");f&&f.addEventListener("click",()=>{Mo()});const e=document.getElementById("crosshair-btn");e&&e.addEventListener("click",()=>{window.location.href="screens/validator.html"});const t=document.getElementById("landscape-btn");t&&t.addEventListener("click",()=>{window.location.href="screens/landscape.html"});const o=document.getElementById("admin-btn");o&&o.addEventListener("click",()=>{window.location.href="screens/admin.html"});const s=document.getElementById("quick-start-btn");s&&s.addEventListener("click",()=>{St.showForced()})}function No(){const f=document.getElementById("user-menu-container"),e=document.getElementById("user-menu-trigger"),t=document.getElementById("user-menu-dropdown"),o=document.getElementById("sign-in-btn"),s=document.getElementById("user-avatar"),i=document.getElementById("user-name-display");function n(c){var d,l,u,p,h,m,y,g,v,w;if(c){(d=q.CROSSHAIR)!=null&&d.ENABLED&&(R.configure(q.CROSSHAIR,c.email||""),R.preload().catch(O=>{console.warn("Crosshair preload after login failed:",O)})),f&&(f.style.display="block"),o&&(o.style.display="none");const b=document.getElementById("crosshair-btn");b&&(b.style.display=(l=q.CROSSHAIR)!=null&&l.ENABLED&&$.isStaffMode()?"":"none");const S=document.getElementById("landscape-btn");S&&(S.style.display=(u=q.CROSSHAIR)!=null&&u.ENABLED&&$.isAdmin()?"":"none");const C=document.getElementById("admin-btn");C&&(C.style.display=$.isAdmin()?"":"none");const P=document.getElementById("staff-divider"),k=(b==null?void 0:b.style.display)!=="none"||(S==null?void 0:S.style.display)!=="none"||(C==null?void 0:C.style.display)!=="none";P&&(P.style.display=k?"":"none");const _=r(c.name);s&&(s.textContent=_),i&&(i.textContent=((p=c.name)==null?void 0:p.split(" ")[0])||"User");const I=(c.email||"").toLowerCase().endsWith("@seima.com.au")&&c.emailVerified===!1;let L=document.getElementById("verify-email-banner");if(I?(L||(L=document.createElement("div"),L.id="verify-email-banner",L.className="verify-email-banner",L.innerHTML=`
        ...
        `,(h=document.getElementById("menu-verify-email"))==null||h.addEventListener("click",()=>{t.style.display="none",Y.showVerifyEmail(c.email,()=>{var U;n($.getCurrentUser()),(U=document.getElementById("verify-email-banner"))==null||U.remove()})}),(m=document.getElementById("menu-refresh"))==null||m.addEventListener("click",async()=>{t.style.display="none";try{const{del:U,keys:fe}=await z(async()=>{const{del:ce,keys:Re}=await import("./vendor-idb-DwnyWBFG.js");return{del:ce,keys:Re}},[]),Ze=await fe(),et=["productCatalogCsv","customerLogo","fredChatHistory","fredChatMessages","fredFeedback","fredQuestionLog"];await Promise.all(Ze.filter(ce=>typeof ce=="string"&&(et.includes(ce)||ce.startsWith("crosshair_"))).map(ce=>U(ce)))}catch{}localStorage.removeItem("configPreferences"),localStorage.removeItem("pdfWizardSettings"),Object.keys(localStorage).filter(U=>U.startsWith("crosshair_")||U.startsWith("fred")).forEach(U=>localStorage.removeItem(U)),window.location.reload()}),(y=document.getElementById("menu-version-footer"))==null||y.addEventListener("click",()=>{t.style.display="none",Oo()}),(g=document.getElementById("menu-profile"))==null||g.addEventListener("click",()=>{t.style.display="none",Y.showEditProfile(U=>{E.clearUserSettings(),n(U)})}),(v=document.getElementById("menu-password"))==null||v.addEventListener("click",()=>{t.style.display="none",Y.showChangePassword()}),(w=document.getElementById("menu-logout"))==null||w.addEventListener("click",()=>{t.style.display="none",E.clearUserSettings(),$.logout(),n(null)})}}else f&&(f.style.display="none"),o&&(o.style.display="block")}function r(c){if(!c)return"?";const d=c.trim().split(" ");return d.length>=2?(d[0][0]+d[d.length-1][0]).toUpperCase():c.substring(0,2).toUpperCase()}function a(c){const d=document.createElement("div");return d.textContent=c||"",d.innerHTML}e&&t&&(e.addEventListener("click",c=>{c.stopPropagation();const d=t.style.display!=="none";t.style.display=d?"none":"block"}),document.addEventListener("click",c=>{f!=null&&f.contains(c.target)||(t.style.display="none")})),o&&o.addEventListener("click",()=>{Y.showLogin(c=>{n(c)})}),$.onAuthChange=n,n($.getCurrentUser())}function Mo(){const f=document.getElementById("user-guide-modal"),e=document.getElementById("user-guide-content");if(!f||!e)return;e.innerHTML=Uo(),f.style.display="flex";const t=document.getElementById("user-guide-close");t&&(t.onclick=()=>{f.style.display="none"}),f.onclick=s=>{s.target===f&&(f.style.display="none")};const o=s=>{s.key==="Escape"&&(f.style.display="none",document.removeEventListener("keydown",o))};document.addEventListener("keydown",o)}function Uo(){return`
