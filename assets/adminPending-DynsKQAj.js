const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/index-EZbr6NQL.js","assets/auth-ui-BdyM06Px.js","assets/auth-ui-in-MCC0p.css","assets/product-search-CeYNHdfP.js","assets/vendor-fuse-Ch1WBRTM.js","assets/product-matching-CsGMOakR.js","assets/pdf-layouts-BftXqYv_.js","assets/data-layer-C5VcJsaC.js","assets/product-synonyms-CeNdtqLO.js","assets/vendor-idb-DwnyWBFG.js"])))=>i.map(i=>d[i]);
import"./auth-ui-BdyM06Px.js";import{_ as K,C as I}from"./config-DcBu6QPE.js";import{c as u}from"./competitor-service-CvNbDbLJ.js";class R{constructor(){this._pending=[],this._filter="all",this._competitorFilter="",this._seimaByCode={}}async init(){const{authService:t}=await K(async()=>{const{authService:e}=await import("./index-EZbr6NQL.js");return{authService:e}},__vite__mapDeps([0,1,2,3,4,5,6])),i=t.getCurrentUser();if(!(!!i&&t.isStaffMode())){document.getElementById("ap-content").style.display="none",document.getElementById("ap-forbidden").style.display="";return}u.isEnabled()||u.configure(I.CROSSHAIR,i.email),await this._loadSeimaData(),await u.preload().catch(()=>{}),await this._loadPending(),this._render()}async _loadSeimaData(){try{const{dataLayer:t}=await K(async()=>{const{dataLayer:a}=await import("./data-layer-C5VcJsaC.js").then(e=>e.a);return{dataLayer:a}},__vite__mapDeps([7,1,2,3,4,8,9]));t.isLoaded||await t.init();const i=t.products||[];for(const a of i){const e=String(a.OrderCode||a["Order Code"]||a.Code||"").trim();e&&(this._seimaByCode[e]=a)}}catch(t){console.error("AdminPending: failed to load Seima catalog",t)}}async _loadPending(){try{const t=await u.getPendingVerifications();this._pending=t.pending||[]}catch(t){console.error("AdminPending: failed to load pending",t),this._pending=[]}}_render(){const t=document.getElementById("ap-content"),i=this._getFilteredItems(),a=this._pending.filter(o=>o.Status==="Pending-User"),e=this._pending.filter(o=>o.Status==="Pending-Dispute"),s=[...new Set(this._pending.map(o=>o.competitor))];let n="";if(n+=`
      <div class="ap-stats">
