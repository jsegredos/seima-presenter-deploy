...
`).filter(r=>r.trim()!==""),n=s.length>0?s[s.length-1]:"Unknown",i=n.split(" - ")[0]||n;o.innerText=i,o.innerText.trim()||(o.innerText="v2.1.0")}}else throw new Error("Version file not found")}catch{const t=document.getElementById("version-number");if(t){const o=B.get("app.version")||"v2.1.0";t.innerText=o}else setTimeout(()=>{const o=document.getElementById("version-number");if(o&&!o.innerText.trim()){const s=B.get("app.version")||"v2.1.0";o.innerText=s}},1e3);console.info("Version loaded from config (GitHub Pages mode)")}}async showProductLookupScreen(){try{const t=await(await fetch("./screens/product-grid.html")).text();document.body.innerHTML=t,this.currentScreen="product-grid";const o=document.createElement("script");o.type="module",o.src="js/app.js",document.body.appendChild(o),setTimeout(()=>{document.querySelectorAll(".back-btn").forEach(s=>s.remove())},100),window.productGridManager&&window.productGridManager.init(),await this.loadVersion(),setTimeout(()=>this.loadVersion(),1e3)}catch(e){console.error("Failed to load product grid screen:",e)}}setupSplitInterface(){const e=document.getElementById("back-to-home");e&&(e.onclick=()=>location.reload());const t=document.getElementById("download-btn"),o=document.getElementById("clear-all-btn");t&&(t.onclick=()=>this.showDownloadFormModal()),o&&(o.onclick=()=>this.showClearConfirmModal()),this.setupSplitProductSearch(),this.setupReviewTable(),this.renderReviewTable(),this.loadInitialSearchResults()}setupSplitProductSearch(){const e=document.getElementById("product-search-input"),t=document.getElementById("search-results-list"),o=document.getElementById("search-loading"),s=document.getElementById("search-no-results");if(!e||!t)return;const n=[],i=S.debounce(r=>{this.performSplitProductSearch(r,t,n,o,s)},200);e.addEventListener("input",()=>{const r=e.value.trim();r?i(r):this.loadInitialSearchResults()}),t.addEventListener("click",r=>{const a=r.target.closest(".result-item");if(!a)return;const c=parseInt(a.getAttribute("data-idx"),10),d=n.length>0?n:this.currentSearchResults||[];!isNaN(c)&&d[c]&&this.showSplitProductDetails(d[c])})}performSplitProductSearch(e,t,o,s,n){if(!L.isLoaded){s.style.display="flex",n.style.display="none",t.innerHTML="";return}o.length=0,o.push(...L.searchProducts(e)),s.style.display="none",o.length===0?(n.style.display="flex",t.innerHTML=""):(n.style.display="none",t.innerHTML=o.map((i,r)=>`
    ...
    `,document.body.appendChild(o),Ae("./assets/seima-logo.png",(n,i,r)=>{const a=JSON.parse(localStorage.getItem("selection")||"[]"),c=JSON.parse(localStorage.getItem(N.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let d=[];c.length>0?d=c.map(h=>({...h.product,Room:h.room,Notes:h.notes,Quantity:h.quantity,Timestamp:new Date(h.timestamp).toISOString()})):d=a;const l=document.getElementById("sort-by"),u=l?l.value:"room";switch(u){case"code":d.sort((h,w)=>{const E=h.OrderCode||h.Code||"",P=w.OrderCode||w.Code||"";return E.localeCompare(P)});break;case"product":d.sort((h,w)=>{const E=h.Description||h.ProductName||"",P=w.Description||w.ProductName||"";return E.localeCompare(P)});break;case"room":default:d.sort((h,w)=>{const E=h.Room||"Blank",P=w.Room||"Blank";return E.localeCompare(P)});break}if(!d.length){alert("No products selected."),t&&(t.style.display="none");return}if(Ke()>0)console.log(`📷 Using ${Ke()} pre-cached images (skipping duplicate preload)`);else{const h=document.getElementById("pdf-processing-notification");if(h){const w=document.createElement("span");w.id="preload-progress",w.style.cssText="display: block; font-size: 12px; margin-top: 8px; color: #1e40af;",w.textContent="Loading images: 0%",h.appendChild(w)}console.log("📷 Starting image preload for",d.length,"products"),ct(d).then(w=>{const E=document.getElementById("preload-progress");E&&(E.textContent=`✓ ${w} images ready`,E.style.color="#059669")}).catch(w=>{console.warn("Image preloading error:",w)})}const g={};u==="room"?d.forEach(h=>{const w=h.Room||"Blank";g[w]||(g[w]=[]),g[w].push(h)}):g.__all__=d;const{jsPDF:y}=window.jspdf,v=new y({orientation:"landscape",unit:"pt",format:"a4",compress:!0,putOnlyUsedFonts:!0,precision:16,userUnit:1,floatPrecision:16}),f=v.internal.pageSize.getWidth(),x=v.internal.pageSize.getHeight();Ae("./assets/seima-logo.png",(h,w,E)=>{const P=G.getCurrentUser(),F=b.getUserSettings(),M=P?{name:P.name,email:P.email,phone:P.phone,position:P.position}:F,$=localStorage.getItem("customerLogo");bt(v,{pageWidth:f,pageHeight:x,seimaLogoDataUrl:h,seimaLogoNaturalW:w,seimaLogoNaturalH:E,customerLogoDataUrl:$,userDetails:p,staffContact:M,footerHeight:Ze.footerHeight}),v.addPage(),Ae("./assets/seima-logo-white.png",(Z,de,le)=>{const gt=p.showRrp&&!p.excludePrice,We=!p.excludePrice,Ye=!p.excludeQty,ft=St(f,{leftMargin:32,rightMargin:32,showRrp:gt,showPrice:We,showQty:Ye,showTotal:We&&Ye}),{colX:ue,colW:we,headers:Te}=ft,xe=Ze.footerHeight;ot();const Xe=(I,C,z,Y,A,O,D)=>{if(!C||typeof C!="string"||C.length<10||!C.startsWith("http://")&&!C.startsWith("https://")&&!C.startsWith("data:")){D&&D();return}if((R=>{if(!R||R.length<25||/\/images\/\d+$/.test(R)||R.endsWith("/0"))return!0;const T=/\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(R),_=R.startsWith("data:");return!T&&!_})(C)){console.warn("Skipping malformed image URL:",(C==null?void 0:C.substring(0,50))+"..."),D&&D();return}if(V.totalImages++,p.emailCompatible){V.failedImages++,D&&D();return}const X=kt(C);if(X&&X.dataUrl)try{const R=X.width/X.height;let T=A,_=A/R;_>O&&(_=O,T=O*R),I.addImage(X.dataUrl,X.format,z,Y,T,_,void 0,"FAST"),V.optimizedImages++,D&&D();return}catch{console.warn("Failed to use cached image, falling back to direct load")}let ee=!1;const me=["https://wsrv.nl/?url=","https://images.weserv.nl/?url=","https://api.codetabs.com/v1/proxy?quest="];let J=0;function Q(){if(ee)return;const R=new Image;R.crossOrigin="Anonymous";let T=null;R.onload=function(){if(!ee){ee=!0,T&&clearTimeout(T);try{const te=ut(0),ke=te.imageMaxWidth,ye=A,K=O;try{const U=document.createElement("canvas"),j=U.getContext("2d"),{width:q,height:Qe}=Xt(R.width,R.height,ke);U.width=q,U.height=Qe,j.imageSmoothingEnabled=!0,j.imageSmoothingQuality="high",j.drawImage(R,0,0,q,Qe);let $e,_e="JPEG";const yt=Yt(U,j),vt=Wt(R);yt||vt?($e=U.toDataURL("image/png",te.imageQuality),_e="PNG"):($e=U.toDataURL("image/jpeg",te.imageQuality),_e="JPEG");const wt=`img_${so(C)}`;I.addImage($e,_e,z,Y,ye,K,wt,"FAST"),V.optimizedImages++,D&&D()}catch(U){console.warn(`Failed to optimize image: ${C}`,U),console.warn("Error details:",U.message,U.stack);try{I.addImage(R,"JPEG",z,Y,ye,K),V.optimizedImages++,D&&D()}catch(j){console.error(`Fallback also failed for: ${C}`,j),V.failedImages++,D&&D()}}}catch(te){console.warn("Failed to add image to PDF:",te),V.failedImages++,D&&D()}}},R.onerror=function(){ee||(T&&clearTimeout(T),console.warn(`Failed to load image with proxy ${J}: ${C}`),console.warn(`Error details for: ${C} - Proxy: ${me[J]}`),J++,J<me.length?setTimeout(()=>{Q()},200):(ee=!0,console.warn("All proxies failed, skipping image"),V.failedImages++,D&&D()))},T=setTimeout(()=>{ee||(console.warn(`⏰ Timeout with proxy ${J}: ${C}`),R.src="",R.onload=null,R.onerror=null,J++,J<me.length?setTimeout(()=>{Q()},200):(ee=!0,console.warn("All proxies timed out, skipping image"),V.failedImages++,D&&D()))},3e3);let _=C;J<me.length&&(_=me[J]+encodeURIComponent(C)),R.src=_}Q()},H=[];Object.keys(g).forEach((I,C)=>{const z=g[I];if(!z||!Array.isArray(z)){console.warn("⚠️ Skipping invalid room items:",I,z);return}z.forEach((Y,A)=>{if(!Y){console.warn("⚠️ Skipping null item in room:",I,"at index:",A);return}H.push({item:Y,room:I,rIdx:C,iIdx:A,isFirstInRoom:A===0,roomCount:z.length})})}),H.reduce((I,C)=>{if(!C||!C.item)return console.warn("⚠️ Skipping null row in data analysis:",C),I;const z=String(C.item.Description||""),Y=String(C.item.LongDescription||""),A=String(C.item.Notes||""),O=String(C.item.OrderCode||"");return I+z.length+Y.length+A.length+O.length},0);let be=0,Ee=0;const Pe=4,Ce=8,De=Math.floor((x-80)/Pe);let Je=xe+8;function Be(){if(!H||!Array.isArray(H)){console.error("❌ Critical error: rowsToDraw is not a valid array:",H),tt(new Error("Invalid product data structure"),"generating PDF","unknown.pdf");return}if(be>=H.length){const A=v.internal.getNumberOfPages()-1;for(let T=2;T<=A+1;T++){v.setPage(T);const te=(T-2)*Pe,ke=Math.min(te+Pe,H.length);let ye=!1;for(let K=te;K<ke;K++)if(H[K]&&H[K].item&&xt(H[K].item)){ye=!0;break}Et(v,{pageWidth:f,colX:ue,colW:we,leftMargin:32,footerHeight:xe,logoDataUrl:Z,logoNaturalW:de,logoNaturalH:le,headers:Te,userDetails:p,skipWelsHeader:!ye}),Pt(v,{pageWidth:f,pageHeight:x,leftMargin:32,footerHeight:xe,pageNumber:T-1,totalPages:A})}const O=new Date,D=String(O.getDate()).padStart(2,"0"),pe=String(O.getMonth()+1).padStart(2,"0"),X=String(O.getFullYear()).slice(-2),ee=String(O.getHours()).padStart(2,"0"),me=String(O.getMinutes()).padStart(2,"0"),Q=`${p.project.replace(/[^a-zA-Z0-9\s]/g,"")}-${D}${pe}${X}.${ee}${me}.pdf`,R=document.getElementById("pdf-processing-notification");R&&R.remove(),no(p.emailCompatible);try{const T=v.output("blob"),_=v.output("string"),te=_?_.match(/\/Type\s*\/XObject/g):null,ke=_?_.match(/Tj\s/g):null,ye=_?_.match(/\/A\s*<</g):null;p.pdfSize=T.size;const K=Qt(T,Q);if(p.sendEmail&&T.size>15*1024*1024){console.warn(`❌ PDF too large for email (${(T.size/1024/1024).toFixed(1)}MB), offering email-compatible version`),io(p,Q);return}const U=Jt(T,K.settings);if(p.sendEmail&&p.email)if(p.exportCsv){const j=Q.replace(/\.pdf$/,".csv");et(p,j).then(q=>{window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:p,pdfBlob:U,csvBlob:q}}))}).catch(q=>{console.error("Async CSV generation for email failed:",q),window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:p,pdfBlob:U,csvBlob:null}}))})}else window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:p,pdfBlob:U,csvBlob:null}}));else if((async()=>{const j=await ao(U);Ge(j,Q,"PDF")})(),p.exportCsv){const j=Q.replace(/\.pdf$/,".csv");setTimeout(()=>{et(p,j).then(q=>{q&&Ge(q,j,"CSV")}).catch(q=>{console.error("CSV generation failed:",q)})},1e3)}}catch(T){console.error("PDF generation failed:",T),tt(T,"generating PDF",Q);const _=document.getElementById("pdf-processing-notification");_&&_.remove()}t&&(t.style.display="none");return}Ee>=Pe&&(v.addPage(),Je=xe+8,Ee=0);const I=H[be];if(!I||!I.item){console.warn(`⚠️  Skipping invalid row at index ${be}:`,I),be++,Be();return}const C=Je+De*Ee;I.isFirstInRoom&&u==="room"&&I.room!=="__all__"&&Ct(v,I.room,I.roomCount,32,C);const z=ue[0],Y=z+90+12;Xe(v,I.item.Image_URL||"",z,C+Ce+16,90,De-Ce*2,()=>{Xe(v,I.item.Diagram_URL||"",Y,C+Ce+16,90,De-Ce*2,()=>{const A=C+28,O=ue[1]+we[1]/2;v.setFontSize(10),v.setTextColor("#222"),v.text(String(I.item.OrderCode||""),O,A+10,{align:"center"}),It(v,I.item,O,A+35);const D=we[2]-10;Rt(v,I.item,ue[2],A+10,D,p.excludeLongDescription);const pe=Te.indexOf("WELS")+1;if(pe>0&&ue[pe]){const X=ue[pe]+we[pe]/2;Lt(v,I.item,X,A+10)}Tt(v,I.item,ue,we,Te,A+10,{excludePrice:p.excludePrice,includeGst:p.includeGst}),be++,Ee++,Be()})})}Be()})})})}function Ae(p,e){const t=new window.Image;t.crossOrigin="Anonymous",t.onload=function(){const o=document.createElement("canvas"),s=o.getContext("2d"),n=400,i=150;let r=t.width,a=t.height;if(r>n||a>i){const d=n/r,l=i/a,u=Math.min(d,l);r=Math.round(r*u),a=Math.round(a*u)}o.width=r,o.height=a,s.imageSmoothingEnabled=!0,s.imageSmoothingQuality="high",s.drawImage(t,0,0,r,a);const c=o.toDataURL("image/png",.9);e(c,r,a)},t.src=p}function jt(){if(!document.getElementById("pdf-spinner")){const p=document.createElement("div");if(p.id="pdf-spinner",p.style.display="none",p.style.position="fixed",p.style.top="0",p.style.left="0",p.style.width="100vw",p.style.height="100vh",p.style.zIndex="9999",p.style.background="rgba(255,255,255,0.7)",p.style.alignItems="center",p.style.justifyContent="center",p.innerHTML='<div style="border:6px solid #e0e0e0;border-top:6px solid #2563eb;border-radius:50%;width:54px;height:54px;animation:spin 1s linear infinite;"></div>',document.body.appendChild(p),!document.getElementById("pdf-spinner-style")){const e=document.createElement("style");e.id="pdf-spinner-style",e.innerHTML="@keyframes spin { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);} }",document.head.appendChild(e)}}}async function et(p,e){return new Promise(async t=>{if(!window.Papa)try{await S.loadScript("https://cdn.jsdelivr.net/npm/papaparse@5.4.1/papaparse.min.js")}catch(i){console.error("Failed to load PapaParse:",i),t(null);return}const o=JSON.parse(localStorage.getItem("selection")||"[]"),s=JSON.parse(localStorage.getItem(N.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let n=[];if(s.length>0?n=s.map(i=>({...i.product,Room:i.room,Notes:i.notes,Quantity:i.quantity,Timestamp:new Date(i.timestamp).toISOString()})):n=o,!n.length){t(null);return}setTimeout(()=>{const i=n.map(r=>{let a,c,d,l;const u=p.excludePrice;let m=0;if(r.UserEditedPrice!==void 0&&r.UserEditedPrice!==null&&r.UserEditedPrice!=="")m=parseFloat(r.UserEditedPrice.toString().replace(/,/g,""));else{const f=r.RRP_EX||r["RRP EX GST"]||r.RRP_EX||r.RRP_EXGST||"";m=parseFloat((f||"0").toString().replace(/,/g,""))}a=m,d="Price ea ex GST",l="Price Total ex GST",c=!isNaN(a)&&a>=0?(a*(r.Quantity||1)).toFixed(2):"";const g=r["WELS STAR"]||r.WELS_STAR||r.WELS_STAR||r.WelsStar||"",y=g&&g.toString().trim()?g.toString().replace(/[^\d.]/g,"").trim():"",v={Code:oe(r.OrderCode||""),Description:oe(r.Description||""),"WELS Star":oe(y),Quantity:r.Quantity||1,Notes:oe(r.Notes||""),Room:oe(r.Room||""),"Image URL":oe(r.Image_URL||""),"Diagram URL":oe(r.Diagram_URL||""),"Datasheet URL":oe(r.Datasheet_URL||""),"Website URL":oe(r.Website_URL||"")};return v[d]=u?"0.00":a>=0?a.toFixed(2):"",v[l]=u?"0.00":c,v});setTimeout(()=>{const r=window.Papa.unparse(i,{quotes:!0,quoteChar:'"',delimiter:",",header:!0,newline:`\r
`,skipEmptyLines:!1,escapeChar:'"',transform:{value(a,c){return typeof a=="string"?a.replace(/\0/g,"").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g,""):a}}});p.sendEmail?setTimeout(()=>{try{const a=btoa(unescape(encodeURIComponent(r)));t({name:e,data:a,contentType:"text/csv",originalSize:r.length,base64Size:a.length})}catch(a){console.error("CSV base64 encoding failed:",a),t(new Blob([r],{type:"text/csv"}))}},0):t(new Blob([r],{type:"text/csv"}))},0)},0)})}function oe(p){return typeof p!="string"&&(p=String(p)),p=p.replace(/\0/g,"").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g,"").replace(/\r?\n|\r/g," ").trim(),p}async function Gt(p,e,t="file"){try{if("showSaveFilePicker"in window){const s=await(await window.showSaveFilePicker({suggestedName:e,types:[{description:`${t} files`,accept:{[p.type]:[`.${e.split(".").pop()}`]}}]})).createWritable();return await s.write(p),await s.close(),!0}}catch(o){console.warn("File System Access API failed:",o)}return!1}function Ht(p,e,t="file"){try{if(p.size>2*1024*1024)return console.warn("File too large for data URI method"),!1;const o=new FileReader;return o.onload=function(s){try{const n=document.createElement("a");n.href=s.target.result,n.download=e,n.style.display="none",document.body.appendChild(n),n.click(),document.body.removeChild(n)}catch(n){console.error("Data URI download failed:",n)}},o.readAsDataURL(p),!0}catch(o){return console.warn("Data URI method failed:",o),!1}}function qt(p,e,t="file"){const o=URL.createObjectURL(p),s=document.createElement("div");s.style.cssText=`
  ...
  `,s.appendChild(n),document.body.appendChild(s),document.getElementById("manual-download-close").onclick=()=>{URL.revokeObjectURL(o),document.body.removeChild(s)},document.getElementById("manual-download-retry").onclick=()=>{URL.revokeObjectURL(o),document.body.removeChild(s),setTimeout(()=>{lt(p,e,t)},1e3)},document.getElementById("copy-url-btn").onclick=()=>{const i=document.getElementById("manual-download-url");i.select(),i.setSelectionRange(0,99999);try{navigator.clipboard.writeText(o).then(()=>{const r=document.getElementById("copy-url-btn");r.textContent="Copied!",r.style.background="#059669",setTimeout(()=>{r.textContent="Copy",r.style.background="#059669"},2e3)}).catch(()=>{document.execCommand("copy");const r=document.getElementById("copy-url-btn");r.textContent="Copied!",setTimeout(()=>r.textContent="Copy",2e3)})}catch{alert("Copy failed. Please select the URL manually and copy it.")}},s.onclick=i=>{i.target===s&&(URL.revokeObjectURL(o),document.body.removeChild(s))},setTimeout(()=>{s.parentElement&&(URL.revokeObjectURL(o),document.body.removeChild(s))},5*60*1e3)}async function lt(p,e,t="file"){try{if(await Vt(p,e))return}catch(o){console.warn("Standard download failed:",o)}await Gt(p,e,t)||Ht(p,e,t)||qt(p,e,t)}function Vt(p,e){return new Promise(t=>{try{const o=URL.createObjectURL(p),s=document.createElement("a");s.href=o,s.download=e,s.style.display="none",document.body.appendChild(s);const n=setTimeout(()=>{i(),t(!1)},3e3),i=()=>{clearTimeout(n),s.parentElement&&document.body.removeChild(s),setTimeout(()=>URL.revokeObjectURL(o),1e3)};s.onclick=()=>{i(),t(!0)},s.click(),setTimeout(()=>{i(),t(!0)},500)}catch(o){console.error("Standard download error:",o),t(!1)}})}function Wt(p){const e=document.createElement("canvas"),t=e.getContext("2d");e.width=Math.min(100,p.width),e.height=Math.min(100,p.height),t.drawImage(p,0,0,e.width,e.height);const s=t.getImageData(0,0,e.width,e.height).data,n=new Set;for(let i=0;i<s.length;i+=4){const r=`${s[i]},${s[i+1]},${s[i+2]}`;n.add(r)}return n.size<1e3}function Yt(p,e){const o=e.getImageData(0,0,p.width,p.height).data;for(let s=3;s<o.length;s+=4)if(o[s]<255)return!0;return!1}function Xt(p,e,t){if(p<=t)return{width:p,height:e};const o=e/p;return{width:t,height:Math.round(t*o)}}function ut(p){return p>25*1024*1024?{compressionLevel:"aggressive",imageQuality:.6,imageMaxWidth:300,removeImages:!1,usePNG:!0,message:"Aggressive compression - maintaining technical diagram clarity"}:p>20*1024*1024?{compressionLevel:"high",imageQuality:.65,imageMaxWidth:350,removeImages:!1,usePNG:!0,message:"High compression - preserving technical diagram details"}:p>15*1024*1024?{compressionLevel:"medium",imageQuality:.7,imageMaxWidth:400,removeImages:!1,usePNG:!0,message:"Medium compression - optimal for technical documentation"}:p>10*1024*1024?{compressionLevel:"light",imageQuality:.75,imageMaxWidth:450,removeImages:!1,usePNG:!0,message:"Light compression - excellent technical diagram quality"}:{compressionLevel:"minimal",imageQuality:.8,imageMaxWidth:500,removeImages:!1,usePNG:!0,message:"Minimal compression - maximum technical diagram quality"}}function Jt(p,e){return p}function Qt(p,e){const t=(p.size/1048576).toFixed(2),o=ut(p.size);if(p.size>15*1024*1024){console.warn(`Large file detected (${t} MB) - exceeds typical email limit, may need email-compatible version`);const s=document.createElement("div");s.style.cssText=`
    ...
    `,s.innerHTML=`
      ...
      ">OK</button>
    `,document.body.appendChild(s),setTimeout(()=>{s.parentElement&&s.remove()},8e3)}else p.size>3*1024*1024;return{size:p.size,sizeInMB:parseFloat(t),settings:o}}function tt(p,e="",t=""){console.error("Detailed error:",p);const o={type:Kt(p),message:p.message||"Unknown error",context:e,filename:t,timestamp:new Date().toISOString(),userAgent:navigator.userAgent},s=document.createElement("div");s.style.cssText=`
    position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
  ...
  `,s.appendChild(n),document.body.appendChild(s),document.getElementById("error-close").onclick=()=>{document.body.removeChild(s)},document.getElementById("error-retry").onclick=()=>{document.body.removeChild(s),console.log("Retry requested for:",e)},document.getElementById("error-report").onclick=()=>{oo(o),alert("Error details copied to clipboard. Please send this to support.")},s.onclick=i=>{i.target===s&&document.body.removeChild(s)},o}function Kt(p){var o,s;const e=((o=p.message)==null?void 0:o.toLowerCase())||"",t=((s=p.stack)==null?void 0:s.toLowerCase())||"";return e.includes("network")||e.includes("fetch")?"network":e.includes("permission")||e.includes("denied")?"permission":e.includes("memory")||e.includes("quota")?"memory":e.includes("blob")||e.includes("url")?"download":e.includes("canvas")||e.includes("image")?"rendering":t.includes("jspdf")||e.includes("pdf")?"pdf":"unknown"}function Zt(p){return{network:"Network Connection Error",permission:"Permission Required",memory:"Insufficient Memory",download:"Download Failed",rendering:"Display Error",pdf:"PDF Generation Error",unknown:"Unexpected Error"}[p]||"Error Occurred"}function eo(p,e,t){return{network:"Unable to load required resources. Please check your internet connection and try again.",permission:`Browser permission required to save ${t}. Please allow downloads and try again.`,memory:"Not enough memory to process this large file. Try closing other browser tabs or use fewer products.",download:`Failed to download ${t}. This may be due to browser security settings or storage limitations.`,rendering:"Unable to display product images properly. Some images may be missing from the final output.",pdf:`PDF generation failed while ${e}. The file may be too large or contain problematic data.`,unknown:`An unexpected error occurred while ${e}. Please try again or contact support.`}[p]||"An unknown error has occurred."}function to(p){const e={network:`
      ...
      </div>`};return e[p]||e.unknown}function pt(){const p=navigator.userAgent;return p.includes("Chrome")?"Chrome":p.includes("Firefox")?"Firefox":p.includes("Safari")?"Safari":p.includes("Edge")?"Edge":p.includes("SamsungBrowser")?"Samsung Internet":"Unknown"}function oo(p){const e=`
...
========================
  `.trim();try{navigator.clipboard.writeText(e)}catch(t){console.error("Failed to copy error report:",t)}}let V={totalImages:0,optimizedImages:0,failedImages:0,totalSavings:0};function so(p){let e=0;if(p.length===0)return e.toString();for(let t=0;t<p.length;t++){const o=p.charCodeAt(t);e=(e<<5)-e+o,e=e&e}return Math.abs(e).toString(36)}function ot(){V={totalImages:0,optimizedImages:0,failedImages:0,totalSavings:0}}function no(p=!1){V.totalImages>0}function io(p,e){const t=document.createElement("div");t.style.cssText=`
    position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
    background: rgba(0,0,0,0.8); z-index: 10001; display: flex; 
    align-items: center; justify-content: center; padding: 20px;
...
Return format: [{"code":"...","name":"...","brand":"...","category":"...","quantity":1,"finish":"...","section":"...","specCode":"...","style":"..."}]`;class uo{constructor(){this.conversationHistory=this._loadHistory(),this.isProcessing=!1}async chat(e,t,o=null){var s,n,i;if(this.isProcessing)throw new Error("A request is already in progress");this.isProcessing=!0;try{const r=[{role:"system",content:co}],a=this._findRelevantProducts(e,t);if(a&&r.push({role:"system",content:`SEIMA PRODUCT DATA (${a.count} products):
...
This can be restored later.`))try{const t=await ie.deleteSelections([e.id]);t.success?(this.showToast("Selection deleted"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to delete")}catch(t){this.showError(t.message)}}async restoreSelection(e){try{const t=await ie.restoreSelections([e.id]);t.success?(this.showToast("Selection restored"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to restore")}catch(t){this.showError(t.message)}}showToast(e){const t=document.createElement("div");t.style.cssText=`
    ...
    `;const a=i.querySelector(".toast-close");if(a.onclick=()=>this.dismiss(n),s&&s.callback){const c=i.querySelector(".toast-action");c.onclick=()=>{s.callback(),this.dismiss(n)}}return this.container.appendChild(i),this.toasts.set(n,i),requestAnimationFrame(()=>{i.classList.add("toast-enter")}),o>0&&setTimeout(()=>this.dismiss(n),o),n}dismiss(e){const t=this.toasts.get(e);t&&(t.classList.add("toast-exit"),t.addEventListener("animationend",()=>{t.remove(),this.toasts.delete(e)}))}dismissAll(){this.toasts.forEach((e,t)=>this.dismiss(t))}success(e,t={}){return this.show({message:e,type:"success",...t})}error(e,t={}){return this.show({message:e,type:"error",duration:6e3,...t})}warning(e,t={}){return this.show({message:e,type:"warning",...t})}info(e,t={}){return this.show({message:e,type:"info",...t})}withUndo(e,t,o=5e3){return this.show({message:e,type:"info",duration:o,action:{label:"Undo",callback:t}})}}const se=new vo;window.toast=se;class wo{constructor(){this.activeDropdown=null,this.updatePositionHandler=null,document.addEventListener("click",e=>{e.target.closest(".global-search-dropdown")||this.hideDropdown()})}showDropdown(e,t,o,s=!1){this.hideDropdown();const n=document.createElement("ul");n.className="global-search-dropdown";const i=300,r=e.getBoundingClientRect(),a=window.innerWidth,c=window.innerHeight,d=r.width,l=c-r.bottom,u=r.top;let m=!1;l<i&&u>l&&(m=!0);let g;m?g=Math.max(8,r.top-i-8):g=Math.min(c-i-8,r.bottom+8);let y=r.left;y+d>a-8&&(y=a-d-8),y<8&&(y=8);const v={position:"fixed",top:`${g}px`,left:`${y}px`,width:`${d}px`,minWidth:`${d}px`,maxWidth:`${d}px`,background:"#fff",border:"1px solid #d1d5db",borderRadius:"8px",boxShadow:"0 10px 25px -5px rgba(0, 0, 0, 0.15), 0 8px 10px -6px rgba(0, 0, 0, 0.10)",maxHeight:`${i}px`,overflowY:"auto",overflowX:"hidden",zIndex:"10010",listStyle:"none",margin:"0",padding:"4px 0",whiteSpace:"normal",wordWrap:"break-word",display:"block",pointerEvents:"auto",transform:"none",contain:"none",isolation:"isolate"};Object.keys(v).forEach(f=>{n.style.setProperty(f,v[f],"important")}),n.dropdownHeight=i,t.length===0?n.innerHTML='<li style="padding: 12px 16px; color: #6b7280; font-style: italic; background: #fff;">No products found</li>':n.innerHTML=t.map(f=>{if(f._crosshairValidate){const $=f._crosshairValidate,Z=$.imageUrl?`<img src="${S.sanitizeInput($.imageUrl)}" alt="" style="width:32px; height:32px; object-fit:contain; border-radius:4px; margin-right:10px; vertical-align:middle; flex-shrink:0;">`:"";return`<li data-validate-competitor="${S.sanitizeInput($.competitor)}" data-validate-code="${S.sanitizeInput($.productCode)}"
            ...
            <div style="flex:1; min-width:0;">
              <div style="font-weight: 600; color: #92400e;">${S.sanitizeInput($.productName)}</div>
              <div style="color: #78716c; font-size: 11px;">${S.sanitizeInput($.competitor)} · #${S.sanitizeInput($.productCode)}${$.finish?" · "+S.sanitizeInput($.finish):""} · ${$.matchCount} suggested match${$.matchCount!==1?"es":""}</div>
            </div>
            <span style="color: #d97706; font-size: 11px; font-weight: 600; white-space: nowrap; margin-left: 8px;">Review →</span>
          </li>`}const x=f.OrderCode||f.Code||"",h=f.Description||f.ProductName||f["Product Name"]||"",w=f.Image_URL||f.image_url||"",E=f._crosshairMatch,P=E?`<span style="display: inline-block; background: ${E.status==="Verified"?"#e8f5e9":"#e3f2fd"}; color: ${E.status==="Verified"?"#2e7d32":"#1565c0"}; font-size: 10px; font-weight: 600; padding: 1px 6px; border-radius: 8px; margin-left: 8px;">↔ ${S.sanitizeInput(E.competitorLabel||E.competitor)}</span>`:"",F=w?`<img src="${S.sanitizeInput(w)}" alt="" style="width:36px; height:36px; object-fit:contain; border-radius:4px; margin-right:10px; flex-shrink:0;" onerror="this.style.display='none'">`:"",M=E?Object.fromEntries(Object.entries(f).filter(([$])=>$!=="_crosshairMatch")):f;return`<li data-product='${JSON.stringify(M).replace(/'/g,"&apos;")}'
      ...
      `;n.push(l),r.forEach(u=>{n.push(this.renderRowHtml(u))})}),e.innerHTML=n.join(""),this.attachSeimaMatchSuggestions()}extractCandidateCodes(e){if(!e||typeof e!="string")return[];const t=new Set,o=[],s=/(?:Code|Part\s*(?:No\.?|Number)|P\/N|Order\s*Code|SKU|Item\s*(?:No\.?|Number)?|#)\s*:?\s*([A-Za-z0-9]{4,14}(?:[.\-][A-Za-z0-9]{2,5})?)/gi;let n;for(;(n=s.exec(e))!==null;){const d=n[1].trim();d&&!t.has(d)&&(t.add(d),o.push(d))}const i=e.split(/\|/).map(d=>d.trim()),r=/^[A-Za-z0-9]{5,14}([.\-][A-Za-z0-9]{2,5})?$/,a=/\d/,c=new Set(["white","stainless","steel","black","grey","gray","chrome","brushed","matte","polished"]);for(const d of i){if(!r.test(d)||!a.test(d))continue;const l=d.toLowerCase();c.has(l)||t.has(d)||(t.add(d),o.push(d))}return o}escapeRegex(e){return String(e).replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}buildPlaceholderHintHtml(e,t){if(!e)return"";let o=S.sanitizeInput(e);if(t.length===0)return o;for(const{code:s,competitor:n,searchCode:i}of t){const r=this.escapeRegex(s),a="(?:(?:Code|Part\\s*(?:No\\.?|Number)|P\\/N|Order\\s*Code|SKU|Item\\s*(?:No\\.?|Number)?|#)\\s*:?\\s*)?",c=new RegExp("("+a+r+")","gi"),d=i??s,l=`<a href="#" class="grid-competitor-code-link" data-competitor="${S.sanitizeInput(n)}" data-code="${S.sanitizeInput(d)}" title="Review in Validator to link to Seima product">$1</a>`;o=o.replace(c,l)}return o}async attachSeimaMatchSuggestions(){var t,o,s;if(!k.isEnabled())return;const e=document.querySelectorAll(".grid-seima-match-wrap");for(const n of e){const i=n.dataset.rowId,r=this.gridRows.find(x=>x.id===i);if(!((t=r==null?void 0:r.product)!=null&&t._placeholder))continue;const a=r.product.Description||"",c=this.extractCandidateCodes(a);if(c.length===0)continue;const d=n.closest(".grid-product-cell"),l=d==null?void 0:d.querySelector(".grid-placeholder-hint"),u=[];for(const x of c)try{const h=await k.findCompetitorEntryByCode(x);if(h){const w=h.matchedCode??((o=h.competitorProduct)==null?void 0:o.product_code)??x;u.push({code:x,competitor:h.competitor,searchCode:w})}else console.log(`Crosshair: no competitor match for code "${x}" (row ${i})`)}catch{}if(l&&u.length>0&&(l.innerHTML=this.buildPlaceholderHintHtml(a,u)),!L.isLoaded)continue;let m=null,g="";for(const x of c)try{const h=await k.findSeimaMatches(x);if(((s=h==null?void 0:h.matches)==null?void 0:s.length)>0){const w=String(h.matches[0].SeimaSKU).trim(),E=L.findProductByCode(w);if(E){m=E;const F=(h.competitorProduct||{}).product_name||E.Description||"";g=`${h.competitor}: ${x} → ${w}${F?" – "+F:""}`;break}}}catch{}if(!m)continue;const y=document.createElement("button");y.type="button",y.className="grid-seima-match-btn";const v=(m.Description||"").trim(),f=v.length>45?v.slice(0,45)+"…":v;y.textContent=`Use Seima match: ${m.OrderCode||""} – ${f}`,y.title=g,y.addEventListener("click",()=>{const x=n.closest(".grid-row"),h=x==null?void 0:x.querySelector(".grid-search-input");h&&this.selectProduct(h,m)}),n.appendChild(y)}}async openValidatorModal(e,t,o){this.closeValidatorModal();const s=document.createElement("div");s.className="grid-validator-overlay";const n=document.createElement("div");n.className="grid-validator-modal",n.innerHTML=`
    ...
    `,document.body.appendChild(s),document.body.appendChild(n);const i=()=>this.closeValidatorModal(o);n.querySelector(".gvm-close").addEventListener("click",i),s.addEventListener("click",i);const r=a=>{a.key==="Escape"&&i()};document.addEventListener("keydown",r),this._validatorModal={overlay:s,modal:n,escHandler:r,competitor:e,productCode:t};try{await k._ensureIndex();const a=await k.getMatches(e),d=(await k.getProducts(e)).find(y=>String(y.product_code)===String(t));if(!d){n.querySelector(".gvm-body").innerHTML=`<div class="gvm-no-matches">Product ${S.sanitizeInput(t)} not found in ${S.sanitizeInput(e)} data.</div>`;return}const l=a.filter(y=>String(y.CompetitorSKU)===String(t)).sort((y,v)=>(y.Rank||99)-(v.Rank||99)),u={Verified:0,Manual:1,Rejected:2,"AI-Suggested":3},m=new Map;for(const y of l){const v=String(y.SeimaSKU),f=m.get(v);(!f||(u[y.Status]??9)<(u[f.Status]??9))&&m.set(v,y)}const g=[...m.values()].sort((y,v)=>(y.Rank||99)-(v.Rank||99));this._renderValidatorModal(d,g,e)}catch(a){console.error("Validator modal load failed:",a),n.querySelector(".gvm-body").innerHTML='<div class="gvm-no-matches">Failed to load data. Check console.</div>'}}_renderValidatorModal(e,t,o){var l;const s=(l=this._validatorModal)==null?void 0:l.modal;if(!s)return;const n=e.product_name||e.product_code,i=e.image_url||e.Image||"",r=u=>S.sanitizeInput(u||"");s.querySelector(".gvm-title").textContent=`${n} — ${o}`;let a;t.length===0?a='<div class="gvm-no-matches">No Seima matches found. Use the search below to add one.</div>':a=t.map(u=>{const m=String(u.SeimaSKU||"").trim(),g=L.findProductByCode(m),y=(g==null?void 0:g.Description)||(g==null?void 0:g.ProductName)||m,v=(g==null?void 0:g.Image_URL)||(g==null?void 0:g.imageUrl)||"",f=(g==null?void 0:g.RRP_INCGST)||(g==null?void 0:g["RRP INC GST"])||"",x=f?`$${Number(f).toLocaleString("en-AU",{minimumFractionDigits:2})}`:"",h=Number(u.Confidence)||0,w=h>=70?"high":h>=40?"med":"low",E=(u.Status||"").toLowerCase().replace(/[\s-]+/g,"-");let P;return u.Status==="Verified"||u.Status==="Manual"?P=`<span class="gvm-status-label gvm-status-verified">✓ Verified</span>
          ...
          <div class="gvm-match-info">
            ...
            <div class="gvm-match-sku">${r(m)} <span class="gvm-badge gvm-badge-${w}">${h}%</span></div>
    ...
    `,this.messagesContainer.appendChild(e),this._scrollToBottom(),e}_formatMarkdown(e){let t=this._escapeHtml(e);return t=t.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),t=t.replace(/`([^`]+)`/g,"<code>$1</code>"),t=t.replace(/^### (.+)$/gm,"<h4>$1</h4>"),t=t.replace(/^## (.+)$/gm,"<h3>$1</h3>"),t=t.replace(/^- (.+)$/gm,"<li>$1</li>"),t=t.replace(/(<li>.*<\/li>\n?)+/g,o=>`<ul>${o}</ul>`),t=t.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),t=t.replace(/\n\n/g,"</p><p>"),t=`<p>${t}</p>`,t=t.replace(/<p><\/p>/g,""),t=t.replace(/<p>(<h[34]>)/g,"$1"),t=t.replace(/(<\/h[34]>)<\/p>/g,"$1"),t=t.replace(/<p>(<ul>)/g,"$1"),t=t.replace(/(<\/ul>)<\/p>/g,"$1"),t=t.replace(/\b(\d{6})\b/g,'<button class="ai-add-product-btn" data-code="$1" title="Add $1 to selection">$1 <span class="ai-add-icon">+</span></button>'),t}_getBasePath(){var e;return(((e=document.querySelector("base"))==null?void 0:e.href)||window.location.origin+"/").replace(/\/$/,"")}_getProductImage(e){return e.Image_URL||e.imageUrl||e.Image||`${this._getBasePath()}/assets/no-image.png`}_getProductName(e){return e.Description||e.ProductName||e["Product Name"]||""}_getProductMeta(e){const t=e.Finish||e.Colour||"",o=e.DimX||e["X Dimension (mm)"]||"",s=e.DimY||e["Y Dimension (mm)"]||"",n=e.DimZ||e["Z Dimension (mm)"]||"",i=o&&o!=="0"?`${o} × ${s||0} × ${n||0}mm`:"";return[t,i].filter(Boolean).join("  ·  ")}_getProductPrice(e){const t=e["RRP INC GST"]||e.RRP_INCGST||e["RRP EX GST"]||e.RRP_EXGST||"";return t?`$${parseFloat(t).toLocaleString("en-AU",{minimumFractionDigits:2})}`:""}_injectProductCards(e){const t=e.querySelectorAll(".ai-add-product-btn");if(!t.length)return;const o=this._getBasePath();t.forEach(s=>{const n=s.dataset.code,i=L.findProductByCode(n);if(!i)return;const r=s.closest("li");if(!(r||s.parentElement))return;const c=this._getProductImage(i),d=this._getProductName(i),l=this._getProductMeta(i),u=this._getProductPrice(i),m=document.createElement("div");m.className="ai-product-card",m.dataset.code=n,m.innerHTML=`
      ...
      <div class="ai-product-modal">
        ...
        <div class="ai-product-modal-body">
          ...
          ${h.length?`<dl class="ai-product-modal-specs">${h.map(([E,P])=>`<dt>${this._escapeHtml(E)}</dt><dd>${this._escapeHtml(P)}</dd>`).join("")}</dl>`:""}
    ...
    `,document.body.insertBefore(n,document.body.firstChild)}setupGlobalEventListeners(){window.addEventListener("generatePdf",e=>{const{tipTailSettings:t,...o}=e.detail;jt(),He(o,t||null)}),window.addEventListener("beforeunload",()=>{}),ve.features.memoryAPI&&setInterval(()=>{const e=ve.memoryInfo;e.memoryPressure==="high"&&console.warn("High memory usage detected:",e)},6e4)}getSelectedProducts(){return b.getSelectedProducts()}clearSelection(){return b.clearAllSelections()}addProduct(e,t,o,s){return b.addProductToSelection(e,t,o,s)}updateSelectionCount(){this.navigationManager&&this.navigationManager.updateSelectionCount()}showError(e){alert(e)}}document.addEventListener("DOMContentLoaded",()=>{window.seimaScanner=new Eo,window.seimaScanner.init()});window.addEventListener("DOMContentLoaded",()=>{fetch("./version.txt").then(p=>p.text()).then(p=>{const e=p.trim().split(`
`);if(e.length>0){const o=e[0].split(" - ")[0].trim(),s=document.getElementById("app-version");s&&(s.textContent=`Ver: ${o}`,s.style.cursor="pointer",s.addEventListener("click",Ro))}}),Po(),Co()});function Po(){const p=document.getElementById("help-btn");p&&p.addEventListener("click",()=>{ko()});const e=document.getElementById("crosshair-btn");e&&e.addEventListener("click",()=>{window.location.href="screens/validator.html"});const t=document.getElementById("landscape-btn");t&&t.addEventListener("click",()=>{window.location.href="screens/landscape.html"});const o=document.getElementById("admin-btn");o&&o.addEventListener("click",()=>{window.location.href="screens/admin.html"});const s=document.getElementById("quick-start-btn");s&&s.addEventListener("click",()=>{ht.showForced()})}function Co(){const p=document.getElementById("user-menu-container"),e=document.getElementById("user-menu-trigger"),t=document.getElementById("user-menu-dropdown"),o=document.getElementById("sign-in-btn"),s=document.getElementById("user-avatar"),n=document.getElementById("user-name-display");function i(c){var d,l,u,m,g,y,v;if(c){(d=N.CROSSHAIR)!=null&&d.ENABLED&&(k.configure(N.CROSSHAIR,c.email||""),k.preload().catch(E=>{console.warn("Crosshair preload after login failed:",E)})),p&&(p.style.display="block"),o&&(o.style.display="none");const f=document.getElementById("crosshair-btn");f&&(f.style.display=(l=N.CROSSHAIR)!=null&&l.ENABLED&&G.isStaffMode()?"":"none");const x=document.getElementById("landscape-btn");x&&(x.style.display=(u=N.CROSSHAIR)!=null&&u.ENABLED&&G.isStaffMode()?"":"none");const h=document.getElementById("admin-btn");h&&(h.style.display=G.isAdmin()?"":"none");const w=r(c.name);s&&(s.textContent=w),n&&(n.textContent=((m=c.name)==null?void 0:m.split(" ")[0])||"User"),t&&(t.innerHTML=`
        ...
        `,(g=document.getElementById("menu-profile"))==null||g.addEventListener("click",()=>{t.style.display="none",W.showEditProfile(E=>{b.clearUserSettings(),i(E)})}),(y=document.getElementById("menu-password"))==null||y.addEventListener("click",()=>{t.style.display="none",W.showChangePassword()}),(v=document.getElementById("menu-logout"))==null||v.addEventListener("click",()=>{t.style.display="none",b.clearUserSettings(),G.logout(),i(null)}))}else p&&(p.style.display="none"),o&&(o.style.display="block")}function r(c){if(!c)return"?";const d=c.trim().split(" ");return d.length>=2?(d[0][0]+d[d.length-1][0]).toUpperCase():c.substring(0,2).toUpperCase()}function a(c){const d=document.createElement("div");return d.textContent=c||"",d.innerHTML}e&&t&&(e.addEventListener("click",c=>{c.stopPropagation();const d=t.style.display!=="none";t.style.display=d?"none":"block"}),document.addEventListener("click",c=>{p!=null&&p.contains(c.target)||(t.style.display="none")})),o&&o.addEventListener("click",()=>{W.showLogin(c=>{i(c)})}),G.onAuthChange=i,i(G.getCurrentUser())}function ko(){const p=document.getElementById("user-guide-modal"),e=document.getElementById("user-guide-content");if(!p||!e)return;e.innerHTML=Io(),p.style.display="flex";const t=document.getElementById("user-guide-close");t&&(t.onclick=()=>{p.style.display="none"}),p.onclick=s=>{s.target===p&&(p.style.display="none")};const o=s=>{s.key==="Escape"&&(p.style.display="none",document.removeEventListener("keydown",o))};document.addEventListener("keydown",o)}function Io(){return`
  ...
  `}async function Ro(){try{const p=document.getElementById("changelog-modal"),e=document.getElementById("changelog-content"),s=(await(await fetch("./version.txt")).text()).trim().split(`
