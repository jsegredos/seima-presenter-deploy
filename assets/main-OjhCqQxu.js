...
`).filter(s=>s.trim()!==""),n=r.length>0?r[r.length-1]:"Unknown",i=n.split(" - ")[0]||n;o.innerText=i,o.innerText.trim()||(o.innerText="v2.1.0")}}else throw new Error("Version file not found")}catch{const t=document.getElementById("version-number");if(t){const o=config.get("app.version")||"v2.1.0";t.innerText=o}else setTimeout(()=>{const o=document.getElementById("version-number");if(o&&!o.innerText.trim()){const r=config.get("app.version")||"v2.1.0";o.innerText=r}},1e3);console.info("Version loaded from config (GitHub Pages mode)")}}async showProductLookupScreen(){try{const t=await(await fetch("./screens/product-grid.html")).text();document.body.innerHTML=t,this.currentScreen="product-grid";const o=document.createElement("script");o.type="module",o.src="js/app.js",document.body.appendChild(o),setTimeout(()=>{document.querySelectorAll(".back-btn").forEach(r=>r.remove())},100),window.productGridManager&&window.productGridManager.init(),await this.loadVersion(),setTimeout(()=>this.loadVersion(),1e3)}catch(e){console.error("Failed to load product grid screen:",e)}}setupSplitInterface(){const e=document.getElementById("back-to-home");e&&(e.onclick=()=>location.reload());const t=document.getElementById("download-btn"),o=document.getElementById("clear-all-btn");t&&(t.onclick=()=>this.showDownloadFormModal()),o&&(o.onclick=()=>this.showClearConfirmModal()),this.setupSplitProductSearch(),this.setupReviewTable(),this.renderReviewTable(),this.loadInitialSearchResults()}setupSplitProductSearch(){const e=document.getElementById("product-search-input"),t=document.getElementById("search-results-list"),o=document.getElementById("search-loading"),r=document.getElementById("search-no-results");if(!e||!t)return;const n=[],i=Utils.debounce(s=>{this.performSplitProductSearch(s,t,n,o,r)},200);e.addEventListener("input",()=>{const s=e.value.trim();s?i(s):this.loadInitialSearchResults()}),t.addEventListener("click",s=>{const c=s.target.closest(".result-item");if(!c)return;const l=parseInt(c.getAttribute("data-idx"),10),d=n.length>0?n:this.currentSearchResults||[];!isNaN(l)&&d[l]&&this.showSplitProductDetails(d[l])})}performSplitProductSearch(e,t,o,r,n){if(!dataLayer.isLoaded){r.style.display="flex",n.style.display="none",t.innerHTML="";return}o.length=0,o.push(...dataLayer.searchProducts(e)),r.style.display="none",o.length===0?(n.style.display="flex",t.innerHTML=""):(n.style.display="none",t.innerHTML=o.map((i,s)=>`
      ...
      <div class="review-room-group">
        ...
        ${i.map((s,c)=>{const l=s.product,d=l.Description||l.description||l.productName||l["Product Name"]||"Product",u=l.OrderCode||l.orderCode||"",p=l.Image_URL||l.imageUrl||"assets/no-image.png",m=l.RRP_EX||l["RRP EX GST"]||l.RRP_EX||l.rrpExGst||l.RRP_EXGST||l.RRP_INCGST||l["RRP INC GST"]||"0";return`
  ...
  `,o.appendChild(r),document.body.appendChild(o),document.getElementById("samsung-help-close").onclick=()=>{document.body.removeChild(o)},document.getElementById("samsung-help-retry").onclick=()=>{document.body.removeChild(o),setTimeout(()=>{downloadWithFallback(a,e,t)},1e3)},o.onclick=n=>{n.target===o&&document.body.removeChild(o)}}async function downloadWithFallback(a,e,t="file"){if(isSamsungDevice$1())try{if(await attemptStandardDownload(a,e))return;console.warn("Download may have failed on Samsung device"),showSamsungDownloadHelp(a,e,t)}catch(o){console.error("Samsung download failed:",o),showSamsungDownloadHelp(a,e,t)}else await downloadWithEnhancedFallbacks(a,e,t)}function showBrowserCompatibilityWarning(){if(isSamsungDevice$1()){if(document.getElementById("samsung-browser-warning"))return;const e=document.createElement("div");e.id="samsung-browser-warning",e.style.cssText=`
    ...
    `,document.body.appendChild(o),loadImageAsDataURL("./assets/seima-logo.png",(n,i,s)=>{const c=JSON.parse(localStorage.getItem("selection")||"[]"),l=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let d=[];if(l.length>0?d=l.map(E=>({...E.product,Room:E.room,Notes:E.notes,Quantity:E.quantity,Timestamp:new Date(E.timestamp).toISOString()})):d=c,!d.length){alert("No products selected."),t&&(t.style.display="none");return}const u={};d.forEach(E=>{u[E.Room]||(u[E.Room]=[]),u[E.Room].push(E)});const{jsPDF:p}=window.jspdf,m=new p({orientation:"landscape",unit:"pt",format:"a4",compress:!0,putOnlyUsedFonts:!0,precision:16,userUnit:1,floatPrecision:16}),h=m.internal.pageSize.getWidth(),f=m.internal.pageSize.getHeight(),S=h/2,y=320,v=90,g=(h-y)/2,b=70;m.setFillColor(255,255,255),m.rect(g,b,y,v,"F");const k=localStorage.getItem("customerLogo");if(k)try{const E=new Image;E.onload=function(){const A=y-20,L=v-20,ce=E.width/E.height;let H=A,q=A/ce;q>L&&(q=L,H=L*ce);const oe=g+(y-H)/2,re=b+(v-q)/2;m.addImage(k,"PNG",oe,re,H,q,void 0,"FAST")},E.src=k}catch(E){console.warn("Failed to draw customer logo:",E)}loadImageAsDataURL("./assets/seima-logo.png",(E,A,L)=>{const H=L&&A?250*L/A:65,q=(h-250)/2,oe=b+v+80;m.addImage(E,"PNG",q,oe,250,H,void 0,"FAST"),m.setFontSize(15),m.setTextColor("#444");const re=S;let le=oe+H+50;const G=[];a!=null&&a.name&&a.name.trim()&&G.push({label:"Name:",value:a.name.trim(),bold:!0}),a!=null&&a.project&&a.project.trim()&&G.push({label:"Project:",value:a.project.trim(),bold:!0}),a!=null&&a.address&&a.address.trim()&&G.push({label:"Address:",value:a.address.trim(),bold:!0}),a!=null&&a.email&&a.email.trim()&&G.push({label:"Email:",value:a.email.trim(),bold:!0}),a!=null&&a.telephone&&a.telephone.trim()&&G.push({label:"Telephone:",value:a.telephone.trim(),bold:!0});const O=32;G.length>0&&G.forEach(W=>{m.setFont("helvetica","normal"),m.text(W.label,re-80,le,{align:"right"}),m.setFont("helvetica","bold"),m.text(W.value,re-35,le,{align:"left"}),le+=26}),m.setFillColor("#9B9184"),m.rect(0,f-O,h,O,"F"),m.setTextColor("#fff"),m.setFontSize(13);const ue=new Date,Re=["January","February","March","April","May","June","July","August","September","October","November","December"],ke=ue.getDate(),Le=Re[ue.getMonth()],Te=ue.getFullYear(),De=String(ue.getHours()).padStart(2,"0"),Be=String(ue.getMinutes()).padStart(2,"0"),Ae=`Printed ${ke} ${Le} ${Te}, ${De}:${Be}`;m.text(Ae,16,f-10),m.text("www.seima.com.au",h-140,f-10);const pe=StorageManager.getUserSettings(),$=pe.staffName||"",X=pe.staffPosition||"",Q=pe.staffEmail||"",K=pe.staffPhone||"";let Z="For more information";$&&K&&Q?Z=`For more information, please contact ${X?`${$}, ${X}`:$} on ${K} or email ${Q}`:$&&K?Z=`For more information, please contact ${X?`${$}, ${X}`:$} on ${K}`:$&&Q?Z=`For more information, please contact ${X?`${$}, ${X}`:$} or email ${Q}`:K&&Q?Z=`For more information, please call ${K} or email ${Q}`:$?Z=`For more information, please contact ${X?`${$}, ${X}`:$}`:K?Z=`For more information, please call ${K}`:Q&&(Z=`For more information, please email ${Q}`),m.setFont("helvetica","normal"),m.setFontSize(14),m.setTextColor("#111"),m.text(Z,S,f-O-18,{align:"center"}),m.addPage(),loadImageAsDataURL("./assets/seima-logo-white.png",(W,xe,Ee)=>{const fe=h-200,ye=h-120,Ce=h-80,ee=[32,236,321,fe,ye,Ce],ne=[90,90,fe-321,ye-fe,Ce-ye,80];resetImageOptimizationStats();const Ie=(w,x,M,V,D,j,C)=>{if(!x||typeof x!="string"||x.length<10||!x.startsWith("http://")&&!x.startsWith("https://")&&!x.startsWith("data:")){C&&C();return}if(imageOptimizationStats.totalImages++,a.emailCompatible){imageOptimizationStats.failedImages++,C&&C();return}if(a.emailCompatible){imageOptimizationStats.failedImages++,C&&C();return}let T=!1;const F=["https://api.codetabs.com/v1/proxy?quest=","https://corsproxy.io/?"];let N=0;function Y(){if(T)return;const B=new Image;B.crossOrigin="Anonymous";let I=null;B.onload=function(){if(!T){T=!0,I&&clearTimeout(I);try{const R=getOptimizedFileSettings(0),U=R.imageMaxWidth,P=D,de=j;try{const _=document.createElement("canvas"),te=_.getContext("2d"),{width:ae,height:J}=calculateOptimizedDimensions(B.width,B.height,U);_.width=ae,_.height=J,te.imageSmoothingEnabled=!0,te.imageSmoothingQuality="high",te.drawImage(B,0,0,ae,J);let z,Se="JPEG";const Ne=detectTransparency(_,te),Fe=isTechnicalDiagram(B);Ne||Fe?(z=_.toDataURL("image/png",R.imageQuality),Se="PNG"):(z=_.toDataURL("image/jpeg",R.imageQuality),Se="JPEG");const Oe=`img_${generateImageHash(x)}`;w.addImage(z,Se,M,V,P,de,Oe,"FAST"),imageOptimizationStats.optimizedImages++,C&&C()}catch(_){console.warn(`Failed to optimize image: ${x}`,_),console.warn("Error details:",_.message,_.stack);try{w.addImage(B,"JPEG",M,V,P,de),imageOptimizationStats.optimizedImages++,C&&C()}catch(te){console.error(`Fallback also failed for: ${x}`,te),imageOptimizationStats.failedImages++,C&&C()}}}catch(R){console.warn("Failed to add image to PDF:",R),imageOptimizationStats.failedImages++,C&&C()}}},B.onerror=function(){T||(I&&clearTimeout(I),console.warn(`Failed to load image with proxy ${N}: ${x}`),console.warn(`Error details for: ${x} - Proxy: ${F[N]}`),N++,N<F.length?setTimeout(()=>{Y()},200):(T=!0,console.warn("All proxies failed, skipping image"),imageOptimizationStats.failedImages++,C&&C()))},I=setTimeout(()=>{T||(console.warn(`⏰ Timeout with proxy ${N}: ${x}`),B.src="",B.onload=null,B.onerror=null,N++,N<F.length?setTimeout(()=>{Y()},200):(T=!0,console.warn("All proxies timed out, skipping image"),imageOptimizationStats.failedImages++,C&&C()))},3e3);let se=x;N<F.length&&(se=F[N]+encodeURIComponent(x)),B.src=se}Y()},ie=[];Object.keys(u).forEach((w,x)=>{const M=u[w];if(!M||!Array.isArray(M)){console.warn("⚠️ Skipping invalid room items:",w,M);return}M.forEach((V,D)=>{if(!V){console.warn("⚠️ Skipping null item in room:",w,"at index:",D);return}ie.push({item:V,room:w,rIdx:x,iIdx:D,isFirstInRoom:D===0,roomCount:M.length})})}),ie.reduce((w,x)=>{if(!x||!x.item)return console.warn("⚠️ Skipping null row in data analysis:",x),w;const M=String(x.item.Description||""),V=String(x.item.LongDescription||""),D=String(x.item.Notes||""),j=String(x.item.OrderCode||"");return w+M.length+V.length+D.length+j.length},0);let me=0,ge=0;const Pe=4,he=8,we=Math.floor((f-80)/Pe);let ve=O+8;function be(){if(!ie||!Array.isArray(ie)){console.error("❌ Critical error: rowsToDraw is not a valid array:",ie),showDetailedErrorMessage(new Error("Invalid product data structure"),"generating PDF","unknown.pdf");return}if(me>=ie.length){const D=m.internal.getNumberOfPages()-1;for(let R=2;R<=D+1;R++)m.setPage(R),drawPDFHeader(m,h,ee,ne,32,O,W,xe,Ee,a),ve=O+8,m.setFillColor("#9B9184"),m.rect(0,f-O,h,O,"F"),m.setTextColor("#fff"),m.setFontSize(11),m.text("www.seima.com.au",h-140,f-10),m.text(`Page ${R-1} of ${D}`,32,f-10);const j=new Date,C=String(j.getDate()).padStart(2,"0"),T=String(j.getMonth()+1).padStart(2,"0"),F=String(j.getFullYear()).slice(-2),N=String(j.getHours()).padStart(2,"0"),Y=String(j.getMinutes()).padStart(2,"0"),I=`${a.project.replace(/[^a-zA-Z0-9\s]/g,"")}-${C}${T}${F}.${N}${Y}.pdf`,se=document.getElementById("pdf-processing-notification");se&&se.remove(),showImageOptimizationSummary(a.emailCompatible);try{const R=m.output("blob"),U=m.output("string"),P=U?U.match(/\/Type\s*\/XObject/g):null,de=U?U.match(/Tj\s/g):null,_=U?U.match(/\/A\s*<</g):null;a.pdfSize=R.size;const te=showFileSizeInfo(R,I);if(a.sendEmail&&R.size>15*1024*1024){console.warn(`❌ PDF too large for email (${(R.size/1024/1024).toFixed(1)}MB), offering email-compatible version`),showEmailCompatibleOption(a,I);return}const ae=createOptimizedBlob(R,te.settings);if(a.sendEmail&&a.email)if(a.exportCsv){const J=I.replace(/\.pdf$/,".csv");generateCsvBlobAsync(a,J).then(z=>{window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:ae,csvBlob:z}}))}).catch(z=>{console.error("Async CSV generation for email failed:",z),window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:ae,csvBlob:null}}))})}else window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:ae,csvBlob:null}}));else if((async()=>{const J=await mergeWithTipTail(ae);downloadWithFallback(J,I,"PDF")})(),a.exportCsv){const J=I.replace(/\.pdf$/,".csv");setTimeout(()=>{generateCsvBlobAsync(a,J).then(z=>{z&&downloadWithFallback(z,J,"CSV")}).catch(z=>{console.error("CSV generation failed:",z)})},1e3)}}catch(R){console.error("PDF generation failed:",R),showDetailedErrorMessage(R,"generating PDF",I);const U=document.getElementById("pdf-processing-notification");U&&U.remove()}t&&(t.style.display="none");return}ge>=Pe&&(m.addPage(),drawPDFHeader(m,h,ee,ne,32,O,W,xe,Ee,a),ve=O+8,ge=0);const w=ie[me];if(!w||!w.item){console.warn(`⚠️  Skipping invalid row at index ${me}:`,w),me++,be();return}const x=ve+we*ge;w.isFirstInRoom&&(m.setFontSize(12),m.setFont("helvetica","bold"),m.setTextColor("#333"),m.text(`${w.room} (${w.roomCount})`,32,x+10),m.setFont("helvetica","normal"));const M=ee[0],V=M+90+12;Ie(m,w.item.Image_URL||"",M,x+he+16,90,we-he*2,()=>{Ie(m,w.item.Diagram_URL||"",V,x+he+16,90,we-he*2,()=>{m.setFontSize(10),m.setTextColor("#222");const D=x+28,C=ee[1]+ne[1]/2;m.text(String(w.item.OrderCode||""),C,D+10,{align:"center"});let T=D+35;if(w.item.Datasheet_URL&&w.item.Datasheet_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Datasheet",C,T,{url:w.item.Datasheet_URL,align:"center"});const P=m.getTextWidth("Datasheet");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(C-P/2,T+1.5,C+P/2,T+1.5),T+=16}if(w.item.Website_URL&&w.item.Website_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Website",C,T,{url:w.item.Website_URL,align:"center"});const P=m.getTextWidth("Website");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(C-P/2,T+1.5,C+P/2,T+1.5),T+=16}let F=D+10;m.setFontSize(10),m.setTextColor("#222");const N=ne[2]-10,Y=ee[2],B=m.splitTextToSize(String(w.item.Description||""),N);if(m.text(B,Y+5,F),F+=B.length*12,!a.excludeLongDescription&&(w.item.LongDescription||w.item["Long Description"]||w.item.longDescription)){const P=w.item.LongDescription||w.item["Long Description"]||w.item.longDescription;m.setFontSize(9),m.setTextColor("#444");const de=m.splitTextToSize(String(P),N);m.text(de,Y+5,F),F+=de.length*11}if(w.item.Notes){m.setFont("helvetica","italic"),m.setFontSize(9),m.setTextColor("#444");const P=m.splitTextToSize(`Notes: ${String(w.item.Notes).replace(/\r?\n|\r/g," ")}`,N);m.text(P,Y+5,F),F+=P.length*11,m.setFont("helvetica","normal")}m.setFontSize(10),m.setTextColor("#222");let I=NaN;if(w.item.UserEditedPrice!==void 0&&w.item.UserEditedPrice!==null&&w.item.UserEditedPrice!=="")I=parseFloat(w.item.UserEditedPrice.toString().replace(/,/g,""));else{const P=w.item.RRP_EX||w.item["RRP EX GST"]||w.item.RRP_EX||w.item.RRP_EXGST;P&&(I=parseFloat(P.toString().replace(/,/g,"")))}!isNaN(I)&&I>0&&a.includeGst&&(I=I*1.1);const se=!isNaN(I)&&I>=0?`$${I.toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2})}`:"";if(!a.excludePrice&&!a.excludeQty){const P=ee[3]+ne[3]/2;m.text(se,P,D+10,{align:"center"})}if(m.setFontSize(10),m.setTextColor("#222"),!a.excludeQty){const P=ee[4]+ne[4]/2;m.text(String(w.item.Quantity||1),P,D+10,{align:"center"})}m.setFontSize(10),m.setTextColor("#222");const R=I*(w.item.Quantity||1),U=!isNaN(I)&&I>=0?`$${R.toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2})}`:"";if(!a.excludePrice&&!a.excludeQty){const P=ee[5]+ne[5]/2;m.text(U,P,D+10,{align:"center"})}me++,ge++,be()})})}be()})})})}function drawPDFHeader(a,e,t,o,r,n,i,s,c,l){const d=n+5.7;if(a.setFillColor("#8B6C2B"),a.rect(0,0,e,d,"F"),i&&s&&c){const y=d*.55,v=s/c;let g=y*v;g>80&&(g=80);const b=(d-y)/2;a.addImage(i,"PNG",r,b,g,y)}a.setFontSize(10),a.setTextColor("#f4f4f4"),a.setFont("helvetica","normal");const u=d-8,p=t[1]+o[1]/2,m=t[2]+o[2]/2,h=t[3]+o[3]/2,f=t[4]+o[4]/2,S=t[5]+o[5]/2;if(a.text("Code",p,u,{align:"center"}),a.text("Description",m,u,{align:"center"}),!l.excludePrice&&!l.excludeQty){if(l.includeGst){a.setFont("helvetica","normal");const y=a.getTextWidth("Price ea "),v=h-a.getTextWidth("Price ea INC GST")/2;a.text("Price ea ",v,u),a.setFont("helvetica","bold"),a.text("INC GST",v+y,u),a.setFont("helvetica","normal")}else{a.setFont("helvetica","normal");const y=a.getTextWidth("Price ea "),v=h-a.getTextWidth("Price ea EX GST")/2;a.text("Price ea ",v,u),a.setFont("helvetica","bold"),a.text("EX GST",v+y,u),a.setFont("helvetica","normal")}a.text("Qty",f,u,{align:"center"}),a.text("Total",S,u,{align:"center"})}else l.excludePrice&&!l.excludeQty&&a.text("Qty",f,u,{align:"center"})}function loadImageAsDataURL(a,e){const t=new window.Image;t.crossOrigin="Anonymous",t.onload=function(){const o=document.createElement("canvas"),r=o.getContext("2d"),n=400,i=150;let s=t.width,c=t.height;if(s>n||c>i){const d=n/s,u=i/c,p=Math.min(d,u);s=Math.round(s*p),c=Math.round(c*p)}o.width=s,o.height=c,r.imageSmoothingEnabled=!0,r.imageSmoothingQuality="high",r.drawImage(t,0,0,s,c);const l=o.toDataURL("image/png",.9);e(l,s,c)},t.src=a}function ensurePdfSpinner(){if(!document.getElementById("pdf-spinner")){const a=document.createElement("div");if(a.id="pdf-spinner",a.style.display="none",a.style.position="fixed",a.style.top="0",a.style.left="0",a.style.width="100vw",a.style.height="100vh",a.style.zIndex="9999",a.style.background="rgba(255,255,255,0.7)",a.style.alignItems="center",a.style.justifyContent="center",a.innerHTML='<div style="border:6px solid #e0e0e0;border-top:6px solid #2563eb;border-radius:50%;width:54px;height:54px;animation:spin 1s linear infinite;"></div>',document.body.appendChild(a),!document.getElementById("pdf-spinner-style")){const e=document.createElement("style");e.id="pdf-spinner-style",e.innerHTML="@keyframes spin { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);} }",document.head.appendChild(e)}}}async function generateCsvBlobAsync(a,e){return new Promise(async t=>{if(!window.Papa)try{await Utils.loadScript("https://cdn.jsdelivr.net/npm/papaparse@5.4.1/papaparse.min.js")}catch(i){console.error("Failed to load PapaParse:",i),t(null);return}const o=JSON.parse(localStorage.getItem("selection")||"[]"),r=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let n=[];if(r.length>0?n=r.map(i=>({...i.product,Room:i.room,Notes:i.notes,Quantity:i.quantity,Timestamp:new Date(i.timestamp).toISOString()})):n=o,!n.length){t(null);return}setTimeout(()=>{const i=n.map(s=>{let c,l,d,u;const p=a.excludePrice;let m=0;if(s.UserEditedPrice!==void 0&&s.UserEditedPrice!==null&&s.UserEditedPrice!=="")m=parseFloat(s.UserEditedPrice.toString().replace(/,/g,""));else{const f=s.RRP_EX||s["RRP EX GST"]||s.RRP_EX||s.RRP_EXGST||"";m=parseFloat((f||"0").toString().replace(/,/g,""))}c=m,d="Price ea ex GST",u="Price Total ex GST",l=!isNaN(c)&&c>=0?(c*(s.Quantity||1)).toFixed(2):"";const h={Code:sanitizeCSVField(s.OrderCode||""),Description:sanitizeCSVField(s.Description||""),Quantity:s.Quantity||1,Notes:sanitizeCSVField(s.Notes||""),Room:sanitizeCSVField(s.Room||""),"Image URL":sanitizeCSVField(s.Image_URL||""),"Diagram URL":sanitizeCSVField(s.Diagram_URL||""),"Datasheet URL":sanitizeCSVField(s.Datasheet_URL||""),"Website URL":sanitizeCSVField(s.Website_URL||"")};return h[d]=p?"0.00":c>=0?c.toFixed(2):"",h[u]=p?"0.00":l,h});setTimeout(()=>{const s=window.Papa.unparse(i,{quotes:!0,quoteChar:'"',delimiter:",",header:!0,newline:`\r
...
This can be restored later.`))try{const t=await presentationLoader.deleteSelections([e.id]);t.success?(this.showToast("Selection deleted"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to delete")}catch(t){this.showError(t.message)}}async restoreSelection(e){try{const t=await presentationLoader.restoreSelections([e.id]);t.success?(this.showToast("Selection restored"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to restore")}catch(t){this.showError(t.message)}}showToast(e){const t=document.createElement("div");t.style.cssText=`
    ...
    `;const r=t.filter(i=>i.type==="critical"),n=e.score<config.get("compatibility.minCompatibilityScore",70);r.length>0||n?o.innerHTML=`
      ...
      `,document.body.insertBefore(o,document.body.firstChild),!n&&r.length===0&&setTimeout(()=>{o.parentElement&&o.remove()},15e3)}setupGlobalEventListeners(){window.addEventListener("generatePdf",e=>{const{tipTailSettings:t,...o}=e.detail;ensurePdfSpinner(),showPdfFormScreen$1(o,t||null)}),window.addEventListener("beforeunload",()=>{}),browserCompatibility.features.memoryAPI&&setInterval(()=>{const e=browserCompatibility.memoryInfo;e.memoryPressure==="high"&&console.warn("High memory usage detected:",e)},6e4)}getSelectedProducts(){return StorageManager.getSelectedProducts()}clearSelection(){return StorageManager.clearAllSelections()}addProduct(e,t,o,r){return StorageManager.addProductToSelection(e,t,o,r)}updateSelectionCount(){this.navigationManager&&this.navigationManager.updateSelectionCount()}showError(e){alert(e)}}document.addEventListener("DOMContentLoaded",()=>{window.seimaScanner=new SeimaScanner,window.seimaScanner.init()});window.addEventListener("DOMContentLoaded",()=>{fetch("./version.txt").then(a=>a.text()).then(a=>{const e=document.getElementById("app-version");if(e){const t=a.trim().split(/\s+|-/)[0];e.textContent=`Ver: ${t}`,e.addEventListener("click",()=>{showChangelog(a.trim(),t)})}}),setupHelpButton(),setupUserMenu()});function setupHelpButton(){const a=document.getElementById("help-btn");a&&a.addEventListener("click",()=>{showUserGuide()});const e=document.getElementById("quick-start-btn");e&&e.addEventListener("click",()=>{onboarding.showForced()})}function setupUserMenu(){const a=document.getElementById("user-menu-container"),e=document.getElementById("user-menu-trigger"),t=document.getElementById("user-menu-dropdown"),o=document.getElementById("sign-in-btn"),r=document.getElementById("user-avatar"),n=document.getElementById("user-name-display");function i(l){var d,u,p,m;if(l){a&&(a.style.display="block"),o&&(o.style.display="none");const h=s(l.name);r&&(r.textContent=h),n&&(n.textContent=((d=l.name)==null?void 0:d.split(" ")[0])||"User"),t&&(t.innerHTML=`
        ...
        `,(u=document.getElementById("menu-profile"))==null||u.addEventListener("click",()=>{t.style.display="none",showProfileModal(l)}),(p=document.getElementById("menu-password"))==null||p.addEventListener("click",()=>{t.style.display="none",showChangePasswordModal()}),(m=document.getElementById("menu-logout"))==null||m.addEventListener("click",()=>{t.style.display="none",authService.logout(),i(null)}))}else a&&(a.style.display="none"),o&&(o.style.display="block")}function s(l){if(!l)return"?";const d=l.trim().split(" ");return d.length>=2?(d[0][0]+d[d.length-1][0]).toUpperCase():l.substring(0,2).toUpperCase()}function c(l){const d=document.createElement("div");return d.textContent=l||"",d.innerHTML}e&&t&&(e.addEventListener("click",l=>{l.stopPropagation();const d=t.style.display!=="none";t.style.display=d?"none":"block"}),document.addEventListener("click",l=>{a!=null&&a.contains(l.target)||(t.style.display="none")})),o&&o.addEventListener("click",()=>{authUI.showLogin(l=>{i(l)})}),authService.onAuthChange=i,i(authService.getCurrentUser())}function showProfileModal(a){var t;const e=`
