...
`).filter(s=>s.trim()!==""),i=r.length>0?r[r.length-1]:"Unknown",n=i.split(" - ")[0]||i;o.innerText=n,o.innerText.trim()||(o.innerText="v2.1.0")}}else throw new Error("Version file not found")}catch{const t=document.getElementById("version-number");if(t){const o=config.get("app.version")||"v2.1.0";t.innerText=o}else setTimeout(()=>{const o=document.getElementById("version-number");if(o&&!o.innerText.trim()){const r=config.get("app.version")||"v2.1.0";o.innerText=r}},1e3);console.info("Version loaded from config (GitHub Pages mode)")}}async showProductLookupScreen(){try{const t=await(await fetch("./screens/product-grid.html")).text();document.body.innerHTML=t,this.currentScreen="product-grid";const o=document.createElement("script");o.type="module",o.src="js/app.js",document.body.appendChild(o),setTimeout(()=>{document.querySelectorAll(".back-btn").forEach(r=>r.remove())},100),window.productGridManager&&window.productGridManager.init(),await this.loadVersion(),setTimeout(()=>this.loadVersion(),1e3)}catch(e){console.error("Failed to load product grid screen:",e)}}setupSplitInterface(){const e=document.getElementById("back-to-home");e&&(e.onclick=()=>location.reload());const t=document.getElementById("download-btn"),o=document.getElementById("clear-all-btn");t&&(t.onclick=()=>this.showDownloadFormModal()),o&&(o.onclick=()=>this.showClearConfirmModal()),this.setupSplitProductSearch(),this.setupReviewTable(),this.renderReviewTable(),this.loadInitialSearchResults()}setupSplitProductSearch(){const e=document.getElementById("product-search-input"),t=document.getElementById("search-results-list"),o=document.getElementById("search-loading"),r=document.getElementById("search-no-results");if(!e||!t)return;const i=[],n=Utils.debounce(s=>{this.performSplitProductSearch(s,t,i,o,r)},200);e.addEventListener("input",()=>{const s=e.value.trim();s?n(s):this.loadInitialSearchResults()}),t.addEventListener("click",s=>{const c=s.target.closest(".result-item");if(!c)return;const l=parseInt(c.getAttribute("data-idx"),10),d=i.length>0?i:this.currentSearchResults||[];!isNaN(l)&&d[l]&&this.showSplitProductDetails(d[l])})}performSplitProductSearch(e,t,o,r,i){if(!dataLayer.isLoaded){r.style.display="flex",i.style.display="none",t.innerHTML="";return}o.length=0,o.push(...dataLayer.searchProducts(e)),r.style.display="none",o.length===0?(i.style.display="flex",t.innerHTML=""):(i.style.display="none",t.innerHTML=o.map((n,s)=>`
    ...
    `,document.body.appendChild(o),loadImageAsDataURL("./assets/seima-logo.png",(i,n,s)=>{const c=JSON.parse(localStorage.getItem("selection")||"[]"),l=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let d=[];l.length>0?d=l.map(C=>({...C.product,Room:C.room,Notes:C.notes,Quantity:C.quantity,Timestamp:new Date(C.timestamp).toISOString()})):d=c;const u=document.getElementById("sort-by"),p=u?u.value:"room";switch(p){case"code":d.sort((C,I)=>{const B=C.OrderCode||C.Code||"",_=I.OrderCode||I.Code||"";return B.localeCompare(_)});break;case"product":d.sort((C,I)=>{const B=C.Description||C.ProductName||"",_=I.Description||I.ProductName||"";return B.localeCompare(_)});break;case"room":default:d.sort((C,I)=>{const B=C.Room||"Blank",_=I.Room||"Blank";return B.localeCompare(_)});break}if(!d.length){alert("No products selected."),t&&(t.style.display="none");return}if(getImageCacheSize()>0)console.log(`📷 Using ${getImageCacheSize()} pre-cached images (skipping duplicate preload)`);else{const C=document.getElementById("pdf-processing-notification");if(C){const I=document.createElement("span");I.id="preload-progress",I.style.cssText="display: block; font-size: 12px; margin-top: 8px; color: #1e40af;",I.textContent="Loading images: 0%",C.appendChild(I)}console.log("📷 Starting image preload for",d.length,"products"),preloadAllProductImages(d).then(I=>{const B=document.getElementById("preload-progress");B&&(B.textContent=`✓ ${I} images ready`,B.style.color="#059669")}).catch(I=>{console.warn("Image preloading error:",I)})}const f={};p==="room"?d.forEach(C=>{const I=C.Room||"Blank";f[I]||(f[I]=[]),f[I].push(C)}):f.__all__=d;const{jsPDF:y}=window.jspdf,m=new y({orientation:"landscape",unit:"pt",format:"a4",compress:!0,putOnlyUsedFonts:!0,precision:16,userUnit:1,floatPrecision:16}),g=m.internal.pageSize.getWidth(),S=m.internal.pageSize.getHeight(),w=g/2,x=320,A=90,oe=(g-x)/2,q=70;m.setFillColor(255,255,255),m.rect(oe,q,x,A,"F");const $=localStorage.getItem("customerLogo");if($)try{const C=new Image;C.onload=function(){const I=x-20,B=A-20,_=C.width/C.height;let K=I,W=I/_;W>B&&(W=B,K=B*_);const pe=oe+(x-K)/2,ge=q+(A-W)/2;m.addImage($,"PNG",pe,ge,K,W,void 0,"FAST")},C.src=$}catch(C){console.warn("Failed to draw customer logo:",C)}loadImageAsDataURL("./assets/seima-logo.png",(C,I,B)=>{const K=B&&I?250*B/I:65,W=(g-250)/2,pe=q+A+80;m.addImage(C,"PNG",W,pe,250,K,void 0,"FAST");const ge="Build with Confidence",De=pe+K+28;m.setFont("helvetica","normal"),m.setFontSize(18),m.setTextColor("#333");const Ae=ge.split(""),$e=(250-m.getTextWidth(ge))/(Ae.length-1);let Be=W;Ae.forEach((ee,Re)=>{m.text(ee,Be,De),Be+=m.getTextWidth(ee)+$e}),m.setFontSize(15),m.setTextColor("#444");const Ne=w;let Pe=De+50;const he=[];a!=null&&a.name&&a.name.trim()&&he.push({label:"Name:",value:a.name.trim(),bold:!0}),a!=null&&a.project&&a.project.trim()&&he.push({label:"Project:",value:a.project.trim(),bold:!0}),a!=null&&a.address&&a.address.trim()&&he.push({label:"Address:",value:a.address.trim(),bold:!0}),a!=null&&a.email&&a.email.trim()&&he.push({label:"Email:",value:a.email.trim(),bold:!0}),a!=null&&a.telephone&&a.telephone.trim()&&he.push({label:"Telephone:",value:a.telephone.trim(),bold:!0});const ae=32;he.length>0&&he.forEach(ee=>{m.setFont("helvetica","normal"),m.text(ee.label,Ne-80,Pe,{align:"right"}),m.setFont("helvetica","bold"),m.text(ee.value,Ne-35,Pe,{align:"left"}),Pe+=26}),m.setFillColor("#9B9184"),m.rect(0,S-ae,g,ae,"F"),m.setTextColor("#fff"),m.setFontSize(13);const we=new Date,_e=["January","February","March","April","May","June","July","August","September","October","November","December"],ze=we.getDate(),Ge=_e[we.getMonth()],je=we.getFullYear(),He=String(we.getHours()).padStart(2,"0"),qe=String(we.getMinutes()).padStart(2,"0"),We=`Printed ${ze} ${Ge} ${je}, ${He}:${qe}`;m.text(We,16,S-10),m.text("www.seima.com.au",g-140,S-10);const xe=StorageManager.getUserSettings(),Z=xe.staffName||"",ce=xe.staffPosition||"",le=xe.staffEmail||"",de=xe.staffPhone||"";let ue="For more information";Z&&de&&le?ue=`For more information, please contact ${ce?`${Z}, ${ce}`:Z} on ${de} or email ${le}`:Z&&de?ue=`For more information, please contact ${ce?`${Z}, ${ce}`:Z} on ${de}`:Z&&le?ue=`For more information, please contact ${ce?`${Z}, ${ce}`:Z} or email ${le}`:de&&le?ue=`For more information, please call ${de} or email ${le}`:Z?ue=`For more information, please contact ${ce?`${Z}, ${ce}`:Z}`:de?ue=`For more information, please call ${de}`:le&&(ue=`For more information, please email ${le}`),m.setFont("helvetica","normal"),m.setFontSize(14),m.setTextColor("#111"),m.text(ue,w,S-ae-18,{align:"center"}),m.addPage(),loadImageAsDataURL("./assets/seima-logo-white.png",(ee,Re,Fe)=>{const Oe=a.showRrp&&!a.excludePrice,ve=!a.excludePrice,be=!a.excludeQty,Me=ve&&be,te=50,Ve=v=>{const b=v["WELS STAR"]||v.WELS_STAR||v.WELS_STAR||v.WelsStar||"";return b&&b.toString().trim()!==""};let N,z,V;if(Oe&&ve&&be&&Me){const v=g-280,b=v-te,T=g-200,M=g-120,D=g-60;N=[32,236,321,b,v,T,M,D],z=[90,90,b-321-10,te,T-v,M-T,D-M,60],V=["Code","Description","WELS","RRP","Price","Qty","Total"]}else if(ve&&be&&Me){const v=g-200,b=v-te,T=g-120,M=g-60;N=[32,236,321,b,v,T,M],z=[90,90,b-321-10,te,T-v,M-T,60],V=["Code","Description","WELS","Price","Qty","Total"]}else if(ve&&!be)if(Oe){const v=g-180,b=v-te,T=g-90;N=[32,236,321,b,v,T],z=[90,90,b-321-10,te,T-v,90],V=["Code","Description","WELS","RRP","Price"]}else{const v=g-90,b=v-te;N=[32,236,321,b,v],z=[90,90,b-321-10,te,90],V=["Code","Description","WELS","Price"]}else if(!ve&&be){const v=g-80,b=v-te;N=[32,236,321,b,v],z=[90,90,b-321-10,te,80],V=["Code","Description","WELS","Qty"]}else{const v=g-32-te;N=[32,236,321,v],z=[90,90,v-321-10,te],V=["Code","Description","WELS"]}resetImageOptimizationStats();const Ue=(v,b,T,M,D,Y,P)=>{if(!b||typeof b!="string"||b.length<10||!b.startsWith("http://")&&!b.startsWith("https://")&&!b.startsWith("data:")){P&&P();return}if((E=>{if(!E||E.length<25||/\/images\/\d+$/.test(E)||E.endsWith("/0"))return!0;const L=/\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(E),k=E.startsWith("data:");return!L&&!k})(b)){console.warn("Skipping malformed image URL:",(b==null?void 0:b.substring(0,50))+"..."),P&&P();return}if(imageOptimizationStats.totalImages++,a.emailCompatible){imageOptimizationStats.failedImages++,P&&P();return}const G=getCachedImage(b);if(G&&G.dataUrl)try{const E=G.width/G.height;let L=D,k=D/E;k>Y&&(k=Y,L=Y*E),v.addImage(G.dataUrl,G.format,T,M,L,k,void 0,"FAST"),imageOptimizationStats.optimizedImages++,P&&P();return}catch{console.warn("Failed to use cached image, falling back to direct load")}let X=!1;const ne=["https://wsrv.nl/?url=","https://images.weserv.nl/?url=","https://api.codetabs.com/v1/proxy?quest="];let J=0;function j(){if(X)return;const E=new Image;E.crossOrigin="Anonymous";let L=null;E.onload=function(){if(!X){X=!0,L&&clearTimeout(L);try{const Q=getOptimizedFileSettings(0),ye=Q.imageMaxWidth,me=D,H=Y;try{const F=document.createElement("canvas"),O=F.getContext("2d"),{width:U,height:R}=calculateOptimizedDimensions(E.width,E.height,ye);F.width=U,F.height=R,O.imageSmoothingEnabled=!0,O.imageSmoothingQuality="high",O.drawImage(E,0,0,U,R);let se,fe="JPEG";const Ye=detectTransparency(F,O),Xe=isTechnicalDiagram(E);Ye||Xe?(se=F.toDataURL("image/png",Q.imageQuality),fe="PNG"):(se=F.toDataURL("image/jpeg",Q.imageQuality),fe="JPEG");const Je=`img_${generateImageHash(b)}`;v.addImage(se,fe,T,M,me,H,Je,"FAST"),imageOptimizationStats.optimizedImages++,P&&P()}catch(F){console.warn(`Failed to optimize image: ${b}`,F),console.warn("Error details:",F.message,F.stack);try{v.addImage(E,"JPEG",T,M,me,H),imageOptimizationStats.optimizedImages++,P&&P()}catch(O){console.error(`Fallback also failed for: ${b}`,O),imageOptimizationStats.failedImages++,P&&P()}}}catch(Q){console.warn("Failed to add image to PDF:",Q),imageOptimizationStats.failedImages++,P&&P()}}},E.onerror=function(){X||(L&&clearTimeout(L),console.warn(`Failed to load image with proxy ${J}: ${b}`),console.warn(`Error details for: ${b} - Proxy: ${ne[J]}`),J++,J<ne.length?setTimeout(()=>{j()},200):(X=!0,console.warn("All proxies failed, skipping image"),imageOptimizationStats.failedImages++,P&&P()))},L=setTimeout(()=>{X||(console.warn(`⏰ Timeout with proxy ${J}: ${b}`),E.src="",E.onload=null,E.onerror=null,J++,J<ne.length?setTimeout(()=>{j()},200):(X=!0,console.warn("All proxies timed out, skipping image"),imageOptimizationStats.failedImages++,P&&P()))},3e3);let k=b;J<ne.length&&(k=ne[J]+encodeURIComponent(b)),E.src=k}j()},re=[];Object.keys(f).forEach((v,b)=>{const T=f[v];if(!T||!Array.isArray(T)){console.warn("⚠️ Skipping invalid room items:",v,T);return}T.forEach((M,D)=>{if(!M){console.warn("⚠️ Skipping null item in room:",v,"at index:",D);return}re.push({item:M,room:v,rIdx:b,iIdx:D,isFirstInRoom:D===0,roomCount:T.length})})}),re.reduce((v,b)=>{if(!b||!b.item)return console.warn("⚠️ Skipping null row in data analysis:",b),v;const T=String(b.item.Description||""),M=String(b.item.LongDescription||""),D=String(b.item.Notes||""),Y=String(b.item.OrderCode||"");return v+T.length+M.length+D.length+Y.length},0);let Se=0,Ee=0;const Ce=4,Ie=8,Le=Math.floor((S-80)/Ce);let ke=ae+8;function Te(){if(!re||!Array.isArray(re)){console.error("❌ Critical error: rowsToDraw is not a valid array:",re),showDetailedErrorMessage(new Error("Invalid product data structure"),"generating PDF","unknown.pdf");return}if(Se>=re.length){const D=m.internal.getNumberOfPages()-1;for(let L=2;L<=D+1;L++){m.setPage(L);const Q=(L-2)*Ce,ye=Math.min(Q+Ce,re.length);let me=!1;for(let H=Q;H<ye;H++)if(re[H]&&re[H].item&&Ve(re[H].item)){me=!0;break}drawPDFHeader(m,g,N,z,32,ae,ee,Re,Fe,a,V,!me),ke=ae+8,m.setFillColor("#9B9184"),m.rect(0,S-ae,g,ae,"F"),m.setTextColor("#fff"),m.setFontSize(11),m.text("www.seima.com.au",g-140,S-10),m.text(`Page ${L-1} of ${D}`,32,S-10)}const Y=new Date,P=String(Y.getDate()).padStart(2,"0"),ie=String(Y.getMonth()+1).padStart(2,"0"),G=String(Y.getFullYear()).slice(-2),X=String(Y.getHours()).padStart(2,"0"),ne=String(Y.getMinutes()).padStart(2,"0"),j=`${a.project.replace(/[^a-zA-Z0-9\s]/g,"")}-${P}${ie}${G}.${X}${ne}.pdf`,E=document.getElementById("pdf-processing-notification");E&&E.remove(),showImageOptimizationSummary(a.emailCompatible);try{const L=m.output("blob"),k=m.output("string"),Q=k?k.match(/\/Type\s*\/XObject/g):null,ye=k?k.match(/Tj\s/g):null,me=k?k.match(/\/A\s*<</g):null;a.pdfSize=L.size;const H=showFileSizeInfo(L,j);if(a.sendEmail&&L.size>15*1024*1024){console.warn(`❌ PDF too large for email (${(L.size/1024/1024).toFixed(1)}MB), offering email-compatible version`),showEmailCompatibleOption(a,j);return}const F=createOptimizedBlob(L,H.settings);if(a.sendEmail&&a.email)if(a.exportCsv){const O=j.replace(/\.pdf$/,".csv");generateCsvBlobAsync(a,O).then(U=>{window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:F,csvBlob:U}}))}).catch(U=>{console.error("Async CSV generation for email failed:",U),window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:F,csvBlob:null}}))})}else window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:a,pdfBlob:F,csvBlob:null}}));else if((async()=>{const O=await mergeWithTipTail(F);downloadWithFallback(O,j,"PDF")})(),a.exportCsv){const O=j.replace(/\.pdf$/,".csv");setTimeout(()=>{generateCsvBlobAsync(a,O).then(U=>{U&&downloadWithFallback(U,O,"CSV")}).catch(U=>{console.error("CSV generation failed:",U)})},1e3)}}catch(L){console.error("PDF generation failed:",L),showDetailedErrorMessage(L,"generating PDF",j);const k=document.getElementById("pdf-processing-notification");k&&k.remove()}t&&(t.style.display="none");return}Ee>=Ce&&(m.addPage(),drawPDFHeader(m,g,N,z,32,ae,ee,Re,Fe,a,V),ke=ae+8,Ee=0);const v=re[Se];if(!v||!v.item){console.warn(`⚠️  Skipping invalid row at index ${Se}:`,v),Se++,Te();return}const b=ke+Le*Ee;v.isFirstInRoom&&p==="room"&&v.room!=="__all__"&&(m.setFontSize(12),m.setFont("helvetica","bold"),m.setTextColor("#333"),m.text(`${v.room} (${v.roomCount})`,32,b+10),m.setFont("helvetica","normal"));const T=N[0],M=T+90+12;Ue(m,v.item.Image_URL||"",T,b+Ie+16,90,Le-Ie*2,()=>{Ue(m,v.item.Diagram_URL||"",M,b+Ie+16,90,Le-Ie*2,()=>{m.setFontSize(10),m.setTextColor("#222");const D=b+28,P=N[1]+z[1]/2;m.text(String(v.item.OrderCode||""),P,D+10,{align:"center"});let ie=D+35;if(v.item.Datasheet_URL&&v.item.Datasheet_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Datasheet",P,ie,{url:v.item.Datasheet_URL,align:"center"});const R=m.getTextWidth("Datasheet");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(P-R/2,ie+1.5,P+R/2,ie+1.5),ie+=16}if(v.item.Website_URL&&v.item.Website_URL!=="#"){m.setFontSize(9),m.setTextColor(0,102,204),m.textWithLink("Website",P,ie,{url:v.item.Website_URL,align:"center"});const R=m.getTextWidth("Website");m.setDrawColor(0,102,204),m.setLineWidth(.8),m.line(P-R/2,ie+1.5,P+R/2,ie+1.5),ie+=16}let G=D+10;m.setFontSize(10),m.setTextColor("#222");const X=z[2]-10,ne=N[2],J=m.splitTextToSize(String(v.item.Description||""),X);if(m.text(J,ne+5,G),G+=J.length*12,!a.excludeLongDescription&&(v.item.LongDescription||v.item["Long Description"]||v.item.longDescription)){const R=v.item.LongDescription||v.item["Long Description"]||v.item.longDescription;m.setFontSize(9),m.setTextColor("#444");const se=m.splitTextToSize(String(R),X);m.text(se,ne+5,G),G+=se.length*11}if(v.item.Notes){m.setFont("helvetica","italic"),m.setFontSize(9),m.setTextColor("#444");const R=m.splitTextToSize(`Notes: ${String(v.item.Notes).replace(/\r?\n|\r/g," ")}`,X);m.text(R,ne+5,G),G+=R.length*11,m.setFont("helvetica","normal")}const j=V.indexOf("WELS")+1;if(j>0&&N[j]){const R=v.item["WELS STAR"]||v.item.WELS_STAR||v.item.WELS_STAR||v.item.WelsStar||"";if(R&&R.toString().trim()){const se=R.toString().replace(/[^\d.]/g,"").trim();if(se){m.setFontSize(9),m.setTextColor("#666");const fe=N[j]+z[j]/2;m.text(`${se} star`,fe,D+10,{align:"center"})}}}m.setFontSize(10),m.setTextColor("#222");let E=NaN;const L=v.item.RRP_EX||v.item["RRP EX GST"]||v.item.RRP_EX||v.item.RRP_EXGST;L&&(E=parseFloat(L.toString().replace(/,/g,"")));let k=NaN;v.item.UserEditedPrice!==void 0&&v.item.UserEditedPrice!==null&&v.item.UserEditedPrice!==""?k=parseFloat(v.item.UserEditedPrice.toString().replace(/,/g,"")):isNaN(E)||(k=E),!isNaN(E)&&E>0&&a.includeGst&&(E=E*1.1),!isNaN(k)&&k>0&&a.includeGst&&(k=k*1.1);const Q=R=>!isNaN(R)&&R>=0?`$${R.toLocaleString("en-AU",{minimumFractionDigits:2,maximumFractionDigits:2})}`:"",ye=Q(E),me=Q(k),H=V.indexOf("RRP")+1,F=V.indexOf("Price")+1,O=V.indexOf("Qty")+1,U=V.indexOf("Total")+1;if(H>0&&N[H]){const R=N[H]+z[H]/2;m.text(ye,R,D+10,{align:"center"})}if(F>0&&N[F]){const R=N[F]+z[F]/2;m.text(me,R,D+10,{align:"center"})}if(O>0&&N[O]){const R=N[O]+z[O]/2;m.text(String(v.item.Quantity||1),R,D+10,{align:"center"})}if(U>0&&N[U]){const R=k*(v.item.Quantity||1),se=Q(R),fe=N[U]+z[U]/2;m.text(se,fe,D+10,{align:"center"})}Se++,Ee++,Te()})})}Te()})})})}function drawPDFHeader(a,e,t,o,r,i,n,s,c,l,d,u=!1){const p=i+5.7;if(a.setFillColor("#8B6C2B"),a.rect(0,0,e,p,"F"),n&&s&&c){const f=p*.55,y=s/c;let m=f*y;m>80&&(m=80);const g=(p-f)/2;a.addImage(n,"PNG",r,g,m,f)}a.setFontSize(10),a.setTextColor("#f4f4f4"),a.setFont("helvetica","normal");const h=p-8;d.forEach((f,y)=>{if(f==="WELS"&&u)return;const m=y+1;if(m<t.length){const g=t[m]+o[m]/2;if(f==="Price"&&!l.excludePrice){a.setFont("helvetica","normal");const S=l.includeGst?"INC GST":"EX GST",w=`Price ${S}`,x=a.getTextWidth("Price "),A=g-a.getTextWidth(w)/2;a.text("Price ",A,h),a.setFont("helvetica","bold"),a.text(S,A+x,h),a.setFont("helvetica","normal")}else if(f==="RRP"){a.setFont("helvetica","normal");const w=`RRP ${l.includeGst?"INC":"EX"}`;a.text(w,g,h,{align:"center"})}else a.text(f,g,h,{align:"center"})}})}function loadImageAsDataURL(a,e){const t=new window.Image;t.crossOrigin="Anonymous",t.onload=function(){const o=document.createElement("canvas"),r=o.getContext("2d"),i=400,n=150;let s=t.width,c=t.height;if(s>i||c>n){const d=i/s,u=n/c,p=Math.min(d,u);s=Math.round(s*p),c=Math.round(c*p)}o.width=s,o.height=c,r.imageSmoothingEnabled=!0,r.imageSmoothingQuality="high",r.drawImage(t,0,0,s,c);const l=o.toDataURL("image/png",.9);e(l,s,c)},t.src=a}function ensurePdfSpinner(){if(!document.getElementById("pdf-spinner")){const a=document.createElement("div");if(a.id="pdf-spinner",a.style.display="none",a.style.position="fixed",a.style.top="0",a.style.left="0",a.style.width="100vw",a.style.height="100vh",a.style.zIndex="9999",a.style.background="rgba(255,255,255,0.7)",a.style.alignItems="center",a.style.justifyContent="center",a.innerHTML='<div style="border:6px solid #e0e0e0;border-top:6px solid #2563eb;border-radius:50%;width:54px;height:54px;animation:spin 1s linear infinite;"></div>',document.body.appendChild(a),!document.getElementById("pdf-spinner-style")){const e=document.createElement("style");e.id="pdf-spinner-style",e.innerHTML="@keyframes spin { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);} }",document.head.appendChild(e)}}}async function generateCsvBlobAsync(a,e){return new Promise(async t=>{if(!window.Papa)try{await Utils.loadScript("https://cdn.jsdelivr.net/npm/papaparse@5.4.1/papaparse.min.js")}catch(n){console.error("Failed to load PapaParse:",n),t(null);return}const o=JSON.parse(localStorage.getItem("selection")||"[]"),r=JSON.parse(localStorage.getItem(CONFIG$1.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let i=[];if(r.length>0?i=r.map(n=>({...n.product,Room:n.room,Notes:n.notes,Quantity:n.quantity,Timestamp:new Date(n.timestamp).toISOString()})):i=o,!i.length){t(null);return}setTimeout(()=>{const n=i.map(s=>{let c,l,d,u;const p=a.excludePrice;let h=0;if(s.UserEditedPrice!==void 0&&s.UserEditedPrice!==null&&s.UserEditedPrice!=="")h=parseFloat(s.UserEditedPrice.toString().replace(/,/g,""));else{const g=s.RRP_EX||s["RRP EX GST"]||s.RRP_EX||s.RRP_EXGST||"";h=parseFloat((g||"0").toString().replace(/,/g,""))}c=h,d="Price ea ex GST",u="Price Total ex GST",l=!isNaN(c)&&c>=0?(c*(s.Quantity||1)).toFixed(2):"";const f=s["WELS STAR"]||s.WELS_STAR||s.WELS_STAR||s.WelsStar||"",y=f&&f.toString().trim()?f.toString().replace(/[^\d.]/g,"").trim():"",m={Code:sanitizeCSVField(s.OrderCode||""),Description:sanitizeCSVField(s.Description||""),"WELS Star":sanitizeCSVField(y),Quantity:s.Quantity||1,Notes:sanitizeCSVField(s.Notes||""),Room:sanitizeCSVField(s.Room||""),"Image URL":sanitizeCSVField(s.Image_URL||""),"Diagram URL":sanitizeCSVField(s.Diagram_URL||""),"Datasheet URL":sanitizeCSVField(s.Datasheet_URL||""),"Website URL":sanitizeCSVField(s.Website_URL||"")};return m[d]=p?"0.00":c>=0?c.toFixed(2):"",m[u]=p?"0.00":l,m});setTimeout(()=>{const s=window.Papa.unparse(n,{quotes:!0,quoteChar:'"',delimiter:",",header:!0,newline:`\r
...
This can be restored later.`))try{const t=await presentationLoader.deleteSelections([e.id]);t.success?(this.showToast("Selection deleted"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to delete")}catch(t){this.showError(t.message)}}async restoreSelection(e){try{const t=await presentationLoader.restoreSelections([e.id]);t.success?(this.showToast("Selection restored"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to restore")}catch(t){this.showError(t.message)}}showToast(e){const t=document.createElement("div");t.style.cssText=`
    ...
    `;this.injectSaveDialogStyles(),document.body.insertAdjacentHTML("beforeend",i);const n=document.getElementById("save-dialog"),s=document.getElementById("save-doc-name"),c=document.getElementById("save-notes");s==null||s.focus(),s==null||s.select(),n.querySelectorAll("button[data-action]").forEach(d=>{d.addEventListener("click",async()=>{const u=d.dataset.action;if(u==="cancel"){n.remove();return}const p=(s==null?void 0:s.value.trim())||"Untitled Selection",h=(c==null?void 0:c.value.trim())||"";this.currentSelectionName=p;const f={...this.prepareSelectionData(),documentName:p,notes:h};n.querySelectorAll("button").forEach(y=>y.disabled=!0),d.textContent="Saving...";try{let y;u==="save-update"?y=await presentationRecorder.updateSelection(this.currentSelectionId,f):(y=await presentationRecorder.saveSelection(f),y.success&&y.id&&(this.currentSelectionId=y.id)),n.remove(),y.success?(this.hasUnsavedChanges=!1,this.lastSaveTime=new Date,this.updateContextHeader(),toast.success(u==="save-update"?"Selection updated!":"Selection saved!")):toast.error("Failed to save: "+(y.error||"Unknown error"))}catch(y){n.remove(),toast.error("Failed to save: "+y.message)}})}),n.addEventListener("click",d=>{d.target===n&&n.remove()});const l=d=>{d.key==="Escape"&&(n.remove(),document.removeEventListener("keydown",l))};document.addEventListener("keydown",l)}injectSaveDialogStyles(){if(document.getElementById("save-dialog-styles"))return;document.head.insertAdjacentHTML("beforeend",`
      <style id="save-dialog-styles">
        .save-dialog-overlay {
          position: fixed;
          top: 0;
    ...
    `,document.body.insertBefore(i,document.body.firstChild)}setupGlobalEventListeners(){window.addEventListener("generatePdf",e=>{const{tipTailSettings:t,...o}=e.detail;ensurePdfSpinner(),showPdfFormScreen$1(o,t||null)}),window.addEventListener("beforeunload",()=>{}),browserCompatibility.features.memoryAPI&&setInterval(()=>{const e=browserCompatibility.memoryInfo;e.memoryPressure==="high"&&console.warn("High memory usage detected:",e)},6e4)}getSelectedProducts(){return StorageManager.getSelectedProducts()}clearSelection(){return StorageManager.clearAllSelections()}addProduct(e,t,o,r){return StorageManager.addProductToSelection(e,t,o,r)}updateSelectionCount(){this.navigationManager&&this.navigationManager.updateSelectionCount()}showError(e){alert(e)}}document.addEventListener("DOMContentLoaded",()=>{window.seimaScanner=new SeimaScanner,window.seimaScanner.init()});window.addEventListener("DOMContentLoaded",()=>{fetch("./version.txt").then(a=>a.text()).then(a=>{const e=document.getElementById("app-version");if(e){const t=a.trim().split(/\s+|-/)[0];e.textContent=`Ver: ${t}`,e.addEventListener("click",()=>{showChangelog(a.trim(),t)})}}),setupHelpButton(),setupUserMenu()});function setupHelpButton(){const a=document.getElementById("help-btn");a&&a.addEventListener("click",()=>{showUserGuide()});const e=document.getElementById("quick-start-btn");e&&e.addEventListener("click",()=>{onboarding.showForced()})}function setupUserMenu(){const a=document.getElementById("user-menu-container"),e=document.getElementById("user-menu-trigger"),t=document.getElementById("user-menu-dropdown"),o=document.getElementById("sign-in-btn"),r=document.getElementById("user-avatar"),i=document.getElementById("user-name-display");function n(l){var d,u,p,h;if(l){a&&(a.style.display="block"),o&&(o.style.display="none");const f=s(l.name);r&&(r.textContent=f),i&&(i.textContent=((d=l.name)==null?void 0:d.split(" ")[0])||"User"),t&&(t.innerHTML=`
        ...
        `,(u=document.getElementById("menu-profile"))==null||u.addEventListener("click",()=>{t.style.display="none",authUI.showEditProfile(y=>{StorageManager.clearUserSettings(),n(y)})}),(p=document.getElementById("menu-password"))==null||p.addEventListener("click",()=>{t.style.display="none",authUI.showChangePassword()}),(h=document.getElementById("menu-logout"))==null||h.addEventListener("click",()=>{t.style.display="none",StorageManager.clearUserSettings(),authService.logout(),n(null)}))}else a&&(a.style.display="none"),o&&(o.style.display="block")}function s(l){if(!l)return"?";const d=l.trim().split(" ");return d.length>=2?(d[0][0]+d[d.length-1][0]).toUpperCase():l.substring(0,2).toUpperCase()}function c(l){const d=document.createElement("div");return d.textContent=l||"",d.innerHTML}e&&t&&(e.addEventListener("click",l=>{l.stopPropagation();const d=t.style.display!=="none";t.style.display=d?"none":"block"}),document.addEventListener("click",l=>{a!=null&&a.contains(l.target)||(t.style.display="none")})),o&&o.addEventListener("click",()=>{authUI.showLogin(l=>{n(l)})}),authService.onAuthChange=n,n(authService.getCurrentUser())}function showUserGuide(){const a=document.getElementById("user-guide-modal"),e=document.getElementById("user-guide-content");if(!a||!e)return;e.innerHTML=getUserGuideContent(),a.style.display="flex";const t=document.getElementById("user-guide-close");t&&(t.onclick=()=>{a.style.display="none"}),a.onclick=r=>{r.target===a&&(a.style.display="none")};const o=r=>{r.key==="Escape"&&(a.style.display="none",document.removeEventListener("keydown",o))};document.addEventListener("keydown",o)}function getUserGuideContent(){return`
